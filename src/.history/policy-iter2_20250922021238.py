...
def estimate_state_space(jobs_data, machines, arrivals):
    ...
    # ACTUAL observations from enumeration:
    if num_jobs == 2 and total_ops == 4:  # SIMPLE
        estimated_states = "318 (ACTUAL from CSV - much smaller than theory!)"
    elif num_jobs == 3 and total_ops == 6:  # MEDIUM  
        estimated_states = "~2K-5K (estimated based on SIMPLE scaling)"
    elif num_jobs == 5 and total_ops == 15:  # LARGER
        estimated_states = "~50K-200K (estimated - still manageable for PI)"
    else:
        # Based on ACTUAL SIMPLE observation: 318 states for next_ops = 9 combinations
        # So roughly 318/9 ≈ 35x multiplier from timing constraints (much lower!)
        scaling_factor = 35
