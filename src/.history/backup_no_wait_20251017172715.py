...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        self.cheat = True  # No cheating with future info
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    ...
    def scenario_hash(jobs_data, machine_list, arrival_times):
        jobs_bytes = pickle.dumps(jobs_data)
        machines_bytes = pickle.dumps(machine_list)
        arrivals_bytes = pickle.dumps(arrival_times)
        hash_input = jobs_bytes + machines_bytes + arrivals_bytes
        return hashlib.md5(hash_input).hexdigest()

    scenario_id = scenario_hash(jobs_data, machine_list, arrival_times)
    cache_filename = f"milp_cache_{scenario_id}.pkl"

    # Try to load cache
    if os.path.exists(cache_filename):
        if verbose:
            print(f"🗂️  Found cached MILP result for scenario: {cache_filename}")
        try:
            with open(cache_filename, "rb") as f:
                cached = pickle.load(f)
            optimal_makespan = cached["optimal_makespan"]
            schedule = cached["schedule"]
            if verbose:
                print(f"✅ Loaded cached MILP makespan: {optimal_makespan:.2f}")
            return optimal_makespan, schedule
        except Exception as e:
            if verbose:
                print(f"⚠️  Failed to load cache: {e}. Will recompute.")
    
    # Save to cache
    try:
        with open(cache_filename, "wb") as f:
            pickle.dump({"optimal_makespan": optimal_makespan, "schedule": schedule}, f)
        if verbose:
            print(f"💾 Saved MILP result to cache: {cache_filename}")
    except Exception as e:
        if verbose:
