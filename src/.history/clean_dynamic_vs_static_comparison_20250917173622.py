...
class StaticFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # Job arrival status (arrived at t=0 for static environment)
        for job_id in self.job_ids:
    ...
    while operations_done < total_ops:
        ...
        for job_id in jobs_data.keys():
            ...
            for machine_name, proc_time in op_data['proc_times'].items():
                ready_ops.append((job_id, next_op_idx, machine_name, proc_time, op_ready_time))
...
def direct_spt_from_test3_backup(jobs_data, machine_list, job_arrival_times):
    """
    Direct copy of heuristic_spt_scheduler from test3_backup.py
    This should achieve makespan around 40-50
    """
    print("\n--- Running Direct SPT from test3_backup.py ---")
    ...
    makespan = max(machine_next_free.values()) if machine_next_free else 0
    print(f"Direct SPT from test3_backup.py Makespan: {makespan:.2f}")
    return makespan, schedule
        ...
        for job_id in arrived_jobs:
            ...
            if next_op_idx < len(jobs_data[job_id]):
                ...
                if next_op_idx == 0:  # First operation
                    ...
                    job_ready_time = operation_end_times[j]