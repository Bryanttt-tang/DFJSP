...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # This part is now disabled to favor rollout-level logging
        return True
    ...
    def _on_rollout_end(self) -> None:
        ...
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            ...
            # Capture rollout metrics (mean reward and length) to align with verbose
            if 'rollout/ep_rew_mean' in log_data:
            ...
            # Maintain a simple rollout counter for x-axis
            TRAINING_METRICS['episode_count'].append(len(TRAINING_METRICS['policy_loss']))

            if self.verbose and len(TRAINING_METRICS['policy_loss']) <= 5:
                print(f"[{self.method_name}] 📊 Rollout #{len(TRAINING_METRICS['policy_loss'])}: "
                      f"ep_rew_mean={log_data.get('rollout/ep_rew_mean', 'N/A'):.2f}, "
                      f"policy_loss={log_data.get('train/policy_gradient_loss', 'N/A'):.6f}, "
