        ...
        for op_data in machine_ops:
            if len(op_data) >= 3:
                ...
                try:
                    ...
                    op_idx = int(job_op.split('O')[1]) - 1  # 0-indexed
...
def main():
    ...
    # Create medium_instances folder
    import os
    folder_name = "medium_instances"
    ...
    # --- MILP VALIDATION TEST WITH 10 JOBS ---
if __name__ == "__main__":
    print("\n=== MILP VALIDATION TEST: 10 JOBS, 3 INITIAL, 7 POISSON ===")
    # Create 10 jobs (first 3 initial, last 7 dynamic)
    jobs_10 = collections.OrderedDict(ENHANCED_JOBS_DATA)
    # Add 3 more jobs for test (simple random jobs)
    for j in range(7, 10):
        jobs_10[j] = [
            {'proc_times': {'M0': random.randint(3, 8), 'M1': random.randint(3, 8)}},
            {'proc_times': {'M1': random.randint(3, 8), 'M2': random.randint(3, 8)}},
            {'proc_times': {'M0': random.randint(3, 8), 'M2': random.randint(3, 8)}}
        ]
    # Initial jobs
    initial_jobs = [0, 1, 2]
    # Generate Poisson arrivals for last 7 jobs
    arrival_rate = 0.5
    np.random.seed(123)
    random.seed(123)
    arrival_times_10 = {j: 0.0 for j in initial_jobs}
    current_time = 0.0
    for j in range(3, 10):
        inter_arrival = np.random.exponential(1.0 / arrival_rate)
        current_time += inter_arrival
        arrival_times_10[j] = round(current_time)
    print(f"Test arrival times: {arrival_times_10}")
    # Run MILP solver
    milp_makespan, milp_schedule = milp_optimal_scheduler(jobs_10, MACHINE_LIST, arrival_times_10)
    print(f"MILP makespan: {milp_makespan}")
    for m in MACHINE_LIST:
        print(f"Machine {m} schedule:")
        for op in milp_schedule.get(m, []):
            print(f"  {op}")
    # Validate schedule
    is_valid, true_makespan = verify_schedule_correctness(milp_schedule, jobs_10, arrival_times_10, "MILP TEST")
    print(f"MILP schedule valid: {is_valid}, True makespan: {true_makespan}")
