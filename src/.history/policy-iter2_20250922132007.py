...
# Helper utilities
# ---------------------------
def make_state(next_ops, ready_jobs, idle_machines):
    """Create a compact event-driven state representation.
    
    Args:
        next_ops: Which operation each job needs next (job progress)
        ready_jobs: Set of jobs whose next operation is ready now
        idle_machines: Set of machines that are currently idle
    """
    return (tuple(next_ops), tuple(sorted(ready_jobs)), tuple(sorted(idle_machines)))
