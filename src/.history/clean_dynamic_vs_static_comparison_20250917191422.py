...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times):
    ...
    if os.path.exists(cache_file):
        try:
            with open(cache_file, 'rb') as f:
                ...
                print(f"✅ Loaded MILP result from cache: makespan = {result[0]:.2f}")
                return result
        except:
            pass  # Cache corrupted, continue with fresh calculation
    ...
    try:
        ...
        for j, oi in ops:
            ...
            prob += lpSum([x[j, oi][m] for m in machine_list if m in jobs_data[j][oi]['proc_times']]) == 1
            
            # 2. Start time >= job arrival time (for first operation)
            if oi == 0:
            ...
            for m in machine_list:
                if m in jobs_data[j][oi]['proc_times']:
                    proc_time = jobs_data[j][oi]['proc_times'][m]
                    prob += c[j, oi] >= s[j, oi] + proc_time * x[j, oi][m]
        ...
        for m in machine_list:
            for j1, oi1 in ops:
                for j2, oi2 in ops:
                    if (j1, oi1) < (j2, oi2) and m in jobs_data[j1][oi1]['proc_times'] and m in jobs_data[j2][oi2]['proc_times']:
                        # Either (j1,oi1) before (j2,oi2) or vice versa
                        prob += c[j1, oi1] <= s[j2, oi2] + BIG_M * (1 - y[j1, oi1, j2, oi2][m])
                        prob += c[j2, oi2] <= s[j1, oi1] + BIG_M * y[j1, oi1, j2, oi2][m]
                        
                        # Only consider this constraint if both operations can be on machine m
                        prob += y[j1, oi1, j2, oi2][m] <= x[j1, oi1][m]
                        prob += y[j1, oi1, j2, oi2][m] <= x[j2, oi2][m]
        ...
        if prob.status == 1 and Cmax.varValue is not None:
            print(f"✅ MILP solved optimally: makespan = {Cmax.varValue:.2f}")
            
            # Extract schedule
            for j, oi in ops:
                for m in machine_list:
                    if m in jobs_data[j][oi]['proc_times'] and x[j, oi][m].varValue > 0.5:
                        start = s[j, oi].varValue
                        end = c[j, oi].varValue
                        schedule[m].append((f"J{j}-O{oi+1}", start, end))
            
            # Sort operations by start time for each machine
            for m in machine_list:
                schedule[m].sort(key=lambda op: op[1])
            
            result = (Cmax.varValue, schedule)
            
            # Cache the result
            try:
                with open(cache_file, 'wb') as f:
                    pickle.dump(result, f)
                print(f"💾 Cached MILP result")
            except:
                pass  # Caching failed, but continue
            
            return result
        else:
            print(f"❌ MILP failed to find optimal solution (status: {prob.status})")
