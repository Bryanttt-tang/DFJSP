...
class StaticFJSPEnv(gym.Env):
    ...
    Based exactly on PerfectKnowledgeFJSPEnv but with all arrival times = 0.
    """
    ...
    def reset(self, seed=None, options=None):
        """Reset environment - based on PerfectKnowledgeFJSPEnv approach."""
        if seed is not None:
            random.seed(seed)
            np.random.seed(seed)
        
        # Initialize state similar to PerfectKnowledgeFJSPEnv
        self.machine_next_free = {m: 0.0 for m in self.machines}
        self.machine_workload = {m: 0.0 for m in self.machines}  # Add missing machine_workload
        ...
        self.num_completed_jobs = 0  # Initialize for compatibility
        
        # All jobs available at t=0 for static scheduling
        self.arrived_jobs = set(self.job_ids)
    ...
    def step(self, action):
        ...
        # Update state using test3_backup.py approach
    ...
    def _get_observation(self):
        ...
        # Job arrival status (all 1.0 for static - all jobs available at t=0)
        for job_id in self.job_ids:
            obs.append(1.0)  # All jobs have arrived in static scheduling
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            print(f"\n=== Makespan: {self.current_makespan:.2f} ===")
...
class StaticRLEvaluationEnv(StaticFJSPEnv):
    """
    Special evaluation environment for Static RL that respects arrival times
    but maintains the same observation space as StaticFJSPEnv.
    """
    
    def __init__(self, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment", seed=None):
        # Initialize as StaticFJSPEnv but with custom arrival times
        super().__init__(jobs_data, machine_list, reward_mode, seed)
        self.arrival_times = arrival_times.copy()
        
    def _reset_state(self):
        """Reset the environment state with arrival time awareness."""
        super()._reset_state()
        
        # Override arrived jobs to respect arrival times (initially only t=0 jobs)
        self.arrived_jobs = {
            job_id for job_id, arrival_time in self.arrival_times.items()
            if arrival_time <= 0
        }
        
    def step(self, action):
        """Modified step that checks for new arrivals based on makespan progress."""
        # Store previous makespan to check for new arrivals
        previous_makespan = self.current_makespan
        
        # Execute normal StaticFJSPEnv step
        obs, reward, terminated, truncated, info = super().step(action)
        
        # Check for newly arrived jobs based on makespan progression
        newly_arrived = {
            job_id for job_id, arrival_time in self.arrival_times.items()
            if previous_makespan < arrival_time <= self.current_makespan and job_id not in self.arrived_jobs
        }
...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    ...
    # Create StaticRLEvaluationEnv that respects arrival times but has same observation space
    test_env = StaticRLEvaluationEnv(jobs_data, machine_list, arrival_times, reward_mode=reward_mode)
...
def main():
    ...
    if milp_makespan != float('inf'):
        fig.suptitle('Main Scheduling Comparison: MILP vs Perfect Knowledge vs Dynamic vs Static RL vs SPT\n' + 
        ...
        fig.suptitle('Main Scheduling Comparison: Perfect Knowledge vs Dynamic vs Static RL vs SPT\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'Static RL evaluated on dynamic scenario with arrivals', 
                     fontsize=16, fontweight='bold')
        schedules_data = [
