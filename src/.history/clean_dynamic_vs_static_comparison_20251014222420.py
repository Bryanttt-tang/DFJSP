...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_next_event_time(self):
        ...
        return next_event_time if next_event_time != float('inf') else self.event_time + 1.0
    ...
    def step(self, action):
        ...
        if not scheduling_actions_available:
            print("At step ", self.episode_step, " with event_time ", self.event_time, " no scheduling actions available. Action chosen: ", action)
        ...
        if job_idx is None:
            ...
            wait_reward = -1000.0 if scheduling_actions_available else -500.0
            ...
            if not new_arrival_happened and not np.any(self.action_masks()[:-1]):
        ...
        print("At step ", self.episode_step, " with event_time ", self.event_time, " job ", job_id, " op ", op_idx, " machine ", machine_idx, " start_time ", start_time, " end_time ", end_time)

        # IMPORTANT: Update event time to the start time and check for new arrivals
        # This ensures that any jobs arriving between current event_time and start_time are revealed
        if start_time == self.event_time:
           num_arrivals = self._update_event_time_and_arrivals(start_time)

        post_event_time = self.event_time
        # Update state
...
def main():
    ...
    dynamic_model = train_dynamic_agent(
        ...
        total_timesteps=dynamic_timesteps, reward_mode="makespan_increment", learning_rate=1e-2
    )
