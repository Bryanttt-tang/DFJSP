...
TRAINING_METRICS = {
    ...
    'episode_makespans': []  # Track makespan per episode
}
...
class TrainingCallback:
    """Enhanced callback to track training metrics including episode-level data."""
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        if hasattr(model, 'ep_info_buffer') and len(model.ep_info_buffer) > 0:
            ...
            latest_episodes = model.ep_info_buffer
            
            for ep_info in latest_episodes:
                if 'r' in ep_info and 'l' in ep_info:
                    TRAINING_METRICS['episode_rewards'].append(ep_info['r'])
                    TRAINING_METRICS['episode_lengths'].append(ep_info['l'])
                    TRAINING_METRICS['episode_numbers'].append(self.episode_count)
                    
                    # Try to get makespan from episode info
                    if 't' in ep_info:  # 't' often contains time/makespan info
                        TRAINING_METRICS['episode_makespans'].append(ep_info['t'])
                    else:
                        TRAINING_METRICS['episode_makespans'].append(0)  # Fallback
...
def generate_test_scenarios(jobs_data, initial_jobs=[0, 1, 2, 3, 4], arrival_rate=0.08, num_scenarios=10):
    """
    Generate diverse test scenarios with expanded job set and max arrival bound.
    """
    ...
    print(f"Max arrival time bound: 200 time units")
    ...
    for i in range(num_scenarios):
        ...
        # Generate Poisson arrivals for remaining jobs with max bound
        ...
        print(f"    Arrivals: {len(arrived_jobs) - len(current_initial)} jobs (bounded by {max_arrival_time})")
...
def plot_training_metrics():
    """
    Enhanced plot showing both training losses and episode rewards/makespans.
    """
    global TRAINING_METRICS
    ...
    print(f"Total episodes completed: {len(TRAINING_METRICS['episode_rewards'])}")
    
    # Create figure with more subplots
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.suptitle('Complete PPO Training Analysis - Losses, Rewards, and Episode Performance', fontsize=16, fontweight='bold')
    ...
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['value_loss']:
        ...
        axes[0, 1].plot(timesteps[:min_len], TRAINING_METRICS['policy_loss'][:min_len], 'r-', linewidth=2, label='Policy Loss')
        axes[0, 1].plot(timesteps[:min_len], TRAINING_METRICS['value_loss'][:min_len], 'g-', linewidth=2, label='Value Loss')
        axes[0, 1].legend()
        axes[0, 1].set_xlabel('Training Steps')
        axes[0, 1].set_ylabel('Loss')
        axes[0, 1].set_title('Training Losses (Should Decrease)')
        axes[0, 1].grid(True, alpha=0.3)
    else:
        axes[0, 1].text(0.5, 0.5, 'No loss data', ha='center', va='center', transform=axes[0, 1].transAxes)
        axes[0, 1].set_title('Training Losses (No Data)')
    
    # Plot 3: Episode Rewards over Episodes
    if TRAINING_METRICS['episode_rewards']:
        episode_nums = range(len(TRAINING_METRICS['episode_rewards']))
        axes[1, 0].plot(episode_nums, TRAINING_METRICS['episode_rewards'], 'purple', linewidth=1, alpha=0.7)
        
        # Add moving average for trend
        if len(TRAINING_METRICS['episode_rewards']) > 10:
            window_size = min(50, len(TRAINING_METRICS['episode_rewards']) // 10)
            moving_avg = []
            for i in range(len(TRAINING_METRICS['episode_rewards'])):
                start_idx = max(0, i - window_size + 1)
                avg = np.mean(TRAINING_METRICS['episode_rewards'][start_idx:i+1])
                moving_avg.append(avg)
            axes[1, 0].plot(episode_nums, moving_avg, 'red', linewidth=3, label=f'Moving Average ({window_size} episodes)')
            axes[1, 0].legend()
        
        axes[1, 0].set_xlabel('Episode Number')
        axes[1, 0].set_ylabel('Episode Reward')
        axes[1, 0].set_title('Episode Rewards (Should Increase Over Time)')
        axes[1, 0].grid(True, alpha=0.3)
        
        # Add trend analysis
        if len(TRAINING_METRICS['episode_rewards']) > 20:
            early_avg = np.mean(TRAINING_METRICS['episode_rewards'][:len(TRAINING_METRICS['episode_rewards'])//4])
            late_avg = np.mean(TRAINING_METRICS['episode_rewards'][-len(TRAINING_METRICS['episode_rewards'])//4:])
            trend = "Improving" if late_avg > early_avg else "Declining"
            axes[1, 0].text(0.02, 0.02, f'Trend: {trend}', transform=axes[1, 0].transAxes, 
                           bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
    else:
        axes[1, 0].text(0.5, 0.5, 'No episode reward data', ha='center', va='center', transform=axes[1, 0].transAxes)
        axes[1, 0].set_title('Episode Rewards (No Data)')
    
    # Plot 4: Episode Lengths
    if TRAINING_METRICS['episode_lengths']:
        episode_nums = range(len(TRAINING_METRICS['episode_lengths']))
        axes[1, 1].plot(episode_nums, TRAINING_METRICS['episode_lengths'], 'orange', linewidth=1, alpha=0.7)
        
        # Add moving average
        if len(TRAINING_METRICS['episode_lengths']) > 10:
            window_size = min(50, len(TRAINING_METRICS['episode_lengths']) // 10)
            moving_avg = []
            for i in range(len(TRAINING_METRICS['episode_lengths'])):
                start_idx = max(0, i - window_size + 1)
                avg = np.mean(TRAINING_METRICS['episode_lengths'][start_idx:i+1])
                moving_avg.append(avg)
            axes[1, 1].plot(episode_nums, moving_avg, 'red', linewidth=3, label=f'Moving Average ({window_size} episodes)')
            axes[1, 1].legend()
        ...
        axes[1, 1].text(0.5, 0.5, 'No episode length data', ha='center', va='center', transform=axes[1, 1].transAxes)
        axes[1, 1].set_title('Episode Lengths (No Data)')
    
    # Plot 5: Episode Makespans (if available)
    if TRAINING_METRICS['episode_makespans'] and any(m > 0 for m in TRAINING_METRICS['episode_makespans']):
        episode_nums = range(len(TRAINING_METRICS['episode_makespans']))
        valid_makespans = [m if m > 0 else np.nan for m in TRAINING_METRICS['episode_makespans']]
        axes[2, 0].plot(episode_nums, valid_makespans, 'green', linewidth=1, alpha=0.7)
        
        # Add moving average for non-zero values
        non_zero_indices = [i for i, m in enumerate(TRAINING_METRICS['episode_makespans']) if m > 0]
        if len(non_zero_indices) > 10:
            window_size = min(20, len(non_zero_indices) // 5)
            moving_avg = []
            for i in non_zero_indices:
                start_idx = max(0, i - window_size)
                end_idx = min(len(TRAINING_METRICS['episode_makespans']), i + window_size)
                valid_values = [m for m in TRAINING_METRICS['episode_makespans'][start_idx:end_idx] if m > 0]
                if valid_values:
                    moving_avg.append((i, np.mean(valid_values)))
            
            if moving_avg:
                avg_x, avg_y = zip(*moving_avg)
                axes[2, 0].plot(avg_x, avg_y, 'red', linewidth=3, label=f'Moving Average')
                axes[2, 0].legend()
        
        axes[2, 0].set_xlabel('Episode Number')
        axes[2, 0].set_ylabel('Episode Makespan')
        axes[2, 0].set_title('Episode Makespans (Should Decrease Over Time)')
        axes[2, 0].grid(True, alpha=0.3)
    else:
        axes[2, 0].text(0.5, 0.5, 'No makespan data', ha='center', va='center', transform=axes[2, 0].transAxes)
        axes[2, 0].set_title('Episode Makespans (No Data)')
...
• Total Timesteps: {max(TRAINING_METRICS['timesteps']) if TRAINING_METRICS['timesteps'] else 0:,}
"""
    
    if TRAINING_METRICS['episode_rewards']:
        summary_text += f"• Avg Episode Reward: {np.mean(TRAINING_METRICS['episode_rewards']):.2f}\n"
        summary_text += f"• Best Episode Reward: {max(TRAINING_METRICS['episode_rewards']):.2f}\n"
    
    if TRAINING_METRICS['episode_lengths']:
        summary_text += f"• Avg Episode Length: {np.mean(TRAINING_METRICS['episode_lengths']):.1f} steps\n"
    
    if TRAINING_METRICS['action_entropy']:
        summary_text += f"• Final Entropy: {TRAINING_METRICS['action_entropy'][-1]:.4f}\n"
    
    # Training quality assessment
    if len(TRAINING_METRICS['episode_rewards']) > 20:
        early_rewards = TRAINING_METRICS['episode_rewards'][:len(TRAINING_METRICS['episode_rewards'])//4]
        late_rewards = TRAINING_METRICS['episode_rewards'][-len(TRAINING_METRICS['episode_rewards'])//4:]
        improvement = np.mean(late_rewards) - np.mean(early_rewards)
        summary_text += f"\nTraining Quality:\n"
        if improvement > 0:
            summary_text += f"✅ Improving (+{improvement:.2f} reward)\n"
        else:
            summary_text += f"❌ Declining ({improvement:.2f} reward)\n"
...
def main():
    ...
    plot_training_metrics()      # Show ENHANCED PPO exploration and episode metrics
...
def validate_schedule_makespan(schedule, jobs_data, arrival_times):
    ...
    Check schedule validity:
      - Each operation appears exactly once on exactly one compatible machine
      - Precedence: op_{k+1} of a job starts >= completion of op_k
      - Start times >= arrival for first op
      - No overlapping operations on same machine
    Returns actual makespan (float) if valid, otherwise raises ValueError.
    """
    # Build per-op info map
    op_map = {}  # (j,oi) -> (machine, start, end)
    # Build reverse index of assigned ops
    for m, tasks in schedule.items():
        for name, st, et in tasks:
            # name format "J{j}-O{oi+1}"
            assert name.startswith("J")
            j_str, o_str = name.split("-")
            j = int(j_str[1:])
            oi = int(o_str[1:]) - 1
            if (j, oi) in op_map:
                raise ValueError(f"Operation J{j}-O{oi+1} assigned more than once")
            op_map[(j, oi)] = (m, float(st), float(et))
            # basic compatibility check
            if m not in jobs_data[j][oi]['proc_times']:
                raise ValueError(f"Operation J{j}-O{oi+1} scheduled on incompatible machine {m}")

            if et < st - 1e-9:
                raise ValueError(f"Operation J{j}-O{oi+1} has end < start")

    # Ensure all operations present
    for j in jobs_data:
        for oi in range(len(jobs_data[j])):
            if (j, oi) not in op_map:
                raise ValueError(f"Missing scheduled operation J{j}-O{oi+1}")

    # Check precedence and arrivals
    for j in jobs_data:
        for oi in range(len(jobs_data[j])):
            m, st, et = op_map[(j, oi)]
            if oi == 0:
                if st + 1e-9 < arrival_times.get(j, 0):
                    raise ValueError(f"J{j}-O1 starts before job arrival")
            else:
                # previous op end
                _, pst, pet = op_map[(j, oi-1)]
                if st + 1e-9 < pet:
                    raise ValueError(f"J{j}-O{oi+1} starts before previous operation finishes")

            # check duration matches proc_time on assigned machine
            expected = jobs_data[j][oi]['proc_times'][m]
            if abs((et - st) - expected) > 1e-6:
                raise ValueError(f"J{j}-O{oi+1} duration mismatch: scheduled {(et-st)} vs expected {expected}")

    # Check no overlap on same machine
    for m in schedule:
        tasks = sorted(schedule[m], key=lambda x: x[1])  # by start
        for i in range(len(tasks)-1):
            _, s1, e1 = tasks[i]
            _, s2, e2 = tasks[i+1]
            if s2 + 1e-9 < e1:
                raise ValueError(f"Overlap on machine {m}: [{s1},{e1}] and [{s2},{e2}]")

    # Compute makespan
    makespan = 0.0
    for (j,oi), (_, st, et) in op_map.items():
        makespan = max(makespan, et)
    return makespan

def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, accept_infeasible=False):
    """
    Corrected MILP scheduler. Returns (optimal_makespan, schedule) or (inf, {}) on failure.
    """
    print("\n--- Running MILP Optimal Scheduler (corrected) ---")
    ops = [(j, oi) for j in sorted(jobs_data.keys()) for oi in range(len(jobs_data[j]))]

    # build list of compatible (op,machine) pairs
    compatible_pairs = []
    proc_time = {}
    for op in ops:
        j, oi = op
        for m in machine_list:
            if m in jobs_data[j][oi]['proc_times']:
                compatible_pairs.append((op, m))
                proc_time[(op, m)] = jobs_data[j][oi]['proc_times'][m]

    # Horizon bound (tight BIG_M)
    # Maximum possible completion time <= sum of all proc times + max arrival
    total_proc_sum = sum(min(jobs_data[j][oi]['proc_times'].values()) for (j,oi) in ops)
    max_arrival = max(arrival_times.values()) if arrival_times else 0
    BIG_M = total_proc_sum + max_arrival + 1000  # small cushion
    # alternative: sum of worst-case processing times:
    worst_case_sum = sum(max(jobs_data[j][oi]['proc_times'].values()) for (j,oi) in ops)
    horizon_ub = worst_case_sum + max_arrival + 10.0

    prob = LpProblem("FJSP_PerfectKnowledge_Optimal", LpMinimize)

    # Decision variables
    x = {}   # x[(op,m)] binary
    for (op, m) in compatible_pairs:
    ...
    # Assignment: each op assigned to exactly one compatible machine
    for op in ops:
        eligible = [ (op,m) for (op,m) in compatible_pairs if op == (op[0], op[1]) ]
        # eligible is list of tuples ((j,oi), m) so we map to x[(op,m)]
        eligible_vars = [ x[(op, m)] for (_, m) in eligible ]
        if not eligible_vars:
            # operation has no compatible machine => infeasible instance
            print(f"Operation {op} has no compatible machines. MILP infeasible.")
            return float('inf'), {}
        prob += lpSum(eligible_vars) == 1

    # Completion time definition and makespan
    for op in ops:
        # c = s + sum(x*proc)
        eligible = [m for (o,m) in compatible_pairs if o == op]
        prob += c[op] == s[op] + lpSum( x[(op,m)] * jobs_data[op[0]][op[1]]['proc_times'][m] for m in eligible )
        # makespan bound
        prob += Cmax >= c[op]
        # start times within horizon (optional but helps numerics)
        prob += s[op] <= horizon_ub
        prob += c[op] <= horizon_ub

    # Precedence constraints inside job
    for j in sorted(jobs_data.keys()):
        for oi in range(1, len(jobs_data[j])):
            prob += s[(j, oi)] >= c[(j, oi-1)]

    # Arrival constraints for first ops
    for j in sorted(jobs_data.keys()):
        prob += s[(j, 0)] >= arrival_times.get(j, 0)

    # Disjunctive machine capacity constraints:
    # For each machine, consider only ops that can go to this machine
    for m in machine_list:
        ops_on_m = [op for op in ops if (op, m) in proc_time]
        # enumerate unordered pairs
        for i in range(len(ops_on_m)):
            for k in range(i+1, len(ops_on_m)):
                op1 = ops_on_m[i]
                op2 = ops_on_m[k]
                # sequencing binary y(op1,op2,m): 1 if op1 before op2 when both assigned to m
                y = LpVariable(f"y_{op1[0]}_{op1[1]}_{op2[0]}_{op2[1]}_{m}", cat=LpBinary)
                # only enforced when both x[(op1,m)] and x[(op2,m)] == 1
                # s_op1 >= c_op2 - BIG_M*(1 - y) - BIG_M*(2 - x1 - x2)
                prob += s[op1] >= c[op2] - BIG_M * (1 - y) - BIG_M * (2 - x[(op1,m)] - x[(op2,m)])
                prob += s[op2] >= c[op1] - BIG_M * y - BIG_M * (2 - x[(op1,m)] - x[(op2,m)])

    # Solve
    print("Solving MILP (CBC)...")
    solver = PULP_CBC_CMD(msg=False, timeLimit=time_limit)
    prob.solve(solver)

    status = LpStatus.get(prob.status, None) if isinstance(LpStatus, dict) else LpStatus[prob.status]
    # More robust: pulp.LpStatus[prob.status] yields string but import depends on pulp version
    try:
    ...
    pulp_status = None
    try:
        from pulp import LpStatus as _LpStatus
        pulp_status = _LpStatus[prob.status]
    except Exception:
        pulp_status = None

    # Accept either optimal or feasible incumbent if time-limited (optional)
    accepted = False
    if pulp_status == 'Optimal':
        accepted = True
    elif pulp_status == 'Not Solved' and prob.status == 0:
        # No solution
        accepted = False
    elif pulp_status == 'Undefined' or pulp_status == 'Infeasible' or pulp_status == 'Unbounded':
        accepted = False
    else:
        # if time limit reached maybe the solver returned an incumbent (status may differ)
        # check if variable values exist and are valid:
        # we'll try to extract and validate schedule; if validation passes we accept the incumbent
        accepted = True

    if not accepted:
        print(f"Solver returned status: {pulp_status}. No acceptable solution.")
        return float('inf'), {}

    # Extract schedule (only include x[(op,m)] that are 1)
    schedule = {m: [] for m in machine_list}
    try:
        for (op,m), var in x.items():
            val = var.value()
            if val is not None and val > 0.5:
                j, oi = op
                st = float(s[op].value())
                ct = float(c[op].value())
                schedule[m].append((f"J{j}-O{oi+1}", st, ct))
    except Exception as e:
        print("Error reading solution variables:", e)
        return float('inf'), {}
    ...
    try:
        ...
        print("MILP produced an invalid schedule:", ex)
        return float('inf'), {}

    print(f"MILP solved: makespan = {actual_makespan:.3f}")
