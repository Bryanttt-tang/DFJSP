...
def edd_heuristic(jobs_data, machine_list, arrival_times):
    ...
    # For EDD, we need due dates. Since we don't have them, we'll estimate based on job arrival + total work
    def estimate_due_date(job_id):
        total_work = sum(min(op['proc_times'].values()) for op in jobs_data[job_id])
        return arrival_times[job_id] + total_work * 2  # Conservative due date
    
    def edd_priority(op_data):
        job_id, op_idx, machine, proc_time = op_data
        return estimate_due_date(job_id)  # Earlier due date = higher priority
    
    return _generic_heuristic(jobs_data, machine_list, arrival_times, "EDD", edd_priority)
