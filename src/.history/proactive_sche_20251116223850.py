...
class DispatchingRuleFJSPEnv(gym.Env):
    """
    EVENT-BASED Dynamic FJSP Environment where actions select DISPATCHING RULES.
    
    Similar to Reactive RL in structure: schedules ONE operation per step.
    The key difference is in the ACTION SPACE:
    - Reactive RL: Action directly selects (job, machine) pair
    - Rule-Based RL: Action selects a dispatching RULE, which then chooses (job, machine)
    
    This implements rule-based scheduling from the literature where the agent
    learns to select among predefined heuristic dispatching rules based on system state.
    
    Action Space (7 actions):
    - 0: FIFO (First In First Out) - Schedule oldest arrived job first
    - 1: SPT (Shortest Processing Time) - Schedule job with shortest operation first
    - 2: LPT (Longest Processing Time) - Schedule job with longest operation first
    - 3: MWKR (Most Work Remaining) - Schedule job with most remaining work
    - 4: LWKR (Least Work Remaining) - Schedule job with least remaining work
    - 5: EDD (Earliest Due Date) - Schedule job with earliest estimated due date
    - 6: WAIT - Wait for next event (job arrival or machine completion)
    
    Episode Structure:
    - Each step either schedules ONE operation (via rule) or WAITs
    - Episode length varies based on wait decisions
    - Agent learns WHICH rule to apply AND WHEN to wait
    - More interpretable than direct (job, machine) selection
    
    The agent learns WHEN to apply WHICH rule based on the current system state.
    """
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # ACTION SPACE: 6 dispatching rules + 1 WAIT action
        # 0=FIFO, 1=SPT, 2=LPT, 3=MWKR, 4=LWKR, 5=EDD, 6=WAIT
    ...
    def _reset_state(self):
        ...
        self.operations_scheduled = 0
        self.episode_step = 0
        # EVENT-BASED: Similar to Reactive RL, one operation per step
    ...
    def _apply_dispatching_rule(self, rule_idx, ready_ops):
        """Apply selected dispatching rule to choose which operation to schedule."""
        if not ready_ops:
            return None
        
        if rule_idx == 0:  # FIFO
            return min(ready_ops, key=lambda x: (x['arrival_time'], x['job_id']))
        elif rule_idx == 1:  # SPT
            return min(ready_ops, key=lambda x: (x['proc_time'], x['arrival_time']))
        elif rule_idx == 2:  # LPT
            return max(ready_ops, key=lambda x: (x['proc_time'], -x['arrival_time']))
        elif rule_idx == 3:  # MWKR (Most Work Remaining)
            return max(ready_ops, key=lambda x: (x['work_remaining'], -x['arrival_time']))
        elif rule_idx == 4:  # LWKR (Least Work Remaining)
            return min(ready_ops, key=lambda x: (x['work_remaining'], x['arrival_time']))
        elif rule_idx == 5:  # EDD (Earliest Due Date - estimated)
            def estimated_due_date(op):
                total_work = sum(min(self.jobs[op['job_id']][i]['proc_times'].values())
                               for i in range(len(self.jobs[op['job_id']])))
                return op['arrival_time'] + total_work * 1.5
            return min(ready_ops, key=lambda x: (estimated_due_date(x), x['arrival_time']))
        else:
            return ready_ops[0]  # Default: first available
    ...
    def action_masks(self):
        """Generate action masks - rules available if ready ops exist, WAIT always available."""
        ...
        # Enable dispatching rules if there are ready operations
        if ready_ops:
            for i in range(6):  # Rules 0-5
                mask[i] = True
        
        # WAIT action: Always available if work remains and we're not done
        has_unarrived_jobs = len(self.arrived_jobs) < len(self.job_ids)
        if has_unarrived_jobs or ready_ops:
    ...
    def step(self, action):
        """
        EVENT-BASED SCHEDULING: Schedule ONE operation per step using selected rule OR WAIT.
        
        Similar to Reactive RL, each step either schedules exactly one operation or waits.
        Actions 0-5: Select a dispatching rule to choose and schedule one operation
        Action 6: WAIT for next event (job arrival or machine completion)
        """
        self.episode_step += 1
        previous_makespan = self.current_makespan
        ...
        # Check termination
        if self.operations_scheduled >= self.total_operations:
            return self._get_observation(), -self.current_makespan, True, False, {"makespan": self.current_makespan}
        
        if self.episode_step >= self.max_episode_steps:
            return self._get_observation(), -self.current_makespan - 1000, True, False, {"error": "timeout"}
        ...
        if not ready_ops:
            # No ready operations - this shouldn't happen if action masks are correct
            # But handle gracefully by advancing time
            next_event_time = self._get_next_event_time()
            if next_event_time > self.event_time and next_event_time != float('inf'):
                self._update_event_time_and_arrivals(next_event_time)
                self.event_time = next_event_time
            ...
            reward = -(self.current_makespan - previous_makespan)
            terminated = self.operations_scheduled >= self.total_operations
            return self._get_observation(), reward, terminated, False, {"info": "no_ready_ops_forced_wait"}
        
        # Apply selected dispatching rule to choose ONE operation
        selected_op = self._apply_dispatching_rule(action, ready_ops)
        
        if selected_op is None:
            return self._get_observation(), -10.0, False, False, {"error": "rule_failed"}
        ...
        # Save machine state before scheduling
        machine_free_before = self.machine_next_free[machine]
        
        # Calculate start time
        machine_avail = self.machine_next_free[machine]
        job_ready = selected_op['job_ready_time']
        start_time = max(machine_avail, job_ready, self.event_time)
        end_time = start_time + proc_time
        ...
        # Event time advancement logic (same as PoissonDynamicFJSPEnv)
        if machine_free_before <= self.event_time:
            other_idle_exists = any(
                ft <= self.event_time
                for m, ft in self.machine_next_free.items()
                if m != machine
            )
            if not other_idle_exists:
                next_event = self._get_next_event_time()
                if next_event > self.event_time and next_event != float('inf'):
                    self.event_time = next_event
                    self._update_event_time_and_arrivals(self.event_time)
        
        reward = -(self.current_makespan - previous_makespan)
        terminated = self.operations_scheduled >= self.total_operations
        
        return self._get_observation(), reward, terminated, False, {"makespan": self.current_makespan}
    ...
    def _get_observation(self):
        ...
        # 3. Machine utilization (time until free / horizon)
        for machine in self.machines:
            free_time = self.machine_next_free[machine]
            utilization = min(1.0, (free_time - self.event_time) / self.max_time_horizon)
            obs.append(max(0.0, utilization))
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        self.wait_durations = [5.0, 10.0, float('inf')]  # 10 units or next event
        num_wait_actions = len(self.wait_durations)
        ...
        obs_size = (
            ...
            len(self.job_ids) +                      # NEW: actual arrival times (normalized, 1.0 if not arrived)
            2                                        # NEW: Estimated arrival rate
        )
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(obs_size,), dtype=np.float32
        )
    ...
    def action_masks(self):
        ...
        # Wait actions: Only enable if there are unarrived jobs OR schedulable work available
        # In Builder Mode, precedence is handled by start_time calculation, not masking
        # So if job arrived, its next op is always "ready" (schedulable at appropriate time)
        has_unarrived_jobs = len(self.arrived_jobs) < len(self.job_ids)
...
def evaluate_rule_based_on_dynamic(rule_model, jobs_data, machine_list, arrival_times, 
    ...
    # Track which rules were selected
    rule_counts = {
        0: 0,  # FIFO
        1: 0,  # SPT
        2: 0,  # LPT
    ...
    rule_dist = {rule_names[i]: f"{rule_counts[i]} ({rule_counts[i]/total_actions*100:.1f}%)" 
                 for i in range(7) if rule_counts[i] > 0}
    print(f"  📊 Rule-Based RL distribution: {rule_dist}")
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    while step_count < max_steps:
        ...
        if action >= test_env.env.wait_action_start:
            ...
            if step_count < 10 or wait_duration != float('inf'):  # Only log first 10 or non-inf waits
                print(f"    Step {step_count}: Wait {wait_duration} units")
        else:
            schedule_count += 1
        ...
        if done or truncated:
            print(f"    Episode completed at step {step_count} (scheduled={schedule_count}, waited={wait_count})")
            break
    
    if step_count >= max_steps:
        print(f"    ⚠️  Hit max steps ({max_steps})! Scheduled {schedule_count} ops, waited {wait_count} times")
    
    print(f"  📊 Proactive RL action breakdown: {schedule_count} scheduling, {wait_count} waits ({wait_count/(schedule_count+wait_count)*100:.1f}% waits)")
...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, rule):
    """
    Event-driven list scheduling for FJSP with dispatching rules.
    
    ARCHITECTURE (Sequencing vs Routing):
    - Sequencing: Determined by 'rule' parameter (FIFO, LIFO, SPT, LPT, EDD)
                 Decides WHICH operation to schedule next from ready operations
    - Routing: Fixed to SPT (each operation assigned to fastest compatible machine)
              Decides WHICH machine to assign the selected operation to
    
    This separation allows testing different sequencing strategies while
    keeping routing consistent. For more flexibility, routing could be
    parameterized separately (future enhancement).
    
    EVENT-DRIVEN SIMULATION:
    - Maintains simulation time and advances to next event (arrival or completion)
    - At each time point, identifies ready operations (arrived + precedence satisfied)
    - Applies dispatching rule to select one operation to schedule
    - Updates machine availability and job state
    - No look-ahead: decisions made greedily based on current state
    
    Args:
        jobs_data: Job definitions with operations and processing times
        machine_list: Available machines
        arrival_times: When each job arrives (dynamic scenario)
        rule: Sequencing rule name (FIFO, LIFO, SPT, LPT, EDD)
    ...
    while completed_operations < total_operations:
        ...
        if not ready_operations:
            ...
            # Check for next job arrival or operation ready time
            for job_id in jobs_data.keys():
        ...
        # Don't update sim_time here - it advances in the ready_operations check
...
def run_heuristic_comparison(jobs_data, machine_list, arrival_times):
    ...
    NOTE: All heuristics use SPT (Shortest Processing Time) for machine routing
    and differ only in job sequencing strategy.
    """
    # Direct mapping of names to heuristic functions (no confusing lambdas)
    heuristics = {
        'FIFO': fifo_heuristic,
        'LIFO': lifo_heuristic, 
        'SPT': spt_heuristic,
    ...
    for name, heuristic_func in heuristics.items():
        try:
            makespan, schedule = heuristic_func(jobs_data, machine_list, arrival_times)
...
def fifo_heuristic(jobs_data, machine_list, arrival_times):
    ...
    Sequencing: Jobs ordered by arrival time (earliest first)
    Routing: SPT (assign each operation to fastest machine)
    """
...
def lifo_heuristic(jobs_data, machine_list, arrival_times):
    ...
    Sequencing: Jobs ordered by arrival time (latest first)
    Routing: SPT (assign each operation to fastest machine)
    """
...
def spt_heuristic(jobs_data, machine_list, arrival_times):
    ...
    Sequencing: Operations ordered by processing time (shortest first)
    Routing: SPT (assign each operation to fastest machine)
    """
...
def lpt_heuristic(jobs_data, machine_list, arrival_times): 
    ...
    Sequencing: Operations ordered by processing time (longest first)
    Routing: SPT (assign each operation to fastest machine)
    """
...
def edd_heuristic(jobs_data, machine_list, arrival_times):
    """
    EDD (Earliest Due Date) - Prioritize jobs with earliest estimated due dates.
    
    Sequencing: Jobs ordered by estimated due date (earliest first)
    Due date estimation: arrival_time + 1.5 × total_work_remaining
    where total_work = sum of minimum processing times across all operations
    
    Routing: SPT (assign each operation to fastest machine)
    
    Note: Since actual due dates are not provided, we estimate them based on
    arrival time plus a conservative estimate of total processing time.
    """
...
def best_heuristic(jobs_data, machine_list, arrival_times):
    """
    Run comparison of all dispatching heuristics and return the best performer.
    
    Compares: FIFO, LIFO, SPT, LPT, EDD
    All use SPT routing (fastest machine) and differ only in job sequencing.
    
    Returns: (makespan, schedule) of the best-performing heuristic
    """
    print(f"  Comparing FIFO, LIFO, SPT, LPT, EDD heuristics with arrival times: {arrival_times}")
...
def main():
    ...
    dynamic_timesteps = 400000   # Increased for better learning with integer timing 
    proactive_timesteps = 400000 
    static_timesteps = 200000    # Increased for better learning
    learning_rate = 3e-4         # Optimized learning rate for PPO (was 10e-4)
  ...
  # Same as reactive RL for fair comparison
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        schedules_for_comparison = [
            ...
            ("Proactive RL", proactive_schedule),
            ("Reactive RL", dynamic_schedule),
            ("Rule-Based RL", rule_based_schedule),
        ...
        for i_method, (method1, sched1) in enumerate(schedules_for_comparison):
            for j_method, (method2, sched2) in enumerate(schedules_for_comparison[i_method+1:], i_method+1):
                if sched1 and sched2 and schedules_identical(sched1, sched2):
                    ...
                    print(f"       This suggests both methods are using the same greedy policy.")
                    print(f"       For Proactive RL vs Reactive RL: Check if wait actions are being used strategically.")
