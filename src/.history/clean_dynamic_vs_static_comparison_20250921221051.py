...
class StaticFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        Components:
        1. Machine next-free time (absolute, normalized)
        2. Next operation for each job (normalized by max_ops_per_job)
        3. Job next-operation-ready-time (absolute, normalized)
        4. Current makespan/time (unnormalized absolute value)
        5. Job arrival times (static: all 0, normalized)
        """
        obs = []
        
        # Calculate estimated maximum possible makespan for normalization
        # Sum all processing times as upper bound estimate
        total_proc_time = 0
        for job_data in self.jobs.values():
            for op in job_data:
                # Use maximum processing time across machines for each operation
                max_proc_time = max(op['proc_times'].values()) if op['proc_times'] else 0
                total_proc_time += max_proc_time
        
        # Use total processing time as normalization scale (upper bound estimate)
        time_scale = max(total_proc_time, 1.0)
        
        # 1. Machine next-free time (absolute, normalized)
        for machine in self.machines:
            machine_free_time = self.machine_next_free[machine]
            normalized_time = min(1.0, machine_free_time / time_scale)
        ...
        for job_id in self.job_ids:
            ...
            if next_op_idx < len(self.jobs[job_id]):
                ...
                normalized_ready_time = min(1.0, job_ready_time / time_scale)
        ...
        # 5. Job arrival times (static: all jobs arrive at t=0, normalized)
        for job_id in self.job_ids:
            # For static environment, all arrival times are 0 (normalized)
            obs.append(0.0)
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        for job_id in self.job_ids:
            ...
            normalized_arrival = min(1.0, arrival_time / time_scale)
...
def plot_training_metrics():
    ...
    if TRAINING_METRICS['policy_loss']:
        axes[0, 1].plot(timesteps[:len(TRAINING_METRICS['policy_loss'])], TRAINING METRICS['policy_loss'], 'r-', linewidth=2)
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    """Shortest Processing Time heuristic for dynamic job arrivals."""
    ...
    # Create list of all operations with their processing times
    operations = []
    for job_id in jobs_data.keys():
        for op_idx, op_data in enumerate(jobs_data[job_id]):
            min_proc_time = min(op_data['proc_times'].values())
            operations.append((job_id, op_idx, min_proc_time, arrival_times.get(job_id, 0.0)))
    ...
    for job_id, op_idx, _, job_arrival_time in operations:
        # Check if job has arrived
        if current_time < job_arrival_time:
            current_time = job_arrival_time
        
        # Check if previous operations are completed
        if op_idx > 0 and not completed_ops[job_id][op_idx - 1]:
            continue
        ...
        job_ready = operation_end_times[job_id][op_idx - 1] if op_idx > 0 else job_arrival_time
        start_time = max(machine_available, job_ready, current_time)
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times):
    """MILP-based optimal scheduler with arrival time constraints."""
    try:
        # Create MILP problem
        prob = LpProblem("FJSP_Makespan", LpMinimize)
        
        # Calculate big M
        total_proc_time = sum(max(op['proc_times'].values()) for job_ops in jobs_data.values() for op in job_ops)
        big_M = total_proc_time * 2
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                start_vars[(job_id, op_idx)] = LpVariable(f"start_{job_id}_{op_idx}", lowBound=0)
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                for machine in jobs_data[job_id][op_idx]['proc_times'].keys():
                    assign_vars[(job_id, op_idx, machine)] = LpVariable(
                        f"assign_{job_id}_{op_idx}_{machine}", cat='Binary')
        
        # Precedence ordering variables for machine conflicts
        order_vars = {}
            ...
            for job_id in jobs_data.keys():
                for op_idx in range(len(jobs_data[job_id])):
                    if machine in jobs_data[job_id][op_idx]['proc_times']:
                        ops_on_machine.append((job_id, op_idx))
            
            for i, op1 in enumerate(ops_on_machine):
                for j, op2 in enumerate(ops_on_machine):
                    if i < j:
                        order_vars[(op1, op2, machine)] = LpVariable(
                            f"order_{op1[0]}_{op1[1]}_{op2[0]}_{op2[1]}_{machine}", cat='Binary')
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                prob += lpSum([assign_vars[(job_id, op_idx, machine)] 
                              for machine in jobs_data[job_id][op_idx]['proc_times'].keys()]) == 1
        
        # 2. Precedence constraints within jobs
        for job_id in jobs_data.keys():
            for op_idx in range(1, len(jobs_data[job_id])):
                prev_op_end = start_vars[(job_id, op_idx-1)] + lpSum([
                    assign_vars[(job_id, op_idx-1, machine)] * jobs_data[job_id][op_idx-1]['proc_times'][machine]
                    for machine in jobs_data[job_id][op_idx-1]['proc_times'].keys()
                ])
                prob += start_vars[(job_id, op_idx)] >= prev_op_end
        
        # 3. Arrival time constraints
        for job_id in jobs_data.keys():
            prob += start_vars[(job_id, 0)] >= arrival_times.get(job_id, 0.0)
        
        # 4. Machine capacity constraints (no overlap)
        for machine in machine_list:
            ops_on_machine = []
            for job_id in jobs_data.keys():
                for op_idx in range(len(jobs_data[job_id])):
            ...
            for i, op1 in enumerate(ops_on_machine):
                for j, op2 in enumerate(ops_on_machine):
                    if i < j:
                        ...
                        # If both operations are assigned to this machine, they can't overlap
                        proc_time1 = jobs_data[job1][op1_idx]['proc_times'][machine]
                        proc_time2 = jobs_data[job2][op2_idx]['proc_times'][machine]
                        
                        # op1 before op2
                        prob += (start_vars[(job2, op2_idx)] >= 
                                start_vars[(job1, op1_idx)] + proc_time1 - 
                                big_M * (3 - assign_vars[(job1, op1_idx, machine)] - 
                                        assign_vars[(job2, op2_idx, machine)] - 
                                        order_vars[(op1, op2, machine)]))
                        
                        # op2 before op1  
                        prob += (start_vars[(job1, op1_idx)] >= 
                                start_vars[(job2, op2_idx)] + proc_time2 - 
                                big_M * (2 - assign_vars[(job1, op1_idx, machine)] - 
                                        assign_vars[(job2, op2_idx, machine)] + 
                                        order_vars[(op1, op2, machine)]))
        ...
        for job_id in jobs_data.keys():
            ...
            last_op_end = start_vars[(job_id, last_op_idx)] + lpSum([
                assign_vars[(job_id, last_op_idx, machine)] * jobs_data[job_id][last_op_idx]['proc_times'][machine]
                for machine in jobs_data[job_id][last_op_idx]['proc_times'].keys()
            ])
            prob += makespan >= last_op_end
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                start_time = start_vars[(job_id, op_idx)].varValue
                
                # Find assigned machine
                assigned_machine = None
                for machine in jobs_data[job_id][op_idx]['proc_times'].keys():
                    if assign_vars[(job_id, op_idx, machine)].varValue > 0.5:
                ...
                if assigned_machine:
                    ...
                    end_time = start_time + proc_time
                    schedule[assigned_machine].append((f"J{job_id}-O{op_idx+1}", start_time, end_time))
...
def calculate_regret_analysis(optimal_makespan, methods_results):
    ...
    for method, makespan in methods_results.items():
        if makespan != float('inf') and optimal_makespan != float('inf'):
            ...
            print(f"{method:25s}: {makespan:.2f} (regret: +{regret:.1f}%)")
        else:
            regret_results[method] = float('inf')
            print(f"{method:25s}: Failed or no solution")
...
def diagnose_performance_similarity(perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan):
    """Diagnose why different methods might have similar performance."""
    print("\n📊 PERFORMANCE SIMILARITY DIAGNOSIS:")
    
    # Check for identical results (potential bugs)
    if abs(perfect_makespan - dynamic_makespan) < 0.01:
        print("🚨 WARNING: Perfect Knowledge RL and Dynamic RL have identical results!")
        print("   This suggests Dynamic RL evaluation may be using perfect information")
    
    if abs(dynamic_makespan - static_makespan) < 0.01:
        print("🚨 WARNING: Dynamic RL and Static RL have identical results!")
        print("   This suggests arrival time differences aren't impacting the schedule")
    
    if abs(perfect_makespan - heuristic_makespan) < 0.01:
        print("🚨 WARNING: Perfect Knowledge RL matches heuristic performance!")
        print("   This suggests the problem may be too easy or RL training insufficient")
    
    # Performance spread analysis
    all_makespans = [perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan]
    valid_makespans = [m for m in all_makespans if m != float('inf')]
    
    if len(valid_makespans) > 1:
        min_makespan = min(valid_makespans)
        max_makespan = max(valid_makespans)
        spread = max_makespan - min_makespan
        relative_spread = (spread / min_makespan) * 100 if min_makespan > 0 else 0
        
        print(f"📈 Performance spread: {spread:.2f} time units ({relative_spread:.1f}%)")
        
        if relative_spread < 5:
            print("   ⚠️  Low performance variation - methods are performing very similarly")
            print("   Possible reasons:")
            print("   - Problem constraints dominate (arrival times don't matter much)")
            print("   - All methods finding similar good solutions")
            print("   - Insufficient training for RL methods")
        elif relative_spread > 20:
            print("   ✅ Good performance variation - clear differentiation between methods")
        else:
            print("   🟡 Moderate performance variation - some differentiation visible")
    else:
        print("📉 Not enough valid makespan data for spread analysis")
        if len(valid_makespans) == 1:
            print("  Only one valid makespan found - others may have failed or no solution")
        else:
            print("  All methods failed to produce a valid makespan")
...
def main():
    ...
    test_scenarios = generate_test_scenarios(ENHANCED_JOBS_DATA, 
                                           initial_jobs=[0, 1, 2], 
                                           arrival_rate=arrival_rate, 
                                           num_scenarios=3)  # Reduced for testing
    ...
    print("- Changed from relative timing to absolute timing")  
    print("- Consistent normalization using estimated max makespan")
    print("- Current makespan kept unnormalized for temporal context")
    print("- All three RL environments updated")
    
    print("\n🎯 Ready to run with improved event-based scheduling observations!")
    print("=" * 80)
