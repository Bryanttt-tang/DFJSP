...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        # EVENT-DRIVEN: Separate event time for WAIT and arrival visibility
    ...
    def _advance_to_next_arrival(self):
        """WAIT ACTION: Advance event_time to next job arrival or next machine completion."""
        ...
        for machine, next_free_time in self.machine_next_free.items():
            if next_free_time > self.event_time:
                next_machine_completion = min(next_machine_completion, next_free_time)
        
        # Choose the earliest event (arrival or machine completion)
        ...
        if next_event_time != float('inf'):
            ...
            # If it's an arrival event, add newly arrived jobs
            if next_event_time == next_arrival_time:
                ...
                # Machine completion event - no new arrivals but time advances
    ...
    def step(self, action):
        """BUILDER MODE: Step function with proper event-driven WAIT action support."""
        self.episode_step += 1
        ...
        # Earliest feasible start = max of all constraints (including event_time for proper ordering)
        ...
        # Advance event_time to at least the start time of this operation
    ...
    def _get_observation(self):
        ...
        # Time since last arrival (relative to event_time)
        ...
        # Current event time (time context)
...
class TrainingCallback:
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
            ...
            # DEBUG: Print all available log keys more frequently to catch episode rewards
            if self.step_count % 50 == 0:  # Print every 50 calls to catch episode rewards
                print(f"\n[DEBUG {self.method_name}] Step {self.step_count} - Available log keys: {list(log_data.keys())}")
            ...
            # ENHANCED: Log episode rewards and lengths from rollout data with more detailed debugging
            if 'rollout/ep_rew_mean' in log_data:
                ...
                print(f"[EPISODE REWARD {self.method_name}] Step {self.step_count}: {ep_reward:.4f} (episode {self.episode_count})")
            ...
            if 'rollout/ep_len_mean' in log_data:
                ...
                print(f"[EPISODE LENGTH {self.method_name}] Step {self.step_count}: {ep_length:.2f}")
