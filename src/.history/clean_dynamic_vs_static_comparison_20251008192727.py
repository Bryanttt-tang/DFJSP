...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        print(f"[{self.method_name}] Training started, initialized metrics")
    ...
    def _on_rollout_end(self) -> None:
        ...
        if self.verbose >= 1:
            print(f"[{self.method_name}] _on_rollout_end called at timestep {self.model.num_timesteps}")
        ...
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            ...
            # DEBUG: Print all available keys if verbose
            if self.verbose >= 1:
                available_keys = list(log_data.keys())
                print(f"[{self.method_name}] Available logger keys: {available_keys}")
            ...
            if 'rollout/ep_rew_mean' in log_data:
                ...
                if self.verbose >= 1:
                    print(f"[{self.method_name}] ✅ Episode reward captured: {ep_reward:.4f} (total captured: {len(TRAINING_METRICS['episode_rewards'])})")
            else:
                if self.verbose >= 1:
            ...
            if 'rollout/ep_len_mean' in log_data:
                ...
                if self.verbose >= 1:
                    print(f"[{self.method_name}] Episode length captured: {log_data['rollout/ep_len_mean']:.2f}")
            ...
            if 'train/policy_gradient_loss' in log_data:
                ...
                if self.verbose >= 1:
                    print(f"[{self.method_name}] Policy loss captured: {log_data['train/policy_gradient_loss']:.6f}")
                
            if 'train/value_loss' in log_data:
                TRAINING_METRICS['value_loss'].append(log_data['train/value_loss'])
                if self.verbose >= 1:
                    print(f"[{self.method_name}] Value loss captured: {log_data['train/value_loss']:.6f}")
            ...
            if self.verbose >= 1:
                ...
                print(f"[{self.method_name}] Model logger exists: {hasattr(self.model, 'logger')}")
                if hasattr(self.model, 'logger'):
                    print(f"[{self.method_name}] Logger has name_to_value: {hasattr(self.model.logger, 'name_to_value')}")
    
    def _on_step(self) -> bool:
        """Update progress bar if available and capture metrics more frequently."""
        if self.pbar is not None:
            self.pbar.n = self.model.num_timesteps
            self.pbar.refresh()
            
        # Also try to capture metrics on every step (as backup)
        if self.model.num_timesteps > self.last_timestep + 512:  # Every 512 steps (roughly a rollout)
            self.last_timestep = self.model.num_timesteps
            if self.verbose >= 1 and hasattr(self.model, 'logger'):
                if hasattr(self.model.logger, 'name_to_value'):
                    log_data = self.model.logger.name_to_value
                    if 'rollout/ep_rew_mean' in log_data:
                        print(f"[{self.method_name}] Step {self.model.num_timesteps}: ep_rew_mean = {log_data['rollout/ep_rew_mean']:.4f}")
...
def plot_training_metrics():
    ...
    print(f"Total training steps recorded: {len(TRAINING_METRICS.get('timesteps', []))}")
    ...
    for key, value in TRAINING_METRICS.items():
        if isinstance(value, list):
            ...
            if len(value) > 0:
                print(f"    Sample: {value[:3]} ... {value[-2:] if len(value) > 3 else []}")
        else:
            print(f"  {key}: {value}")
    ...
    if TRAINING_METRICS.get('value_loss'):
        ...
        print(f"✅ Value loss captured successfully!")
    else:
        print("❌ NO VALUE LOSS RECORDED!")
    
    # Policy loss diagnostic  
    if TRAINING_METRICS.get('policy_loss'):
        policy_losses = TRAINING_METRICS['policy_loss']
        print(f"Policy loss sample (last 5): {policy_losses[-5:]}")
        print(f"Policy loss range: {min(policy_losses):.4f} to {max(policy_losses):.4f}")
        print(f"✅ Policy loss captured successfully!")
    else:
        print("❌ NO POLICY LOSS RECORDED!")
    
    # Try to create plots only if we have enough data
    try:
        import matplotlib.pyplot as plt
        ...
        if TRAINING_METRICS.get('episode_rewards') and len(TRAINING_METRICS['episode_rewards']) > 0:
            ep_rewards = TRAINING_METRICS['episode_rewards']
            ep_timesteps = timesteps[:len(ep_rewards)] if len(timesteps) >= len(ep_rewards) else list(range(len(ep_rewards)))
            
            print(f"[DEBUG] Plotting {len(ep_rewards)} episode rewards")
            
            axes[0, 0].plot(ep_timesteps, ep_rewards, 'g-', linewidth=2, alpha=0.7, label='Episode Reward')
            axes[0, 0].set_title('Episode Rewards Over Time', fontweight='bold')
            axes[0, 0].set_xlabel('Training Steps')
            axes[0, 0].set_ylabel('Mean Episode Reward')
            axes[0, 0].grid(True, alpha=0.3)
            axes[0, 0].legend()
        else:
            axes[0, 0].text(0.5, 0.5, 'No episode reward data available',
                           ha='center', va='center', transform=axes[0, 0].transAxes, fontsize=12)
            axes[0, 0].set_title('Episode Rewards (No Data)', fontweight='bold')
        
        # Plot 2: Value Loss (Top Right)
        if TRAINING_METRICS.get('value_loss') and len(TRAINING_METRICS['value_loss']) > 0:
            value_losses = TRAINING_METRICS['value_loss']
            loss_timesteps = timesteps[:len(value_losses)] if len(timesteps) >= len(value_losses) else list(range(len(value_losses)))
            axes[0, 1].plot(loss_timesteps, value_losses, 'r-', linewidth=2, alpha=0.7)
            axes[0, 1].set_title('Value Loss Over Time', fontweight='bold')
            axes[0, 1].set_xlabel('Training Steps')
            axes[0, 1].set_ylabel('Value Loss')
            axes[0, 1].grid(True, alpha=0.3)
        else:
            axes[0, 1].text(0.5, 0.5, 'No value loss data available',
                           ha='center', va='center', transform=axes[0, 1].transAxes, fontsize=12)
            axes[0, 1].set_title('Value Loss (No Data)', fontweight='bold')
        
        # Plot 3: Policy Loss (Bottom Left)
        if TRAINING_METRICS.get('policy_loss') and len(TRAINING_METRICS['policy_loss']) > 0:
            policy_losses = TRAINING_METRICS['policy_loss']
            loss_timesteps = timesteps[:len(policy_losses)] if len(timesteps) >= len(policy_losses) else list(range(len(policy_losses)))
            axes[1, 0].plot(loss_timesteps, policy_losses, 'b-', linewidth=2, alpha=0.7)
            axes[1, 0].set_title('Policy Loss Over Time', fontweight='bold')
            axes[1, 0].set_xlabel('Training Steps')
            axes[1, 0].set_ylabel('Policy Loss')
            axes[1, 0].grid(True, alpha=0.3)
        else:
            axes[1, 0].text(0.5, 0.5, 'No policy loss data available',
                           ha='center', va='center', transform=axes[1, 0].transAxes, fontsize=12)
            axes[1, 0].set_title('Policy Loss (No Data)', fontweight='bold')
        
        # Plot 4: Episode Lengths (Bottom Right)
        if TRAINING_METRICS.get('episode_lengths') and len(TRAINING_METRICS['episode_lengths']) > 0:
            ep_lengths = TRAINING_METRICS['episode_lengths']
            length_timesteps = timesteps[:len(ep_lengths)] if len(timesteps) >= len(ep_lengths) else list(range(len(ep_lengths)))
            axes[1, 1].plot(length_timesteps, ep_lengths, 'm-', linewidth=2, alpha=0.7)
            axes[1, 1].set_title('Episode Lengths Over Time', fontweight='bold')
            axes[1, 1].set_xlabel('Training Steps')
            axes[1, 1].set_ylabel('Episode Length')
            axes[1, 1].grid(True, alpha=0.3)
        else:
            axes[1, 1].text(0.5, 0.5, 'No episode length data available',
                           ha='center', va='center', transform=axes[1, 1].transAxes, fontsize=12)
            axes[1, 1].set_title('Episode Lengths (No Data)', fontweight='bold')
        
        # Save and show plot
        plt.tight_layout()
        plt.savefig('training_metrics.png', dpi=300, bbox_inches='tight')
        print(f"✅ Training metrics plot saved as 'training_metrics.png'")
        plt.show()
