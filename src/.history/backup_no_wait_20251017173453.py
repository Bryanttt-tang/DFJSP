...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    ...
    if os.path.exists(cache_file):
        try:
            ...
            if verbose:
                print(f"✅ Found cached MILP result (makespan={cached_result[0]:.2f})")
            return cached_result
        except Exception as e:
            if verbose:
                print(f"⚠️  Cache file corrupted, recomputing: {e}")
            # Continue to solve fresh
    
    # No cache found or cache invalid - solve fresh
    if verbose:
        print(f"🔍 No cache found, solving MILP optimization...")
    ...
    if status == "Optimal" or status == "Not Solved" or status == "Optimal":  # Prefer explicit 'Optimal' but accept near-optimal?
        ...
        if math.isfinite(actual_makespan) and abs(actual_makespan - optimal_makespan) <= 1e-6:
            ...
            try:
                ...
                if verbose:
                    print(f"💾 Cached MILP result to {cache_file}")
            except Exception as e:
                if verbose:
