...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def _get_ready_operations(self):
        """
        Get list of operations that can be scheduled (BUILDER MODE).
        In builder mode, an operation is schedulable if:
        1. Job has arrived
        2. Job is not completed
        3. It's the next operation in the job (precedence satisfied by start_time calculation)
        
        No event_time check - machines always available, just with different free times.
        """
        ready_ops = []
        ...
        for job_id in self.arrived_jobs:
            ...
            if op_idx < len(self.jobs[job_id]):
                ...
                # Job ready time (for precedence - used in scheduling, not filtering)
    ...
    def _apply_rule_combination(self, action_idx, ready_ops):
        ...
        def routing_score(machine):
            ...
            start_time = max(machine_avail, job_ready)  # Builder mode - no event_time
    ...
    def step(self, action):
        """
        BUILDER MODE SCHEDULING: Schedule ONE operation per step using selected rule combination.
        
        Builder mode characteristics:
        - Machines always available (just with different free times)
        - No wait actions - always schedule next operation
        - Operations placed at earliest feasible time considering:
          * Machine availability (machine_next_free)
          * Job precedence (previous operation must complete first)
          * Job arrival time (can't schedule before job arrives)
        
        Action: Index 0-9 selecting one of 10 rule combinations (5 sequencing × 2 routing)
        """
        self.episode_step += 1
        previous_makespan = self.current_makespan
        ...
        # Discover new arrivals up to current makespan (builder mode discovery)
        # In builder mode, we discover jobs as makespan advances
        for job_id, arr_time in self.job_arrival_times.items():
            if job_id not in self.arrived_jobs and arr_time <= self.current_makespan:
                self.arrived_jobs.add(job_id)
        
        # Get schedulable operations (arrived jobs with next operation unscheduled)
        ready_ops = self._get_ready_operations()
        
        if not ready_ops:
            # All arrived jobs scheduled - advance makespan to next arrival or machine completion
            next_arrival = float('inf')
            for job_id, arr_time in self.job_arrival_times.items():
                if job_id not in self.arrived_jobs:
                    next_arrival = min(next_arrival, arr_time)
            
            next_machine_completion = min(self.machine_next_free.values()) if self.machine_next_free else float('inf')
            next_event = min(next_arrival, next_machine_completion)
            
            if next_event != float('inf') and next_event > self.current_makespan:
                self.current_makespan = next_event
                # Discover new arrivals at this makespan
                for job_id, arr_time in self.job_arrival_times.items():
                    if job_id not in self.arrived_jobs and arr_time <= self.current_makespan:
                        self.arrived_jobs.add(job_id)
                # Retry getting operations
                ready_ops = self._get_ready_operations()
            
            if not ready_ops:
                # Still no ops - return with penalty
                reward = -(self.current_makespan - previous_makespan)
                terminated = self.operations_scheduled >= self.total_operations
        ...
        # Schedule the selected operation on the selected machine (BUILDER MODE)
        job_id = selected_op['job_id']
        op_idx = selected_op['op_idx']
        operation = selected_op['operation']
        proc_time = operation['proc_times'][best_machine]
        
        # Calculate earliest start time (builder mode - no event_time constraint)
        machine_avail = self.machine_next_free[best_machine]
        job_ready = selected_op['job_ready_time']  # Considers arrival + precedence
        start_time = max(machine_avail, job_ready)
        end_time = start_time + proc_time
        ...
        # Update makespan (builder clock)
        self.current_makespan = max(self.machine_next_free.values())
        
        # Discover any jobs that arrived within new makespan
        for job_id, arr_time in self.job_arrival_times.items():
            if job_id not in self.arrived_jobs and arr_time <= self.current_makespan:
                self.arrived_jobs.add(job_id)
...
def evaluate_rule_based_on_dynamic(rule_model, jobs_data, machine_list, arrival_times, 
    ...
    # Track which rules were selected (10 combinations now)
    rule_counts = {i: 0 for i in range(10)}
    ...
    rule_dist = {rule_names[i]: f"{rule_counts[i]} ({rule_counts[i]/total_actions*100:.1f}%)" 
                 for i in range(10) if rule_counts[i] > 0}
