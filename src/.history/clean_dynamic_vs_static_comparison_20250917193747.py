...
def plot_gantt_chart_consistent_scale(schedule, makespan, title, max_scale=None):
    """
    Plot Gantt chart with consistent scale across different plots for fair comparison.
    
    Args:
        schedule: Dictionary with machine schedules
        makespan: The makespan of this schedule
        title: Title for the plot
        max_scale: Maximum time scale to use (if None, uses makespan + 10% buffer)
    """
    if not schedule or not any(schedule.values()):
        print(f"No schedule data available for {title}")
        return
    
    # Set consistent scale - use max_scale if provided, otherwise add buffer to makespan
    if max_scale is None:
        time_limit = makespan * 1.1  # Add 10% buffer
    else:
        time_limit = max_scale
    ...
    # Add makespan line
    plt.axvline(x=makespan, color='red', linestyle='--', alpha=0.7, 
