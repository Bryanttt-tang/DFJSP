...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # Enhanced relative timing observation space
        obs_size = (
...
def create_deterministic_test_scenario():
    """
    Create the exact deterministic scenario from test3_backup.py that achieved makespan=43.
    This serves as our benchmark for dynamic RL performance.
    """
    return {
        'scenario_id': 'deterministic_benchmark',
        'arrival_times': DETERMINISTIC_ARRIVAL_TIMES.copy(),
        'initial_jobs': [0, 1, 2],  # Jobs arriving at t=0
        'arrival_rate': None,  # Deterministic, not stochastic
        'seed': 42
    }
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    dynamic_timesteps = 100000  # Reduced for smaller 7-job dataset test
    ...
    # Step 2: Use deterministic benchmark scenario (matching test3_backup.py success case)
    print("\n2. USING DETERMINISTIC BENCHMARK SCENARIO")
    print("-" * 40)
    print("Using exact scenario from test3_backup.py that achieved makespan=43")
    deterministic_scenario = create_deterministic_test_scenario()
    print(f"Deterministic arrivals: {deterministic_scenario['arrival_times']}")
    print("Expected: Dynamic RL ~43, Static RL ~50, SPT ~45-50")
    ...
    # Use deterministic benchmark scenario for all evaluations
    benchmark_arrivals = deterministic_scenario['arrival_times']
    
    print(f"Benchmark scenario arrivals: {benchmark_arrivals}")
    
    # Static RL on deterministic scenario
    print("Evaluating Static RL on deterministic benchmark...")
    static_makespan, static_schedule = evaluate_static_on_dynamic(
        static_model, ENHANCED_JOBS_DATA, MACHINE_LIST, benchmark_arrivals)
    
    # Dynamic RL on deterministic scenario
    print("Evaluating Dynamic RL on deterministic benchmark...")
    dynamic_makespan, dynamic_schedule = evaluate_dynamic_on_dynamic(
        dynamic_model, ENHANCED_JOBS_DATA, MACHINE_LIST, benchmark_arrivals)
    
    # SPT Heuristic on deterministic scenario
    print("Evaluating SPT Heuristic on deterministic benchmark...")
    spt_makespan, spt_schedule = spt_heuristic_poisson(ENHANCED_JOBS_DATA, MACHINE_LIST, benchmark_arrivals)
    ...
    colors = plt.cm.tab20.colors
    schedules_data = [
        {'schedule': dynamic_schedule, 'makespan': dynamic_makespan, 'title': 'Dynamic RL', 'arrival_times': benchmark_arrivals},
        {'schedule': static_schedule, 'makespan': static_makespan, 'title': 'Static RL', 'arrival_times': benchmark_arrivals},
        {'schedule': spt_schedule, 'makespan': spt_makespan, 'title': 'SPT Heuristic', 'arrival_times': benchmark_arrivals}
    ]
