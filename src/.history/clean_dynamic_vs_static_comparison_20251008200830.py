...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            ...
            # DEBUG: Print all available keys (only first few times)
            ...
            # Track episode count from environment statistics
            current_num_episodes = 0
            if hasattr(self.model.env, 'get_attr'):
                try:
                    # Get episode count from VecMonitor
                    episode_counts = self.model.env.get_attr('episode_count')
                    if episode_counts and len(episode_counts) > 0:
                        current_num_episodes = episode_counts[0]
                except:
                    pass
            ...
            if 'rollout/ep_rew_mean' in log_data:
                ...
                # Use episode count if available, otherwise increment our counter
                if current_num_episodes > self.last_num_episodes:
                    self.episode_count = current_num_episodes
                    self.last_num_episodes = current_num_episodes
                else:
...
def plot_training_metrics():
    ...
    # Plot 3: Value Loss over Episodes (Middle Left)
    ...
    # Plot 4: Action Entropy over Episodes (Middle Right)
    if TRAINING_METRICS['action_entropy']:
        entropy = TRAINING_METRICS['action_entropy']
        # Align with episode numbers
        if len(entropy) <= len(episode_numbers):
            entropy_episodes = episode_numbers[:len(entropy)]
        else:
            entropy_episodes = np.linspace(episode_numbers[0], episode_numbers[-1], len(entropy))
        
        axes[1, 1].plot(entropy_episodes, entropy, 'purple', linewidth=2, alpha=0.7, label='Action Entropy')
        ...
        if len(entropy) > 10:
            ...
            if window_size > 0:
                ...
                moving_episodes = entropy_episodes[window_size-1:]
                axes[1, 1].plot(moving_episodes, moving_avg, 'indigo', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[1, 1].legend()
    ...
    # Plot 5: Episode Length over Episodes (Bottom Left)
    if TRAINING_METRICS['episode_lengths']:
        ...
        axes[2, 0].plot(length_episodes, ep_lengths, 'orange', linewidth=2, alpha=0.7, label='Episode Length')
        ...
        if len(ep_lengths) > 10:
            ...
            if window_size > 0:
                ...
                axes[2, 0].plot(moving_episodes, moving_avg, 'darkorange', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[2, 0].legend()
        
        axes[2, 0].set_xlabel('Episode Number')
        axes[2, 0].set_ylabel('Episode Length')
        axes[2, 0].set_title('Episode Length (Steps per Episode)')
        axes[2, 0].grid(True, alpha=0.3)
    else:
        axes[2, 0].text(0.5, 0.5, 'No episode length data', ha='center', va='center', 
                       transform=axes[2, 0].transAxes, fontsize=14)
        axes[2, 0].set_title('Episode Length (No Data)')
    ...
    if TRAINING_METRICS['learning_rate'] or TRAINING_METRICS['explained_variance']:
        ...
        if TRAINING_METRICS['learning_rate']:
            ...
            lr_episodes = episode_numbers[:len(lr)]
            line1 = ax_lr.plot(lr_episodes, lr, 'orange', linewidth=2, label='Learning Rate')
        ...
        if TRAINING_METRICS['explained_variance']:
            ax_ev = ax_lr.twinx() if TRAINING_METRICS['learning_rate'] else ax_lr
            ev = TRAINING_METRICS['explained_variance']
            ev_episodes = episode_numbers[:len(ev)]
            line2 = ax_ev.plot(ev_episodes, ev, 'cyan', linewidth=2, label='Explained Variance')
