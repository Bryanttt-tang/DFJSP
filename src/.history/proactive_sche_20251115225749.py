...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # SIMPLIFIED observation space (removed redundant/noisy features)
        # Goal: Reduce dimensionality for faster convergence
        obs_size = (
            len(self.job_ids) +                      # Job ready time (when can start next op)
            len(self.job_ids) +                      # Job progress (ops completed / total ops)
            len(self.machines) +                     # Machine free time
            len(self.job_ids) * len(self.machines) + # Processing times for next ops
            len(self.job_ids) +                      # Predicted arrival times (time until predicted arrival)
            2                                        # Global: arrival progress, makespan progress
        )
        # OLD: 12 + 6 + 72 + 12 + 12 + 12 + 2 = 128 dims
        # NEW: 12 + 12 + 6 + 72 + 12 + 2 = 116 dims (removed actual arrivals + rate)
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(obs_size,), dtype=np.float32
        )
    ...
    def _get_observation(self):
        ...
        # 5. Predicted arrival times (time until predicted arrival, normalized)
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs or job_id in self.completed_jobs:
                obs_parts.append(0.0)  # Already arrived
            elif job_id in self.predicted_arrival_times:
                pred_time = self.predicted_arrival_times[job_id]
                time_until = max(0, pred_time - self.event_time)
                ...
                obs_parts.append(1.0)  # Far future

        # 6. Global progress indicators
        arrival_progress = len(self.arrived_jobs) / len(self.job_ids)
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    # CRITICAL: Add observation normalization for stable learning
    # This normalizes observations to have mean=0, std=1, improving gradient flow
    vec_env = VecNormalize(
        vec_env,
        norm_obs=True,           # Normalize observations
        norm_reward=False,       # Don't normalize rewards (makespan increment is already meaningful)
        clip_obs=10.0,           # Clip normalized obs to [-10, 10]
        clip_reward=10000.0,     # Don't clip rewards
        gamma=1.0,               # Match PPO gamma
        epsilon=1e-8
    )
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    print(f"  Arrival times set: {test_env.env.job_arrival_times}")
    ...
    while step_count < max_steps:
        ...
        if not any(action_masks):
            print(f"    ❌ No valid actions at step {step_count}")
            print(f"       Arrived jobs: {test_env.env.arrived_jobs}")
            print(f"       Completed jobs: {test_env.env.completed_jobs}")
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 150000    # Perfect knowledge with MULTIPLE INITIALIZATIONS
