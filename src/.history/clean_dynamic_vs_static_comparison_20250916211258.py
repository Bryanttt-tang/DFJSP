...
class StaticFJSPEnv(gym.Env):
    ...
    def _is_valid_action(self, job_idx, op_idx, machine_idx):
        ...
        # Check if job has arrived (always true for static - all jobs at t=0)
    ...
    def step(self, action):
        """Step function - based on PerfectKnowledgeFJSPEnv"""
        self.episode_step += 1
        ...
        job_ready_time = (self.operation_end_times[job_id][op_idx - 1] if op_idx > 0 
                         else self.job_arrival_times.get(job_id, 0.0))  # Always 0.0 for static
        ...
        # Update makespan (key improvement)
    ...
    def _get_observation(self):
        ...
        # 1. Machine availability (normalized by current makespan)
        for m in self.machines:
            value = float(self.machine_next_free.get(m, 0.0)) / norm_factor
            obs.append(max(0.0, min(1.0, value)))

        # 2. Operation completion status (padded to max_ops_per_job)
        ...
        # 3. Job progress (proportion of operations completed)
        for job_id in self.job_ids:
        ...
        # 4. Job arrival status (all 1.0 for static - all jobs available at t=0)
        for job_id in self.job_ids:
            obs.append(1.0)  # All jobs have arrived in static scheduling

        # 5. Current makespan (normalized)
