...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # 5.1. Time since last arrival (for each job, 0 if initial jobs, 1 if not arrived)
        for job_id in self.job_ids:
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
            if job_id in self.initial_job_ids:
                # Initial jobs: 0
                obs.append(0.0)
                ...
                # Arrived: normalized time since arrival
                time_since_arrival = max(0, self.event_time - arrival_time)
                normalized_time = min(1.0, time_since_arrival / self.max_time_horizon)
                obs.append(normalized_time)
