...
def generate_gantt_charts(gantt_scenarios_data, arrival_rate, folder_name=None):
    ...
    for scenario_idx in range(len(gantt_scenarios_data)):
        ...
        # Determine number of methods available
        available_methods = [(name, data) for name, data in schedules.items() 
                            if data[0] != float('inf') and data[1] is not None]
        ...
        max_makespan_scenario = max([makespan for makespan, _ in available_methods])
...
def generate_focused_comparison_chart(gantt_scenarios_data, arrival_rate):
    ...
    max_makespan = max([makespan for makespan, _ in available_methods])
