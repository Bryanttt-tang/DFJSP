...
from utils import (generate_fjsp_dataset, generate_simplified_fjsp_dataset, print_dataset_info)
...
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    machine_speed_variance=0.6,  # High machine heterogeneity for strategic waiting
...
class ArrivalPredictor:
    ...
    def _update_mle_estimate(self):
        ...
        FIXED: Use explicit weighted mean calculation (memory efficient).
        """
        total_n = self.global_n + self.current_n
        
        if total_n > 0:
            # FIXED: Explicit weighted mean calculation
            if self.current_n >= 3:
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
                 max_time_horizon=200, reward_mode="makespan_increment", 
                 use_predictor_for_wait=True, max_wait_time=10.0, seed=None):
        """
        ...
        Args:
            ...
            use_predictor_for_wait: Whether to use arrival predictor for wait rewards
            max_wait_time: Maximum time agent can wait in single wait action
            seed: Random seed
        """
        super().__init__()
        ...
        # Initialize arrival predictor (learns arrival rate from episodes!)
        ...
        # - Wait actions: Multiple wait durations (1, 2, 3, 5, 10 time units) OR wait to next event
        num_scheduling_actions = len(self.job_ids) * len(self.machines)
        
        # Define wait actions: [1, 2, 3, 5, 10, "next_event"]
        self.wait_durations = [1.0, 2.0, 3.0, 5.0, 10.0, float('inf')]  # inf = next event
    ...
    def _execute_wait_action_flexible(self, wait_duration):
        """Execute wait action with flexible duration (not predictor-guided)."""
        if wait_duration == float('inf'):
            # Wait to next event
            return self._execute_wait_action()
        ...
        if actual_wait_time <= self.event_time:
            done = len(self.completed_jobs) == len(self.job_ids)
            return 0.0, done
        
        # Advance time
        wait_amount = actual_wait_time - self.event_time
        self.event_time = actual_wait_time
        ...
        # Simple penalty proportional to wait duration
        reward = -wait_amount * 0.1
        done = False
        
        return reward, done
    
    def _execute_wait_action_with_predictor_guidance(self, wait_duration):
        """Execute wait with predictor-guided reward calculation."""
        # Store state before wait
        arrived_before = len(self.arrived_jobs)
        event_time_before = self.event_time
        
        # Get predictions before wait
        predicted_arrivals = self.arrival_predictor.predict_next_arrivals(
            current_time=self.event_time,
            num_jobs_to_predict=min(3, len(self.job_ids) - len(self.arrived_jobs))
        )
        confidence = self.arrival_predictor.get_confidence()
        
        # Calculate wait metrics BEFORE executing wait
        num_idle_machines = sum(
            1 for m_time in self.machine_end_times.values() 
            if m_time <= self.event_time
        )
        num_schedulable_jobs = sum(
            1 for jid in self.arrived_jobs 
            if self.job_progress[jid] < len(self.jobs[jid])
        )
        ...
        if wait_duration == float('inf'):
            ...
            actual_duration = max(0, next_event_time - self.event_time)
        else:
            target_time = self.event_time + wait_duration
            next_event_time = self._get_next_event_time()
            target_time = min(target_time, next_event_time)
            actual_duration = max(0, target_time - self.event_time)
        
        # Execute the wait
        if target_time <= self.event_time or target_time == float('inf'):
            done = len(self.completed_jobs) == len(self.job_ids)
            return 0.0, done
        ...
        # Check what happened during wait
        arrived_after = len(self.arrived_jobs)
        num_new_arrivals = arrived_after - arrived_before
        
        # PREDICTOR-GUIDED REWARD CALCULATION
        base_wait_penalty = -actual_duration * 0.1
        
        # Factor 1: Predicted arrival alignment
        # If predictor predicted arrival soon and we got one, reduce penalty
        predicted_soon = False
        if predicted_arrivals and len(predicted_arrivals) > 0:
            earliest_predicted = predicted_arrivals[0]
            time_to_predicted = earliest_predicted - event_time_before
            
            # "Soon" means within 2x our wait duration (or within 10 time units)
            soon_threshold = min(10.0, wait_duration * 2.0) if wait_duration != float('inf') else 10.0
            predicted_soon = (time_to_predicted <= soon_threshold and time_to_predicted >= 0)
        
        if predicted_soon and num_new_arrivals > 0:
            # Good wait: predicted arrival and got it
            alignment_bonus = 0.5 * confidence  # Up to +0.5 based on confidence
            base_wait_penalty += alignment_bonus
        elif predicted_soon and num_new_arrivals == 0:
            # Bad wait: predicted but nothing arrived
            misprediction_penalty = -0.3 * confidence
            base_wait_penalty += misprediction_penalty
        
        # Factor 2: Opportunity cost (idle resources + available work)
        # Waiting is BAD if: many idle machines AND work available
        if num_idle_machines > 0 and num_schedulable_jobs > 0:
            # Heavy penalty for waiting when we could be working
            idle_penalty = -num_idle_machines * num_schedulable_jobs * 0.2
            base_wait_penalty += idle_penalty
        
        # Factor 3: Strategic wait bonus
        # If we waited and got a job (any job), small bonus for being patient
        if num_new_arrivals > 0:
            patience_bonus = 0.2 * num_new_arrivals
            base_wait_penalty += patience_bonus
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        model = MaskablePPO(
            ...
            n_epochs=15,               # ⭐ More epochs = better learning per rollout
            gamma=1.0,                 # Undiscounted (makespan)
            gae_lambda=0.95,           # GAE for advantage estimation
            clip_range=0.2,            # Standard PPO clipping
            ent_coef=0.0001,           # ⭐ VERY LOW entropy (deterministic scenario!)
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    3. **Arrival Prediction**: Agent learns Poisson arrival rate across episodes
    ...
    - Learns arrival rate via MLE across episodes
    - Gets reward shaping based on wait effectiveness
    ...
    for i in range(num_scenarios):
        ...
        for job_id in dynamic_job_ids:
            ...
            if current_time <= 300:  # Extended time horizon
                arrival_times[job_id] = round(current_time, 1)
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    Returns:
        makespan, schedule
    """
    print(f"  Proactive RL evaluation (with predictor)...")
    ...
    while step_count < max_steps:
        ...
        if action >= test_env.env.wait_action_start:
            ...
            if wait_duration != float('inf'):
                print(f"    Wait action: {wait_duration} time units")
...
def main():
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        print(f"    MILP Optimal: {milp_makespan:.2f}")
        milp_end_time = time.time()
        print(f"    MILP Optimal training time: {milp_end_time - milp_start_time:.2f} seconds")
        ...
        perfect_model = train_perfect_knowledge_agent(
            ...
            reward_mode="makespan_increment", 
            learning_rate=learning_rate,
            num_initializations=5,  # ⭐ Try 5 random initializations
            milp_optimal=milp_makespan  # ⭐ Pass MILP for early stopping
        )
        if i == 0:
        ...
        if perfect_makespan < milp_makespan - 0.01:  # Allow tiny numerical tolerance
            ...
            print(f"    🚨 This should be impossible - check for bugs!")
