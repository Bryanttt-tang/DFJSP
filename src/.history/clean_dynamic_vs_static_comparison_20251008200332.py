...
class EnhancedTrainingCallback(BaseCallback):
    """Enhanced BaseCallback to consistently track all training metrics."""
    ...
    def _on_training_start(self) -> None:
        ...
        if self.verbose >= 1:
            print(f"[{self.method_name}] Training started - metrics tracking initialized")
        
    def _on_rollout_end(self) -> None:
        """Called at the end of each rollout - capture all available metrics."""
        global TRAINING_METRICS
        self.rollout_count += 1
        
        # Get logger data at rollout boundary
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            log_data = self.model.logger.name_to_value
            
            # Always record timesteps first
            current_timesteps = self.model.num_timesteps
            TRAINING_METRICS['timesteps'].append(current_timesteps)
            
            # Episode-level metrics (most important)
            if 'rollout/ep_rew_mean' in log_data:
                ep_reward = float(log_data['rollout/ep_rew_mean'])
                TRAINING_METRICS['episode_rewards'].append(ep_reward)
                self.episode_count += 1
                TRAINING_METRICS['episode_count'].append(self.episode_count)
                if self.verbose >= 1:
                    print(f"[{self.method_name}] Episode {self.episode_count}: reward={ep_reward:.4f}, timesteps={current_timesteps}")
            
            if 'rollout/ep_len_mean' in log_data:
                ep_length = float(log_data['rollout/ep_len_mean'])
                TRAINING_METRICS['episode_lengths'].append(ep_length)
            
            # Training loss metrics (capture whenever available)
            if 'train/policy_gradient_loss' in log_data:
                policy_loss = float(log_data['train/policy_gradient_loss'])
            ...
            if 'train/value_loss' in log_data:
                value_loss = float(log_data['train/value_loss'])
            ...
            if 'train/entropy_loss' in log_data:
                entropy_loss = float(log_data['train/entropy_loss'])
                TRAINING_METRICS['action_entropy'].append(entropy_loss)
            
            if 'train/learning_rate' in log_data:
                lr = float(log_data['train/learning_rate'])
                TRAINING_METRICS['learning_rate'].append(lr)
            
            if 'train/explained_variance' in log_data:
                exp_var = float(log_data['train/explained_variance'])
                TRAINING_METRICS['explained_variance'].append(exp_var)
            
            # Debug: Show metrics capture status
            if self.verbose >= 1 and self.rollout_count % 5 == 0:  # Every 5 rollouts
                metrics_status = {
                    ...
                    'timesteps': len(TRAINING_METRICS['timesteps'])
                }
                print(f"[{self.method_name}] Rollout {self.rollout_count} - Metrics captured: {metrics_status}")
        else:
            if self.verbose >= 1:
                print(f"[{self.method_name}] ❌ No logger available at rollout {self.rollout_count}")
...
def plot_training_metrics():
    """
    Comprehensive training metrics visualization with proper error handling.
    Shows episode rewards, training losses, and learning progress over time.
    """
    global TRAINING_METRICS
    
    # Check if we have any data
    if not TRAINING_METRICS or not TRAINING_METRICS.get('timesteps'):
        print("❌ No training metrics recorded!")
        print("   - Check if the training callback is working properly")
        print("   - Ensure verbose=1 is set during training")
        return
    
    method_name = TRAINING_METRICS.get('method_name', 'RL Agent')
    timesteps = TRAINING_METRICS['timesteps']
    
    print(f"\n=== TRAINING METRICS ANALYSIS: {method_name} ===")
    print(f"Total timesteps recorded: {len(timesteps):,}")
    
    # Prepare data for plotting
    metrics_data = {}
    
    # Episode rewards (most important metric)
    if TRAINING_METRICS.get('episode_rewards'):
        rewards = TRAINING_METRICS['episode_rewards']
        metrics_data['episode_rewards'] = rewards
        print(f"Episode rewards: {len(rewards)} entries")
        print(f"  Range: {min(rewards):.3f} to {max(rewards):.3f}")
    ...
    if TRAINING_METRICS.get('policy_loss'):
        ...
        metrics_data['policy_loss'] = policy_losses
        print(f"Policy losses: {len(policy_losses)} entries")
        print(f"  Range: {min(policy_losses):.6f} to {max(policy_losses):.6f}")
    else:
        print("❌ No policy losses captured")
    
    # Value loss
    if TRAINING_METRICS.get('value_loss'):
        value_losses = TRAINING_METRICS['value_loss']
        metrics_data['value_loss'] = value_losses
        print(f"Value losses: {len(value_losses)} entries")
        print(f"  Range: {min(value_losses):.6f} to {max(value_losses):.6f}")
    else:
        print("❌ No value losses captured")
    
    # Episode lengths
    if TRAINING_METRICS.get('episode_lengths'):
        lengths = TRAINING_METRICS['episode_lengths']
        metrics_data['episode_lengths'] = lengths
        print(f"Episode lengths: {len(lengths)} entries")
        print(f"  Average length: {np.mean(lengths):.1f} steps")
    
    # Create figure with subplots
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle(f'Training Metrics: {method_name}', fontsize=16, fontweight='bold')
    
    # Plot 1: Episode Rewards (Top Left)
    ax1 = axes[0, 0]
    if 'episode_rewards' in metrics_data:
        rewards = metrics_data['episode_rewards']
        reward_timesteps = timesteps[:len(rewards)]  # Match timesteps to rewards
        
        ax1.plot(reward_timesteps, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward')
        
        # Add moving average if enough data
        if len(rewards) > 10:
            window_size = min(20, len(rewards) // 4)
            moving_avg = np.convolve(rewards, np.ones(window_size)/window_size, mode='valid')
            avg_timesteps = reward_timesteps[window_size-1:]
            ax1.plot(avg_timesteps, moving_avg, 'r-', linewidth=3, alpha=0.9, 
        ...
        ax1.annotate(f'Final: {final_reward:.3f}', 
                    xy=(reward_timesteps[-1], final_reward),
                    xytext=(10, 10), textcoords='offset points',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='yellow', alpha=0.7),
                    arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0'))
    else:
        ax1.text(0.5, 0.5, 'No Episode Rewards Data', ha='center', va='center', 
                transform=ax1.transAxes, fontsize=14)
        ax1.set_title('Episode Rewards (No Data)')
    
    # Plot 2: Policy Loss (Top Right)
    ax2 = axes[0, 1]
    if 'policy_loss' in metrics_data:
        policy_losses = metrics_data['policy_loss']
        loss_timesteps = timesteps[:len(policy_losses)]
        
        ax2.plot(loss_timesteps, policy_losses, 'b-', linewidth=1.5, alpha=0.7)
        ...
        ax2.text(0.5, 0.5, 'No Policy Loss Data', ha='center', va='center', 
                transform=ax2.transAxes, fontsize=14)
        ax2.set_title('Policy Loss (No Data)')
    
    # Plot 3: Value Loss (Bottom Left)
    ax3 = axes[1, 0]
    if 'value_loss' in metrics_data:
        value_losses = metrics_data['value_loss']
        vloss_timesteps = timesteps[:len(value_losses)]
        
        ax3.plot(vloss_timesteps, value_losses, 'r-', linewidth=1.5, alpha=0.7)
    ...
    ax4 = axes[1, 1]
    if 'episode_lengths' in metrics_data:
        lengths = metrics_data['episode_lengths']
        length_timesteps = timesteps[:len(lengths)]
        
        ax4.plot(length_timesteps, lengths, 'm-', linewidth=1.5, alpha=0.7)
        ...
        ax4.axhline(y=avg_length, color='orange', linestyle='--', linewidth=2, 
                   label=f'Average: {avg_length:.1f}')
        ax4.legend()
    else:
        ax4.text(0.5, 0.5, 'No Episode Length Data', ha='center', va='center', 
                transform=ax4.transAxes, fontsize=14)
        ax4.set_title('Episode Lengths (No Data)')
    
    plt.tight_layout()
    
    # Save the plot
    timestamp = int(time.time())
    filename = f'training_metrics_{method_name.lower().replace(" ", "_")}_{timestamp}.png'
    ...
    if 'episode_rewards' in metrics_data:
        ...
        # Learning progress
        if len(rewards) > 10:
            early_avg = np.mean(rewards[:len(rewards)//4])
            late_avg = np.mean(rewards[-len(rewards)//4:])
            print(f"Early average: {early_avg:.3f}")
            print(f"Late average: {late_avg:.3f}")
            print(f"Learning progress: {late_avg - early_avg:.3f}")
