...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Validate action with softer penalty like successful environments
        if action >= len(self.valid_actions):
            # Invalid action - give negative reward but continue (softer penalty)
        ...
        # Double-check validity with softer penalties
        if next_op_idx >= len(self.jobs[job_id]):
        ...
        # Ensure job has actually arrived (with softer penalty)
        ...
        if self.current_time < job_arrival_time:
            ...
            return self._get_observation(), -50.0, False, False, {"error": "Scheduling before arrival"}
    ...
    def _calculate_reward(self, proc_time, idle_time, done, num_new_arrivals, 
        ...
        if self.reward_mode == "makespan_increment":
            # Simplified reward: focus on makespan increment like successful environments
            makespan_increment = current_time - previous_time
            reward = -makespan_increment  # Primary objective: minimize makespan growth
            
            # Small penalty for idle time to encourage efficiency
            reward -= idle_time * 0.1
            
            # Bonus for utilizing newly arrived jobs (dynamic advantage)
            if num_new_arrivals > 0:
                reward += 2.0 * num_new_arrivals  # Reduced from 5.0 for stability
            
            # Completion bonus
            if done:
                reward += 50.0  # Consistent with other environments
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment"):
    ...
    model = MaskablePPO(
        ...
        gamma=0.99,                # Match PerfectKnowledgeFJSPEnv
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,
        vf_coef=0.5,
