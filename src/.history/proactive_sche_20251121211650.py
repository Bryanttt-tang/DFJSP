...
def main():
    ...
    # Create a dedicated plot for mean episode reward
    if TRAINING_METRICS['rollout_ep_rew_mean']:
        plt.figure(figsize=(12, 6))
        rollout_numbers = list(range(1, len(TRAINING_METRICS['rollout_ep_rew_mean']) + 1))
        plt.plot(rollout_numbers, TRAINING_METRICS['rollout_ep_rew_mean'], 'b-', linewidth=2, label='Mean Episode Reward')
        plt.xlabel('Rollout Number', fontsize=14)
        plt.ylabel('Mean Episode Reward', fontsize=14)
        plt.title(f'Proactive RL: Mean Episode Reward (λ={arrival_rate}, {proactive_timesteps:,} timesteps)', fontsize=16, fontweight='bold')
        plt.grid(True, alpha=0.3)
