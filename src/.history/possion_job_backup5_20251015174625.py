...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if job_idx is None:
            ...
            wait_reward = -500.0 if scheduling_actions_available else -100.0
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Use parallel environments for better gradient estimates
    n_envs = 8  # Use 8 parallel environments
    vec_env = SubprocVecEnv([make_perfect_env for _ in range(n_envs)])
    # Add VecMonitor to properly capture episode statistics
    vec_env = VecMonitor(vec_env)
    
    # OPTIMIZED hyperparameters for stable training (aligned with Static RL)
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=1e-4,        # Reduced for stability
        n_steps=256,               # Increased: 8 envs × 256 = 2048 samples
        batch_size=512,            # Larger batches for stable gradients
        n_epochs=4,                # Standard PPO epochs
        gamma=1,                   # Episodic tasks
        gae_lambda=0.95,           # Lower variance
        clip_range=0.2,
        ent_coef=0.01,
        vf_coef=0.5,
        max_grad_norm=0.5,
...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Use parallel environments for better gradient estimates
    n_envs = 8  # Use 8 parallel environments
    vec_env = SubprocVecEnv([make_static_env for _ in range(n_envs)])
    # Add VecMonitor to properly capture episode statistics
    vec_env = VecMonitor(vec_env)
    
    # OPTIMIZED hyperparameters for stable training
    # Key changes: larger batch_size (512), fewer epochs (4), adjusted learning rate
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,  # Minimal output
        learning_rate=1e-4,        # Reduced from 3e-4 for stability
        n_steps=256,               # Keep same: 8 envs × 256 = 2048 samples per rollout
        batch_size=512,            # INCREASED: 2048/512 = 4 minibatches (was 16)
        n_epochs=4,                # REDUCED: standard PPO uses 3-4 (was 10)
        gamma=1,                   # Keep same for episodic tasks
        gae_lambda=0.95,           # Slightly reduced for lower variance
        clip_range=0.2,            # Keep same
        ent_coef=0.01,             # Keep same
        ...
        policy_kwargs=dict(
            net_arch=[256, 256, 128],  # Keep same architecture
            activation_fn=torch.nn.ReLU
        )
    )
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Use parallel environments for better gradient estimates
    n_envs = 8  # Use 8 parallel environments
    vec_env = SubprocVecEnv([make_dynamic_env for _ in range(n_envs)])
    # Add VecMonitor to properly capture episode statistics
    vec_env = VecMonitor(vec_env)
    
    # OPTIMIZED hyperparameters for stable training
    # Dynamic RL may need slightly different settings due to varying task difficulty
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,
        learning_rate=1e-4,        # Reduced for stability
        n_steps=256,               # Reduced: 8 envs × 256 = 2048 samples (was 4096)
        batch_size=512,            # Larger batches: 2048/512 = 4 minibatches
        n_epochs=4,                # Standard PPO epochs (was 10)
        gamma=1,                   # Episodic tasks
...
def plot_training_metrics():
    ...
    if TRAINING_METRICS['kl_divergence']:
        ...
        if kl_values:
            axes[2, 1].plot(rollout_numbers[:len(kl_values)], kl_values, 'darkred', linewidth=2, alpha=0.7, label='KL Divergence')
            axes[2, 1].legend()
            axes[2, 1].set_xlabel('Rollout Number')
            axes[2, 1].set_ylabel('KL Divergence')
            axes[2, 1].set_title('KL Divergence (Policy Updates)')
            axes[2, 1].grid(True, alpha=0.3)
            
            # Add horizontal line at KL divergence threshold (typically 0.01-0.02 for PPO)
            axes[2, 1].axhline(y=0.01, color='red', linestyle='--', alpha=0.5, label='Typical Threshold')
            axes[2, 1].legend()
        else:
            axes[2, 1].text(0.5, 0.5, 'No valid KL divergence data', ha='center', va='center', transform=axes[2, 1].transAxes, fontsize=14)
            axes[2, 1].set_title('KL Divergence (No Data)')
    else:
        axes[2, 1].text(0.5, 0.5, 'No KL divergence data', ha='center', va='center', transform=axes[2, 1].transAxes, fontsize=14)
        axes[2, 1].set_title('KL Divergence (No Data)')
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    ...
    # Create a cache key based on the problem instance
    def create_cache_key():
        # Create a deterministic string representation of the problem
        jobs_str = str(sorted([
            (j, [(tuple(sorted(op['proc_times'].items()))) for op in ops])
            for j, ops in jobs_data.items()
        ]))
        machines_str = str(sorted(machine_list))
        arrivals_str = str(sorted(arrival_times.items()))
    ...
    # Check if cached solution exists
    if os.path.exists(cache_file):
        try:
            with open(cache_file, 'rb') as f:
                cached_result = pickle.load(f)
            if verbose:
                print(f"✅ Using cached MILP solution: makespan = {cached_result[0]:.2f}")
            return cached_result
        except Exception as e:
            if verbose:
                print(f"⚠️ Failed to load cache file, computing fresh: {e}")
            try:
                os.remove(cache_file)
            except:
                pass
    ...
    if verbose:
        print(f"🔄 Computing fresh MILP solution (cache key: {cache_key[:8]}...)...")
    ...
    if status == "Optimal" or status == "Not Solved" or status == "Optimal":  # Prefer explicit 'Optimal' but accept near-optimal?
        ...
        if math.isfinite(actual_makespan) and abs(actual_makespan - optimal_makespan) <= 1e-6:
            ...
            try:
                result = (optimal_makespan, schedule)
                with open(cache_file, 'wb') as f:
                    pickle.dump(result, f)
                if verbose:
                    print(f"💾 Cached MILP solution for future use")
            except Exception as e:
                if verbose:
                    print(f"⚠️ Failed to cache solution: {e}")
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 1000000    # Perfect knowledge needs less training
    dynamic_timesteps = 100000   # Increased for better learning with integer timing  
    static_timesteps = 1000000    # Increased for better learning
    learning_rate = 1e-4       # Standard learning rate for PPO
