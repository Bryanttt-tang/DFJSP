...
# Use medium problem first (3 jobs, 3 machines)
SIMPLE_JOBS_DATA = MEDIUM_JOBS_DATA
SIMPLE_MACHINES = MEDIUM_MACHINES  
SIMPLE_ARRIVALS = MEDIUM_ARRIVALS
...
def estimate_state_space(jobs_data, machines, arrivals):
    """Estimate upper bound on state space size"""
    print("\n=== STATE SPACE ESTIMATION ===")
    ...
    # Estimate maximum makespan (upper bound)
    max_proc_time = 0
    for job_ops in jobs_data.values():
        for op in job_ops:
            max_proc_time = max(max_proc_time, max(op['proc_times'].values()))
    
    max_makespan = total_ops * max_proc_time  # Very loose upper bound
    print(f"Max processing time: {max_proc_time}, Rough max makespan: {max_makespan}")
    
    # State components:
    # 1. next_ops: each job can be at operation 0, 1, ..., len(job_ops)
    ...
    # 2. machine_next_free: each machine can be free at time 0, 1, ..., max_makespan
    machine_time_combinations = (max_makespan + 1) ** num_machines
    print(f"Machine time combinations (loose): {machine_time_combinations}")
    
    # 3. job_ready_time: each job can be ready at time 0, 1, ..., max_makespan  
    job_time_combinations = (max_makespan + 1) ** num_jobs
    print(f"Job ready time combinations (loose): {job_time_combinations}")
    
    # Total state space (very loose upper bound)
    total_loose = next_ops_combinations * machine_time_combinations * job_time_combinations
    print(f"Total state space (loose upper bound): {total_loose:.2e}")
    
    # More realistic estimate: machines/jobs won't all have max time
    # Typical makespan is more like sum of critical path
    realistic_makespan = sum(max(op['proc_times'].values()) for job_ops in jobs_data.values() for op in job_ops) // num_jobs
    realistic_makespan = min(realistic_makespan, max_makespan // 2)
    
    realistic_machine_times = (realistic_makespan + 1) ** num_machines
    realistic_job_times = (realistic_makespan + 1) ** num_jobs
    realistic_total = next_ops_combinations * realistic_machine_times * realistic_job_times
    
    print(f"Realistic makespan estimate: {realistic_makespan}")
    print(f"Realistic state space estimate: {realistic_total:.2e}")
