...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # We don't collect individual episodes here anymore - only at rollout boundaries
        return True

    def _on_rollout_end(self) -> None:
        """Called at the end of each rollout - record ALL metrics here to align with verbose output."""
        global TRAINING_METRICS
        
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            log_data = dict(self.model.logger.name_to_value)  # copy
            
            # Capture rollout-level episode metrics (these are what verbose shows)
            if 'rollout/ep_rew_mean' in log_data:
                TRAINING_METRICS['episode_rewards'].append(float(log_data['rollout/ep_rew_mean']))
            if 'rollout/ep_len_mean' in log_data:
                TRAINING_METRICS['episode_lengths'].append(float(log_data['rollout/ep_len_mean']))
            
            # Capture training losses (these are also what verbose shows)
            if 'train/policy_gradient_loss' in log_data:
            ...
            # Record rollout number and timestep for alignment
            rollout_num = len(TRAINING_METRICS['policy_loss'])  # Use policy_loss length as rollout counter
            TRAINING_METRICS['episode_count'].append(rollout_num)
            TRAINING_METRICS['timesteps'].append(getattr(self.model, "num_timesteps", 0))
            
            if self.verbose and rollout_num <= 5:
                ep_rew = TRAINING_METRICS['episode_rewards'][-1] if TRAINING_METRICS['episode_rewards'] else 0
                pol_loss = TRAINING_METRICS['policy_loss'][-1] if TRAINING_METRICS['policy_loss'] else 0
                val_loss = TRAINING_METRICS['value_loss'][-1] if TRAINING_METRICS['value_loss'] else 0
                print(f"[{self.method_name}] 📊 Rollout #{rollout_num}: "
                      f"ep_rew_mean={ep_rew:.3f}, "
                      f"policy_loss={pol_loss:.6f}, "
                      f"value_loss={val_loss:.6f}, "
...
def plot_training_metrics():
    ...
    print(f"Episode rewards (ep_rew_mean) recorded: {len(TRAINING_METRICS['episode_rewards'])}")
    ...
    # Create rollout numbers for x-axis (should be the same for all metrics now)
    num_rollouts = len(TRAINING_METRICS['policy_loss'])
    rollout_numbers = list(range(1, num_rollouts + 1))
    
    # Verify alignment
    if len(TRAINING_METRICS['episode_rewards']) != num_rollouts:
        print(f"⚠️  WARNING: Episode rewards ({len(TRAINING_METRICS['episode_rewards'])}) != Policy loss ({num_rollouts}) rollouts")
    
    # DEBUG: Print sample data
    rewards = TRAINING_METRICS['episode_rewards'][:num_rollouts]  # Ensure same length
    print(f"Episode rewards (ep_rew_mean) sample (first 5): {rewards[:5]}")
    print(f"Episode rewards (ep_rew_mean) sample (last 5): {rewards[-5:]}")
    print(f"Episode rewards range: {min(rewards):.4f} to {max(rewards):.4f}")
    print(f"Rollout numbers range: {min(rollout_numbers)} to {max(rollout_numbers)}")
    
    # Create comprehensive figure with 3x2 subplots
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.suptitle(f'Training Progress - {TRAINING_METRICS.get("method_name", "RL Agent")} ({num_rollouts} Rollouts)', 
                 fontsize=16, fontweight='bold')
    
    # Plot 1: Episode Rewards (ep_rew_mean) over Rollouts (Top Left) - MAIN PLOT
    axes[0, 0].plot(rollout_numbers, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward Mean')
    
    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Rollout Number')
    axes[0, 0].set_ylabel('Episode Reward Mean')
    axes[0, 0].set_title('Episode Reward Mean Over Training (Aligned with Verbose)')
