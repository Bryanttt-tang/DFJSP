...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def __init__(self, method_name, pbar=None, verbose=0):
        ...
        self.last_episode_count = 0  # Track episodes from VecMonitor
    ...
    def _on_rollout_end(self) -> None:
        """Called at the end of each rollout - capture training metrics."""
        global TRAINING_METRICS
        
        # Get episode statistics directly from VecMonitor
        if hasattr(self.model, 'env') and hasattr(self.model.env, 'get_attr'):
            try:
                # Get episode rewards from VecMonitor episode buffer
                episode_rewards = self.model.env.get_attr('episode_rewards')
                episode_lengths = self.model.env.get_attr('episode_lengths')
                
                if episode_rewards and len(episode_rewards) > 0 and len(episode_rewards[0]) > 0:
                    # Get new episodes since last check
                    current_episodes = len(episode_rewards[0])
                    if current_episodes > self.last_episode_count:
                        # Add new episode rewards
                        new_rewards = episode_rewards[0][self.last_episode_count:]
                        for reward in new_rewards:
                            TRAINING_METRICS['episode_rewards'].append(reward)
                            self.episode_count += 1
                            TRAINING_METRICS['episode_count'].append(self.episode_count)
                            
                            if self.verbose >= 1 and self.episode_count <= 3:
                                print(f"[{self.method_name}] ✅ Episode reward: {reward:.4f} (episode #{self.episode_count})")
                        
                        # Add episode lengths if available
                        if episode_lengths and len(episode_lengths) > 0 and len(episode_lengths[0]) > self.last_episode_count:
                            new_lengths = episode_lengths[0][self.last_episode_count:]
                            TRAINING_METRICS['episode_lengths'].extend(new_lengths)
                        
                        self.last_episode_count = current_episodes
                        
            except Exception as e:
                if self.verbose >= 1 and len(TRAINING_METRICS['episode_rewards']) < 3:
                    print(f"[{self.method_name}] Could not get episode stats from VecMonitor: {e}")
        
        # Get training metrics from logger
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            ...
            if 'train/value_loss' in log_data:
                ...
                if self.verbose >= 1 and len(TRAINING_METRICS['value_loss']) <= 3:
                    print(f"[{self.method_name}] Value loss: {log_data['train/value_loss']:.6f}")
            ...
            # Always record timesteps for reference
            TRAINING_METRICS['timesteps'].append(self.model.num_timesteps)
    
    def _on_step(self) -> bool:
        """Called at each step - capture episode rewards when episodes complete."""
        global TRAINING_METRICS
        ...
        # Check if we have episode rewards available in the logger
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            log_data = self.model.logger.name_to_value
            
            # Sometimes episode rewards are available in logger during steps
            if 'rollout/ep_rew_mean' in log_data:
                ep_reward = log_data['rollout/ep_rew_mean']
                # Only add if this is a new value (to avoid duplicates)
                if len(TRAINING_METRICS['episode_rewards']) == 0 or ep_reward != TRAINING_METRICS['episode_rewards'][-1]:
            ...
            if 'rollout/ep_len_mean' in log_data:
                ep_length = log_data['rollout/ep_len_mean']
                if len(TRAINING_METRICS['episode_lengths']) == 0 or ep_length != TRAINING_METRICS['episode_lengths'][-1]:
                    TRAINING_METRICS['episode_lengths'].append(ep_length)
        
        # Try to get episode rewards directly from VecMonitor if available
        if hasattr(self.model, 'env') and hasattr(self.model.env, 'get_attr'):
            try:
                episode_rewards = self.model.env.get_attr('episode_rewards')
                if episode_rewards and len(episode_rewards) > 0 and len(episode_rewards[0]) > self.last_episode_count:
                    ...
                    for reward in new_rewards:
                        ...
                        if self.verbose >= 1 and self.episode_count <= 5:
                            print(f"[{self.method_name}] ✅ Episode reward from VecMonitor: {reward:.4f} (episode #{self.episode_count})")
                    
                    self.last_episode_count = len(episode_rewards[0])
            except:
                pass  # VecMonitor access may not always work
...
def plot_training_metrics():
    ...
    X-axis is now episode number for better interpretability.
    """
    global TRAINING_METRICS
    ...
    # Plot 2: Policy Loss over Episodes (Top Right)
    if TRAINING_METRICS['policy_loss']:
        policy_loss = TRAINING_METRICS['policy_loss']
        # Align with episode numbers (losses may be recorded less frequently)
        if len(policy_loss) <= len(episode_numbers):
            loss_episodes = episode_numbers[:len(policy_loss)]
        else:
            # If more losses than episodes, create episode-aligned indices
            loss_episodes = np.linspace(episode_numbers[0], episode_numbers[-1], len(policy_loss))
        
        axes[0, 1].plot(loss_episodes, policy_loss, 'r-', linewidth=2, alpha=0.7, label='Policy Loss')
        ...
        if len(policy_loss) > 10:
            ...
            if window_size > 0:
                ...
                moving_episodes = loss_episodes[window_size-1:]
                axes[0, 1].plot(moving_episodes, moving_avg, 'darkred', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[0, 1].legend()
    ...
    # Plot 3: Value Loss over Episodes (Bottom Left)
    if TRAINING_METRICS['value_loss']:
        value_loss = TRAINING_METRICS['value_loss']
        # Align with episode numbers
        if len(value_loss) <= len(episode_numbers):
            loss_episodes = episode_numbers[:len(value_loss)]
        else:
            loss_episodes = np.linspace(episode_numbers[0], episode_numbers[-1], len(value_loss))
        
        axes[1, 0].plot(loss_episodes, value_loss, 'b-', linewidth=2, alpha=0.7, label='Value Loss')
        ...
        if len(value_loss) > 10:
            ...
            if window_size > 0:
                ...
                moving_episodes = loss_episodes[window_size-1:]
                axes[1, 0].plot(moving_episodes, moving_avg, 'darkblue', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[1, 0].legend()
    ...
    # Plot 4: Episode Length over Episodes (Bottom Right)
    if TRAINING_METRICS['episode_lengths']:
        ...
        axes[1, 1].plot(length_episodes, ep_lengths, 'orange', linewidth=2, alpha=0.7, label='Episode Length')
        ...
        if len(ep_lengths) > 10:
            ...
            if window_size > 0:
                ...
                axes[1, 1].plot(moving_episodes, moving_avg, 'darkorange', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[1, 1].legend()
        
        axes[1, 1].set_xlabel('Episode Number')
        axes[1, 1].set_ylabel('Episode Length')
        axes[1, 1].set_title('Episode Length (Steps per Episode)')
        axes[1, 1].grid(True, alpha=0.3)
    else:
        axes[1, 1].text(0.5, 0.5, 'No episode length data', ha='center', va='center', 
                       transform=axes[1, 1].transAxes, fontsize=14)
        axes[1, 1].set_title('Episode Length (No Data)')
