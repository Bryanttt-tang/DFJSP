...
def train_agent(jobs_data, machine_list, train_arrivals, log_name, total_timesteps=150000):
    ...
    def make_train_env():
        env = Dy(jobs_data, machine_list, train_arrivals)
...
if __name__ == "__main__":
    """
    Main execution demonstrating comprehensive comparison of scheduling methods for Dynamic FJSP:
    - MILP Optimal Solver
    - Deep Reinforcement Learning (DRL) Agent
    - SPT (Shortest Processing Time) Heuristic
    """
    print("="*80)
    print("DYNAMIC FJSP - COMPREHENSIVE SCHEDULING METHOD COMPARISON")
    print("="*80)
    ...
    # Evaluate all methods on default instance
    print("=== DEFAULT INSTANCE (4 jobs) ===")
    
    # 1. MILP Optimal Solver
    print("\n1. Running MILP Optimal Solver...")
    milp_makespan_default, milp_schedule_default = milp_optimal_scheduler(
        DEFAULT_JOBS_DATA, DEFAULT_MACHINE_LIST, DEFAULT_ARRIVAL_TIMES, time_limit=120
    )
    ...
    # Evaluate on extended instance (MILP may be slower for larger instances)
    print("\n1. Running MILP Optimal Solver...")
    milp_makespan_extended, milp_schedule_extended = milp_optimal_scheduler(
        EXTENDED_JOBS_DATA, EXTENDED_MACHINE_LIST, EXTENDED_ARRIVAL_TIMES, time_limit=300
    )
    ...
    print("-" * 50)
    print("DEFAULT INSTANCE (4 jobs):")
    if milp_makespan_default is not None:
        print(f"  MILP Optimal Makespan:  {milp_makespan_default:.2f}")
    else:
        print(f"  MILP Optimal Makespan:  N/A (solver failed)")
    ...
    if milp_makespan_default is not None and milp_makespan_default > 0:
        drl_gap_default = ((drl_makespan_default - milp_makespan_default) / milp_makespan_default * 100)
        spt_gap_default = ((spt_makespan_default - milp_makespan_default) / milp_makespan_default * 100)
        print(f"  DRL Optimality Gap:     {drl_gap_default:+.1f}%")
        print(f"  SPT Optimality Gap:     {spt_gap_default:+.1f}%")
    
    print("\nEXTENDED INSTANCE (7 jobs):")
    if milp_makespan_extended is not None:
        print(f"  MILP Optimal Makespan:  {milp_makespan_extended:.2f}")
    else:
        print(f"  MILP Optimal Makespan:  N/A (solver failed)")
    print(f"  DRL Agent Makespan:     {drl_makespan_extended:.2f}")
    print(f"  SPT Heuristic Makespan: {spt_makespan_extended:.2f}")
    
    if milp_makespan_extended is not None and milp_makespan_extended > 0:
        drl_gap_extended = ((drl_makespan_extended - milp_makespan_extended) / milp_makespan_extended * 100)
        spt_gap_extended = ((spt_makespan_extended - milp_makespan_extended) / milp_makespan_extended * 100)
        print(f"  DRL Optimality Gap:     {drl_gap_extended:+.1f}%")
        print(f"  SPT Optimality Gap:     {spt_gap_extended:+.1f}%")
    
    print("\n4. GANTT CHART VISUALIZATIONS")
    print("-" * 50)
    
    # Create comprehensive comparison plots
    def plot_three_method_comparison_subplots(methods_data, machines, title, save_path):
        """Create a 1x3 subplot comparison of three scheduling methods"""
        ...
        for idx, (method_name, (schedule, makespan)) in enumerate(methods_data.items()):
            ...
            # Add optimality gap to title if MILP solution exists
            if 'MILP' in method_name:
                ax.set_title(f"{method_name}\nMakespan: {makespan:.1f}")
            else:
                if methods_data.get('MILP Optimal', (None, None))[1] is not None:
                    optimal_makespan = methods_data['MILP Optimal'][1]
                    gap = ((makespan - optimal_makespan) / optimal_makespan * 100) if optimal_makespan > 0 else 0
                    ax.set_title(f"{method_name}\nMakespan: {makespan:.1f} (Gap: +{gap:.1f}%)")
                else:
                    ax.set_title(f"{method_name}\nMakespan: {makespan:.1f}")
        ...
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
        print(f"Saved three-method comparison to {save_path}")
    
    # Generate comparison plots for default instance
    if milp_makespan_default is not None:
        plot_three_method_comparison_subplots(
            {
                'MILP Optimal': (milp_schedule_default, milp_makespan_default),
                'Dynamic RL': (drl_schedule_default, drl_makespan_default),
                'SPT Heuristic': (spt_schedule_default, spt_makespan_default)
            },
            DEFAULT_MACHINE_LIST,
            "Scheduling Method Comparison - Default Instance (4 Jobs)",
            "comparison_default_instance.png"
        )
    
    # Generate comparison plots for extended instance  
    if milp_makespan_extended is not None:
        plot_three_method_comparison_subplots(
            {
                'MILP Optimal': (milp_schedule_extended, milp_makespan_extended),
                'Dynamic RL': (drl_schedule_extended, drl_makespan_extended),
                'SPT Heuristic': (spt_schedule_extended, spt_makespan_extended)
            },
            EXTENDED_MACHINE_LIST,
            "Scheduling Method Comparison - Extended Instance (7 Jobs)",
            "comparison_extended_instance.png"
        )
    
    # Individual detailed Gantt charts
    print("\nGenerating detailed individual Gantt charts...")
    
    if milp_schedule_default:
        plot_gantt(milp_schedule_default, DEFAULT_MACHINE_LIST, 
                  f"MILP Optimal Schedule - Default Instance (Makespan: {milp_makespan_default:.1f})")
    
    plot_gantt(drl_schedule_default, DEFAULT_MACHINE_LIST, 
              f"Dynamic RL Schedule - Default Instance (Makespan: {drl_makespan_default:.1f})")
    
    plot_gantt(spt_schedule_default, DEFAULT_MACHINE_LIST, 
              f"SPT Heuristic Schedule - Default Instance (Makespan: {spt_makespan_default:.1f})")
    
    print("\n5. PERFORMANCE ANALYSIS")
    print("-" * 50)
    
    if milp_makespan_default is not None and milp_makespan_extended is not None:
        print("✅ MILP solver found optimal solutions for both instances")
        print(f"📊 DRL avg optimality gap: {((drl_gap_default + drl_gap_extended) / 2):+.1f}%")
        print(f"📊 SPT avg optimality gap: {((spt_gap_default + spt_gap_extended) / 2):+.1f}%")
        
        if drl_gap_default < spt_gap_default and drl_gap_extended < spt_gap_extended:
            print("🎯 DRL consistently outperforms SPT heuristic on both instances")
        elif drl_gap_default < spt_gap_default:
            print("🎯 DRL outperforms SPT on default instance")
        elif drl_gap_extended < spt_gap_extended:
            print("🎯 DRL outperforms SPT on extended instance")
    else:
        print("⚠️  MILP solver could not find optimal solution within time limits")
        drl_vs_spt_default = ((spt_makespan_default - drl_makespan_default) / spt_makespan_default * 100)
        drl_vs_spt_extended = ((spt_makespan_extended - drl_makespan_extended) / spt_makespan_extended * 100)
        print(f"📊 DRL vs SPT (default): {drl_vs_spt_default:+.1f}%")
        print(f"📊 DRL vs SPT (extended): {drl_vs_spt_extended:+.1f}%")
    
    print("\n" + "="*80)
    print("COMPREHENSIVE SCHEDULING COMPARISON COMPLETED")
    print("Generated comparison visualizations:")
    print("- comparison_default_instance.png (3-method comparison, 4 jobs)")
    print("- comparison_extended_instance.png (3-method comparison, 7 jobs)")
    print("- Individual detailed Gantt charts for each method")
    print("="*80)
