...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        model = MaskablePPO(
            ...
            vec_env,
            verbose=0,
            learning_rate=1e-4,        # ⭐ MUCH LOWER for stability
            n_steps=256,               # ⭐ Smaller rollouts = more frequent updates
            batch_size=128,            # ⭐ Larger relative to n_steps for stability
            ...
            ent_coef=0.001,            # ⭐ VERY LOW entropy (deterministic!)
            ...
            policy_kwargs=dict(
                net_arch=dict(
                    pi=[256, 256, 128],    # ⭐ Slightly smaller for stability
                    vf=[256, 256, 128]     # Separate value network
                ),
