...
def plot_training_metrics():
    ...
    # Create rollout numbers for x-axis (all metrics are aligned to rollouts now)
    num_rollouts = len(TRAINING_METRICS['episode_rewards'])
    ...
    # Plot 1: Episode Rewards over Rollouts (Top Left) - MAIN PLOT
    axes[0, 0].plot(rollout_numbers, rewards, 'g-', linewidth=2, alpha=0.8, label='Mean Episode Reward')
    
    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Rollout Number')
    axes[0, 0].set_ylabel('Mean Episode Reward')
    axes[0, 0].set_title('Mean Episode Rewards Over Training (Rollout-level)')
