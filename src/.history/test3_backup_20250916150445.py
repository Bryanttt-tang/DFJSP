...
EXTENDED_ARRIVAL_TIMES = {0: 0, 1: 0, 2: 0, 3: 10, 4: 15, 5: 20, 6: 25}
...
class DynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, job_arrival_times=None, reward_mode="improved"):
        super().__init__()
        self.jobs = jobs_data
        ...
        self.reward_mode = reward_mode  # "improved", "makespan_increment", "processing_time"
    ...
    def step(self, action):
        ...
        reward = self._calculate_reward(proc_time, max(0, start_time - machine_available_time), terminated, previous_makespan, self.current_makespan)
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_makespan=None, current_makespan=None):
        if self.reward_mode == "makespan_increment":
            ...
            # Simple negative processing time reward (like in policy-iter.py)
            reward = -proc_time
            if done:
                reward += 50.0
            return reward
...
def train_agent(jobs_data, machine_list, train_arrivals, log_name, total_timesteps=50000, reward_mode="makespan_increment"):
    ...
    def make_env():
        env = DynamicFJSPEnv(jobs_data, machine_list, train_arrivals, reward_mode=reward_mode)
        # Wrap with ActionMasker to enable MaskablePPO
    ...
    # Enhanced MaskablePPO configuration with better hyperparameters
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,
        learning_rate=3e-4,  # Standard learning rate for MaskablePPO
        n_steps=2048,        # Increased for better experience collection
        batch_size=128,      # Larger batch size for stable updates
        n_epochs=20,         # More epochs for better policy updates
        gamma=0.99,
        ...
        policy_kwargs=dict(
            net_arch=[512, 512, 256],  # Larger network for complex state space
...
def evaluate_agent(model, jobs_data, machine_list, eval_arrivals, scenario_name, reward_mode="makespan_increment"):
    ...
    test_env = DynamicFJSPEnv(jobs_data, machine_list, eval_arrivals, reward_mode=reward_mode)
...
if __name__ == "__main__":
    ...
    env = DynamicFJSPEnv(jobs_data, machine_list, arrival_times, reward_mode="makespan_increment")
    obs, _ = env.reset()
    ...
    try:
        static_model = train_agent(jobs_data, machine_list, static_arrival_times, 
                                 "Static_RL", total_timesteps=25000, reward_mode="makespan_increment")
        ...
        static_rl_on_static_makespan, static_rl_on_static_schedule = evaluate_agent(
            static_model, jobs_data, machine_list, static_arrival_times, "Static_RL_on_Static", reward_mode="makespan_increment")
        ...
        static_rl_makespan, static_rl_schedule = evaluate_agent(
            static_model, jobs_data, machine_list, arrival_times, "Static_RL_on_Dynamic", reward_mode="makespan_increment")
    ...
    try:
        dynamic_model = train_agent(jobs_data, machine_list, arrival_times, 
                                  "Dynamic_RL", total_timesteps=25000, reward_mode="makespan_increment")
        print("Evaluating dynamic RL...")
        dynamic_rl_makespan, dynamic_rl_schedule = evaluate_agent(
            dynamic_model, jobs_data, machine_list, arrival_times, "Dynamic_RL", reward_mode="makespan_increment")
