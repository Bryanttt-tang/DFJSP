...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, seq_rule, route_rule="MIN"):
    ...
    while completed_operations < total_operations:
        ...
        # CRITICAL: Check if we should advance event_time after scheduling
        # (Same logic as Rule-Based RL - advance if no more ready operations)
        # First, check for new arrivals at current sim_time
        for job_id, arr_time in arrival_times.items():
            if job_id not in arrived_jobs and arr_time <= sim_time:
                arrived_jobs.add(job_id)
        
        # Check if there are ready operations after scheduling
        has_ready_ops = False
        for job_id in arrived_jobs:
            if job_next_op[job_id] < len(jobs_data[job_id]):
                op_idx = job_next_op[job_id]
        ...
        # If no ready operations but work remains, advance to next event
        if not has_ready_ops and completed_operations < total_operations:
            next_time = float('inf')
