...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    # CRITICAL: Calculate actual arrival rate from test scenario
    dynamic_arrivals = [t for t in arrival_times.values() if t > 1e-6 and t < float('inf')]
    if len(dynamic_arrivals) > 1:
        # Estimate rate from observed arrivals: λ ≈ n / T_max
        estimated_rate = (len(dynamic_arrivals) - 1) / max(dynamic_arrivals)
    else:
        estimated_rate = 0.05  # Fallback
    
    print(f"  📊 Test scenario estimated arrival rate: {estimated_rate:.4f}")
    
    test_env = ProactiveDynamicFJSPEnv(
        jobs_data, machine_list,
        initial_jobs=initial_job_ids,
        arrival_rate=estimated_rate,  # Use estimated rate from test scenario
        reward_mode=reward_mode,
        predictor_mode=predictor_mode,
        seed=GLOBAL_SEED,
        max_time_horizon=1000  # Extended horizon to allow for WAIT actions
    ...
    print(f"  📊 Predictor initial state: rate={predictor_stats['estimated_rate']:.4f}, "
    ...
    # The predictor starts fresh at evaluation (global_obs=0) with NO training history!
    # During training, the predictor learns across episodes, but this knowledge is lost.
    # The model (neural network) is saved, but predictor statistics are NOT saved.
    # 
    # This explains poor performance:
    # - Training: Predictor accumulates 1000+ episodes of arrival data → good predictions
    # - Evaluation: Predictor has 0 episodes of data → relies only on prior → poor predictions
    # 
    # SOLUTION: The predictor's learned statistics (global_n, global_sum) should be:
    # 1. Saved with the model (pickle the predictor state)
    # 2. Loaded during evaluation (restore predictor state)
    # 3. OR use the TRUE arrival rate from training as initial_rate_guess
    
    if stats['num_global_observations'] == 0:
        print(f"  \u26a0\ufe0f  WARNING: Predictor has NO global observations (fresh start)!")
        print(f"      This means the predictor did NOT carry over training knowledge.")
        print(f"      Expected: {proactive_timesteps//100} episodes * observations")
        print(f"      Actual: {stats['num_global_observations']} observations")
        print(f"      \u2192 Poor predictions lead to suboptimal wait decisions!")
