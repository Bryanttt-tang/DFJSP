...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            ...
            if 'train/policy_gradient_loss' in log_data:
                old_value = self._latest_train_metrics['policy_loss']
                self._latest_train_metrics['policy_loss'] = float(log_data['train/policy_gradient_loss'])
                if old_value != self._latest_train_metrics['policy_loss']:
            ...
            if metrics_updated and self.verbose >= 1 and getattr(self.model, "num_timesteps", 0) <= 5000:
                timestep = getattr(self.model, "num_timesteps", 0)
                if timestep % 512 == 0:  # Only print occasionally
            ...
            # Debug: Show when logger is not available
            if self.verbose >= 2 and getattr(self.model, "num_timesteps", 0) % 1000 == 0:
                print(f"[{self.method_name}] No logger available at timestep {getattr(self.model, 'num_timesteps', 0)}")
        ...
        if ep_buf is not None:
            ...
            if current_len > self._processed_episodes:
                ...
                for idx in range(self._processed_episodes, current_len):
                    ...
                    # This ensures all metrics have same length as episode_rewards
                    ...
                    if self.verbose >= 1 and len(TRAINING_METRICS['episode_rewards']) <= 5:
                ...
                # Debug: Show when no new episodes
                if self.verbose >= 2 and getattr(self.model, "num_timesteps", 0) % 1000 == 0:
                    print(f"[{self.method_name}] No new episodes yet (buffer length: {current_len})")
        else:
            # Debug: Show when buffer is None
            if self.verbose >= 2 and getattr(self.model, "num_timesteps", 0) % 1000 == 0:
                print(f"[{self.method_name}] Episode buffer is None")
