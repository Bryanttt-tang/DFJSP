...
GLOBAL_SEED = 1234  # Changed from 42 to force fresh results
...
class StaticFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        4. STATIC RL: No future arrival info, no future processing info (all zeros)
        """
        obs = []
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        2. Processing times for ready operations on each machine
        3. Which machines are currently idle
        4. DYNAMIC RL: Only normalized delay for unarrived jobs (no exact arrival times, no future processing info)
        """
        obs = []
        ...
        for job_id in self.job_ids:
            if job_id not in self.arrived_jobs:
                ...
                if arrival_time != float('inf') and arrival_time <= self.current_makespan + 30:
                    ...
                    normalized_delay = min(1.0, delay / 30.0)
        ...
        # 5. DYNAMIC RL: No future processing times (all zeros)
        for job_id in self.job_ids:
            for machine in self.machines:
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        2. Processing times for ready operations on each machine
        3. Which machines are currently idle
        4. PERFECT RL: Exact normalized arrival times for unarrived jobs
        5. PERFECT RL: Future processing times for unarrived jobs
        """
        obs = []
        ...
        for job_id in self.job_ids:
            if job_id not in self.arrived_jobs:
                ...
                if arrival_time != float('inf'):
                    delay = max(0, arrival_time - self.current_makespan)
                    normalized_delay = min(1.0, delay / 50.0)
                    obs.append(normalized_delay)
                else:
                    obs.append(0.0)
            else:
            ...
            if job_id not in self.arrived_jobs and len(self.jobs[job_id]) > 0:
                ...
                for machine in self.machines:
                    if machine in first_op['proc_times']:
                        ...
                        normalized_proc = min(1.0, proc_time / 10.0)
...
def generate_test_scenarios(jobs_data, initial_jobs=[0, 1, 2, 3, 4], arrival_rate=0.08, num_scenarios=10):
    ...
    for i in range(num_scenarios):
        test_seed = 5000 + i  # Changed from 1000 to 5000 range for fresh test scenarios
...
def verify_schedule_correctness(schedule, jobs_data, arrival_times, method_name):
    ...
    print(f"    Scheduled ops: {scheduled_ops}")
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times):
    ...
    print("\n--- Running MILP Optimal Scheduler (Fresh Computation - No Cache) ---")
...
def calculate_regret_analysis(optimal_makespan, methods_results):
    ...
    # Check if hierarchy is as expected
    expected_order = (
        valid_methods['Perfect Knowledge RL']['absolute_regret'] <= 
        valid_methods['Dynamic RL']['absolute_regret'] <= 
        valid_methods['Static RL (dynamic)']['absolute_regret']
    )
    ...
    # Recommendations
    print(f"\nRecommendations:")
    if 'Dynamic RL' in valid_methods and 'Static RL (dynamic)' in valid_methods:
        dynamic_regret = valid_methods['Dynamic RL']['relative_regret_percent']
        static_regret = valid_methods['Static RL (dynamic)']['relative_regret_percent']
        
        if dynamic_regret < static_regret:
            print("- Dynamic RL is outperforming Static RL (dynamic), consider using Dynamic RL")
        else:
            print("- Static RL (dynamic) is outperforming Dynamic RL, investigate potential issues with Dynamic RL")
    
    if 'Perfect Knowledge RL' in valid_methods:
        pk_regret = valid_methods['Perfect Knowledge RL']['relative_regret_percent']
        if pk_regret > 10:
            print("- Perfect Knowledge RL has high regret, consider: longer training, better hyperparameters, or state representation issues")
...
def main():
    ...
    print("OPTIMIZED: Unified observation space (59 elements) with informational hierarchy:")
    print("       - All RL methods use same observation size for evaluation compatibility")
    print("       - Static RL: Ready jobs + Machine status + Processing times + ZEROS")
    print("       - Dynamic RL: + Future arrival time hints + ZEROS for processing times")  
    print("       - Perfect RL: + Exact arrival times + Future processing times")
    print("       - This creates clear advantage hierarchy: Perfect > Dynamic > Static")
