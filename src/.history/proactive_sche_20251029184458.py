...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
        ...
        # ENHANCED Perfect knowledge observation space for optimal planning
        obs_size = (
            self.num_jobs +                         # Ready job indicators (all 1.0 for perfect knowledge)
            self.num_jobs +                         # Job progress (completed_ops / total_ops)
            len(self.machines) +                    # ⭐ Machine next_free times (NORMALIZED - CRITICAL!)
            self.num_jobs * len(self.machines) +    # Processing times for NEXT operations
            self.num_jobs +                         # ⭐ Job remaining processing time (sum of unscheduled ops)
            self.num_jobs +                         # ⭐ PERFECT: Exact arrival times (normalized)
            self.num_jobs +                         # ⭐ Time until/since arrival (relative to current makespan)
            1                                       # ⭐ Current scheduling progress (operations_scheduled / total)
        )
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(obs_size,), dtype=np.float32
        )
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_makespan, current_makespan):
        """ENHANCED reward for perfect knowledge - guides to optimal schedules."""
        
        if self.reward_mode == "makespan_increment":
            # Primary reward: Minimize makespan increment (main objective)
            makespan_increment = current_makespan - previous_makespan
            reward = -makespan_increment
            
            # ⭐ CRITICAL: Penalty for machine idle time (encourages efficiency)
            # Idle time means we could have scheduled this operation earlier
            idle_penalty = idle_time * 0.3
            reward -= idle_penalty
            
            # ⭐ Small progress bonus (encourages taking actions)
            progress_bonus = 0.05
            reward += progress_bonus
            
            # ⭐ CRITICAL: Large completion bonus (strongly encourage finishing)
            if done:
                # Inverse makespan bonus: shorter makespan = higher reward
                # This creates strong pressure to minimize final makespan
                completion_bonus = 50.0 / max(current_makespan, 1.0)
                reward += completion_bonus
            
            return reward
        else:
            # Alternative reward mode (not typically used for perfect knowledge)
            reward = 0.0
    ...
    def _get_observation(self):
        """ENHANCED Perfect knowledge observation for optimal planning."""
        obs = []
        
        # 1. Ready job indicators - For Perfect Knowledge: ALL jobs with remaining ops are ready
        for job_id in self.job_ids:
            if self.next_operation[job_id] < len(self.jobs[job_id]):
                obs.append(1.0)  # Job has remaining operations (always schedulable with perfect knowledge)
            else:
        ...
        # 3. ⭐ CRITICAL: Machine next_free times (normalized) - Essential for planning!
        max_machine_time = max(self.machine_next_free.values()) if self.machine_next_free else 0.0
        time_horizon_estimate = max(self.max_time_horizon * 0.5, max_machine_time + 50)
        
        for machine in self.machines:
            machine_free_time = self.machine_next_free[machine]
            # Normalize by estimated time horizon
        ...
        # 5. ⭐ NEW: Job remaining processing time (critical for prioritization)
        for job_id in self.job_ids:
            remaining_time = 0.0
            ...
            # Normalize by reasonable upper bound
            normalized_remaining = min(1.0, remaining_time / (self.max_proc_time * 10))
            obs.append(normalized_remaining)
        
        # 6. ⭐ PERFECT KNOWLEDGE: Exact arrival times (normalized)
        for job_id in self.job_ids:
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
            normalized_arrival = min(1.0, arrival_time / self.max_time_horizon)
            obs.append(normalized_arrival)
        
        # 7. ⭐ NEW: Time until/since arrival (relative to current makespan)
        for job_id in self.job_ids:
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
            time_diff = arrival_time - self.current_makespan
            
            if time_diff > 0:
                # Job arrives in future
                normalized_until = min(1.0, time_diff / 50.0)
                obs.append(normalized_until)
            else:
                # Job already arrived (or at t=0)
                obs.append(0.0)
        
        # 8. ⭐ NEW: Overall scheduling progress
        progress = self.operations_scheduled / self.total_operations if self.total_operations > 0 else 1.0
        obs.append(progress)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # ENHANCED hyperparameters for Perfect Knowledge RL
    # Goal: Enable agent to learn near-optimal policies by providing stable training
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=1e-4,        # ⭐ LOWER learning rate for stability (was 3e-4)
        ...
        gamma=0.99,                # ⭐ CRITICAL: Discount factor < 1 for long-horizon planning (was 1.0!)
        ...
        policy_kwargs=dict(
            net_arch=[512, 512, 256, 128],  # ⭐ LARGER network for complex planning (was [256, 256, 128])
