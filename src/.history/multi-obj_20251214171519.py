...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # REWARD: Makespan increment + tardiness penalty
        makespan_increment = self.current_makespan - previous_makespan
        
        # Calculate tardiness penalty if job completed
        tardiness_penalty = 0.0
        if self.job_progress[job_id] >= len(self.jobs[job_id]):
            # Job just completed - check tardiness
            if self.job_due_dates.get(job_id) is not None:
                due_date = self.job_due_dates[job_id]
                completion_time = self.job_end_times[job_id]
