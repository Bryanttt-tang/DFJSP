...
class StaticFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        job_id = self.job_ids[job_idx]
        # Debug: Print job, op, machine, and arrival status
        print(f"[StaticFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={self.machines[machine_idx]}, arrived={job_id in self.arrived_jobs}, next_op={self.next_operation[job_id]}")
        ...
        # After scheduling, print scheduled jobs so far
        scheduled_jobs = set()
                ...
                if len(op_data) >= 3:
                    ...
                    if 'J' in job_op:
                        try:
                            ...
                            pass
        print(f"[StaticFJSPEnv] Scheduled jobs so far: {sorted(scheduled_jobs)}")
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        job_id = self.job_ids[job_idx]
        # Debug: Print job, op, machine, and arrival status
        print(f"[PoissonDynamicFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={self.machines[machine_idx]}, arrived={job_id in self.arrived_jobs}, next_op={self.next_operation[job_id]}")
        ...
        # Debug: Print scheduled jobs and missing jobs
        scheduled_jobs = set()
        for machine_ops in self.schedule.values():
            for op_data in machine_ops:
                if len(op_data) >= 3:
        ...
        print(f"[Eval Dynamic on Dynamic] Scheduled jobs: {sorted(scheduled_jobs)}, Missing jobs: {sorted(missing_jobs)}")
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        job_id = self.job_ids[job_idx]
        # Debug: Print job, op, machine, and arrival status
        print(f"[PerfectKnowledgeFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={self.machines[machine_idx]}, arrived={job_id in self.arrived_jobs}, next_op={self.next_operation[job_id]}")
