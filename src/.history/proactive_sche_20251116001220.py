...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        Args:
            ...
            use_predictor_for_wait: Whether to use arrival predictor
            max_wait_time: Maximum time agent can wait in single wait action (not used with simplified waits)
    ...
    def _execute_wait_action_with_predictor_guidance(self, wait_duration):
        ...
        Note: Predictor is still used for learning/observations, but reward is simplified.
        """
        previous_makespan = self.current_makespan
        ...
        # Update makespan: ensure it's at least event_time (time passes even if idle)
        self.current_makespan = max(self.current_makespan, self.event_time)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        model = MaskablePPO(
            ...
            n_epochs=150               # ⭐ More epochs = better learning per rollout
            gamma=1.0,                 # Undiscounted (makespan)
            gae_lambda=0.95,           # GAE for advantage estimation
            clip_range=0.2,            # Standard PPO clipping
            ent_coef=0.00,           # ⭐ VERY LOW entropy (deterministic scenario!)
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    print(f"  Total steps: {step_count} (scheduled={schedule_count}, waited={wait_count})")
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    ...
    print(f"\n  🔍 MILP OPTIMAL using arrival times: {arrival_times12")
    ...
    # CRITICAL WARNING: CBC solver can return "Optimal" even when stopped on time limit!
    # The solver output above shows:
    # - "Result - Stopped on time limit" means it did NOT prove optimality
    # - "Gap: X" shows the optimality gap (e.g., 3% means solution could be 3% worse than true optimal)
    # - "Lower bound: Y" gives the proven lower bound on the optimal objective
    #
    # If you see "Stopped on time limit", the solution is FEASIBLE but NOT proven optimal!
    
    if LpStatus[prob.status] == "Optimal":
        print("✅ Solver returned 'Optimal' status.")
        print("⚠️  WARNING: Check solver output above for 'Stopped on time limit'")
        print("⚠️  If time limit was hit, solution is FEASIBLE but NOT proven optimal!")
        print("⚠️  The 'Gap' value shows how far from true optimal it might be.")
    else:
    ...
    if status == "Optimal" or status == "Not Solved" or status == "Optimal":  # Prefer explicit 'Optimal' but accept near-optimal?
        ...
        if math.isfinite(actual_makespan) and abs(actual_makespan - optimal_makespan) <= 1e-6:
            if verbose:
                print(f"✅ MILP solution validated. Makespan = {optimal_makespan:.2f} (PROVEN OPTIMAL)")
            ...
            if verbose:
                ...
                print(f"   MILP claim: {optimal_makespan:.2f}, actual: {actual_makespan:.2f}")
            return float('inf'), {}
    else:
        if verbose:
  ...
  # Same as reactive RL for fair comparison
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        perfect_model = train_perfect_knowledge_agent(
            ...
            reward_mode="makespan_increment", 
            learning_rate=learning_rate,
            num_initializations=3,  # ⭐ Try 5 random initializations
        ...
        if perfect_makespan < milp_makespan - 0.01:  # Allow tiny numerical tolerance
            gap = ((milp_makespan - perfect_makespan) / milp_makespan) * 100
            print(f"    🎯 EXCELLENT: Perfect RL ({perfect_makespan:.2f}) beats MILP ({milp_makespan:.2f}) by {gap:.2f}%!")
            print(f"    � EXPLANATION: MILP solver likely stopped on TIME LIMIT (not proven optimal)")
            print(f"    📊 Check solver output above for 'Stopped on time limit' and 'Gap' value")
            print(f"    ✅ Perfect RL found better solution than MILP's time-limited result")
