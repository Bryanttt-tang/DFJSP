...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        obs_size = (
            ...
            self.num_jobs +                         # Job next-op-ready-time
            1 +                                     # Current makespan/time
            self.num_jobs                           # Job arrival times (normalized)
        )
    ...
    def _get_observation(self):
        ...
        3. Job next-operation-ready-time (absolute time, normalized)
        4. Current makespan/time (unnormalized absolute time)
        5. Job arrival times (dynamic: exact arrival times, normalized)
        """
        obs = []
        ...
        # 5. Job arrival times (dynamic: exact arrival times, normalized)
        # Dynamic RL gets same arrival information as Perfect Knowledge RL
        for job_id in self.job_ids:
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Use SAME global seed for fair comparison with Dynamic and Static RL
    # This ensures all methods have the same training randomness
    arrival_seed = GLOBAL_SEED  # Use same seed as Dynamic and Static RL
    print(f"    Using consistent global seed: {arrival_seed} (same as Dynamic/Static RL)")
    
    def make_perfect_env():
        # Use SAME environment class as Dynamic RL for consistency!
        # Create PoissonDynamicFJSPEnv but override arrival times to be deterministic
        env = PoissonDynamicFJSPEnv(
            jobs_data, machine_list,
            initial_jobs=[k for k, v in arrival_times.items() if v == 0],
            arrival_rate=0.1,  # Rate doesn't matter since we'll override with exact times
            reward_mode=reward_mode,
            seed=arrival_seed
        )
        # Override with exact arrival times (this makes it "perfect knowledge")
    ...
    # Use IDENTICAL hyperparameters as Dynamic RL for fair comparison
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=learning_rate,
        n_steps=2048,              # Match Dynamic RL exactly
        batch_size=128,            # Match Dynamic RL exactly
        n_epochs=10,               # Match Dynamic RL exactly
        gamma=1,                   # Match Dynamic RL exactly
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,             # Match Dynamic RL exactly (not higher)
        vf_coef=0.5,
        max_grad_norm=0.5,
        seed=arrival_seed,         # Use scenario-specific seed
        policy_kwargs=dict(
            net_arch=[512, 512, 256],  # Match Dynamic RL network size exactly
    ...
    # Do a quick evaluation before training to establish baseline using SAME environment as training
    ...
    test_env.job_arrival_times = arrival_times.copy()  # Override with exact times
    ...
    with tqdm(total=total_timesteps, desc=f"Perfect Knowledge RL (Scenario {arrival_seed % 100})", 
    ...
    # Comprehensive post-training evaluation with multiple runs
    print(f"    Running post-training evaluation (5 runs)...")
    post_training_makespans = []
    ...
    for eval_run in range(5):
        try:
            ...
            while not done and steps < 200:  # Increased step limit
                ...
                obs, reward, done, truncated, info = test_env.step(action)
                steps += 1
            post_training_makespans.append(test_env.env.current_makespan)
        except Exception as e:
            print(f"    ⚠️  Evaluation run {eval_run+1} failed: {e}")
            post_training_makespans.append(999.0)
    
    if post_training_makespans:
        best_makespan = min(post_training_makespans)
        avg_makespan = np.mean(post_training_makespans)
        print(f"    Post-training: Best={best_makespan:.2f}, Avg={avg_makespan:.2f}")
        
        if best_makespan < random_makespan:
            improvement = random_makespan - best_makespan
            print(f"    ✅ Training improved by {improvement:.2f} (random: {random_makespan:.2f})")
        else:
            print(f"    ⚠️  Training did not improve (random: {random_makespan:.2f}, best trained: {best_makespan:.2f})")
            print(f"    ⚠️  This suggests Perfect Knowledge RL is not converging properly!")
    else:
        print(f"    ❌ All post-training evaluations failed!")
    
    print(f"    ✅ Perfect knowledge training completed for this scenario!")
    return model

def train_static_agent(jobs_data, machine_list, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,         # Match Dynamic RL exactly
        vf_coef=0.5,           # Match Dynamic RL exactly
        max_grad_norm=0.5,     # Match Dynamic RL exactly
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    """
    ...
    # IDENTICAL hyperparameters for fair comparison across all RL methods
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=learning_rate,        # Same as Perfect Knowledge and Static RL
        n_steps=2048,              # Same as Perfect Knowledge and Static RL
        batch_size=128,            # Same as Perfect Knowledge and Static RL
        n_epochs=10,               # Same as Perfect Knowledge and Static RL
        gamma=1,                   # Same as Perfect Knowledge and Static RL
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,             # Same as Perfect Knowledge and Static RL
        vf_coef=0.5,               # Same as Perfect Knowledge and Static RL
        max_grad_norm=0.5,         # Same as Perfect Knowledge and Static RL
        seed=GLOBAL_SEED,          # Ensure reproducibility
        policy_kwargs=dict(
            net_arch=[512, 512, 256],  # Same as Perfect Knowledge and Static RL
            activation_fn=torch.nn.ReLU
        )
    )
...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    ...
    test_env = PoissonDynamicFJSPEnv(
        jobs_data, machine_list,
        initial_jobs=[k for k, v in arrival_times.items() if v == 0],
        arrival_rate=0.1,  # Rate doesn't matter since we override
        reward_mode=reward_mode,
        seed=eval_seed
    )
    ...
    # Reset after overriding arrival times
    obs, _ = test_env.reset()
    
    # Ensure the override persists after reset
...
def evaluate_perfect_knowledge_on_scenario(perfect_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    """Evaluate perfect knowledge agent using the SAME environment as Dynamic RL."""
    print(f"  Perfect Knowledge RL evaluation (using same environment as Dynamic RL)...")
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 100000    # Same timesteps for fair comparison
    dynamic_timesteps = 100000   # Same timesteps for fair comparison  
    static_timesteps = 100000    # Same timesteps for fair comparison
    ...
    print("UPDATED: Dynamic RL observation now includes exact arrival times (same as Perfect Knowledge)")
    print("         This provides fair comparison between Perfect Knowledge and Dynamic RL agents")
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        if perfect_makespan > dynamic_makespan:
            ...
            print(f"      Perfect RL should always be better since it's trained specifically for this scenario!")
            print(f"      Possible issues: insufficient training, poor convergence, or hyperparameter problems")        # Check if Dynamic RL is giving same result as previous scenarios
