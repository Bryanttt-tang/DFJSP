...
class StaticFJSPEnv(gym.Env):
    ...
    Uses compact event-driven observation for efficient learning.
    """
    ...
    def __init__(self, jobs_data, machine_list, reward_mode="makespan_increment", seed=None):
        ...
        # Use SAME action space as other environments
        self.action_space = spaces.Discrete(
            min(self.num_jobs * self.max_ops_per_job * len(self.machines), 1000)
        )
        
        # COMPACT observation space: job ready flags + processing times + machine idle flags
        # For each job: 1 ready flag + 3 processing times = 4 values per job
        # For machines: 1 idle flag per machine = 3 values
        obs_size = self.num_jobs * (1 + len(self.machines)) + len(self.machines)
