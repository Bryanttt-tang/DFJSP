...
# Global metrics store (you can also import this from your module)
TRAINING_METRICS = {
    'method_name': None,
    'timesteps': [],            # recorded when we snapshot train/* scalars
    'episode_timesteps': [],    # model.num_timesteps at episode end
    'episode_rewards': [],      # episodic returns (r)
    'episode_lengths': [],      # episodic lengths (l)
    'episode_count': [],        # 1..N
    # Per-episode snapshots of training scalars (may be None if not present)
    'policy_loss': [],
    'value_loss': [],
    'action_entropy': [],
    'learning_rate': [],
    'explained_variance': [],
    # Optional: raw logger dump per episode (debug)
    'logger_snapshot': []
}
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _advance_to_next_arrival(self):
        ...
        # If there are future arrivals, advance time to the next one.
        if next_arrival_time != float('inf'):
            self.event_time = next_arrival_time
            # Update the set of arrived jobs
            newly_arrived = set()
            for job_id, arrival_time in self.job_arrival_times.items():
            ...
            return len(newly_arrived) > 0
        
        # If no future arrivals, it means we are waiting for a machine to be free.
        # In this simplified model, we can just advance time by a fixed step.
        self.event_time += 1.0
        return False
    ...
    def step(self, action):
        ...
        if job_idx is None:
            ...
            new_arrival_happened = self._advance_to_next_arrival()
            ...
            if not new_arrival_happened and not future_arrivals_possible and not np.any(self.action_masks()[:-1]):
    ...
    def _get_observation(self):
        ...
        # 4. DYNAMIC RL: Arrival pattern features
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
        ...
        # Simplified action space without WAIT action (like possion_job_backup3.py)
...
def reset_training_metrics(method_name=None):
    global TRAINING_METRICS
    for k in TRAINING_METRICS.keys():
        if isinstance(TRAINING_METRICS[k], list):
            TRAINING_METRICS[k].clear()
    if method_name:
        TRAINING_METRICS['method_name'] = method_name
...
#     """
#     Enhanced callback that logs ALL metrics per episode:
#       - Episode rewards from ep_info_buffer (per episode)
#       - Training metrics from logger cached and assigned to episodes
#       - All metrics aligned to episode numbers for consistent plotting
#     """
...
#         # Cache latest training metrics to assign to new episodes
...
#         if self.verbose:
#             print(f"[{self.method_name}] Callback init. Will record ALL metrics per episode.")
#             print(f"[{self.method_name}] Initial metrics cache: {self._latest_train_metrics}")
...
#         # Update cached training metrics from logger (every step to keep them fresh)
#         if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
#             log_data = dict(self.model.logger.name_to_value)
            
#             # Cache latest training metrics (will be assigned to next completed episode)
#             metrics_updated = False
#             old_policy_loss = self._latest_train_metrics['policy_loss']
            
#             if 'train/policy_gradient_loss' in log_data:
#                 self._latest_train_metrics['policy_loss'] = float(log_data['train/policy_gradient_loss'])
#                 if old_policy_loss != self._latest_train_metrics['policy_loss']:
#                     metrics_updated = True
#             if 'train/value_loss' in log_data:
#                 self._latest_train_metrics['value_loss'] = float(log_data['train/value_loss'])
#             if 'train/entropy_loss' in log_data:
#                 self._latest_train_metrics['entropy_loss'] = float(log_data['train/entropy_loss'])
#             if 'train/loss' in log_data:
#                 self._latest_train_metrics['total_loss'] = float(log_data['train/loss'])
#             if 'train/learning_rate' in log_data:
#                 self._latest_train_metrics['learning_rate'] = float(log_data['train/learning_rate'])
#             if 'train/explained_variance' in log_data:
#                 self._latest_train_metrics['explained_variance'] = float(log_data['train/explained_variance'])
            
#             # Debug: Show when training metrics get updated (first few times only)
#             if metrics_updated and self.verbose >= 1:
#                 timestep = getattr(self.model, "num_timesteps", 0)
#                 if timestep <= 5000 and timestep % 512 == 0:  # Only print occasionally
#                     print(f"[{self.method_name}] Training metrics updated at timestep {timestep}: "
#                           f"policy_loss={self._latest_train_metrics['policy_loss']:.6f}, "
#                           f"value_loss={self._latest_train_metrics['value_loss']:.6f if self._latest_train_metrics['value_loss'] is not None else 'N/A'}")
...
#                 # Debug: Show when episodes are being processed
#                 if self.verbose >= 1:
#                     print(f"[{self.method_name}] Processing {current_len - self._processed_episodes} new episodes (buffer length: {current_len})")
...
#                     current_timestep = getattr(self.model, "num_timesteps", 0)
                    
#                     # Record episode metrics
#                     TRAINING_METRICS['episode_rewards'].append(episode_reward)
#                     TRAINING_METRICS['episode_lengths'].append(episode_length)
#                     TRAINING_METRICS['episode_timesteps'].append(current_timestep)
#                     TRAINING_METRICS['episode_count'].append(len(TRAINING_METRICS['episode_rewards']))
                    
#                     # Assign cached training metrics to this episode
#                     # Use 0.0 as default for early episodes before training metrics are available
#                     TRAINING_METRICS['policy_loss'].append(
#                         self._latest_train_metrics['policy_loss'] if self._latest_train_metrics['policy_loss'] is not None else 0.0)
#                     TRAINING_METRICS['value_loss'].append(
#                         self._latest_train_metrics['value_loss'] if self._latest_train_metrics['value_loss'] is not None else 0.0)
#                     TRAINING_METRICS['action_entropy'].append(
#                         self._latest_train_metrics['entropy_loss'] if self._latest_train_metrics['entropy_loss'] is not None else 0.0)
#                     TRAINING_METRICS['total_loss'].append(
#                         self._latest_train_metrics['total_loss'] if self._latest_train_metrics['total_loss'] is not None else 0.0)
#                     TRAINING_METRICS['learning_rate'].append(
#                         self._latest_train_metrics['learning_rate'] if self._latest_train_metrics['learning_rate'] is not None else 0.0)
#                     TRAINING_METRICS['explained_variance'].append(
#                         self._latest_train_metrics['explained_variance'] if self._latest_train_metrics['explained_variance'] is not None else 0.0)
#                     TRAINING_METRICS['timesteps'].append(current_timestep)
                    
#                     # Debug logging for first few episodes
#                     if self.verbose >= 1 and len(TRAINING_METRICS['episode_rewards']) <= 10:
#                         policy_loss_str = f"{self._latest_train_metrics['policy_loss']:.4f}" if self._latest_train_metrics['policy_loss'] is not None else "N/A"
#                         value_loss_str = f"{self._latest_train_metrics['value_loss']:.4f}" if self._latest_train_metrics['value_loss'] is not None else "N/A"
#                         print(f"[{self.method_name}] ✅ Episode {len(TRAINING_METRICS['episode_rewards'])}: "
#                               f"reward={episode_reward:.2f}, length={episode_length}, policy_loss={policy_loss_str}, value_loss={value_loss_str}")
...
class EnhancedTrainingCallback(BaseCallback):
    """
    Callback that:
     - Drains model.ep_info_buffer each step to capture EVERY finished episode.
     - For each finished episode, snapshots available train/* logger scalars so
       you can align losses/entropy with episode boundaries.
     - Records model.num_timesteps at episode end so plotting is aligned.
    Usage:
        cb = EnhancedEpisodeCallback(method_name="Dynamic RL", verbose=1, print_every_n_episodes=50)
        model.learn(..., callback=cb)
    """

    def __init__(self, method_name="RL", pbar=None, verbose=0, print_every_n_episodes=50):
        super().__init__(verbose)
        self.method_name = method_name
        self.pbar = pbar
    ...
    def _on_training_start(self) -> None:
        ...
        if self.verbose:
            print(f"[{self.method_name}] EnhancedEpisodeCallback initialized.")

    def _drain_ep_info_buffer(self):
        """
        Drain SB3 ep_info_buffer (deque). Return list of ep_info dicts popped.
        ep_info entries typically contain keys: {'r': return, 'l': length}
        """
        drained = []
        ep_buf = getattr(self.model, "ep_info_buffer", None)
        if ep_buf is None:
        ...
        while len(ep_buf) > 0:
            try:
                ...
                # defensive: some SB3 versions expose different types
                info = None
            if info is not None:
                drained.append(info)
        return drained

    def _snapshot_logger_scalars(self):
        """
        Grab a safe copy of model.logger.name_to_value and extract keys we care about.
        Returns a dict of selected scalars (may be missing keys).
        """
        snapshot = {}
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            # copy to avoid racing with logger updates
            log_copy = dict(self.model.logger.name_to_value)
            # Extract relevant keys if present
            snapshot['policy_loss'] = float(log_copy.get('train/policy_gradient_loss', np.nan)) if 'train/policy_gradient_loss' in log_copy else None
            snapshot['value_loss']  = float(log_copy.get('train/value_loss', np.nan)) if 'train/value_loss' in log_copy else None
            snapshot['action_entropy'] = float(log_copy.get('train/entropy_loss', np.nan)) if 'train/entropy_loss' in log_copy else None
            snapshot['learning_rate']  = float(log_copy.get('train/learning_rate', np.nan)) if 'train/learning_rate' in log_copy else None
            snapshot['explained_variance'] = float(log_copy.get('train/explained_variance', np.nan)) if 'train/explained_variance' in log_copy else None
            snapshot['raw'] = log_copy  # optional: full copy for debugging
        else:
            snapshot = {
                'policy_loss': None,
                'value_loss': None,
    ...
    def _on_step(self) -> bool:
        ...
        # Drain finished episodes (may be zero, one, or many per call depending on vectorization)
        drained_eps = self._drain_ep_info_buffer()
        if drained_eps:
            # snapshot logger scalars ONCE per batch of drained episodes
            logger_snapshot = self._snapshot_logger_scalars()
            current_timesteps = getattr(self.model, "num_timesteps", 0)

            # For each finished episode, record episode data plus snapshot (same snapshot used for all
            # episodes drained in this call - that's typical since episodes finish between rollouts)
            for info in drained_eps:
                # Standard ep_info format: {'r': total_reward, 'l': length}
                ep_r = float(info.get('r', np.nan))
                ep_l = int(info.get('l', -1))

                TRAINING_METRICS['episode_rewards'].append(ep_r)
                TRAINING_METRICS['episode_lengths'].append(ep_l)
                TRAINING_METRICS['episode_timesteps'].append(current_timesteps)
                TRAINING_METRICS['episode_count'].append(len(TRAINING_METRICS['episode_rewards']))

                # append scalar snapshots
                TRAINING_METRICS['policy_loss'].append(logger_snapshot.get('policy_loss'))
                TRAINING_METRICS['value_loss'].append(logger_snapshot.get('value_loss'))
                TRAINING_METRICS['action_entropy'].append(logger_snapshot.get('action_entropy'))
                TRAINING_METRICS['learning_rate'].append(logger_snapshot.get('learning_rate'))
                TRAINING_METRICS['explained_variance'].append(logger_snapshot.get('explained_variance'))
                TRAINING_METRICS['logger_snapshot'].append(logger_snapshot.get('raw'))

                self._local_episode_counter += 1

                # small verbose prints for first few episodes and periodic summary
                if self.verbose and self._local_episode_counter <= 5:
                    print(f"[{self.method_name}] Episode {self._local_episode_counter}: reward={ep_r:.3f}, length={ep_l}, t={current_timesteps}")
                elif self.verbose and (self._local_episode_counter % self.print_every_n_episodes == 0):
                    elapsed = time.time() - (self._start_time or time.time())
                    avg_reward = np.nanmean(TRAINING_METRICS['episode_rewards'][-self.print_every_n_episodes:])
                    print(f"[{self.method_name}] Episodes {self._local_episode_counter - self.print_every_n_episodes + 1}-{self._local_episode_counter}: "
                          f"avg_reward={avg_reward:.3f}, total_eps={self._local_episode_counter}, t={current_timesteps}, elapsed={elapsed:.1f}s")
    ...
    def _on_training_end(self) -> None:
        # final summary
        if self.verbose:
            total_eps = len(TRAINING_METRICS['episode_rewards'])
            total_steps = getattr(self.model, "num_timesteps", 0)
            print(f"[{self.method_name}] Training finished: {total_eps} episodes, {total_steps} timesteps recorded.")
...
def plot_training_metrics():
    """
    Plot training metrics with all data aligned to episode numbers.
    Shows raw data without moving averages as requested.
    All metrics should have the same length as episode_rewards.
    """
    global TRAINING_METRICS
    ...
    num_episodes = len(TRAINING_METRICS['episode_rewards'])
    metrics_lengths = {
        'episode_rewards': len(TRAINING_METRICS['episode_rewards']),
        'policy_loss': len(TRAINING_METRICS['policy_loss']),
        'value_loss': len(TRAINING_METRICS['value_loss']),
        'total_loss': len(TRAINING_METRICS['total_loss']),
        'action_entropy': len(TRAINING_METRICS['action_entropy']),
        'episode_lengths': len(TRAINING_METRICS['episode_lengths'])
    }
    
    print(f"Metrics lengths: {metrics_lengths}")
    
    # Create episode numbers for x-axis
    if TRAINING_METRICS['episode_count']:
        episode_numbers = TRAINING_METRICS['episode_count'][:num_episodes]
    else:
        episode_numbers = list(range(1, num_episodes + 1))
    
    # DEBUG: Print sample data
    rewards = TRAINING_METRICS['episode_rewards']
    print(f"Episode rewards sample (first 5): {rewards[:5]}")
    print(f"Episode rewards sample (last 5): {rewards[-5:]}")
    print(f"Episode rewards range: {min(rewards):.4f} to {max(rewards):.4f}")
    print(f"Episode numbers range: {min(episode_numbers)} to {max(episode_numbers)}")
    
    # Create comprehensive figure with 3x2 subplots
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.suptitle(f'Training Progress - {TRAINING_METRICS.get("method_name", "RL Agent")} ({num_episodes} Episodes)', 
                 fontsize=16, fontweight='bold')
    
    # Plot 1: Episode Rewards over Episodes (Top Left) - MAIN PLOT
    axes[0, 0].plot(episode_numbers, rewards, 'g-', linewidth=1.5, alpha=0.8, label='Episode Reward')
    axes[0, 0].set_xlabel('Episode Number')
    axes[0, 0].set_ylabel('Episode Reward')
    axes[0, 0].set_title('Episode Rewards Over Training (Raw Data)')
    axes[0, 0].grid(True, alpha=0.3)
    axes[0, 0].legend()
    
    # Add improvement analysis
    if len(rewards) > 20:
        early_reward = np.mean(rewards[:len(rewards)//4])
        late_reward = np.mean(rewards[-len(rewards)//4:])
        improvement = late_reward - early_reward
        
        improvement_text = f'Early: {early_reward:.2f} → Late: {late_reward:.2f}'
        if improvement > 0:
            axes[0, 0].text(0.02, 0.98, f'✅ Improving\n{improvement_text}\n(+{improvement:.2f})', 
                           transform=axes[0, 0].transAxes, verticalalignment='top',
                           bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
        else:
            axes[0, 0].text(0.02, 0.98, f'🔴 Declining\n{improvement_text}\n({improvement:.2f})', 
                           transform=axes[0, 0].transAxes, verticalalignment='top',
                           bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    
    # Plot 2: Total Loss over Episodes (Top Right)
    if TRAINING_METRICS['total_loss']:
        total_loss = TRAINING_METRICS['total_loss'][:num_episodes]
        loss_episodes = episode_numbers[:len(total_loss)]
        
        axes[0, 1].plot(loss_episodes, total_loss, 'purple', linewidth=1.5, alpha=0.8, label='Total Loss')
        axes[0, 1].set_xlabel('Episode Number')
        axes[0, 1].set_ylabel('Total Loss')
        axes[0, 1].set_title('Total Training Loss (Raw Data)')
        axes[0, 1].grid(True, alpha=0.3)
        axes[0, 1].legend()
        
        # Add trend analysis
        if len(total_loss) > 10:
            early_loss = np.mean(total_loss[:len(total_loss)//4])
            late_loss = np.mean(total_loss[-len(total_loss)//4:])
            reduction = early_loss - late_loss
            
            if reduction > 0:
                axes[0, 1].text(0.02, 0.98, f'✅ Decreasing\n(-{reduction:.6f})', 
                               transform=axes[0, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[0, 1].text(0.02, 0.98, f'🔴 Increasing\n(+{abs(reduction):.6f})', 
                               transform=axes[0, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        axes[0, 1].text(0.5, 0.5, 'No total loss data', ha='center', va='center', 
                       transform=axes[0, 1].transAxes, fontsize=14)
        axes[0, 1].set_title('Total Loss (No Data)')
    
    # Plot 3: Policy Loss over Episodes (Middle Left)
    if TRAINING_METRICS['policy_loss']:
        policy_loss = TRAINING_METRICS['policy_loss'][:num_episodes]
        loss_episodes = episode_numbers[:len(policy_loss)]
        
        axes[1, 0].plot(loss_episodes, policy_loss, 'r-', linewidth=1.5, alpha=0.8, label='Policy Loss')
        axes[1, 0].set_xlabel('Episode Number')
        axes[1, 0].set_ylabel('Policy Loss')
        axes[1, 0].set_title('Policy Gradient Loss (Raw Data)')
        ...
        if len(policy_loss) > 10:
            ...
            if reduction > 0:
                axes[1, 0].text(0.02, 0.98, f'✅ Decreasing\n(-{reduction:.6f})', 
                               transform=axes[1, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[1, 0].text(0.02, 0.98, f'🔴 Increasing\n(+{abs(reduction):.6f})', 
                               transform=axes[1, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
    ...
    # Plot 4: Value Loss over Episodes (Middle Right)
    if TRAINING_METRICS['value_loss']:
        value_loss = TRAINING_METRICS['value_loss'][:num_episodes]
        loss_episodes = episode_numbers[:len(value_loss)]
        
        axes[1, 1].plot(loss_episodes, value_loss, 'b-', linewidth=1.5, alpha=0.8, label='Value Loss')
        axes[1, 1].set_xlabel('Episode Number')
        axes[1, 1].set_ylabel('Value Loss')
        axes[1, 1].set_title('Value Function Loss (Raw Data)')
        ...
        if len(value_loss) > 10:
            ...
            if reduction > 0:
                axes[1, 1].text(0.02, 0.98, f'✅ Decreasing\n(-{reduction:.6f})', 
                               transform=axes[1, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[1, 1].text(0.02, 0.98, f'🔴 Increasing\n(+{abs(reduction):.6f})', 
                               transform=axes[1, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        axes[1, 1].text(0.5, 0.5, 'No value loss data', ha='center', va='center', 
                       transform=axes[1, 1].transAxes, fontsize=14)
        axes[1, 1].set_title('Value Loss (No Data)')
    
    # Plot 5: Entropy Loss over Episodes (Bottom Left)
    if TRAINING_METRICS['action_entropy']:
        entropy_loss = TRAINING_METRICS['action_entropy'][:num_episodes]
        loss_episodes = episode_numbers[:len(entropy_loss)]
        
        axes[2, 0].plot(loss_episodes, entropy_loss, 'orange', linewidth=1.5, alpha=0.8, label='Entropy Loss')
        axes[2, 0].set_xlabel('Episode Number')
        axes[2, 0].set_ylabel('Entropy Loss')
        axes[2, 0].set_title('Policy Entropy Loss (Raw Data)')
        axes[2, 0].grid(True, alpha=0.3)
        axes[2, 0].legend()
        
        # Add trend analysis
        if len(entropy_loss) > 10:
            early_loss = np.mean(entropy_loss[:len(entropy_loss)//4])
            late_loss = np.mean(entropy_loss[-len(entropy_loss)//4:])
            change = late_loss - early_loss
            
            # For entropy loss, more negative is better (higher exploration)
            if change < 0:
                axes[2, 0].text(0.02, 0.98, f'✅ More Exploration\n({change:.6f})', 
                               transform=axes[2, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[2, 0].text(0.02, 0.98, f'⚠️ Less Exploration\n(+{change:.6f})', 
                               transform=axes[2, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.7))
    else:
        axes[2, 0].text(0.5, 0.5, 'No entropy loss data', ha='center', va='center', 
                       transform=axes[2, 0].transAxes, fontsize=14)
        axes[2, 0].set_title('Entropy Loss (No Data)')
    
    # Plot 6: Episode Length over Episodes (Bottom Right)
    if TRAINING_METRICS['episode_lengths']:
        ep_lengths = TRAINING_METRICS['episode_lengths'][:num_episodes]
        length_episodes = episode_numbers[:len(ep_lengths)]
        
        axes[2, 1].plot(length_episodes, ep_lengths, 'teal', linewidth=1.5, alpha=0.8, label='Episode Length')
        axes[2, 1].set_xlabel('Episode Number')
        axes[2, 1].set_ylabel('Episode Length')
        axes[2, 1].set_title('Episode Length (Steps per Episode)')
        axes[2, 1].grid(True, alpha=0.3)
        axes[2, 1].legend()
        
        # Add trend analysis
        if len(ep_lengths) > 10:
            early_length = np.mean(ep_lengths[:len(ep_lengths)//4])
            late_length = np.mean(ep_lengths[-len(ep_lengths)//4:])
            change = late_length - early_length
            
            if change < 0:
                axes[2, 1].text(0.02, 0.98, f'✅ More Efficient\n({change:.1f} steps)', 
                               transform=axes[2, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            elif change > 0:
                axes[2, 1].text(0.02, 0.98, f'⚠️ Longer Episodes\n(+{change:.1f} steps)', 
                               transform=axes[2, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.7))
            else:
                axes[2, 1].text(0.02, 0.98, f'➡️ Stable Length\n({late_length:.1f} steps)', 
                               transform=axes[2, 1].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
    else:
        axes[2, 1].text(0.5, 0.5, 'No episode length data', ha='center', va='center', 
                       transform=axes[2, 1].transAxes, fontsize=14)
        axes[2, 1].set_title('Episode Length (No Data)')
    
    plt.tight_layout()
    plt.savefig('enhanced_ppo_training_metrics_rl_agent.png', dpi=300, bbox_inches='tight')
    print("✅ Enhanced training metrics plot saved: enhanced_ppo_training_metrics_rl_agent.png")
    plt.show()
