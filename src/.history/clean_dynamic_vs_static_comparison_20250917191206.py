...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        idle_time = max(0, start_time - machine_available_time)
        reward = self._calculate_reward(proc_time, idle_time, terminated, previous_makespan, self.current_makespan, len(newly_arrived))
...
# Enhanced heuristic functions based on the backup file
...
def improved_dispatching_heuristic(jobs_data, machine_list, arrival_times, job_sequencing_rule='SPT', machine_selection_rule='LWR'):
    """
    FIXED Two-stage heuristic: corrected time advancement and event handling.
    ...
    machine_workload = {m: 0.0 for m in machine_list}  # Total processing time assigned
    ...
    while operations_scheduled < total_operations:
        ...
        for job_id in arrived_jobs:
            ...
            if op_idx < len(jobs_data[job_id]):
                ...
                if job_ready_time <= current_time:
                    # Calculate minimum processing time for this operation
                    min_proc_time = min(op_data['proc_times'].values())
                    ready_operations.append((job_id, op_idx, op_data, job_ready_time, min_proc_time))
        ...
        if not ready_operations:
            ...
            for job_id in arrived_jobs:
                ...
                if op_idx > 0 and op_idx < len(jobs_data[job_id]):
                    next_event_times.append(operation_end_times[job_id][op_idx - 1])
            
            # Next job arrival
            for job_id, arr_time in arrival_times.items():
                if job_id not in arrived_jobs and arr_time > current_time:
                    next_event_times.append(arr_time)
        ...
        if job_sequencing_rule == 'SPT':
            selected_operation = min(ready_operations, key=lambda x: x[4])  # By min processing time
        elif job_sequencing_rule == 'LPT':
            selected_operation = max(ready_operations, key=lambda x: x[4])  # By max processing time
        elif job_sequencing_rule == 'FIFO':
            selected_operation = min(ready_operations, key=lambda x: arrival_times.get(x[0], 0.0))  # By arrival time
        elif job_sequencing_rule == 'LIFO':
            selected_operation = max(ready_operations, key=lambda x: arrival_times.get(x[0], 0.0))  # By arrival time (reverse)
        else:
            selected_operation = ready_operations[0]  # Default: first available
        
        job_id, op_idx, op_data, job_ready_time = selected_operation[:4]
        ...
        if machine_selection_rule == 'SPT':
            ...
            selected_machine = available_machines[0]  # Default: first available
        ...
        # Advance time to when this operation completes
...
def run_heuristic_comparison(jobs_data, machine_list, arrival_times):
    ...
    Based on backup approach with proper separation of concerns.
    """
...
def simple_spt_heuristic(jobs_data, machine_list, arrival_times):
    """
    Ultra-simple SPT heuristic based directly on backup logic.
    Just schedule the shortest available operation at each step.
    """
    print("\n--- Running Ultra-Simple SPT Heuristic ---")
    ...
    while operations_scheduled < total_operations:
        debug_iteration += 1
        if debug_iteration > 1000:  # Safety break
            print(f"Warning: Breaking after 1000 iterations, may have infinite loop")
            break
            
        candidate_operations = []
        
        # Check if we need to advance time for job arrivals
        current_min_machine_time = min(machine_next_free.values())
        for job_id, arrival_time in arrival_times.items():
            if job_id not in arrived_jobs and arrival_time <= current_min_machine_time:
                arrived_jobs.add(job_id)
            ...
            if op_idx < len(jobs_data[job_id]):
                ...
                for machine_name, proc_time in operation['proc_times'].items():
                    ...
                    candidate_operations.append((
                        proc_time, start_time, job_id, op_idx, machine_name
                    ))
        
        if not candidate_operations:
            # No operations available, need to advance time for job arrivals
            future_arrivals = [arr for job_id, arr in arrival_times.items() 
                             if job_id not in arrived_jobs and arr > current_min_machine_time]
            if not future_arrivals:
                break
            
            next_arrival = min(future_arrivals)
            # Advance all machines to this time
            for m in machine_list:
                machine_next_free[m] = max(machine_next_free[m], next_arrival)
            
            continue  # Try again with newly arrived jobs
        ...
        # Update arrived jobs based on new machine times
        current_max_time = max(machine_next_free.values())
        for job_id, arrival_time in arrival_times.items():
            if job_id not in arrived_jobs and arrival_time <= current_max_time:
                arrived_jobs.add(job_id)
    
    makespan = max(machine_next_free.values()) if machine_next_free else 0
    print(f"Ultra-Simple SPT Makespan: {makespan:.2f} (iterations: {debug_iteration})")
    return makespan, schedule
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    ...
    Uses enhanced heuristics based on the working backup.
    """
    print(f"  Testing improved heuristics with arrival times: {arrival_times}")
    ...
    # Also run the two-stage comparison
    two_stage_makespan, two_stage_schedule = run_heuristic_comparison(jobs_data, machine_list, arrival_times)
    
    # Return the better result
    if simple_spt_makespan <= two_stage_makespan:
        print(f"  Using Simple SPT (better than two-stage)")
        return simple_spt_makespan, simple_spt_schedule
    else:
        print(f"  Using Two-stage heuristic (better than simple SPT)")
        return two_stage_makespan, two_stage_schedule

def basic_greedy_scheduler(jobs_data, machine_list, arrival_times):
    """Basic greedy scheduler using improved dispatching heuristic."""
    return improved_dispatching_heuristic(jobs_data, machine_list, arrival_times, 'FIFO', 'EAM')

def optimized_spt_scheduler(jobs_data, machine_list, arrival_times):
    """Optimized SPT scheduler using improved dispatching heuristic."""
...
def earliest_completion_scheduler(jobs_data, machine_list, arrival_times):
    """Earliest completion scheduler using improved dispatching heuristic."""
    return improved_dispatching_heuristic(jobs_data, machine_list, arrival_times, 'SPT', 'EAM')
def fifo_heuristic(jobs_data, machine_list, arrival_times):
    """FIFO (First In First Out) - Use our basic greedy scheduler."""
...
def lifo_heuristic(jobs_data, machine_list, arrival_times):
    """LIFO (Last In First Out) - Use our optimized SPT scheduler.""" 
    return optimized_spt_scheduler(jobs_data, machine_list, arrival_times)


def spt_heuristic_simple(jobs_data, machine_list, arrival_times):
    """SPT (Shortest Processing Time) - Use our simple SPT heuristic."""
    return simple_spt_heuristic(jobs_data, machine_list, arrival_times)


def lpt_heuristic(jobs_data, machine_list, arrival_times): 
    """LPT (Longest Processing Time) - Use our earliest completion scheduler."""
    return earliest_completion_scheduler(jobs_data, machine_list, arrival_times)
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    ...
    opt_spt_makespan, opt_spt_schedule = optimized_spt_scheduler(jobs_data, machine_list, arrival_times)
    ...
    all_results = [
        ...
        (greedy_makespan, greedy_schedule, "Basic Greedy"),
        (opt_spt_makespan, opt_spt_schedule, "Optimized SPT"), 
        (ec_makespan, ec_schedule, "Earliest Completion")
    ]
...
def optimized_spt_scheduler(jobs_data, machine_list, arrival_times):
    ...
    while operations_scheduled < total_operations:
        ...
        if not ready_operations:
            ...
            next_time = min(next_events)
            for m in machine_list:
                machine_next_free[m] = max(machine_next_free[m], next_time)
            continue
