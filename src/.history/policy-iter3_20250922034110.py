...
# To test medium instance (bigger) uncomment below:
...
def canonicalize_state(next_ops, machine_next_free, job_ready_time, current_time, time_unit=1, cap=None):
    """
    Convert an internal absolute-time state into a canonical compact state matching RL observation space:
    UNIFIED observation space (59 elements total for 2 jobs, 3 machines):
    - Ready job indicators (2)
    - Machine idle status (3) 
    - Processing times for ready ops (6)
    - Future arrival info (2) - always zeros for static scenario
    - Future processing times (6) - always zeros for static scenario
    
    Returns hashable tuple: (ready_jobs, machine_idle, proc_times, future_arrivals, future_procs)
    """
    num_jobs = len(job_ids)
    num_machines = len(machines)
    
    # 1. Ready job indicators (which jobs have operations ready now)
    ready_jobs = []
    for j_idx, job_id in enumerate(job_ids):
        op_idx = next_ops[j_idx]
        if op_idx < len(JOBS_DATA[job_id]):
            # Job has remaining operations
            job_ready_time_val = job_ready_time[j_idx] if j_idx < len(job_ready_time) else ARRIVALS[job_id]
            is_ready = job_ready_time_val <= current_time
            ready_jobs.append(1 if is_ready else 0)
        else:
            ready_jobs.append(0)  # Job completed
    
    # 2. Machine idle status (which machines are idle now)
    machine_idle = []
    for m_idx in range(num_machines):
        machine_free_time = machine_next_free[m_idx] if m_idx < len(machine_next_free) else 0
        is_idle = machine_free_time <= current_time
        machine_idle.append(1 if is_idle else 0)
    
    # 3. Processing times for ready operations (normalized)
    max_proc_time = 10.0  # Same normalization as RL environments
    proc_times = []
    for j_idx, job_id in enumerate(job_ids):
        op_idx = next_ops[j_idx]
        if (op_idx < len(JOBS_DATA[job_id]) and 
            ready_jobs[j_idx] == 1):  # Job has ready operation
            operation = JOBS_DATA[job_id][op_idx]
            proc_dict = operation['proc_times']
            
            # Add processing time for each machine (0 if incompatible)
            for machine in machines:
                if machine in proc_dict:
                    proc_time = proc_dict[machine]
                    normalized_time = min(1.0, proc_time / max_proc_time)
                    proc_times.append(normalized_time)
                else:
                    proc_times.append(0.0)  # Machine cannot process this operation
        else:
            # Job not ready: add zeros for all machines
            for machine in machines:
                proc_times.append(0.0)
    
    # 4. Future arrival info (all zeros for static scenario - policy iteration assumes all jobs at t=0)
    future_arrivals = [0.0] * num_jobs
    
    # 5. Future processing times (all zeros for static scenario - no future job info in policy iteration)
    future_procs = [0.0] * (num_jobs * num_machines)

    return (tuple(ready_jobs), tuple(machine_idle), tuple(proc_times), 
            tuple(future_arrivals), tuple(future_procs))

def pretty_state(s):
    ready_jobs, machine_idle, proc_times, future_arrivals, future_procs = s
    return f"ready_jobs={ready_jobs}, machine_idle={machine_idle}, proc_times={proc_times[:6]}..."  # truncate for readability
...
while q and len(id_to_state) < MAX_STATES:
    ...
    for action in legal_actions:
        ...
        if len(id_to_state) <= nsid:
            # if newly created canonical state, add to BFS queue internal repr
...
# Show breakdown by next_ops
print("\n=== STATE SPACE BY next_ops (canonical) ===")
next_ops_counts = {}
for sid, canon in enumerate(id_to_state):
    next_ops = canon[0]
    next_ops_counts[next_ops] = next_ops_counts.get(next_ops, 0) + 1
for no, cnt in sorted(next_ops_counts.items()):
    print(f"  {no}: {cnt} states")
...
with open(csv_filename, 'w', newline='') as csvfile:
    ...
    writer.writerow(['state_id', 'next_ops', 'machine_rem', 'job_rem', 'num_actions', 'actions'])
    for sid, canon in enumerate(id_to_state):
        next_ops, mrem, jrem = canon
        acts = transitions.get(sid, [])
        actions_str = "; ".join([ (f"{a[0]}->sid{nsid}" if isinstance(a[0], tuple) and a[0][0]=='WAIT' else f"J{job_ids[a[0]]}-Op{a[1]+1}-M{a[2]+1}->sid{nsid}") for (a, nsid, r) in acts ])
        writer.writerow([sid, list(next_ops), list(mrem), list(jrem), len(acts), actions_str])
print("CSV saved.")
...
if incomplete_mdp:
    ...
    # init policy: pick first available action for each state
    policy = {}
    for sid in range(num_states):
        ...
        if not acts:
            ...
            # choose first non-WAIT if exists, else WAIT
            first_non_wait = next((a for a in acts if not (isinstance(a[0], tuple) and a[0][0]=='WAIT')), acts[0])
            policy[sid] = first_non_wait[0]  # store the action tuple (or ('WAIT',), ...)
    ...
    while True:
        ...
        # reward
        old_mk = compute_makespan_internal(cur_mnext, cur_jready)  # note: compute after update replicates style above
        new_mk = compute_makespan_internal(cur_mnext, cur_jready)
        reward = -(new_mk - old_mk)
        total_reward += reward
