...
# Generate dataset dynamically using utility functions
# Default configuration: 3 initial jobs, 4 future jobs, 3 machines
# To use a different configuration, call generate_fjsp_dataset() with desired parameters

# Use default predefined dataset (can be replaced with generated data)
# Uncomment below to use generated data:
...
# # Print the generated dataset structure in a table format
...
class ArrivalPredictor:
    """
    Predicts job arrival times using Maximum Likelihood Estimation (MLE) for Poisson processes.
    ...
    def __init__(self, initial_rate_guess=0.05):
        ...
        # Cross-episode learning: Store ALL inter-arrival times from all past episodes
        self.global_inter_arrivals = []  # Complete history across all episodes
        
        # Current episode tracking
        self.current_episode_arrivals = []  # Arrival times in current episode (sorted)
        self.current_episode_inter_arrivals = []  # Inter-arrival times computed this episode
    ...
    def reset_episode(self):
        ...
        # Keep global_inter_arrivals - this is the key to cross-episode learning!
    ...
    def finalize_episode(self, all_arrival_times):
        ...
        """
        # Extract all inter-arrival times from complete episode
        ...
        # ADD TO GLOBAL HISTORY - this accumulates across ALL episodes
    ...
    def _update_mle_estimate(self):
        ...
        This is the key to using past 100 episodes to improve current predictions!
        """
        # Combine historical data with current episode data
        all_data = self.global_inter_arrivals + self.current_episode_inter_arrivals
        
        if len(all_data) > 0:
            # Weight recent data more heavily (optional - can adjust)
            if len(self.current_episode_inter_arrivals) >= 3:
                # If we have enough current data, weight it 2x
    ...
    def _update_global_mle(self):
        """Update global MLE estimate using ALL historical data."""
        if len(self.global_inter_arrivals) > 0:
            mean_inter_arrival = np.mean(self.global_inter_arrivals)
            if mean_inter_arrival > 0:
    ...
    def predict_next_arrivals(self, current_time, num_jobs_to_predict, last_known_arrival=None):
        ...
        # If we have enough historical data, also consider variance
        if len(self.global_inter_arrivals) >= 10:
            # Add some stochasticity based on observed variance
            std_inter_arrival = np.std(self.global_inter_arrivals)
            # For now, use mean (can add confidence intervals later)
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    REALISTIC: Only arrived jobs can be scheduled + INTELLIGENT WAIT actions.
    
    ENHANCED WAIT ACTION DESIGN:
    - Uses ArrivalPredictor to learn arrival patterns across episodes
    - Supports multiple wait durations (short, medium, to-next-event)
    - Intelligent reward that considers opportunity cost vs. long-term benefit
    """
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        self.use_smart_wait = use_smart_wait  # Enable intelligent wait actions
        
        # Initialize arrival predictor for learning patterns
        ...
        # - WAIT_SHORT: Wait for predicted_mean_inter_arrival / 2
        # - WAIT_MEDIUM: Wait for predicted_mean_inter_arrival
        # - WAIT_TO_NEXT_EVENT: Wait until next arrival or machine completion (original behavior)
        max_scheduling_actions = self.num_jobs * len(self.machines)
        
        if self.use_smart_wait:
            self.num_wait_actions = 3  # SHORT, MEDIUM, TO_NEXT_EVENT
            self.action_space = spaces.Discrete(max_scheduling_actions + self.num_wait_actions)
            self.WAIT_SHORT_ACTION = max_scheduling_actions
            self.WAIT_MEDIUM_ACTION = max_scheduling_actions + 1
            self.WAIT_TO_NEXT_EVENT_ACTION = max_scheduling_actions + 2
        else:
            self.num_wait_actions = 1  # Only TO_NEXT_EVENT (original)
            self.action_space = spaces.Discrete(max_scheduling_actions + 1)
            self.WAIT_ACTION = max_scheduling_actions  # Backward compatibility
        
        self.cheat = False  # No cheating with future info
        
        # ENHANCED observation space (includes arrival prediction features when use_smart_wait=True)
        if self.use_smart_wait:
            obs_size = (
                ...
                self.num_jobs +                         # Time since arrival (per job)
                2 +                                     # Arrival progress and makespan progress
                # NEW: Arrival prediction features
                1 +                                     # Predicted next arrival time (normalized)
                1 +                                     # Prediction confidence (0-1)
                1 +                                     # Estimated arrival rate (normalized)
                1 +                                     # Time since last arrival (normalized)
                len(self.machines)                      # Idle machine indicators (binary)
            )
        else:
            # Original observation space (backward compatibility)
            obs_size = (
                self.num_jobs +                         # Ready job indicators
                self.num_jobs +                         # Job progress (completed_ops / total_ops)
                len(self.machines) +                    # Machine next_free times (normalized)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset arrival predictor for new episode (keeps cross-episode learning)
        if self.use_smart_wait:
    ...
    def _decode_action(self, action):
        """
        Decode action - includes multiple WAIT action types.
        
        Returns:
            - For scheduling actions: (job_idx, op_idx, machine_idx)
            - For wait actions: (None, None, wait_type_str) where wait_type_str in 
              ['WAIT_SHORT', 'WAIT_MEDIUM', 'WAIT_TO_NEXT_EVENT', 'WAIT'] (backward compat)
        """
        action = int(action)
        
        # Check if it's a wait action
        num_machines = len(self.machines)
        max_scheduling_actions = self.num_jobs * num_machines
        
        if self.use_smart_wait:
            if action == self.WAIT_SHORT_ACTION:
                return None, None, 'WAIT_SHORT'
            elif action == self.WAIT_MEDIUM_ACTION:
                return None, None, 'WAIT_MEDIUM'
            elif action == self.WAIT_TO_NEXT_EVENT_ACTION:
                return None, None, 'WAIT_TO_NEXT_EVENT'
        else:
            if action == self.WAIT_ACTION:
                return None, None, 'WAIT'  # Backward compatibility
    ...
    def action_masks(self):
        """Generate action masks based on arrival and machine compatibility."""
        ...
        for job_idx, job_id in enumerate(self.job_ids):
            ...
            for machine_idx, machine in enumerate(self.machines):
                if machine in self.jobs[job_id][next_op_idx]['proc_times']:
                    ...
                    if action < self.num_jobs * len(self.machines):
                        mask[action] = True
        
        # All WAIT actions are always valid (unless episode is over)
        if self.use_smart_wait:
            mask[self.WAIT_SHORT_ACTION] = True
            mask[self.WAIT_MEDIUM_ACTION] = True
            mask[self.WAIT_TO_NEXT_EVENT_ACTION] = True
        else:
    ...
    def _execute_wait_action(self, wait_type, scheduling_actions_available):
        """
        Execute wait action with different strategies.
        
        Args:
            wait_type: Type of wait action ('WAIT_SHORT', 'WAIT_MEDIUM', 'WAIT_TO_NEXT_EVENT', 'WAIT')
            scheduling_actions_available: Whether there were valid scheduling actions before waiting
            
        Returns:
            (new_event_time, new_arrivals, wait_reward)
        """
        old_event_time = self.event_time
        
        # Determine wait duration based on wait type
        if wait_type == 'WAIT_SHORT':
            # Wait for half of predicted mean inter-arrival time
            if self.use_smart_wait:
                stats = self.arrival_predictor.get_stats()
                mean_inter_arrival = stats['mean_inter_arrival']
                if mean_inter_arrival != float('inf'):
                    wait_duration = mean_inter_arrival / 2.0
                else:
                    wait_duration = 5.0  # Default if no predictions yet
            else:
                wait_duration = 5.0  # Fallback
            
            target_event_time = self.event_time + wait_duration
            
        elif wait_type == 'WAIT_MEDIUM':
            # Wait for predicted mean inter-arrival time
            if self.use_smart_wait:
                stats = self.arrival_predictor.get_stats()
                mean_inter_arrival = stats['mean_inter_arrival']
                if mean_inter_arrival != float('inf'):
                    wait_duration = mean_inter_arrival
                else:
                    wait_duration = 10.0  # Default
            else:
                wait_duration = 10.0  # Fallback
            
            target_event_time = self.event_time + wait_duration
            
        else:  # 'WAIT_TO_NEXT_EVENT' or 'WAIT' (backward compat)
            # Original behavior: wait until next arrival or machine completion
            target_event_time = self._get_next_event_time()
        
        # Advance event time and check for arrivals
        new_arrivals = self._update_event_time_and_arrivals(target_event_time)
        
        # Update predictor if new arrivals observed
        if self.use_smart_wait and new_arrivals > 0:
            for job_id in self.arrived_jobs:
                if job_id not in self.job_arrival_times or self.job_arrival_times[job_id] == self.event_time:
                    # This job just arrived
                    self.arrival_predictor.observe_arrival(self.event_time)
        
        # INTELLIGENT REWARD DESIGN:
        # Factors to consider:
        # 1. Opportunity cost: Were there scheduling actions available?
        # 2. Idle machines: How many machines are currently idle?
        # 3. Prediction quality: Did we wait based on good predictions?
        # 4. Result: Did waiting reveal new jobs?
        
        # Count idle machines
        num_idle_machines = sum(1 for m in self.machines if self.machine_next_free[m] <= old_event_time)
        
        # Base penalty for time passing
        time_penalty = -(target_event_time - old_event_time) * 0.1
        
        # Opportunity cost: Heavy penalty if we could have scheduled instead
        if scheduling_actions_available:
            opportunity_penalty = -5.0 * num_idle_machines  # More idle machines = worse to wait
        else:
            opportunity_penalty = -1.0  # Small penalty if no choice
        
        # Reward for successful prediction (if new jobs arrived)
        if new_arrivals > 0:
            prediction_reward = 5.0  # Reward for correctly anticipating arrivals
            
            # Extra reward if we used SHORT/MEDIUM waits successfully
            if wait_type in ['WAIT_SHORT', 'WAIT_MEDIUM']:
                prediction_reward += 3.0  # Bonus for efficient waiting
        else:
            prediction_reward = 0.0
            
            # Extra penalty for inefficient waits that didn't reveal jobs
            if wait_type in ['WAIT_SHORT', 'WAIT_MEDIUM']:
                prediction_reward = -2.0  # Penalize speculative waits that failed
        
        # Combine rewards
        wait_reward = time_penalty + opportunity_penalty + prediction_reward
        
        return target_event_time, new_arrivals, wait_reward

    def step(self, action):
        """Simplified step function for Poisson Dynamic environment with intelligent wait actions."""
        self.episode_step += 1
        ...
        if self.operations_scheduled >= self.total_operations:
            ...
            # Finalize episode for predictor (cross-episode learning)
            if self.use_smart_wait:
        ...
        if self.episode_step >= self.max_episode_steps:
            ...
            # Still finalize for learning
            if self.use_smart_wait:
                self.arrival_predictor.finalize_episode(self.job_arrival_times)
            
            return self._get_observation(), penalty, True, False, {"error": "Max episode steps reached"}

        job_idx, op_idx, wait_type_or_machine_idx = self._decode_action(action)

        # Handle WAIT actions (all types)
        if job_idx is None:
            wait_type = wait_type_or_machine_idx  # This is a wait type string
            action_mask = self.action_masks()
            # Check if there were any valid scheduling actions
            num_machines = len(self.machines)
            num_scheduling_actions = self.num_jobs * num_machines
            scheduling_actions_available = np.any(action_mask[:num_scheduling_actions])
            
            # Execute wait action with intelligent reward
            new_event_time, new_arrivals, wait_reward = self._execute_wait_action(
                wait_type, scheduling_actions_available
            )
            
            # Check for termination after waiting
            future_arrivals_possible = any(
                t < float('inf') for j, t in self.job_arrival_times.items() 
                if j not in self.arrived_jobs
            )
            if not new_arrivals and not future_arrivals_possible and not np.any(self.action_masks()[:num_scheduling_actions]):
                return self._get_observation(), -100, True, False, {"error": "Stuck with no future arrivals"}

            info = {
                "action_type": wait_type, 
                "event_time": self.event_time,
                "new_arrivals": new_arrivals,
                "time_advanced": new_event_time - current_event_time
            }
            return self._get_observation(), wait_reward, False, False, info

        # Handle scheduling action - wait_type_or_machine_idx is actually machine_idx here
        machine_idx = wait_type_or_machine_idx
        ...
        # Event time advancement logic (unchanged)
        if machine_free_before_scheduling <= self.event_time:
            ...
            if not other_idle_machines_exist:
                ...
                if next_event_time > self.event_time and next_event_time != float('inf'):
                    ...
                    # Update predictor with new arrivals
                    if self.use_smart_wait and new_arrivals > 0:
                        for job_id_check in self.arrived_jobs:
                            if (job_id_check in self.job_arrival_times and 
                                self.job_arrival_times[job_id_check] == self.event_time):
                                self.arrival_predictor.observe_arrival(self.event_time)
        ...
        # Finalize episode for predictor
        if terminated and self.use_smart_wait:
            self.arrival_predictor.finalize_episode(self.job_arrival_times)
    ...
    def _get_observation(self):
        ...
        if not self.cheat:    
            ...
            # 6. ENHANCED: Arrival prediction features (if smart wait enabled)
            if self.use_smart_wait:
                # 6.1. Predicted time until next arrival (normalized)
                stats = self.arrival_predictor.get_stats()
                mean_inter_arrival = stats['mean_inter_arrival']
                
                # Estimate time until next arrival based on time since last arrival
                if len(self.arrived_jobs) > 0:
                    last_arrival_time = max(
                        self.job_arrival_times.get(j, 0.0) 
                        for j in self.arrived_jobs
                    )
                    time_since_last = self.event_time - last_arrival_time
                else:
                    time_since_last = self.event_time
                
                # Predicted next arrival (simple: current time + mean inter-arrival - time since last)
                if mean_inter_arrival != float('inf'):
                    predicted_next_arrival = max(0, mean_inter_arrival - time_since_last)
                    normalized_predicted = min(1.0, predicted_next_arrival / self.max_time_horizon)
                else:
                    normalized_predicted = 1.0  # Unknown
                obs.append(normalized_predicted)
                
                # 6.2. Prediction confidence (0-1, directly from predictor)
                confidence = stats['confidence']
                obs.append(confidence)
                
                # 6.3. Estimated arrival rate (normalized, assume typical range 0-0.2)
                estimated_rate = stats['estimated_rate']
                normalized_rate = min(1.0, estimated_rate / 0.2)
                obs.append(normalized_rate)
                
                # 6.4. Time since last arrival (normalized)
                normalized_time_since = min(1.0, time_since_last / self.max_time_horizon)
                obs.append(normalized_time_since)
                
                # 6.5. Idle machine indicators (binary: 1 if idle, 0 if busy)
                for machine in self.machines:
                    is_idle = 1.0 if self.machine_next_free[machine] <= self.event_time else 0.0
                    obs.append(is_idle)
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        Args:
            ...
            arrival_rate: TRUE arrival rate (hidden from agent, only for generating arrivals)
            prediction_window: Time horizon for proactive scheduling (e.g., 10.0 means
                              can schedule jobs predicted to arrive in next 10 time units)
        ...
        self.prediction_window = prediction_window  # How far ahead agent can schedule
        self.max_time_horizon = max_time_horizon
        self.reward_mode = reward_mode
        
        # Initialize arrival predictor
        self.arrival_predictor = ArrivalPredictor(initial_rate_guess=0.05)
        
        # Action space: job_idx * num_machines + machine_idx + 1 WAIT action
        num_scheduling_actions = len(self.job_ids) * len(self.machines)
        self.action_space = spaces.Discrete(num_scheduling_actions + 1)
        self.wait_action_idx = num_scheduling_actions
        ...
        obs_size = (
            ...
            len(self.job_ids) +                      # NEW: Prediction confidence per job
            3                                        # NEW: Estimated arrival rate
        )
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(obs_size,), dtype=np.float32
        )
    ...
    def action_masks(self):
        ...
        - Can schedule arrived jobs (traditional)
        - Can schedule predicted jobs within prediction_window (PROACTIVE)
        - Can always WAIT
        """
        ...
        # Always allow WAIT
        mask[self.wait_action_idx] = True
        
        return mask
    
    def step(self, action):
        """Execute action with proactive scheduling support."""
        previous_makespan = self.current_makespan
        
        # Handle WAIT action
        if action == self.wait_action_idx:
            reward, done = self._execute_wait_action()
    ...
    def _execute_wait_action(self):
        """Advance time to next event (similar to PoissonDynamicFJSPEnv)."""
        # Get the next event time (considers both arrivals and machine completions)
        next_event_time = self._get_next_event_time()
        
        # If no future events, check if done
    ...
    def _get_observation(self):
        ...
        for job_id in self.job_ids:
            ...
            if job_id in self.arrived_jobs:
                # Arrived jobs: normalized arrival time
                normalized_arrival_time = min(1.0, arrival_time / self.max_time_horizon)
                obs_parts.append(normalized_arrival_time)
            else:
                # Not yet arrived: 1
                obs_parts.append(1.0)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        n_steps=4096,              # ⭐ LARGER rollout buffer for better estimates (was 1024)
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        n_steps=1024,              # IDENTICAL across all RL methods
        batch_size=256,            # IDENTICAL across all RL methods
        n_epochs=2,               # IDENTICAL across all RL methods
        gamma=1,                # IDENTICAL across all RL methods
        gae_lambda=0.99,
        clip_range=0.2,
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
                          prediction_window=10.0, total_timesteps=500000, 
                          reward_mode="makespan_increment", learning_rate=3e-4):
    """
    Train a PROACTIVE RL agent that learns to predict job arrivals and schedule proactively.
    
    Key differences from reactive dynamic agent:
    1. Uses ProactiveDynamicFJSPEnv with arrival prediction
    2. Can schedule jobs predicted to arrive within prediction_window
    3. Learns arrival rate across episodes using MLE
    4. Gets penalized for mispredictions
    
    Args:
        jobs_data: Job specifications
        machine_list: List of machines
        initial_jobs: Initial jobs or list of job IDs
        arrival_rate: TRUE arrival rate (hidden from agent)
        prediction_window: Time horizon for proactive scheduling
    ...
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    print(f"Prediction Window: {prediction_window} time units")
    print(f"⚠️  Agent does NOT know true arrival rate (λ={arrival_rate})")
    print(f"✅ Agent LEARNS arrival rate via MLE across episodes")
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
                                  prediction_window=10.0, reward_mode="makespan_increment"):
    """
    Evaluate PROACTIVE model on dynamic scenario.
    
    The model uses its learned arrival predictor to schedule proactively.
    ...
    # Initialize predictor (it will learn as jobs arrive during evaluation)
    obs = test_env.env._get_observation()
    ...
    while step_count < max_steps:
        ...
        # Track if this was a proactive decision
        if action != test_env.env.wait_action_idx:
            job_id = action // len(machine_list)
            if job_id not in test_env.env.arrived_jobs and job_id in test_env.env.predicted_arrival_times:
    ...
    print(f"  Proactive scheduling decisions: {proactive_decisions}")
...
def main():
    ...
    # Main comparison with 5-6 plots (added Proactive RL)
    num_plots = 6 if milp_makespan != float('inf') else 5
    fig, axes = plt.subplots(num_plots, 1, figsize=(18, num_plots * 3.5))
    
    if milp_makespan != float('inf'):
        fig.suptitle('Main Scheduling Comparison: MILP vs Perfect vs Proactive vs Dynamic vs Static RL vs Heuristic\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'Proactive RL learns arrival patterns and schedules proactively', 
        ...
        fig.suptitle('Main Scheduling Comparison: Perfect vs Proactive vs Dynamic vs Static RL vs Heuristic\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'Proactive RL learns arrival patterns and schedules proactively', 
                     fontsize=16, fontweight='bold')
        schedules_data = [
    ...
    for scenario_idx in range(len(gantt_scenarios_data)):
        ...
        print(f"Arrival times: {arrival_times}")
        num_methods = 6 if schedules['MILP Optimal'][0] != float('inf') else 5
