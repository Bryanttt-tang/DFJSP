...
class DynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            # Instead of terminating, give negative reward and continue
            return self._get_observation(), -10.0, False, False, {"error": "Invalid action, continuing"}
    ...
    def _calculate_reward(self, proc_time, idle_time, done):
        ...
        # Progress reward
        reward += 5.0  # Base reward for making progress
        
        # Completion bonus
        if done:
            reward += 100.0  # Large completion bonus
            # Small penalty for makespan to encourage shorter schedules
            reward -= self.current_makespan * 0.01
    ...
    def _get_observation(self):
        ...
        for m in self.machines:
            obs.append(self.machine_next_free.get(m, 0.0) / norm_factor)
        ...
        for job_id in self.job_ids:
            ...
            progress = self.next_operation[job_id] / max(total_ops, 1) if total_ops > 0 else 1.0
            obs.append(progress)
        ...
        obs.append(self.current_makespan / max(norm_factor, 1.0))
...
def train_agent(jobs_data, machine_list, train_arrivals, log_name, total_timesteps=150000):
    ...
    # Simplified hyperparameters compatible with MaskablePPO
    model = MaskablePPO(
        ...
        policy_kwargs=dict(
            net_arch=[256, 256]  # Simpler network architecture
        )
    )
...
if __name__ == "__main__":
    ...
    try:
        static_model = train_agent(jobs_data, machine_list, static_arrival_times, 
                                 "Static_RL", total_timesteps=5000)  # Reduced for faster testing
    ...
    try:
        dynamic_model = train_agent(jobs_data, machine_list, arrival_times, 
                                  "Dynamic_RL", total_timesteps=7500)  # Reduced for faster testing
