...
def plot_training_metrics():
    ...
    try:
        ...
        if TRAINING_METRICS.get('episode_rewards') and len(TRAINING_METRICS['episode_rewards']) > 1:
            ...
            if len(ep_rewards) > 10:
                ...
                axes[0, 0].plot(moving_timesteps, moving_avg, 'r-', linewidth=3, alpha=0.8, 
            ...
            axes[0, 0].text(0.5, 0.5, f'No episode reward data\\n(Recorded: {len(TRAINING_METRICS.get("episode_rewards", []))} rewards)',
                           ha='center', va='center', transform=axes[0, 0].transAxes, fontsize=12)
            axes[0, 0].set_title('Episode Rewards Over Time (No Data)', fontweight='bold')
        
        # Plot 2: Value Loss (Top Right)
        if TRAINING_METRICS.get('value_loss') and len(TRAINING_METRICS['value_loss']) > 1:
            value_losses = TRAINING_METRICS['value_loss']
            loss_timesteps = timesteps[:len(value_losses)]
            ...
            axes[0, 1].text(0.5, 0.5, f'No value loss data\\n(Recorded: {len(TRAINING_METRICS.get("value_loss", []))} values)',
                           ha='center', va='center', transform=axes[0, 1].transAxes, fontsize=12)
            axes[0, 1].set_title('Value Loss Over Time (No Data)', fontweight='bold')
        
        # Save and show plot
        plt.tight_layout()
        plt.savefig('enhanced_ppo_training_metrics.png', dpi=300, bbox_inches='tight')
        print(f"✅ Training metrics plot saved as 'enhanced_ppo_training_metrics.png'")
        plt.show()
        
    except Exception as e:
        print(f"❌ Error creating plots: {e}")
        print("This might be due to missing matplotlib or insufficient data")
