...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Use similar hyperparameters as Dynamic RL for fair comparison
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=learning_rate,
        n_steps=2048,              # Match Dynamic RL
        batch_size=128,            # Match Dynamic RL
        n_epochs=10,               # Match Dynamic RL
        gamma=1,                   # Match Dynamic RL
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.02,             # Higher entropy for better exploration of specific scenario
        vf_coef=0.5,
        max_grad_norm=0.5,
        seed=arrival_seed,         # Use scenario-specific seed
        policy_kwargs=dict(
            net_arch=[512, 512, 256],  # Match Dynamic RL network size
