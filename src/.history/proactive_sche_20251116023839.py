...
class DispatchingRuleFJSPEnv(gym.Env):
    """
    SCHEDULE BUILDER MODE: Dynamic FJSP Environment where actions select DISPATCHING RULES.
    
    Unlike reactive RL which schedules one (job, machine) pair per step, this environment
    applies a selected dispatching rule to schedule ALL ready operations in a single step.
    
    This implements the "schedule builder" paradigm from the literature where the agent
    makes high-level decisions (which rule to apply) rather than low-level decisions
    (which specific job-machine pair to schedule).
    
    Action Space (6 dispatching rules):
    ...
    Key Differences from Reactive RL:
    - Episodes have ~O(num_jobs) steps instead of O(total_operations) steps
    - Each step schedules multiple operations using the selected rule
    - Agent learns WHEN to switch rules based on system state
    - More strategic, less tactical than low-level scheduling
    
    The agent learns WHEN to apply WHICH rule as jobs arrive and system state evolves.
    """
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # ACTION SPACE: Select from 6 dispatching rules (no WAIT - pure rule selection)
        # 0=FIFO, 1=SPT, 2=LPT, 3=MWKR, 4=LWKR, 5=EDD
    ...
    def _reset_state(self):
        ...
        self.operations_scheduled = 0
        self.episode_step = 0
        # BUILDER MODE: Much fewer steps needed since each step schedules multiple operations
        # Expect roughly one decision per job arrival event
        self.max_episode_steps = len(self.job_ids) * 3  # Conservative upper bound
    ...
    def action_masks(self):
        """Generate action masks - all rules available if work exists."""
        ...
        # Enable all dispatching rules if there are ready operations
        if ready_ops:
    ...
    def step(self, action):
        """
        SCHEDULE BUILDER MODE: Apply selected rule to schedule ALL ready operations.
        
        Unlike reactive RL which schedules ONE operation per step, this applies
        the selected dispatching rule repeatedly until no more operations are ready
        at the current event time.
        """
        self.episode_step += 1
        previous_makespan = self.current_makespan
        operations_scheduled_this_step = 0
        ...
        # SCHEDULE BUILDER MODE: Apply selected rule repeatedly to schedule ALL ready operations
        while self.operations_scheduled < self.total_operations:
            ready_ops = self._get_ready_operations()
            
            if not ready_ops:
                # No ready operations - advance time to next event
                next_event_time = self._get_next_event_time()
                if next_event_time > self.event_time and next_event_time != float('inf'):
                    self._update_event_time_and_arrivals(next_event_time)
                    ...
                    # After time advancement, check for ready operations
                    ready_ops = self._get_ready_operations()
                    
                    # If new operations became ready, schedule them with the selected rule
                    if ready_ops:
                        continue  # Continue scheduling with same rule
                    else:
                        # No more work can be done at this decision point
                        break
                else:
                    # No future events - done
                    break
            
            # Apply selected dispatching rule to choose best operation
            selected_op = self._apply_dispatching_rule(action, ready_ops)
            
            if selected_op is None:
                break  # Rule failed to select operation
            ...
            self.next_operation[job_id] += 1
            self.operations_scheduled += 1
            operations_scheduled_this_step += 1
            self.current_makespan = max(self.current_makespan, end_time)
            self.schedule[machine].append((f"J{job_id}-O{op_idx+1}", start_time, end_time))
            
            # Continue scheduling more operations with same rule if they're ready
            # (This implements the "builder" aspect - apply rule until no more ready ops)
        
        # After scheduling all ready operations, advance event time if all machines busy
        all_machines_busy = all(
            self.machine_next_free[m] > self.event_time 
            for m in self.machines
        )
        
        if all_machines_busy and self.operations_scheduled < self.total_operations:
            # All machines busy - advance to next event
        ...
        # Reward is negative makespan increment (minimize makespan)
        reward = -(self.current_makespan - previous_makespan)
        terminated = self.operations_scheduled >= self.total_operations
        
        info = {
            "makespan": self.current_makespan,
            "ops_scheduled": operations_scheduled_this_step,
            "total_ops": self.operations_scheduled
        }
...
def evaluate_rule_based_on_dynamic(rule_model, jobs_data, machine_list, arrival_times, 
    ...
    rule_names = ['FIFO', 'SPT', 'LPT', 'MWKR', 'LWKR', 'EDD']
    ...
    rule_dist = {rule_names[i]: f"{rule_counts[i]} ({rule_counts[i]/total_actions*100:.1f}%)" 
                 for i in range(6) if rule_counts[i] > 0}
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    print(f"  Initial arrived jobs: {test_env.env.arrived_jobs}")
