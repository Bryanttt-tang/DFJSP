...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # CRITICAL FIX: start_time should NOT depend on event_time!
        # event_time only controls job VISIBILITY, not scheduling timeline
        # This allows backfilling: scheduling operations in earlier idle slots
        ...
        # IMPORTANT: After scheduling, check if we should advance event_time
        # to reveal jobs that might arrive before the next decision point
        # But DON'T force future operations to be scheduled after event_time!
        # Only advance event_time if the start_time we just scheduled is in the "future"
        if start_time > self.event_time:
            # We scheduled something in the future, reveal any jobs that arrive by then
