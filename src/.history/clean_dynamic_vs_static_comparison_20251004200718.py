...
class TrainingCallback:
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
            ...
            # DEBUG: Print all available log keys every 1000 calls to see what's available
            if self.step_count % 1000 == 0:
                print(f"\nDEBUG - Available log keys for {self.method_name}: {list(log_data.keys())}")
            ...
            # ENHANCED: Try multiple possible episode reward keys
            episode_reward = None
            ...
            for key in possible_reward_keys:
                if key in log_data:
                    episode_reward = log_data[key]
                    if self.step_count % 1000 == 0:
                        print(f"DEBUG - Found episode reward in key: {key}, value: {episode_reward}")
                    break
            ...
            # ENHANCED: Try multiple possible episode length keys
            episode_length = None
            possible_length_keys = [
                'rollout/ep_len_mean',
                'episode/length_mean',
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    def make_perfect_env():
        ...
        env = Monitor(env)  # Monitor should log episode statistics
        return env
    ...
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,  # INCREASED verbose to enable episode logging
    ...
    # DEBUG: Print final training metrics summary
    print(f"DEBUG - Final metrics summary for {callback.method_name}:")
    print(f"  Episode rewards logged: {len(TRAINING_METRICS['episode_rewards'])}")
    print(f"  Policy loss logged: {len(TRAINING_METRICS['policy_loss'])}")
    print(f"  Value loss logged: {len(TRAINING_METRICS['value_loss'])}")
    print(f"  Timesteps logged: {len(TRAINING_METRICS['timesteps'])}")
    if TRAINING_METRICS['episode_rewards']:
        print(f"  Sample episode rewards: {TRAINING_METRICS['episode_rewards'][:5]}...{TRAINING_METRICS['episode_rewards'][-5:]}")
    ...
    for job_id in jobs_data.keys():
        if (job_id, 0) in job_op_times:
            ...
            if first_op_start < arrival_time:
                print(f"    ❌ {method_name}: Job {job_id} operation {op_idx} starts at {first_op_start:.2f} before arrival at {arrival_time:.2f}")
                return False, float('inf')
