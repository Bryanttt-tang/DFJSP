...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # CRITICAL FIX: Smart event_time advancement during scheduling
        # Only advance event_time if we scheduled on a machine that was BUSY (in the future)
        # If machine was idle at current event_time, keep event_time unchanged
        if machine_available_time > self.event_time:
            # Machine was busy in the future, we had to wait for it
            # Advance event_time to when we actually scheduled (start_time)
            # But also check for any job arrivals that happen before start_time
            new_event_time = min(start_time, self._get_next_event_time())
            if new_event_time > self.event_time:
                self._update_event_time_and_arrivals(new_event_time)
        # else: machine was idle at event_time, keep event_time unchanged
        # This allows scheduling multiple operations in parallel at current event_time
