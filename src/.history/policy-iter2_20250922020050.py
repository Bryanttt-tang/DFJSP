...
def estimate_state_space(jobs_data, machines, arrivals):
    """Estimate state space size - but note this is very rough since actual reachable states are much smaller"""
    print("\n=== STATE SPACE ESTIMATION (ROUGH BOUNDS) ===")
    ...
    # NOTE: The actual enumeration shows that reachable states are MUCH smaller than these bounds!
    # For MEDIUM (3 jobs, 2 ops each): ~320 reachable states (not thousands!)
    # For SIMPLE (2 jobs, 2 ops each): even fewer states
    
    # State components - THEORETICAL upper bounds (most unreachable):
    ...
    print(f"Next_ops combinations (theoretical): {next_ops_combinations}")
    
    # 2. Timing constraints make most combinations impossible
    # The actual reachable states depend heavily on:
    # - Precedence constraints (operations must be sequential within jobs)
    # - Resource constraints (machines can only process one op at a time)
    # - Processing time dependencies
    
    print(f"IMPORTANT: Theoretical bounds are VASTLY larger than reachable states!")
    print(f"Actual enumeration shows:")
    print(f"  - SIMPLE (2 jobs × 2 ops): likely ~50-100 reachable states")
    print(f"  - MEDIUM (3 jobs × 2 ops): ~320 reachable states (as observed)")
    print(f"  - LARGER (5 jobs × 3 ops): ~10K-100K reachable states (estimate)")
    
    # The explosion happens because:
    # 1. More jobs = more parallel decision points
    # 2. More operations per job = longer horizons
    # 3. More machines = more assignment choices at each step
    
    return next_ops_combinations  # Just return the operation combinations as a rough indicator
