...
# --- Flexible JSP Dataset Generation ---
def generate_fjsp_dataset(num_jobs, num_machines, max_ops_per_job=5, min_ops_per_job=1, 
                          min_proc_time=1, max_proc_time=10, seed=None):
    """
    Generate a flexible JSP dataset with random parameters.
    
    Args:
        num_jobs: Total number of jobs to generate
        num_machines: Total number of machines available
        max_ops_per_job: Maximum number of operations per job (default: 5)
        min_ops_per_job: Minimum number of operations per job (default: 1)
        min_proc_time: Minimum processing time (default: 1)
        max_proc_time: Maximum processing time (default: 10)
        seed: Random seed for reproducibility (optional)
    
    Returns:
        jobs_data: OrderedDict with job structure
    ...
    for job_id in range(num_jobs):
        # Random number of operations for this job (at least 1, at most max_ops_per_job)
        num_ops = np.random.randint(min_ops_per_job, max_ops_per_job + 1)
        
        job_operations = []
        for op_idx in range(num_ops):
            # Random number of available machines for this operation (at least 1)
            num_available_machines = np.random.randint(1, num_machines + 1)
            
            # Randomly select which machines are available
            available_machines = np.random.choice(machine_list, size=num_available_machines, replace=False)
            
            # Generate processing times for available machines
            proc_times = {}
            for machine in available_machines:
                proc_time = np.random.randint(min_proc_time, max_proc_time + 1)
                proc_times[machine] = proc_time
            
            job_operations.append({'proc_times': proc_times})
        
        jobs_data[job_id] = job_operations
    
    return jobs_data, machine_list

def generate_arrival_times(num_initial_jobs, num_dynamic_jobs, arrival_rate=0.08, 
                           max_time_horizon=200, seed=None):
    """
    Generate arrival times for dynamic jobs using Poisson process.
    
    Args:
        num_initial_jobs: Number of jobs that arrive at t=0
        num_dynamic_jobs: Number of jobs that arrive dynamically
        arrival_rate: Rate parameter for Poisson process (default: 0.08)
        max_time_horizon: Maximum time for arrivals (default: 200)
        seed: Random seed for reproducibility (optional)
    ...
    for job_id in range(num_initial_jobs):
        arrival_times[job_id] = 0
    
    # Generate Poisson arrivals for dynamic jobs
    current_time = 0.0
    for job_id in range(num_initial_jobs, num_initial_jobs + num_dynamic_jobs):
...
def create_custom_problem_instance(num_initial_jobs=5, num_dynamic_jobs=2, num_machines=3,
                                   max_ops_per_job=5, arrival_rate=0.08, seed=None):
    """
    Convenience function to create a complete problem instance with jobs and arrival times.
    
    Args:
        num_initial_jobs: Number of jobs that arrive at t=0 (default: 5)
        num_dynamic_jobs: Number of jobs that arrive dynamically (default: 2)
        num_machines: Total number of machines (default: 3)
        max_ops_per_job: Maximum operations per job (default: 5)
        arrival_rate: Poisson arrival rate for dynamic jobs (default: 0.08)
        seed: Random seed for reproducibility (optional)
    
    Returns:
        jobs_data: Generated jobs dataset
        machine_list: List of machine names
        arrival_times: Dictionary of arrival times
    
    Example:
        >>> # Generate a small problem: 5 initial jobs + 2 dynamic jobs, 3 machines
        ...
        ...     seed=42
        ... )
        >>> print(f"Generated {len(jobs)} jobs on {len(machines)} machines")
        >>> print(f"Arrival times: {arrivals}")
    """
    total_jobs = num_initial_jobs + num_dynamic_jobs
    ...
    arrival_times = generate_arrival_times(
        ...
        num_dynamic_jobs=num_dynamic_jobs,
...
# --- Predefined Dataset (for backward compatibility) ---
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        self.action_space = spaces.Discrete(max_scheduling_actions)  # No WAIT action
    ...
    def _get_next_event_time(self):
        ...
        # Find earliest machine completion time
        ...
        result = next_event_time if next_event_time != float('inf') else self.event_time + 1.0
    ...
    #     return self._update_event_time_and_arrivals(next_event_time)

    def step(self, action):
        """Simplified step function for Poisson Dynamic environment without WAIT action."""
        self.episode_step += 1

        # print("step:", self.episode_step, 'Action masks:', self.action_masks())  # Debug: Print action masks
        # previous_mask = self.action_masks().copy()
        # print(" Same as previous mask?:", np.array_equal(previous_mask, self.action_masks()))  # Debug: Check if mask changed
    ...
    def _get_observation(self):
        ...
        if not self.cheat:    
            ...
            # for machine in self.machines:
            #     machine_free_time = self.machine_next_free[machine]
            #     relative_busy_time = max(0, self.current_makespan - machine_free_time)
            #     normalized_busy = min(1.0, relative_busy_time / self.max_time_horizon)
            #     obs.append(normalized_busy)
            for machine in self.machines:
                machine_free_time = self.machine_next_free[machine]
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # 3. Machine idle status (binary: 1 if idle, 0 if busy)
        for machine in self.machines:
            machine_free_time = self.machine_next_free[machine]
            is_idle = machine_free_time <= self.current_makespan
            obs.append(1.0 if is_idle else 0.0)

        # 4. Processing times for ready operations (normalized)
        for job_id in self.job_ids:
            if self.next_operation[job_id] < len(self.jobs[job_id]):
                next_op_idx = self.next_operation[job_id]
                operation = self.jobs[job_id][next_op_idx]
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    ...
    # === CACHING LOGIC ===
    # Generate a unique hash for this scenario
    def generate_scenario_hash(jobs_data, machine_list, arrival_times):
        """Generate a unique hash for the problem instance."""
        # Create a canonical representation of the problem
        scenario_dict = {
