...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 
        # Rule: Only advance event_time if scheduling on a machine that was IDLE
        #       (i.e., machine_free_before_scheduling <= event_time)
        # 
        # Case 1: Machine was IDLE (free_time <= event_time)
        #         Example: event_time=0, M0 free at t=0, M1 free at t=5
        #         Agent schedules on M0 → event_time stays 0 (can schedule on other machines)
        #         
        # Case 2: Machine was BUSY (free_time > event_time)  
        #         Example: event_time=0, M0 free at t=5
        #         Agent schedules on M0 → event_time stays 0 (already in future)
        #         The machine was already busy, no need to advance time
        #
        # This allows:
        # - Parallel scheduling on all idle machines at current event_time
        # - Scheduling on busy machines without advancing time (they're already in future)
        
        if machine_free_before_scheduling <= self.event_time:
            # Machine was IDLE - it just became busy, advance event_time if needed
            # Advance to the minimum of: next arrival time, or earliest machine completion
            next_event_time = self._get_next_event_time()
        ...
        #       → event_time stays same, this scheduling doesn't affect current time
