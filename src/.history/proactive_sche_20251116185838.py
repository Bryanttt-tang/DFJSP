...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Calculate timing - CRITICAL: must respect arrival time for ALL operations
        machine_available_time = self.machine_next_free.get(machine, 0.0)
        
        # Job precedence: must wait for previous operation to complete
        if op_idx > 0:
            job_ready_time = self.operation_end_times[job_id][op_idx - 1]
        else:
            job_ready_time = 0.0
        
        # CRITICAL FIX: Must also respect job arrival time for ALL operations (not just first)
        job_arrival_time = self.job_arrival_times.get(job_id, 0.0)
        
        # Start time is when ALL constraints are satisfied
        start_time = max(machine_available_time, job_ready_time, job_arrival_time)
        proc_time = self.jobs[job_id][op_idx]['proc_times'][machine]
        end_time = start_time + proc_time
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        n_epochs=5,                 # IDENTICAL to proactive (more epochs)
