...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #         Example: event_time=5, M0 free at t=3 → M0 is idle
        #         Agent schedules on M0 → M0 becomes busy, may need to advance event_time
        #         
        # Case 2: Machine was BUSY in future (free_time > event_time)  
        #         Example: event_time=0, M1 free at t=5 → M1 is busy until t=5
        #         Agent schedules on M1 → event_time STAYS at 0 (no change)
        #         The machine was already in the future, we don't advance time
        #
        # This allows:
        ...
        if machine_free_before_scheduling <= self.event_time:
            # Machine was IDLE at current event_time
            # Now it's busy, so advance event_time to next event
            next_event_time = self._get_next_event_time()
        ...
        #       Example: event_time=0, M1 free at t=5
        #       Scheduling on M1 doesn't change event_time (stays at 0)
