...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def action_masks(self):
        ...
        if ready_ops:
            ...
            # CRITICAL FIX: If no ready ops but jobs remain, enable action 0 to allow step() to be called
            # The step() function will automatically advance event_time via its while loop (lines 1527-1537)
            # This prevents evaluation loops from breaking prematurely when waiting for job arrivals
            if len(self.arrived_jobs) < len(self.job_ids):
                mask[0] = True  # Enable first action as a dummy to trigger time advancement
    ...
    def step(self, action):
        ...
        # EVENT-DRIVEN: Update event_time based on machine state (matching Reactive RL logic)
        machine_free_before_scheduling = machine_avail
        if machine_free_before_scheduling <= self.event_time:
            # Machine was idle - check if other idle machines remain
            other_idle_machines = [
                m for m in self.machines 
                if m != best_machine and self.machine_next_free[m] <= self.event_time
            ]
            
            if not other_idle_machines:
                # No other idle machines - advance event_time
                next_event_time = self._get_next_event_time()
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def _get_next_event_time(self):
        ...
        CRITICAL FIX: When all current jobs are completed but new jobs haven't arrived,
        we should jump to the next arrival time (not stay at current event_time).
        """
        # Find next arrival time (future arrivals only)
        next_arrival_time = float('inf')
        for job_id, arrival_time in self.job_arrival_times.items():
        ...
        # CRITICAL FIX: If all machines are idle (no future completions) but jobs haven't arrived,
        # jump directly to next arrival instead of staying stuck
        if next_machine_completion == float('inf') and next_arrival_time != float('inf'):
            # All machines idle, waiting for new jobs - jump to next arrival
