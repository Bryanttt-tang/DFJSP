...
def plot_training_metrics(metrics_data, method_name="RL Agent"):
    """
    Enhanced plot of training metrics from a specific training run.
    """
    if not metrics_data or not metrics_data.get('timesteps'):
        print(f"No training metrics recorded for {method_name}!")
        return
    
    print(f"\n=== ENHANCED TRAINING METRICS ANALYSIS for {method_name} ===")
    print(f"Total training steps recorded: {len(metrics_data['timesteps'])}")
    print(f"Episode rewards recorded: {len(metrics_data['episode_rewards'])}")
    
    # Create comprehensive figure with subplots
    fig, axes = plt.subplots(3, 2, figsize=(20, 15))
    fig.suptitle(f'Comprehensive PPO Training Analysis - {method_name}', 
                 fontsize=16, fontweight='bold')
    
    timesteps = metrics_data['timesteps']
    
    # Plot 1: Episode Rewards over Time
    if metrics_data.get('episode_rewards'):
        ep_rewards = metrics_data['episode_rewards']
        # Ensure timesteps and rewards align
        reward_timesteps = timesteps[:len(ep_rewards)]
        axes[0, 0].plot(reward_timesteps, ep_rewards, 'g-', linewidth=2, alpha=0.7, label='Mean Episode Reward')
        
        if len(ep_rewards) > 10:
            window_size = min(50, len(ep_rewards) // 4)
            moving_avg = pd.Series(ep_rewards).rolling(window=window_size, min_periods=1).mean()
            axes[0, 0].plot(reward_timesteps, moving_avg, 'b--', label=f'Moving Avg (w={window_size})')
        ...
        axes[0, 0].text(0.5, 0.5, 'No Episode Reward Data', ha='center', va='center')

    # Plot 2: Episode Lengths
    if metrics_data.get('episode_lengths'):
        ep_lengths = metrics_data['episode_lengths']
        length_timesteps = timesteps[:len(ep_lengths)]
        axes[0, 1].plot(length_timesteps, ep_lengths, 'c-', alpha=0.7, label='Mean Episode Length')
        ...
        axes[0, 1].text(0.5, 0.5, 'No Episode Length Data', ha='center', va='center')

    # Plot 3: Policy & Value Loss
    if metrics_data.get('policy_loss') and metrics_data.get('value_loss'):
        policy_loss = metrics_data['policy_loss']
        value_loss = metrics_data['value_loss']
        loss_timesteps = timesteps[:len(policy_loss)]
        
        ax_loss = axes[1, 0]
        ax_loss.plot(loss_timesteps, policy_loss, 'r-', alpha=0.7, label='Policy Loss')
        ax_loss.set_xlabel('Timesteps')
        ax_loss.set_ylabel('Policy Loss', color='r')
        ax_loss.tick_params(axis='y', labelcolor='r')
        
        ax_val = ax_loss.twinx()
        ax_val.plot(loss_timesteps, value_loss, 'b-', alpha=0.7, label='Value Loss')
        ax_val.set_ylabel('Value Loss', color='b')
        ax_val.tick_params(axis='y', labelcolor='b')
        
        ax_loss.set_title('Policy and Value Loss')
    ...
    if metrics_data.get('action_entropy'):
        entropy = metrics_data['action_entropy']
        entropy_timesteps = timesteps[:len(entropy)]
        axes[1, 1].plot(entropy_timesteps, entropy, 'm-', alpha=0.7, label='Entropy')
    ...
    if metrics_data.get('learning_rate'):
        lr = metrics_data['learning_rate']
        lr_timesteps = timesteps[:len(lr)]
        axes[2, 0].plot(lr_timesteps, lr, 'k-', label='Learning Rate')
    ...
    if metrics_data.get('explained_variance'):
        exp_var = metrics_data['explained_variance']
        var_timesteps = timesteps[:len(exp_var)]
        axes[2, 1].plot(var_timesteps, exp_var, 'y-', label='Explained Variance')
        ...
        axes[2, 1].text(0.5, 0.5, 'No Explained Variance Data', ha='center', va='center')

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    # Save the figure with a unique name
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    filename = f"training_metrics_{method_name.replace(' ', '_')}_{timestamp}.png"
...
def main():
    ...
    plot_training_metrics(dynamic_rl_metrics, "Dynamic RL")
    ...
    plot_training_metrics(static_rl_metrics, "Static RL")
