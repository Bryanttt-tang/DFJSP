...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        # BUILDER MODE: Use makespan as the clock (no event_time)
        self.current_makespan = 0.0
        self.operations_scheduled = 0
        self.episode_step = 0
        self.max_episode_steps = self.total_operations * 2  # No waits, so tighter limit
        
        # BUILDER MODE: Jobs discovered as makespan advances (not event-driven)
        self.arrived_jobs = set(self.initial_job_ids)  # Initial jobs available immediately
        self.job_arrival_times = {}
        self._generate_poisson_arrivals()
        
        # No event_time - jobs discovered when makespan >= arrival_time
