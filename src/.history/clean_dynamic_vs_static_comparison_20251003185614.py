...
class TrainingCallback:
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        # Extract episode-level data from the environment
        if hasattr(model, 'env') and hasattr(model.env, 'get_attr'):
            try:
                # Get episode rewards and lengths from vectorized environment
                ep_rew_mean = None
                ep_len_mean = None
                
                # Try to get episode statistics from the logger
                if 'rollout/ep_rew_mean' in log_data:
                    ep_rew_mean = log_data['rollout/ep_rew_mean']
                    TRAINING_METRICS['episode_rewards'].append(ep_rew_mean)
                
                if 'rollout/ep_len_mean' in log_data:
                    ep_len_mean = log_data['rollout/ep_len_mean']
                    TRAINING_METRICS['episode_lengths'].append(ep_len_mean)
                ...
                pass  # Silently handle missing episode data
...
def plot_training_metrics():
    ...
    # Set method name for figure title
    method_name = TRAINING_METRICS.get('method_name', None)
    if not method_name:
        # Try to infer from callback (if available)
        if len(TRAINING_METRICS['episode_count']) > 0:
            method_name = "RL Agent"
        else:
            method_name = "RL Agent"
    else:
        method_name = str(method_name)

    print(f"\n=== ENHANCED TRAINING METRICS ANALYSIS ({method_name}) ===")
    ...
    fig.suptitle(f'PPO Training Metrics - {method_name}', fontsize=16, fontweight='bold')
    ...
    if TRAINING_METRICS['episode_rewards'] and len(TRAINING_METRICS['episode_rewards']) > 1:
        ...
        if len(ep_rewards) > 10:
            ...
            axes[0, 0].plot(moving_timesteps, moving_avg, 'darkgreen', linewidth=3, label=f'Moving Avg (window={window_size})')
        ...
        axes[0, 0].text(0.5, 0.5, 'No episode reward data', ha='center', va='center', transform=axes[0, 0].transAxes)
    ...
    if TRAINING_METRICS['policy_loss']:
        ...
        axes[0, 1].plot(loss_timesteps, policy_loss, 'r-', linewidth=2, alpha=0.7, label='Policy Loss')
        if len(policy_loss) > 10:
            ...
            axes[0, 1].plot(moving_timesteps, moving_avg, 'darkred', linewidth=3, label=f'Moving Avg (window={window_size})')
    ...
    if TRAINING_METRICS['value_loss']:
        ...
        axes[1, 0].plot(loss_timesteps, value_loss, 'b-', linewidth=2, alpha=0.7, label='Value Loss')
        if len(value_loss) > 10:
            ...
            moving_timesteps = loss_timesteps[window_size-1:]
            axes[1, 0].plot(moving_timesteps, moving_avg, 'darkblue', linewidth=3, label=f'Moving Avg (window={window_size})')
    ...
    if TRAINING_METRICS['action_entropy']:
        ...
        axes[1, 1].plot(entropy_timesteps, entropy, 'purple', linewidth=2, alpha=0.7, label='Action Entropy')
        if len(entropy) > 10:
            ...
            axes[1, 1].plot(moving_timesteps, moving_avg, 'indigo', linewidth=3, label=f'Moving Avg (window={window_size})')
        axes[1, 1].set_xlabel('Training Steps')
        axes[1, 1].set_ylabel('Action Entropy')
        axes[1, 1].set_title('Action Entropy')
        axes[1, 1].legend()
    ...
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['value_loss']:
        ...
        axes[2, 0].plot(combined_timesteps, TRAINING_METRICS['policy_loss'][:min_len], 'r-', linewidth=2, label='Policy Loss', alpha=0.7)
        axes[2, 0].plot(combined_timesteps, TRAINING_METRICS['value_loss'][:min_len], 'b-', linewidth=2, label='Value Loss', alpha=0.7)
        axes[2, 0].legend()
        axes[2, 0].set_xlabel('Training Steps')
        axes[2, 0].set_ylabel('Loss')
        axes[2, 0].grid(True, alpha=0.3)
    else:
        axes[2, 0].text(0.5, 0.5, 'Insufficient loss data', ha='center', va='center', transform=axes[2, 0].transAxes)
    ...
    if TRAINING_METRICS['learning_rate'] or TRAINING_METRICS['explained_variance']:
        ...
        if TRAINING_METRICS['learning_rate']:
            ...
            ax_lr.plot(lr_timesteps, lr, 'orange', linewidth=2, label='Learning Rate')
        ...
        if TRAINING_METRICS['explained_variance']:
            ...
            ax_ev.plot(ev_timesteps, ev, 'cyan', linewidth=2, label='Explained Variance')
            ax_ev.set_ylabel('Explained Variance', color='cyan')
            ax_ev.tick_params(axis='y', labelcolor='cyan')
        axes[2, 1].set_xlabel('Training Steps')
        axes[2, 1].set_title('Learning Rate & Explained Variance')
        axes[2, 1].grid(True, alpha=0.3)
    else:
        axes[2, 1].text(0.5, 0.5, 'No LR/EV data', ha='center', va='center', transform=axes[2, 1].transAxes)
        axes[2, 1].set_title('Learning Rate & Explained Variance (No Data)')

    plt.tight_layout()
    plt.savefig(f'enhanced_ppo_training_metrics_{method_name.lower().replace(" ","_")}.png', dpi=300, bbox_inches='tight')
    print(f"✅ Enhanced training metrics plot saved: enhanced_ppo_training_metrics_{method_name.lower().replace(' ','_')}.png")
    plt.show()
