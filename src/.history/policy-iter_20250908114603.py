...
class SimplifiedFJSPEnv:
    ...
    def transition(self, action, verbose=True):
        ...
        Enhanced to handle timing information in state
        """
        ...
        completed_ops = list(self.state[:num_jobs])
        machine_free_times = list(self.state[num_jobs:num_jobs+num_machines])
        current_makespan = self.state[-1]
        ...
        # Job ready time: arrival time for first operation, or previous op completion time
        job_ready_time = self.job_arrival_times[job_id]
        if completed_ops[job_idx] > 0:
            # For subsequent operations, job is ready when previous operation completes
            # This is approximated as current makespan (in reality we'd track exact job completion times)
            job_ready_time = current_makespan
        ...
        # Create new state
        new_state = tuple(completed_ops) + tuple(machine_free_times) + (new_makespan,)
    ...
    def is_done(self, state=None):
        ...
        # Extract operation counts from enhanced state
        num_jobs = len(self.job_ids)
        completed_ops = state[:num_jobs]
