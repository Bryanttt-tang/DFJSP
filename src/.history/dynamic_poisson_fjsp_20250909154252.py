...
def evaluate_static_agent_on_dynamic(model, jobs_data, machine_list, arrival_times, debug=False):
    """
    Evaluate a static-trained agent on dynamic scenarios.
    The agent was trained on static problems but tested on dynamic arrivals.
    """
    ...
    if debug:
        ...
        print(f"Arrival schedule: {test_env.arrival_times}")
    ...
    print(f"Static agent on dynamic scenario makespan: {makespan:.2f}")
...
def comprehensive_dynamic_static_comparison():
    ...
    arrival_rate = 0.08           # Moderate Poisson arrival rate
    ...
    dynamic_env = DummyVecEnv([lambda: make_dynamic_env(0.5)])
...
if __name__ == "__main__":
    ...
    num_test_scenarios = 1    # Number of test scenarios for fair comparison
    
    # CRITICAL FIX: Generate consistent test scenarios for fair comparison
    print(f"\n0. GENERATING {num_test_scenarios} TEST SCENARIOS...")
    print("-" * 50)
    test_scenarios = generate_test_scenarios(
        num_scenarios=num_test_scenarios,
        initial_jobs=initial_jobs,
        arrival_rate=arrival_rate,
        base_seed=2000  # Different from training seeds
    )
    
    print(f"Generated {len(test_scenarios)} test scenarios")
    print("Sample scenario arrival times:")
    ...
    print("\n1. TRAINING DYNAMIC RL AGENT (ENHANCED)...")
    print("-" * 50)
    print("Enhanced training with curriculum learning and dynamic rewards...")
    
    # Enhanced Dynamic RL training with curriculum
    def make_dynamic_env_curriculum(phase=1):
        # Adjust complexity based on training phase
        if phase == 1:
            rate = arrival_rate * 0.5  # Easier scenario
            # For list initial_jobs, reduce complexity by using fewer initial jobs
            if isinstance(initial_jobs, list):
                init_jobs = initial_jobs[:max(1, len(initial_jobs) - 1)]  # Remove last job
            else:
                init_jobs = max(1, initial_jobs - 1)
        elif phase == 2:
            rate = arrival_rate * 0.75  # Medium scenario
            init_jobs = initial_jobs
        else:
            rate = arrival_rate  # Full complexity
            init_jobs = initial_jobs
            
        env = PoissonDynamicFJSPEnv(
            ENHANCED_JOBS_DATA, MACHINE_LIST,
            initial_jobs=init_jobs,
            arrival_rate=rate,
            reward_mode="makespan_increment"
        )
        return ActionMasker(env, mask_fn)
    ...
    print("  Phase 1/3: Training on simplified scenarios...")
    phase1_env = DummyVecEnv([lambda: make_dynamic_env_curriculum(1)])
    dynamic_rl_model = MaskablePPO(
        "MlpPolicy", phase1_env, verbose=1,
        learning_rate=3e-4, n_steps=512, batch_size=128, n_epochs=20,
        gamma=0.99, gae_lambda=0.95, clip_range=0.2, ent_coef=0.02,
        policy_kwargs=dict(net_arch=[256, 256, 128], activation_fn=torch.nn.ReLU)
    )
    dynamic_rl_model.learn(total_timesteps=70000)
    
    # Phase 2: Medium complexity
    print("  Phase 2/3: Training on medium complexity...")
    phase2_env = DummyVecEnv([lambda: make_dynamic_env_curriculum(2)])
    dynamic_rl_model.set_env(phase2_env)
    dynamic_rl_model.learn(total_timesteps=70000)
    
    # Phase 3: Full complexity  
    print("  Phase 3/3: Training on full complexity...")
    phase3_env = DummyVecEnv([lambda: make_dynamic_env_curriculum(3)])
    dynamic_rl_model.set_env(phase3_env)
    dynamic_rl_model.learn(total_timesteps=60000)
    
    print("\n2. TRAINING STATIC RL AGENT (ENHANCED)...")
    print("-" * 50)
    print("Enhanced training on static arrivals (all jobs at t=0)...")
    
    # Enhanced static RL agent with same architecture as dynamic
    def make_static_env():
        env = PoissonDynamicFJSPEnv(
            ENHANCED_JOBS_DATA, MACHINE_LIST,
    ...
    static_rl_model = MaskablePPO(
        ...
        learning_rate=3e-4,      # Same as dynamic
        n_steps=512,             # Same as dynamic
        batch_size=128,          # Same as dynamic  
        n_epochs=20,             # Same as dynamic
        gamma=0.99,              # Same as dynamic
        gae_lambda=0.95,         # Same as dynamic
        clip_range=0.2,          # Same as dynamic
        ent_coef=0.02,           # Same as dynamic
        policy_kwargs=dict(
            net_arch=[256, 256, 128],  # Same architecture as dynamic
            activation_fn=torch.nn.ReLU
        )
    )
    static_rl_model.learn(total_timesteps=200000)
    
    print("\n3. EVALUATING ALL METHODS ON SAME TEST SCENARIOS...")
    print("="*80)
    ...
    # Evaluate Dynamic RL
    dynamic_rl_makespan, dynamic_rl_schedule, dynamic_rl_arrival_times = evaluate_on_test_scenarios(
        dynamic_rl_model, test_scenarios, "Dynamic RL"
    )
    ...
    # Evaluate Static RL
    static_rl_makespan, static_rl_schedule, static_rl_arrival_times = evaluate_on_test_scenarios(
        static_rl_model, test_scenarios, "Static RL"
    )
    ...
    print("Performance Ranking (Best Performance on Test Scenarios):")
    ...
    # Analysis
    print("\nAnalysis:")
    if dynamic_rl_makespan < static_rl_makespan:
        improvement = ((static_rl_makespan - dynamic_rl_makespan) / static_rl_makespan * 100)
        print(f"✓ Dynamic RL outperforms Static RL by {improvement:.1f}%")
    else:
        gap = ((dynamic_rl_makespan - static_rl_makespan) / static_rl_makespan * 100)
        print(f"✗ Dynamic RL underperforms Static RL by {gap:.1f}%")
    
    print("\n5. CREATING 3-METHOD COMPARISON PLOT...")
    print("-" * 50)
    print("Using best-performing scenario from each method for visualization")
    
    # Use the arrival times from the best SPT scenario for consistent visualization
    ...
    print(f"All methods evaluated on the same {num_test_scenarios} test scenarios for fair comparison")
    print("="*80)
