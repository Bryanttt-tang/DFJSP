...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # All training metrics are now collected at rollout boundaries in _on_rollout_end
        # This ensures alignment with verbose output
        return True
    ...
    def _on_rollout_end(self) -> None:
        ...
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            ...
            # Record episode reward from rollout (aligned with verbose output)
            if 'rollout/ep_rew_mean' in log_data:
            ...
            # Record episode length from rollout if available
            if 'rollout/ep_len_mean' in log_data:
            ...
            # Only print verbose output if we actually logged some metrics
            if (self.verbose and len(TRAINING_METRICS['policy_loss']) <= 5 and 
                len(TRAINING_METRICS['policy_loss']) > 0 and len(TRAINING_METRICS['value_loss']) > 0):
                ep_rew = TRAINING_METRICS['episode_rewards'][-1] if TRAINING_METRICS['episode_rewards'] else 0.0
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Increase ep_info_buffer size to capture all episodes (default is 100)
    model.ep_info_buffer.maxlen = 10000  # Enough for most training runs
...
def plot_training_metrics():
    ...
    # Create episode numbers and timesteps for x-axis
    ...
    # Use timesteps for episode rewards to align with other plots
    episode_timesteps = TRAINING_METRICS['timesteps'][:num_episodes] if len(TRAINING_METRICS['timesteps']) >= num_episodes else list(range(num_episodes))
    ...
    print(f"Episode rewards recorded at timesteps: {episode_timesteps[:5]} ... {episode_timesteps[-5:] if len(episode_timesteps) > 5 else episode_timesteps}")
    print(f"Total rollouts/training updates: {len(TRAINING_METRICS['timesteps'])}")
    
    # Create comprehensive figure with 3x2 subplots
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.suptitle(f'Training Progress - {TRAINING_METRICS.get("method_name", "RL Agent")} ({num_episodes} Rollouts)', 
                 fontsize=16, fontweight='bold')
    
    # Plot 1: Episode Rewards over Timesteps (Top Left) - ALIGNED WITH OTHER PLOTS
    axes[0, 0].plot(episode_timesteps, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward (ep_rew_mean)')
    
    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Timesteps')
    axes[0, 0].set_ylabel('Episode Reward (ep_rew_mean)')
    axes[0, 0].set_title('Episode Rewards Over Training (Aligned with Rollouts)')
    ...
    # Plot 6: Episode Length over Timesteps (Bottom Right) - ALIGNED WITH OTHER PLOTS
    if TRAINING_METRICS['episode_lengths']:
        ...
        axes[2, 1].plot(length_timesteps, ep_lengths, 'teal', linewidth=2, alpha=0.7, label='Episode Length (ep_len_mean)')
        axes[2, 1].legend()
        
        axes[2, 1].set_xlabel('Timesteps')
        axes[2, 1].set_ylabel('Episode Length (ep_len_mean)')
        axes[2, 1].set_title('Episode Length Over Training (Aligned with Rollouts)')
