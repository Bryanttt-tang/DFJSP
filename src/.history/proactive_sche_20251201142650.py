...
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    proc_time_variance_range=(1, 10),  # HUGE variance (was 5-30) - breaks SPT/LPT
    seed=GLOBAL_SEED
)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        # STABILIZED hyperparameters for deterministic Perfect RL
        # Key fixes for stability:
        # 1. Much lower learning rate (1e-4 instead of 3e-4)
        # 2. Larger batches for more stable gradients
        # 3. Smaller rollouts for more frequent updates
        # 4. Very low entropy coefficient (deterministic problem)
        # 5. Tighter gradient clipping
        model = MaskablePPO(
            "MlpPolicy",
            vec_env,
            verbose=0,
            learning_rate=linear_schedule(learning_rate),  # ⭐ VERY LOW + decay
            n_steps=512,               # ⭐ Larger rollouts for better value estimates
            batch_size=128,             # ⭐ Smaller batches = more gradient steps
            n_epochs=3,                # ⭐ Minimal epochs to avoid overfitting
            gamma=1.0,                 # Undiscounted (makespan)
            gae_lambda=0.99,           # ⭐ Higher GAE for smoother advantages
            clip_range=0.2,            # ⭐ VERY TIGHT clipping (half default)
            clip_range_vf=0.1,         # ⭐ Also clip value function
            ent_coef=0.01,              # ⭐ ZERO entropy (pure exploitation)
            ...
            policy_kwargs=dict(
                net_arch=dict(
                    pi=[256, 256],         # ⭐ Smaller network = less overfitting
                    vf=[256, 256]          # Separate value network
                ),
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    model = MaskablePPO(
        ...
        n_steps=512,               # Increased from 1024 - match other RL methods
        batch_size=256,            # Increased from 256 for more stable gradients
        n_epochs=10,                # More epochs (proactive is complex!)
        gamma=1.0,                  # Undiscounted (makespan objective)
        gae_lambda=0.95,            # GAE for advantage estimation
        clip_range=0.2,             # Standard PPO clipping
        ent_coef=0.05,              # Increased from 0.01 - critical for wait exploration
        vf_coef=0.5,                # Value function coefficient
        max_grad_norm=0.5,          # Gradient clipping for stability
        normalize_advantage=True,   # Normalize advantages
        seed=GLOBAL_SEED,
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 300000    # Perfect knowledge with MULTIPLE INITIALIZATIONS
    dynamic_timesteps = 10000  # Increased from 300k - Reactive needs more time
    proactive_timesteps = 300000  # Increased from 300k - Proactive needs even more (prediction + scheduling)
    rule_based_timesteps = 10000  # Same as reactive - needs to explore all rule combos
