...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def _get_ready_operations(self):
        """Get list of ready operations that can be scheduled (without pre-selecting machines)."""
        ready_ops = []
    ...
    def action_masks(self):
        """Generate action masks - all 10 rule combinations available if ready ops exist."""
        ...
        # No WAIT action - we advance time automatically if no ready ops
        if ready_ops:
            mask[:] = True  # All 10 actions available
