...
class EnhancedTrainingCallback(BaseCallback):
    """Enhanced BaseCallback to track comprehensive training metrics at proper rollout boundaries."""
    ...
    def _on_rollout_end(self) -> None:
        ...
        # Get logger data at rollout boundary when episode stats are fresh
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            ...
            if self.verbose >= 1:
                ...
                print(f"[{self.method_name}] Logger keys: {available_keys}")
            
            # Episode-level metrics (available at rollout boundaries)
            if 'rollout/ep_rew_mean' in log_data:
                ...
                if self.verbose >= 1:
                    print(f"[{self.method_name}] ✅ Episode reward captured: {ep_reward:.4f} (count: {self.episode_count})")
            else:
                if self.verbose >= 1:
                    print(f"[{self.method_name}] ❌ No 'rollout/ep_rew_mean' in logger data")
            
            if 'rollout/ep_len_mean' in log_data:
                TRAINING_METRICS['episode_lengths'].append(log_data['rollout/ep_len_mean'])
            
            # Training metrics (also refreshed at rollout boundaries)
            ...
            if 'train/value_loss' in log_data:
                ...
                if self.verbose >= 1:
                    print(f"[{self.method_name}] Value loss: {log_data['train/value_loss']:.4f}")
            ...
            if self.verbose >= 1:
                print(f"[{self.method_name}] ❌ No logger or name_to_value available")
...
def plot_training_metrics():
    ...
    print(f"TRAINING_METRICS type: {type(TRAINING_METRICS)}")
    print(f"TRAINING_METRICS keys: {list(TRAINING_METRICS.keys())}")
    
    if not TRAINING_METRICS.get('timesteps', []):
        print("❌ No training metrics recorded in TRAINING_METRICS['timesteps']!")
        print("This suggests the callback is not properly collecting data")
        # Let's still try to show what we have
        if TRAINING_METRICS:
            print("But we do have some TRAINING_METRICS data:")
    ...
    print(f"Episode rewards recorded: {len(TRAINING_METRICS.get('episode_rewards', []))}")
    print(f"Policy loss records: {len(TRAINING_METRICS.get('policy_loss', []))}")
    print(f"Value loss records: {len(TRAINING_METRICS.get('value_loss', []))}")
    ...
    if TRAINING_METRICS.get('episode_rewards', []):
        ...
        print("This suggests the VecMonitor or callback is not working properly")
    
    # Value loss diagnostic
    if TRAINING_METRICS.get('value_loss', []):
        value_losses = TRAINING_METRICS['value_loss']
        print(f"Value loss sample (last 5): {value_losses[-5:]}")
        print(f"Value loss range: {min(value_losses):.4f} to {max(value_losses):.4f}")
    else:
        print("❌ NO VALUE LOSS RECORDED!")
    
    # DEBUG: Print all TRAINING_METRICS keys and lengths for diagnostics
    print(f"\\nTRAINING_METRICS diagnostics:")
    for key, value in TRAINING_METRICS.items():
        if isinstance(value, list):
            print(f"  {key}: {len(value)} items - Sample: {value[:3] if value else 'empty'}")
        else:
            print(f"  {key}: {value}")
    ...
    if episode_rewards and len(episode_rewards) > 1:
        ...
        ep_timesteps = timesteps[:len(episode_rewards)] if timesteps else list(range(len(episode_rewards)))
        
        print(f"[DEBUG] Plotting {len(episode_rewards)} episode rewards against {len(ep_timesteps)} timesteps")
        
        axes[0, 0].plot(ep_timesteps, episode_rewards, 'g-', linewidth=2, alpha=0.7, label='Mean Episode Reward')
        ...
        if len(episode_rewards) > 10:
            ...
            if window_size > 0:
                ...
                moving_timesteps = ep_timesteps[window_size-1:] if len(ep_timesteps) >= window_size else ep_timesteps
                if len(moving_timesteps) == len(moving_avg):
                    axes[0, 0].plot(moving_timesteps, moving_avg, 'darkgreen', linewidth=3, 
                                   label=f'Moving Avg (window={window_size})')
        
        axes[0, 0].legend()
        axes[0, 0].set_xlabel('Training Steps' if timesteps else 'Episode Count')
        ...
        if len(episode_rewards) > 20:
            early_reward = np.mean(episode_rewards[:len(episode_rewards)//4])
            late_reward = np.mean(episode_rewards[-len(episode_rewards)//4:])
    ...
    if policy_loss:
        loss_timesteps = timesteps[:len(policy_loss)] if timesteps else list(range(len(policy_loss)))
        ...
        if len(policy_loss) > 10:
            ...
            if window_size > 0:
                ...
                moving_timesteps = loss_timesteps[window_size-1:] if len(loss_timesteps) >= window_size else loss_timesteps
                if len(moving_timesteps) == len(moving_avg):
                    axes[0, 1].plot(moving_timesteps, moving_avg, 'darkred', linewidth=3,
                                   label=f'Moving Avg (window={window_size})')
                    axes[0, 1].legend()
        
        axes[0, 1].set_xlabel('Training Steps' if timesteps else 'Update Count')
        ...
        axes[0, 1].text(0.5, 0.5, f'No policy loss data\n(Recorded: {len(policy_loss)} items)', ha='center', va='center', 
    ...
    if value_loss:
        loss_timesteps = timesteps[:len(value_loss)] if timesteps else list(range(len(value_loss)))
        ...
        axes[1, 0].set_xlabel('Training Steps' if timesteps else 'Update Count')
        ...
        axes[1, 0].text(0.5, 0.5, f'No value loss data\n(Recorded: {len(value_loss)} items)', ha='center', va='center', 
                       transform=axes[1, 0].transAxes)
        axes[1, 0].set_title('Value Loss (No Data)')
    
    # Plot 4: Action Entropy over Time (Middle Right)
    action_entropy = TRAINING_METRICS.get('action_entropy', [])
    if action_entropy:
        entropy_timesteps = timesteps[:len(action_entropy)] if timesteps else list(range(len(action_entropy)))
        
        axes[1, 1].plot(entropy_timesteps, action_entropy, 'purple', linewidth=2, alpha=0.7)
        
        # Add moving average
        if len(action_entropy) > 10:
            window_size = min(20, len(action_entropy) // 4)
            if window_size > 0:
                moving_avg = np.convolve(action_entropy, np.ones(window_size)/window_size, mode='valid')
                moving_timesteps = entropy_timesteps[window_size-1:] if len(entropy_timesteps) >= window_size else entropy_timesteps
                if len(moving_timesteps) == len(moving_avg):
                    axes[1, 1].plot(moving_timesteps, moving_avg, 'indigo', linewidth=3,
                                   label=f'Moving Avg (window={window_size})')
                    axes[1, 1].legend()
        
        axes[1, 1].set_xlabel('Training Steps' if timesteps else 'Update Count')
        axes[1, 1].set_ylabel('Action Entropy')
        axes[1, 1].set_title('Action Entropy (Exploration Level)')
        axes[1, 1].grid(True, alpha=0.3)
        
        # Add interpretation
        final_entropy = action_entropy[-1]
        ...
        axes[1, 1].text(0.5, 0.5, f'No entropy data\n(Recorded: {len(action_entropy)} items)', ha='center', va='center', 
    ...
    if policy_loss and value_loss:
        min_len = min(len(policy_loss), len(value_loss))
        combined_timesteps = timesteps[:min_len] if timesteps else list(range(min_len))
        
        axes[2, 0].plot(combined_timesteps, policy_loss[:min_len], 
                       'r-', linewidth=2, label='Policy Loss', alpha=0.7)
        axes[2, 0].plot(combined_timesteps, value_loss[:min_len], 
                       'b-', linewidth=2, label='Value Loss', alpha=0.7)
        axes[2, 0].legend()
        axes[2, 0].set_xlabel('Training Steps' if timesteps else 'Update Count')
        axes[2, 0].set_ylabel('Loss')
        axes[2, 0].grid(True, alpha=0.3)
    else:
        axes[2, 0].text(0.5, 0.5, f'Insufficient loss data\nPolicy: {len(policy_loss)}, Value: {len(value_loss)}', 
                       ha='center', va='center', transform=axes[2, 0].transAxes)
    
    # Plot 6: Learning Rate and Explained Variance (Bottom Right)
    learning_rate = TRAINING_METRICS.get('learning_rate', [])
    explained_variance = TRAINING_METRICS.get('explained_variance', [])
    if learning_rate or explained_variance:
        ax_lr = axes[2, 1]
        ax_ev = ax_lr.twinx()
        
        if learning_rate:
            lr_timesteps = timesteps[:len(learning_rate)] if timesteps else list(range(len(learning_rate)))
            line1 = ax_lr.plot(lr_timesteps, learning_rate, 'orange', linewidth=2, label='Learning Rate')
            ax_lr.set_ylabel('Learning Rate', color='orange')
            ax_lr.tick_params(axis='y', labelcolor='orange')
        
        if explained_variance:
            ev_timesteps = timesteps[:len(explained_variance)] if timesteps else list(range(len(explained_variance)))
            line2 = ax_ev.plot(ev_timesteps, explained_variance, 'cyan', linewidth=2, label='Explained Variance')
            ax_ev.set_ylabel('Explained Variance', color='cyan')
            ax_ev.tick_params(axis='y', labelcolor='cyan')
        
        axes[2, 1].set_xlabel('Training Steps' if timesteps else 'Update Count')
        axes[2, 1].set_title('Learning Rate & Value Function Quality')
        axes[2, 1].grid(True, alpha=0.3)
    else:
        axes[2, 1].text(0.5, 0.5, f'No LR/EV data\nLR: {len(learning_rate)}, EV: {len(explained_variance)}', 
    ...
    if episode_rewards:
        ...
        print(f"  Initial reward: {episode_rewards[0]:.4f}")
        ...
        if len(episode_rewards) > 20:
            early_rewards = episode_rewards[:len(episode_rewards)//4]
            late_rewards = episode_rewards[-len(episode_rewards)//4:]
        ...
        print("❌ No episode rewards recorded for summary")
    
    policy_loss = TRAINING_METRICS.get('policy_loss', [])
    if policy_loss:
        print(f"\nPolicy Loss:")
        print(f"  Initial loss: {policy_loss[0]:.6f}")
        print(f"  Final loss: {policy_loss[-1]:.6f}")
        print(f"  Mean loss: {np.mean(policy_loss):.6f}")
        print(f"  Std loss: {np.std(policy_loss):.6f}")
    ...
    if value_loss:
        print(f"\nValue Loss:")
        print(f"  Initial loss: {value_loss[0]:.6f}")
        print(f"  Final loss: {value_loss[-1]:.6f}")
        print(f"  Mean loss: {np.mean(value_loss):.6f}")
        print(f"  Std loss: {np.std(value_loss):.6f}")
    ...
    print("=" * 50)
    return True  # Indicate successful plotting
