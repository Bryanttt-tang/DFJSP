...
class ArrivalPredictor:
    """
    Maximum Likelihood Estimator for Poisson arrival process.
    Updates the estimated arrival rate based on observed inter-arrival times.
    """
    def __init__(self, initial_rate_guess=0.05):
        self.estimated_rate = initial_rate_guess
        self.observed_inter_arrivals = []  # Track inter-arrival times
        self.episode_observations = []  # Observations within current episode
        
    def update_with_episode_data(self, arrival_times_dict):
        """
        Update MLE after observing an episode's arrival times.
        arrival_times_dict: {job_id: arrival_time} for jobs that arrived
        """
        # Extract dynamic job arrivals (excluding initial jobs at t=0)
        arrivals = sorted([t for j, t in arrival_times_dict.items() if t > 0 and t != float('inf')])
        
        if len(arrivals) > 1:
            # Calculate inter-arrival times
            inter_arrivals = [arrivals[i+1] - arrivals[i] for i in range(len(arrivals)-1)]
            self.observed_inter_arrivals.extend(inter_arrivals)
            
            # MLE for Poisson process: rate = 1 / mean_inter_arrival_time
            if self.observed_inter_arrivals:
                mean_inter_arrival = np.mean(self.observed_inter_arrivals)
                if mean_inter_arrival > 0:
                    self.estimated_rate = 1.0 / mean_inter_arrival
                    
    def update_within_episode(self, new_arrival_time, previous_arrival_time):
        """
        Update prediction within an episode as new jobs arrive.
        """
        if new_arrival_time > previous_arrival_time and previous_arrival_time >= 0:
            inter_arrival = new_arrival_time - previous_arrival_time
            self.episode_observations.append(inter_arrival)
            
            # Update estimate using all observations so far
            all_observations = self.observed_inter_arrivals + self.episode_observations
            if all_observations:
                mean_inter_arrival = np.mean(all_observations)
                if mean_inter_arrival > 0:
                    self.estimated_rate = 1.0 / mean_inter_arrival
    
    def predict_next_arrival(self, current_time, last_arrival_time=None):
        """
        Predict the expected time of next arrival.
        Returns: expected_next_arrival_time
        """
        if last_arrival_time is None or last_arrival_time < current_time:
            base_time = current_time
        else:
            base_time = last_arrival_time
            
        # Expected inter-arrival time for Poisson process
        expected_inter_arrival = 1.0 / self.estimated_rate if self.estimated_rate > 0 else 10.0
        
        return base_time + expected_inter_arrival
    
    def reset_episode(self):
        """Reset episode-specific observations."""
        self.episode_observations = []


class ProactiveDynamicFJSPEnv(gym.Env):
    """
    PROACTIVE Dynamic FJSP Environment with arrival time prediction.
    - Uses MLE to estimate Poisson arrival rate from observed data
    - Allows scheduling of "soon-to-arrive" jobs based on predictions
    - Updates predictions as more arrivals are observed
    """
    
    metadata = {"render.modes": ["human"]}

    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
                 max_time_horizon=100, reward_mode="makespan_increment", 
                 prediction_window=10.0, seed=None):
        super().__init__()
        ...
        self.true_arrival_rate = arrival_rate  # Hidden from agent
        self.max_time_horizon = max_time_horizon
        self.reward_mode = reward_mode
        self.prediction_window = prediction_window  # How far ahead to allow proactive scheduling
        
        # Initialize arrival predictor (agent's belief)
        ...
        # Action space: job_idx * num_machines + machine_idx + WAIT
        ...
        # Observation space includes prediction information
        obs_size = (
            self.num_jobs +                         # Ready/arrived job indicators
            ...
            self.num_jobs +                         # Predicted arrival times (normalized)
            self.num_jobs +                         # Confidence in predictions
            3                                       # Event time, makespan progress, estimated rate
        )
    ...
    def _reset_state(self):
        ...
        self.last_observed_arrival = 0.0
        
        # Generate true arrival times (hidden from agent)
        self._generate_poisson_arrivals()
        
        # Reset predictor's episode-specific data
        self.arrival_predictor.reset_episode()
        
        # Track predictions for debugging
        self.predicted_arrival_times = {}  # Agent's belief
    ...
    def _generate_poisson_arrivals(self):
        ...
        for job_id in self.dynamic_job_ids:
            inter_arrival_time = np.random.exponential(1.0 / self.true_arrival_rate)
            current_time += inter_arrival_time
            
            if current_time <= self.max_time_horizon:
                self.job_arrival_times[job_id] = round(current_time, 2)
            else:
    ...
    def reset(self, seed=None, options=None):
        ...
        # Update predictor with episode data after first episode
        if TRAINING_EPISODE_COUNT > 0:
            self.arrival_predictor.update_with_episode_data(self.job_arrival_times)
        ...
        for job_id, arr_time in self.job_arrival_times.items():
            if arr_time != float('inf') and arr_time > 0:
    ...
    def _update_predictions(self):
        """Update predicted arrival times for unarrived jobs."""
        # Find the last observed arrival
        observed_arrivals = [t for j, t in self.job_arrival_times.items() 
                           if j in self.arrived_jobs and t > 0]
        if observed_arrivals:
            self.last_observed_arrival = max(observed_arrivals)
        
        # Predict next arrivals for unarrived jobs
        unarrived_jobs = [j for j in self.dynamic_job_ids if j not in self.arrived_jobs]
        
        if unarrived_jobs:
            next_predicted = self.arrival_predictor.predict_next_arrival(
                self.event_time, self.last_observed_arrival
            )
            
            # Assign predictions (simplified: assume sequential arrivals)
            for idx, job_id in enumerate(sorted(unarrived_jobs)):
                self.predicted_arrival_times[job_id] = next_predicted + idx * (1.0 / self.arrival_predictor.estimated_rate)

    def action_masks(self):
        """
        PROACTIVE: Generate action masks allowing scheduling of:
        1. Arrived jobs
        2. Jobs predicted to arrive within prediction_window
        """
        ...
        # Update predictions before generating masks
        ...
        for job_idx, job_id in enumerate(self.job_ids):
            ...
            if job_id in self.arrived_jobs:
                can_schedule = True
            # 2. Job predicted to arrive soon (PROACTIVE SCHEDULING)
    ...
    def _is_valid_action(self, job_idx, op_idx, machine_idx):
        """Check if scheduling action is valid (includes proactive scheduling)."""
        if job_idx is None:
            return True
        ...
        # Check if job can be scheduled (arrived or predicted soon)
        can_schedule = job_id in self.arrived_jobs
        if not can_schedule and job_id in self.predicted_arrival_times:
            predicted_time = self.predicted_arrival_times[job_id]
            can_schedule = predicted_time <= self.event_time + self.prediction_window
    ...
    def _update_event_time_and_arrivals(self, new_event_time):
        """Update event time and reveal arrived jobs."""
        old_arrivals = len(self.arrived_jobs)
        ...
        for job_id, arrival_time in self.job_arrival_times.items():
            if (job_id not in self.arrived_jobs and 
                ...
                # Update predictor with newly observed arrival
                if self.last_observed_arrival > 0:
                    self.arrival_predictor.update_within_episode(arrival_time, self.last_observed_arrival)
    ...
    def step(self, action):
        ...
        if self.operations_scheduled >= self.total_operations:
            final_reward = -self.current_makespan
            return self._get_observation(), final_reward, True, False, {
                "makespan": self.current_makespan, "status": "completed"
            }

        if self.episode_step >= self.max_episode_steps:
            penalty = -self.current_makespan - 1000
            return self._get_observation(), penalty, True, False, {
                "error": "Max episode steps reached"
            }
        ...
        if job_idx is None:
            ...
            return self._get_observation(), wait_reward, False, False, {
                "action_type": "WAIT", "event_time": self.event_time
            }

        # Handle scheduling action
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            return self._get_observation(), -500.0, False, False, {
                "error": "Invalid scheduling action"
            }
        ...
        # PROACTIVE: For not-yet-arrived jobs, use predicted arrival time
        if job_id not in self.arrived_jobs:
            predicted_arrival = self.predicted_arrival_times.get(job_id, job_ready_time)
            job_ready_time = max(job_ready_time, predicted_arrival)
        
        # Start time respects event_time, machine availability, and job readiness
        ...
        # Penalty for misprediction (if scheduled before actual arrival)
        if job_id not in self.arrived_jobs:
            actual_arrival = self.job_arrival_times[job_id]
            if actual_arrival > start_time:
                # Scheduled too early - add penalty
        ...
        return self._get_observation(), reward, terminated, False, {
            "makespan": self.current_makespan, "action_type": "SCHEDULE", 
            "event_time": self.event_time
        }

    def _get_observation(self):
        """PROACTIVE observation including prediction information."""
        obs = []
        
        # Update predictions
        self._update_predictions()
        
        # 1. Job availability indicators (arrived or predicted soon)
        for job_id in self.job_ids:
            is_available = job_id in self.arrived_jobs
            if not is_available and job_id in self.predicted_arrival_times:
                predicted_time = self.predicted_arrival_times[job_id]
                is_available = predicted_time <= self.event_time + self.prediction_window
            
            if is_available and self.next_operation[job_id] < len(self.jobs[job_id]):
                obs.append(1.0)
            else:
                obs.append(0.0)
        ...
        # 4. Processing times for available operations
        for job_id in self.job_ids:
            is_available = (job_id in self.arrived_jobs or 
                          (job_id in self.predicted_arrival_times and 
                           self.predicted_arrival_times[job_id] <= self.event_time + self.prediction_window))
        ...
        # 5. PROACTIVE: Predicted arrival times (normalized)
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs:
                obs.append(0.0)  # Already arrived
            elif job_id in self.predicted_arrival_times:
                predicted_time = self.predicted_arrival_times[job_id]
                relative_time = max(0, predicted_time - self.event_time)
                normalized_time = min(1.0, relative_time / self.max_time_horizon)
                obs.append(normalized_time)
            else:
                obs.append(1.0)  # Unknown
        
        # 6. Prediction confidence (based on number of observations)
        confidence = min(1.0, len(self.arrival_predictor.observed_inter_arrivals) / 10.0)
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs:
                obs.append(1.0)  # Certain
            elif job_id in self.predicted_arrival_times:
        ...
        normalized_rate = min(1.0, self.arrival_predictor.estimated_rate * 10.0)
...
def main():
    ...
    dynamic_model = train_dynamic_agent(
        ...
        total_timesteps=dynamic_timesteps, reward_mode="makespan_increment", learning_rate=3e-4
    )
    ...
    # Create reactive folder
    import os
    folder_name = f"reactive_{arrival_rate}_rate"
