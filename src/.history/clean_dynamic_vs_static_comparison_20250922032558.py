...
class StaticFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, reward_mode="makespan_increment", seed=None):
        ...
        # EVENT-DRIVEN observation space with STATIC RL baseline (no future info)
        obs_size = (
            ...
            # NOTE: Static RL gets NO future arrival information - this is the baseline
        )
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # EVENT-DRIVEN observation space with DYNAMIC RL arrival awareness
        obs_size = (
            self.num_jobs +                         # Ready job indicators
            len(self.machines) +                    # Machine idle status
            self.num_jobs * len(self.machines) +    # Processing times for ready ops
    ...
    def _get_observation(self):
        ...
        # 3. Processing times for ready operations (normalized) + future arrival awareness
        max_proc_time = 10.0  # Reasonable upper bound for normalization
        ...
        for job_id in self.job_ids:
            if (job_id in self.arrived_jobs and 
                ...
                # Job not arrived yet: provide arrival time hint for dynamic advantage
                arrival_time = self.job_arrival_times.get(job_id, float('inf'))
                if arrival_time != float('inf') and arrival_time <= self.current_makespan + 20:
                    # Job will arrive soon: add normalized arrival delay
                    delay = max(0, arrival_time - self.current_makespan)
                    normalized_delay = min(1.0, delay / 20.0)  # Normalize by 20 time units
                    for machine in self.machines:
                        obs.append(normalized_delay)  # Same delay for all machines
                else:
                    # Job won't arrive soon: add zeros
                    for machine in self.machines:
                        obs.append(0.0)
            else:
                # Job arrived but no more operations: add zeros
                for machine in self.machines:
                    obs.append(0.0)
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment", seed=None):
        ...
        # EVENT-DRIVEN MINIMAL observation space matching other environments
        obs_size = (
            self.num_jobs +                         # Ready job indicators
            len(self.machines) +                    # Machine idle status
            self.num_jobs * len(self.machines)      # Processing times for ready ops
    ...
    def _get_observation(self):
        ...
        # 3. Processing times for ready operations (normalized) + PERFECT future knowledge
        max_proc_time = 10.0  # Reasonable upper bound for normalization
        ...
        for job_id in self.job_ids:
            if (job_id in self.arrived_jobs and 
                ...
                # PERFECT KNOWLEDGE: Exact future arrival information
                arrival_time = self.job_arrival_times.get(job_id, float('inf'))
                if arrival_time != float('inf'):
                    ...
                    # Also include the processing times of the first operation for future planning
                    if len(self.jobs[job_id]) > 0:
                        ...
                        for machine in self.machines:
                            if machine in first_op['proc_times']:
                                ...
                                # Combine arrival delay and processing time info
                                combined_info = (normalized_delay + normalized_proc) / 2.0
                                obs.append(combined_info)
                            else:
                                obs.append(normalized_delay)  # Just arrival delay
                    else:
                        for machine in self.machines:
                            obs.append(normalized_delay)
                else:
                    # Job won't arrive: add zeros
                    for machine in self.machines:
                        obs.append(0.0)
            else:
                # Job arrived but no more operations: add zeros
...
def main():
    ...
    print("OPTIMIZED: Switched to event-driven minimal observation space:")
    print("       - Ready job indicators + Machine idle status + Processing times")
    print(f"       - Observation size reduced from {7*4+1} to {7+3+7*3} = {7+3+21} elements")
    print("       - Perfect RL gets enhanced future arrival information for advantage")
