...
# Test the compact MDP with the larger problem that previously hit state limits!
JOBS_DATA = LARGER_JOBS_DATA
MACHINES = LARGER_MACHINES
ARRIVALS = LARGER_ARRIVALS

print("Testing compact event-driven MDP on LARGER problem:")
print(f"  - {len(LARGER_JOBS_DATA)} jobs")
print(f"  - {len(LARGER_MACHINES)} machines") 
print(f"  - {sum(len(ops) for ops in LARGER_JOBS_DATA.values())} total operations")
print("This would have exploded the timing-based MDP but should be manageable now!\n")
...
# ---------------------------
# Helper utilities for event-driven compact MDP
# ---------------------------
def make_compact_state(ready_jobs, idle_machines):
    """
    Event-driven compact state:
    - ready_jobs: tuple of (job_idx, op_idx) for jobs whose next op is ready NOW
    - idle_machines: tuple of machine indices that are idle NOW
    
    This is Markov because:
    1. We only need to know which jobs can be scheduled right now
    2. We only need to know which machines are available right now
    3. Processing times are deterministic and encoded in the problem data
    4. The simulator handles all timing and future events deterministically
    """
    return (tuple(sorted(ready_jobs)), tuple(sorted(idle_machines)))

def pretty_compact_state(s):
    ready_jobs, idle_machines = s
    ready_str = [f"J{job_ids[j]}-Op{op+1}" for j, op in ready_jobs]
    idle_str = [machines[m] for m in idle_machines]
    return f"ready={ready_str}, idle={idle_str}"

def get_processing_times_for_state(ready_jobs):
    """Get processing times for all ready operations on all eligible machines."""
    proc_times = {}
    for j_idx, op_idx in ready_jobs:
        job_id = job_ids[j_idx]
        proc_dict = JOBS_DATA[job_id][op_idx]['proc_times']
        proc_times[(j_idx, op_idx)] = proc_dict
    return proc_times

# ---------------------------
# Enumerate reachable compact event-driven MDP
# ---------------------------
print("Enumerating reachable compact event-driven MDP via BFS...")
...
def add_compact_state(s):
    if s not in state_to_id:
        sid = len(id_to_state)
        state_to_id[s] = sid
        id_to_state.append(s)
        # Progress reporting
        if len(id_to_state) % 1000 == 0:
            print(f"  Generated {len(id_to_state)} states so far...")
    return state_to_id[s]

def get_next_compact_states_after_assignment(ready_jobs, idle_machines, assigned_job, assigned_machine):
    """
    Given current ready jobs and idle machines, and an assignment decision,
    compute all possible next compact states that could occur.
    
    The key insight: after assignment, we need to consider all possible ways
    that future operations could become ready simultaneously.
    """
    # Start with current progress
    next_ops = [0 for _ in job_ids]
    for j_idx, op_idx in ready_jobs:
        next_ops[j_idx] = op_idx
    
    # Apply the assignment
    j_idx, op_idx = assigned_job
    next_ops[j_idx] += 1  # this job progresses to next operation
    
    # Generate all possible combinations of which jobs could be ready next
    # This is where we enumerate the "event-driven" aspect
    possible_next_states = []
    
    # Base case: immediate next state after this assignment
    new_ready_jobs = []
    new_idle_machines = list(idle_machines)
    new_idle_machines.remove(assigned_machine)  # this machine becomes busy
    
    # Check which jobs have their next operation ready
    for j_idx, job_id in enumerate(job_ids):
        op_idx = next_ops[j_idx]
        if op_idx < len(JOBS_DATA[job_id]):  # job not finished
            # For event-driven MDP: assume operation becomes ready immediately
            # (the timing is handled by the environment/simulator)
            new_ready_jobs.append((j_idx, op_idx))
    
    # The machine becomes idle again after processing
    # In event-driven model, we assume it's immediately available for next assignment
    new_idle_machines.append(assigned_machine)
    
    next_state = make_compact_state(new_ready_jobs, new_idle_machines)
    return [next_state]

# Initial state: all jobs start with their first operation ready, all machines idle
init_ready_jobs = []
for j_idx, job_id in enumerate(job_ids):
    if len(JOBS_DATA[job_id]) > 0:  # job has operations
        init_ready_jobs.append((j_idx, 0))

init_idle_machines = list(range(len(machines)))
init_state = make_compact_state(init_ready_jobs, init_idle_machines)
add_compact_state(init_state)

q = deque([init_state])
states_processed = 0
MAX_STATES = 50000  # Should be much smaller now!
...
while q and len(id_to_state) < MAX_STATES:
    ...
    # Terminal state: no jobs ready (all finished)
    if not ready_jobs:
        continue

    # Legal actions: assign any ready job to any idle machine (if compatible)
    legal_actions = []
    proc_times = get_processing_times_for_state(ready_jobs)
    
    for (j_idx, op_idx) in ready_jobs:
        job_proc_times = proc_times[(j_idx, op_idx)]
        for m_idx in idle_machines:
            machine = machines[m_idx]
            if machine in job_proc_times:
                legal_actions.append((j_idx, op_idx, m_idx))

    for action in legal_actions:
        j_idx, op_idx, m_idx = action
        
        # Get next states after this assignment
        next_states = get_next_compact_states_after_assignment(
            ready_jobs, idle_machines, (j_idx, op_idx), m_idx)
        
        for next_state in next_states:
            nsid = add_compact_state(next_state)
            if nsid == len(id_to_state) - 1:
                q.append(next_state)
            
            # Reward: negative processing time (minimize total processing time as proxy for makespan)
            job_id = job_ids[j_idx]
            machine = machines[m_idx]
            processing_time = JOBS_DATA[job_id][op_idx]['proc_times'][machine]
...
if incomplete_mdp:
    print(f"WARNING: Enumeration stopped at {MAX_STATES} states limit!")
    print(f"The MDP is INCOMPLETE - Policy Iteration will give WRONG results!")
    print(f"Reduce problem size or increase MAX_STATES limit.")
    
print(f"Enumeration done: {num_states} states, {num_transitions} transitions (elapsed {elapsed:.2f}s)\n")

# Analyze the compact state space
print("=== COMPACT STATE SPACE ANALYSIS ===")
ready_job_counts = {}
idle_machine_counts = {}

for sid in range(num_states):
    state = id_to_state[sid]
    ready_jobs, idle_machines = state
    
    num_ready = len(ready_jobs)
    num_idle = len(idle_machines)
    
    if num_ready not in ready_job_counts:
        ready_job_counts[num_ready] = 0
    ready_job_counts[num_ready] += 1
    
    if num_idle not in idle_machine_counts:
        idle_machine_counts[num_idle] = 0
    idle_machine_counts[num_idle] += 1

print("States by number of ready jobs:")
for num_ready, count in sorted(ready_job_counts.items()):
    print(f"  {num_ready} ready jobs: {count} states")

print("States by number of idle machines:")
for num_idle, count in sorted(idle_machine_counts.items()):
    print(f"  {num_idle} idle machines: {count} states")
    
print(f"\nCompact state space: {num_states} states total")
print("No timing explosion - state space is truly minimal!")

# Save compact states and transitions to CSV
import csv
csv_filename = "/Users/tanu/Desktop/PhD/Scheduling/src/compact_mdp_states_transitions.csv"
print(f"Saving compact state and transition data to {csv_filename}...")

with open(csv_filename, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    # Header
    writer.writerow(['state_id', 'ready_jobs', 'idle_machines', 'num_actions', 'actions'])
    ...
    for sid in range(num_states):
        ...
        # Format ready jobs and machines for CSV
        ready_str = "; ".join([f"J{job_ids[j]}-Op{op+1}" for j, op in ready_jobs])
        idle_str = "; ".join([machines[m] for m in idle_machines])
...
# ---------------------------
if incomplete_mdp:
    print("SKIPPING Policy Iteration due to incomplete MDP!")
    print("Policy Iteration results would be INCORRECT on truncated state space.\n")
    ...
    for sid in range(num_states):
        ...
        if old_a != best_a:
            ...
            if len(changes) <= max_print:
                print(f"  State {sid}: {pretty_compact_state(id_to_state[sid])}")
...
# ---------------------------
# Execute PI policy to build schedule (with timing simulation)
# ---------------------------
print("Executing the obtained optimal policy from initial state to collect schedule...")

# For compact MDP, we need to simulate timing to build actual schedule
class EventSimulator:
    def __init__(self, jobs_data, machines, arrivals):
        self.jobs_data = jobs_data
        self.machines = machines
        self.arrivals = arrivals
        self.job_ids = list(jobs_data.keys())
        
        # Simulation state
        self.current_time = 0
        self.machine_free_time = [0] * len(machines)
        self.job_next_op = [0] * len(self.job_ids)
        self.job_ready_time = [arrivals[job_id] for job_id in self.job_ids]
        self.schedule = {m: [] for m in machines}
        
    def get_current_compact_state(self):
        """Get compact state representation at current simulation time."""
        ready_jobs = []
        idle_machines = []
        
        # Jobs ready now
        for j_idx, job_id in enumerate(self.job_ids):
            op_idx = self.job_next_op[j_idx]
            if (op_idx < len(self.jobs_data[job_id]) and 
                self.job_ready_time[j_idx] <= self.current_time):
                ready_jobs.append((j_idx, op_idx))
        
        # Machines idle now
        for m_idx in range(len(self.machines)):
            if self.machine_free_time[m_idx] <= self.current_time:
                idle_machines.append(m_idx)
                
        return make_compact_state(ready_jobs, idle_machines)
    
    def execute_action(self, action):
        """Execute an action and advance simulation."""
        j_idx, op_idx, m_idx = action
        job_id = self.job_ids[j_idx]
        machine = self.machines[m_idx]
        
        # Get processing time
        proc_time = self.jobs_data[job_id][op_idx]['proc_times'][machine]
        
        # Start time is when both job and machine are ready
        start_time = max(self.job_ready_time[j_idx], self.machine_free_time[m_idx])
        end_time = start_time + proc_time
        
        # Update simulation state
        self.machine_free_time[m_idx] = end_time
        self.job_next_op[j_idx] += 1
        self.job_ready_time[j_idx] = end_time  # ready for next operation
        self.current_time = end_time  # advance simulation time
        ...
        return proc_time  # return processing time as cost

# Simulate PI policy execution
if not incomplete_mdp:
    simulator = EventSimulator(JOBS_DATA, machines, ARRIVALS)
    cur_sid = init_id
    step = 0
    total_reward = 0.0
    
    while True:
        # Get current compact state from simulation
        sim_state = simulator.get_current_compact_state()
        
        # Find matching state ID (this assumes our enumeration is complete)
        if sim_state not in state_to_id:
            print(f"ERROR: simulation state not found in MDP: {pretty_compact_state(sim_state)}")
            break
            
        cur_sid = state_to_id[sim_state]
        a = policy[cur_sid]
        if a is None:
            break
        ...
        proc_time = simulator.execute_action(a)
        total_reward -= proc_time  # negative processing time
        
        # Check if all jobs finished
        if all(simulator.job_next_op[j] >= len(JOBS_DATA[job_ids[j]]) 
               for j in range(len(job_ids))):
            break

    exec_schedule_pi = simulator.schedule
    pi_makespan = max(max(task['end'] for task in tasks) if tasks else 0 
                      for tasks in exec_schedule_pi.values())
    
    print(f"PI execution finished in {step} steps. PI makespan = {pi_makespan:.3f}\n")
else:
    exec_schedule_pi = {m: [] for m in machines}
    pi_makespan = float('inf')
