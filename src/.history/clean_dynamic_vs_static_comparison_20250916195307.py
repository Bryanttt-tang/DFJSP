...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    ...
    while step_count < 1000:
        ...
        # Map action masks to static model's action space
        static_action_space_size = static_model.action_space.n
        if len(action_masks) != static_action_space_size:
            # Create properly sized action masks for static model
            static_action_masks = np.zeros(static_action_space_size, dtype=bool)
            # Enable all actions (since we can't perfectly map)
            static_action_masks[:] = True
        else:
            static_action_masks = action_masks
        
        action, _ = static_model.predict(static_obs, action_masks=static_action_masks, deterministic=True)
