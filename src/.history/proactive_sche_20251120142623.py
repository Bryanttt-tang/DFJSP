...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=1200, verbose=True, solver='gurobi'):
    ...
    Args:
        ...
        solver: 'gurobi' (default, fastest), 'cbc' (open-source), or 'cplex' (commercial)
    ...
    print(f"  Using solver: {solver.upper()}")
    
    # Imports required for pulp
    from pulp import (LpProblem, LpMinimize, LpVariable, lpSum, GUROBI_CMD, PULP_CBC_CMD, CPLEX_CMD, LpStatus)
    import math
    import os
    import itertools
    ...
    # Solve with selected solver (with fallback to CBC if commercial solver unavailable)
    if solver.lower() == 'gurobi':
        try:
            solver_instance = GUROBI_CMD(msg=1 if verbose else 0, timeLimit=time_limit)
            prob.solve(solver_instance)
        except Exception as e:
            if verbose:
                print(f"⚠️  Gurobi not available ({str(e)}), falling back to CBC...")
            solver_instance = PULP_CBC_CMD(msg=1 if verbose else 0, timeLimit=time_limit)
            prob.solve(solver_instance)
    elif solver.lower() == 'cplex':
        try:
            solver_instance = CPLEX_CMD(msg=1 if verbose else 0, timeLimit=time_limit)
            prob.solve(solver_instance)
        except Exception as e:
            if verbose:
                print(f"⚠️  CPLEX not available ({str(e)}), falling back to CBC...")
            solver_instance = PULP_CBC_CMD(msg=1 if verbose else 0, timeLimit=time_limit)
            prob.solve(solver_instance)
    else:  # Default to CBC
        solver_instance = PULP_CBC_CMD(msg=1 if verbose else 0, timeLimit=time_limit)
