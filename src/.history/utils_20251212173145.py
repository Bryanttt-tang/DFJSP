...
def generate_simplified_fjsp_dataset(num_initial_jobs, num_future_jobs, total_num_machines,
                                     ...
                                     due_date_tightness=1.5, seed=None):
    """
    ...
    4. Due dates for tardiness minimization objective
    ...
    due_date_tightness : float
        Controls how tight due dates are relative to total processing time
        Formula: due_date = arrival_time + due_date_tightness * total_processing_time
        - 1.0: Very tight (challenging to meet)
        - 1.5: Moderate (realistic, default)
        - 2.0: Loose (easier to meet)
    seed : int, optional
        Random seed for reproducibility
    ...
    --------
    jobs_data : collections.OrderedDict
        Dictionary mapping job_id to list of operations + due_date
        Format: {job_id: [{'proc_times': {machine: time, ...}}, ...], 'due_date': float}
    machine_list : list
        List of machine names ['M0', 'M1', ..., 'M{total_num_machines-1}']
    ...
    for job_id in range(total_jobs):
        ...
        operations = []
        total_min_proc_time = 0  # Track minimum total processing time for due date
        
        for op_idx in range(num_operations):
            # Sample base processing time with HIGH VARIANCE (uniform distribution)
            base_proc_time = random.uniform(*proc_time_variance_range)
            ...
            proc_times = {}
            min_time_for_op = float('inf')
            for machine in compatible_machines:
                speed_factor = machine_metadata[machine]['speed_factor']
                # Actual time = base_time * speed_factor
                actual_proc_time = float(base_proc_time * speed_factor)
                actual_proc_time = max(1, actual_proc_time)
                proc_times[machine] = actual_proc_time
                min_time_for_op = min(min_time_for_op, actual_proc_time)
            
            operations.append({'proc_times': proc_times})
            total_min_proc_time += min_time_for_op
        
        # Store job with operations and due date
        jobs_data[job_id] = operations
...
def assign_due_dates(jobs_data, arrival_times, due_date_tightness=1.5):
    """
    Assign due dates to jobs based on their arrival times and total processing times.
    ...
    Returns:
    --------
    jobs_with_due_dates : collections.OrderedDict
        Dictionary with due_date added: {job_id: {'operations': [...], 'due_date': float}}
    """
    jobs_with_due_dates = collections.OrderedDict()
    ...
    for job_id, operations in jobs_data.items():
        ...
        total_min_proc_time = sum(
            min(op['proc_times'].values()) for op in operations
        )
        
        # Calculate due date
        arrival_time = arrival_times.get(job_id, 0.0)
        due_date = arrival_time + due_date_tightness * total_min_proc_time
        
        # Store job with operations and due date
        jobs_with_due_dates[job_id] = {
            'operations': operations,
            'due_date': float(due_date)
        }
    
    return jobs_with_due_dates
