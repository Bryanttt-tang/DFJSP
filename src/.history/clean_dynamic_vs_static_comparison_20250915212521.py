...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        IMPROVED RELATIVE TIMING STATE REPRESENTATION for better generalization.
        
        Key insight: Instead of removing ALL timing info, use RELATIVE timing features
        that capture scheduling urgency without being tied to specific arrival times.
        
        This preserves critical scheduling information while enabling generalization!
        """
        obs = []
        
        # 1. MACHINE AVAILABILITY (relative to current scheduling horizon)
        # Normalize by a scheduling window rather than absolute time
        scheduling_window = 20.0  # Fixed scheduling horizon
        for m in self.machines:
            machine_free_time = self.machine_next_free.get(m, 0.0)
            relative_busy_time = max(0.0, machine_free_time - self.current_time)
            normalized_busy = min(1.0, relative_busy_time / scheduling_window)
            obs.append(1.0 - normalized_busy)  # 1.0 = free, 0.0 = very busy
        
        # 2. OPERATION COMPLETION STATUS (structural)
        for job_id in self.job_ids:
            for op_idx in range(self.max_ops_per_job):
                if op_idx < len(self.jobs[job_id]):
                    completed = 1.0 if self.completed_ops[job_id][op_idx] else 0.0
        ...
        # 4. IMPROVED: JOB ARRIVAL URGENCY (relative timing categories)
        # Use urgency categories instead of binary available/not-available
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs:
                obs.append(1.0)  # Available now - highest urgency
                ...
                if time_until_arrival <= 5.0:  # Arriving very soon
                    obs.append(0.0)  # Not available now
                    obs.append(1.0)  # Arriving soon - high urgency
                    obs.append(0.0)  # Not arriving later
                elif time_until_arrival <= 15.0:  # Arriving later
                    obs.append(0.0)  # Not available now
                    obs.append(0.0)  # Not arriving soon
                    obs.append(1.0)  # Arriving later - medium urgency
                else:  # Arriving much later or never
                    obs.append(0.0)  # Not available now
                    obs.append(0.0)  # Not arriving soon
                    obs.append(0.0)  # Not a concern for current decisions
        ...
        # 10. LOAD BALANCING OPPORTUNITY (using relative machine busy times)
        ...
        if len(machine_busy_times) > 1 and max(machine_busy_times) > 0:
            load_std = np.std(machine_busy_times)
            load_mean = np.mean(machine_busy_times)
            if load_mean > 0:
...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="combined_makespan_utilization"):
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="combined_makespan_utilization"):
    """
