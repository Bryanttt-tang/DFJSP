...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _generate_poisson_arrivals(self):
        ...
        for job_id in self.dynamic_job_ids:
            ...
            # BUG FIX: For low arrival rates, ensure reasonable arrival times
            # Don't round to nearest integer - use float values for better precision
    ...
    def _get_observation(self):
        ...
        # 4. DYNAMIC RL: ARRIVAL PATTERN FEATURES - Learn from observation, not direct future access
        # Features to help RL learn arrival patterns without giving direct future knowledge:
        
        # 4a. Time since last job arrival (helps RL learn arrival intervals)
        ...
        obs.append(min(1.0, time_since_last_arrival / 30.0))  # Normalized by 30 time units
        
        # 4b. Number of jobs arrived so far (progress indicator)
        arrival_progress = len(self.arrived_jobs) / len(self.job_ids)
        obs.append(arrival_progress)
        
        # 4c. Current time/makespan (time context for arrival pattern learning)
        normalized_time = min(1.0, self.current_makespan / 100.0)  # Normalize by reasonable time horizon
        obs.append(normalized_time)
        
        # Fill remaining arrival positions with zeros (to match observation space size with other methods)
        remaining_positions = len(self.job_ids) - 3  # 3 features added above
...
class TrainingCallback:
    ...
    def __call__(self, locals_dict, globals_dict):
        global TRAINING_METRICS, METHOD_TRAINING_HISTORY
        ...
        if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
            ...
            # Track timestep-level metrics
            if 'train/entropy_loss' in log_data:
                TRAINING_METRICS['action_entropy'].append(log_data['train/entropy_loss'])
                METHOD_TRAINING_HISTORY[self.method_name]['entropy'].append(log_data['train/entropy_loss'])
            
            if 'train/policy_gradient_loss' in log_data:
                TRAINING_METRICS['policy_loss'].append(log_data['train/policy_gradient_loss'])
                METHOD_TRAINING_HISTORY[self.method_name]['losses'].append(log_data['train/policy_gradient_loss'])
            
            if 'train/value_loss' in log_data:
                TRAINING_METRICS['value_loss'].append(log_data['train/value_loss'])
            
            # NEW: Track additional training metrics
            if 'train/learning_rate' in log_data:
                TRAINING_METRICS['learning_rates'].append(log_data['train/learning_rate'])
            
            if 'train/clip_fraction' in log_data:
                TRAINING_METRICS['clip_fractions'].append(log_data['train/clip_fraction'])
            
            if 'train/explained_variance' in log_data:
                TRAINING_METRICS['explained_variance'].append(log_data['train/explained_variance'])
            
            # Track episode returns (cumulative reward per episode)
            if 'rollout/ep_rew_mean' in log_data:
                episode_return = log_data['rollout/ep_rew_mean']
                TRAINING_METRICS['episode_returns'].append(episode_return)
                TRAINING_METRICS['episode_numbers'].append(len(TRAINING_METRICS['episode_returns']))
                METHOD_TRAINING_HISTORY[self.method_name]['episodes'].append(len(TRAINING_METRICS['episode_returns']))
                METHOD_TRAINING_HISTORY[self.method_name]['returns'].append(episode_return)
                
                # Print periodic updates every 50 episodes
                if len(TRAINING_METRICS['episode_returns']) % 50 == 0:
                    print(f"    {self.method_name} - Episode {len(TRAINING_METRICS['episode_returns'])}: "
                          f"Return = {episode_return:.2f}, "
                          f"Entropy = {TRAINING_METRICS['action_entropy'][-1]:.4f}" if TRAINING_METRICS['action_entropy'] else "")
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # 4. PERFECT KNOWLEDGE: Exact future arrival times for all jobs
        for job_id in self.job_ids:
            if job_id not in self.arrived_jobs:
                # Job not arrived yet: provide exact arrival time information
...
def plot_training_metrics():
    """
    Enhanced training metrics visualization with episode-level tracking.
    Shows loss trends, reward trends, and learning progress across episodes.
    """
    global TRAINING_METRICS, METHOD_TRAINING_HISTORY
    ...
    print(f"Total episodes recorded: {len(TRAINING_METRICS['episode_returns'])}")
    
    # Create comprehensive training visualization
    fig, axes = plt.subplots(3, 3, figsize=(20, 15))
    fig.suptitle('Comprehensive PPO Training Analysis - Loss & Reward Trends by Episode', 
                 fontsize=18, fontweight='bold')
    
    timesteps = TRAINING_METRICS['timesteps']
    
    # Plot 1: Episode Returns (Cumulative Reward) over Episodes
    if TRAINING_METRICS['episode_returns']:
        axes[0, 0].plot(TRAINING_METRICS['episode_numbers'], TRAINING_METRICS['episode_returns'], 
                       'b-', linewidth=2, alpha=0.7)
        
        # Add trend line
        if len(TRAINING_METRICS['episode_returns']) > 10:
            z = np.polyfit(TRAINING_METRICS['episode_numbers'], TRAINING_METRICS['episode_returns'], 1)
            p = np.poly1d(z)
            axes[0, 0].plot(TRAINING_METRICS['episode_numbers'], p(TRAINING_METRICS['episode_numbers']), 
                           "r--", alpha=0.8, linewidth=2, label=f'Trend (slope: {z[0]:.3f})')
            axes[0, 0].legend()
        
        axes[0, 0].set_xlabel('Episode Number')
        axes[0, 0].set_ylabel('Episode Return (Cumulative Reward)')
        axes[0, 0].set_title('Episode Returns Over Time')
        axes[0, 0].grid(True, alpha=0.3)
        
        # Add interpretation
        if len(TRAINING_METRICS['episode_returns']) > 20:
            early_avg = np.mean(TRAINING_METRICS['episode_returns'][:len(TRAINING_METRICS['episode_returns'])//4])
            late_avg = np.mean(TRAINING_METRICS['episode_returns'][-len(TRAINING_METRICS['episode_returns'])//4:])
        ...
        axes[0, 0].text(0.5, 0.5, 'No episode return data', ha='center', va='center', 
                       transform=axes[0, 0].transAxes)
        axes[0, 0].set_title('Episode Returns (No Data)')
    
    # Plot 2: Policy Loss over Training Steps
    if TRAINING_METRICS['policy_loss']:
        axes[0, 1].plot(timesteps[:len(TRAINING_METRICS['policy_loss'])], 
                       TRAINING_METRICS['policy_loss'], 'r-', linewidth=1.5, alpha=0.7)
        
        # Add smoothed trend
        if len(TRAINING_METRICS['policy_loss']) > 50:
            window_size = min(50, len(TRAINING_METRICS['policy_loss'])//10)
            smoothed = np.convolve(TRAINING_METRICS['policy_loss'], 
                                 np.ones(window_size)/window_size, mode='valid')
            axes[0, 1].plot(timesteps[window_size-1:len(smoothed)+window_size-1], smoothed, 
                           'darkred', linewidth=3, alpha=0.8, label='Smoothed Trend')
            axes[0, 1].legend()
        
        axes[0, 1].set_xlabel('Training Steps')
        axes[0, 1].set_ylabel('Policy Loss')
        axes[0, 1].set_title('Policy Gradient Loss Over Training')
        axes[0, 1].grid(True, alpha=0.3)
        
        # Check if loss is decreasing
        if len(TRAINING_METRICS['policy_loss']) > 100:
            recent_trend = np.mean(TRAINING_METRICS['policy_loss'][-50:]) - np.mean(TRAINING_METRICS['policy_loss'][:50])
            if recent_trend < 0:
                axes[0, 1].text(0.02, 0.98, '✅ Loss decreasing', transform=axes[0, 1].transAxes,
                               verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[0, 1].text(0.02, 0.98, '⚠️ Loss not decreasing', transform=axes[0, 1].transAxes,
    ...
    # Plot 3: Value Loss over Training Steps
    if TRAINING_METRICS['value_loss']:
        axes[0, 2].plot(timesteps[:len(TRAINING_METRICS['value_loss'])], 
                       TRAINING_METRICS['value_loss'], 'g-', linewidth=1.5, alpha=0.7)
        
        # Add smoothed trend
        if len(TRAINING_METRICS['value_loss']) > 50:
            window_size = min(50, len(TRAINING_METRICS['value_loss'])//10)
            smoothed = np.convolve(TRAINING_METRICS['value_loss'], 
                                 np.ones(window_size)/window_size, mode='valid')
            axes[0, 2].plot(timesteps[window_size-1:len(smoothed)+window_size-1], smoothed, 
                           'darkgreen', linewidth=3, alpha=0.8, label='Smoothed Trend')
            axes[0, 2].legend()
        
        axes[0, 2].set_xlabel('Training Steps')
        axes[0, 2].set_ylabel('Value Loss')
        axes[0, 2].set_title('Value Function Loss Over Training')
        axes[0, 2].grid(True, alpha=0.3)
    else:
        axes[0, 2].text(0.5, 0.5, 'No value loss data', ha='center', va='center', 
                       transform=axes[0, 2].transAxes)
        axes[0, 2].set_title('Value Loss (No Data)')
    
    # Plot 4: Action Entropy over Training Steps
    if TRAINING_METRICS['action_entropy']:
        axes[1, 0].plot(timesteps[:len(TRAINING_METRICS['action_entropy'])], 
                       TRAINING_METRICS['action_entropy'], 'purple', linewidth=2, alpha=0.7)
        axes[1, 0].set_xlabel('Training Steps')
        axes[1, 0].set_ylabel('Action Entropy')
        axes[1, 0].set_title('Action Entropy (Exploration Level)')
        axes[1, 0].grid(True, alpha=0.3)
        
        # Add interpretation
        final_entropy = TRAINING_METRICS['action_entropy'][-1]
        if final_entropy > 0.5:
            axes[1, 0].text(0.02, 0.98, '✅ High exploration', transform=axes[1, 0].transAxes, 
                           verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
        elif final_entropy > 0.1:
            axes[1, 0].text(0.02, 0.98, '🟡 Moderate exploration', transform=axes[1, 0].transAxes, 
                           verticalalignment='top', bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
        else:
            axes[1, 0].text(0.02, 0.98, '🔴 Low exploration', transform=axes[1, 0].transAxes, 
                           verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        axes[1, 0].text(0.5, 0.5, 'No entropy data', ha='center', va='center', transform=axes[1, 0].transAxes)
        axes[1, 0].set_title('Action Entropy (No Data)')
    
    # Plot 5: Learning Rate over Training
    if TRAINING_METRICS['learning_rates']:
        axes[1, 1].plot(timesteps[:len(TRAINING_METRICS['learning_rates'])], 
                       TRAINING_METRICS['learning_rates'], 'orange', linewidth=2)
    ...
    # Plot 6: Clip Fraction (PPO-specific metric)
    if TRAINING_METRICS['clip_fractions']:
        axes[1, 2].plot(timesteps[:len(TRAINING_METRICS['clip_fractions'])], 
                       TRAINING_METRICS['clip_fractions'], 'brown', linewidth=2)
        axes[1, 2].set_xlabel('Training Steps')
        axes[1, 2].set_ylabel('Clip Fraction')
        axes[1, 2].set_title('PPO Clip Fraction (Policy Updates)')
        axes[1, 2].grid(True, alpha=0.3)
        
        # Add interpretation
        if TRAINING_METRICS['clip_fractions']:
            avg_clip = np.mean(TRAINING_METRICS['clip_fractions'][-100:])
            if avg_clip > 0.3:
                axes[1, 2].text(0.02, 0.98, '⚠️ High clipping (large updates)', 
                               transform=axes[1, 2].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
            elif avg_clip > 0.1:
                axes[1, 2].text(0.02, 0.98, '✅ Moderate clipping', 
                               transform=axes[1, 2].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                axes[1, 2].text(0.02, 0.98, '🔴 Low clipping (small updates)', 
                               transform=axes[1, 2].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        axes[1, 2].text(0.5, 0.5, 'No clip fraction data', ha='center', va='center', 
                       transform=axes[1, 2].transAxes)
        axes[1, 2].set_title('Clip Fraction (No Data)')
    
    # Plot 7: Explained Variance (Value function quality)
    if TRAINING_METRICS['explained_variance']:
        axes[2, 0].plot(timesteps[:len(TRAINING_METRICS['explained_variance'])], 
                       TRAINING_METRICS['explained_variance'], 'teal', linewidth=2)
        axes[2, 0].set_xlabel('Training Steps')
        axes[2, 0].set_ylabel('Explained Variance')
        axes[2, 0].set_title('Value Function Explained Variance')
        axes[2, 0].grid(True, alpha=0.3)
        
        # Add interpretation
        if TRAINING_METRICS['explained_variance']:
            final_ev = TRAINING_METRICS['explained_variance'][-1]
            if final_ev > 0.8:
                axes[2, 0].text(0.02, 0.98, '✅ Good value function', 
                               transform=axes[2, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            elif final_ev > 0.5:
                axes[2, 0].text(0.02, 0.98, '🟡 Moderate value function', 
                               transform=axes[2, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
            else:
                axes[2, 0].text(0.02, 0.98, '🔴 Poor value function', 
                               transform=axes[2, 0].transAxes, verticalalignment='top',
                               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        axes[2, 0].text(0.5, 0.5, 'No explained variance data', ha='center', va='center', 
                       transform=axes[2, 0].transAxes)
        axes[2, 0].set_title('Explained Variance (No Data)')
    
    # Plot 8: Combined Losses with Episode Markers
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['value_loss']:
        axes[2, 1].set_title('Combined Training Losses')
        min_len = min(len(TRAINING_METRICS['policy_loss']), len(TRAINING_METRICS['value_loss']))
        axes[2, 1].plot(timesteps[:min_len], TRAINING_METRICS['policy_loss'][:min_len], 
                       'r-', linewidth=2, label='Policy Loss', alpha=0.7)
        axes[2, 1].plot(timesteps[:min_len], TRAINING_METRICS['value_loss'][:min_len], 
                       'g-', linewidth=2, label='Value Loss', alpha=0.7)
        axes[2, 1].legend()
        axes[2, 1].set_xlabel('Training Steps')
        axes[2, 1].set_ylabel('Loss')
        axes[2, 1].grid(True, alpha=0.3)
    else:
        axes[2, 1].text(0.5, 0.5, 'Insufficient loss data', ha='center', va='center', 
                       transform=axes[2, 1].transAxes)
        axes[2, 1].set_title('Combined Losses (No Data)')
    
    # Plot 9: Episode Length Distribution (if available)
    if TRAINING_METRICS['episode_lengths']:
        axes[2, 2].hist(TRAINING_METRICS['episode_lengths'], bins=30, alpha=0.7, edgecolor='black')
        axes[2, 2].set_xlabel('Episode Length (Steps)')
        axes[2, 2].set_ylabel('Frequency')
        axes[2, 2].set_title('Episode Length Distribution')
        axes[2, 2].grid(True, alpha=0.3)
        
        avg_length = np.mean(TRAINING_METRICS['episode_lengths'])
        axes[2, 2].axvline(avg_length, color='red', linestyle='--', linewidth=2, 
                          label=f'Mean: {avg_length:.1f}')
        axes[2, 2].legend()
    else:
        # Show episode return moving average instead
        if TRAINING_METRICS['episode_returns'] and len(TRAINING_METRICS['episode_returns']) > 10:
            window = min(20, len(TRAINING_METRICS['episode_returns'])//5)
            moving_avg = np.convolve(TRAINING_METRICS['episode_returns'], 
                                   np.ones(window)/window, mode='valid')
            axes[2, 2].plot(range(window-1, len(moving_avg)+window-1), moving_avg, 
                           'b-', linewidth=3, label=f'Moving Average (window={window})')
            axes[2, 2].plot(TRAINING_METRICS['episode_numbers'], TRAINING_METRICS['episode_returns'], 
                           'lightblue', alpha=0.5, linewidth=1, label='Raw Returns')
            axes[2, 2].set_xlabel('Episode Number')
            axes[2, 2].set_ylabel('Episode Return')
            axes[2, 2].set_title('Episode Return Moving Average')
            axes[2, 2].legend()
            axes[2, 2].grid(True, alpha=0.3)
        else:
            axes[2, 2].text(0.5, 0.5, 'No episode data available', ha='center', va='center', 
                           transform=axes[2, 2].transAxes)
            axes[2, 2].set_title('Episode Analysis (No Data)')
    
    plt.tight_layout()
    plt.savefig('comprehensive_training_analysis.png', dpi=300, bbox_inches='tight')
    print("✅ Comprehensive training analysis plot saved: comprehensive_training_analysis.png")
    plt.show()
    ...
    if TRAINING_METRICS['episode_returns']:
        returns = TRAINING_METRICS['episode_returns']
        print(f"Episode Returns:")
        print(f"  Episodes: {len(returns)}")
        print(f"  Initial (first 10%): {np.mean(returns[:max(1, len(returns)//10)]):.2f}")
        print(f"  Final (last 10%): {np.mean(returns[-max(1, len(returns)//10):]):.2f}")
        print(f"  Overall mean: {np.mean(returns):.2f} ± {np.std(returns):.2f}")
        print(f"  Best episode: {np.max(returns):.2f}")
        print(f"  Worst episode: {np.min(returns):.2f}")
        
        # Calculate learning trend
        if len(returns) > 20:
            early_returns = np.mean(returns[:len(returns)//4])
            late_returns = np.mean(returns[-len(returns)//4:])
            improvement = late_returns - early_returns
            improvement_pct = (improvement / abs(early_returns)) * 100 if early_returns != 0 else 0
            
            print(f"  Learning trend: {improvement:+.2f} ({improvement_pct:+.1f}%)")
            if improvement > 0:
                print("  ✅ Agent is learning (returns improving)")
            else:
                print("  ⚠️ Agent may not be learning effectively (returns declining)")
    
    if TRAINING_METRICS['policy_loss']:
        policy_losses = TRAINING_METRICS['policy_loss']
        print(f"\nPolicy Loss:")
        print(f"  Updates: {len(policy_losses)}")
        print(f"  Initial: {np.mean(policy_losses[:max(1, len(policy_losses)//10)]):.6f}")
        print(f"  Final: {np.mean(policy_losses[-max(1, len(policy_losses)//10):]):.6f}")
        print(f"  Mean: {np.mean(policy_losses):.6f} ± {np.std(policy_losses):.6f}")
        
        # Check convergence
        if len(policy_losses) > 100:
            recent_trend = np.mean(policy_losses[-50:]) - np.mean(policy_losses[-100:-50])
            if abs(recent_trend) < np.std(policy_losses) * 0.1:
                print("  ✅ Loss appears to be converging")
            elif recent_trend < 0:
                print("  📈 Loss still decreasing (good)")
            else:
                print("  ⚠️ Loss increasing (may indicate instability)")
    ...
    if TRAINING_METRICS['action_entropy']:
        ...
        print(f"  Mean: {np.mean(entropy_data):.4f} ± {np.std(entropy_data):.4f}")
        ...
        if len(entropy_data) > 10:
            ...
            if entropy_change < -50:
                ...
                print("  🟡 CAUTION: Entropy decreased - normal but monitor exploration vs exploitation")
            else:
                print("  ✅ Entropy maintained reasonably well")
    
    print("=" * 60)

def plot_method_comparison_training():
    """
    Plot training progress comparison across all three methods.
    Shows which method learned most effectively.
    """
    global METHOD_TRAINING_HISTORY
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Training Progress Comparison: Perfect Knowledge vs Dynamic vs Static RL', 
                 fontsize=16, fontweight='bold')
    
    colors = {'Perfect Knowledge RL': 'blue', 'Dynamic RL': 'green', 'Static RL': 'red'}
    
    # Plot 1: Episode Returns Comparison
    axes[0, 0].set_title('Episode Returns Learning Curves')
    for method, data in METHOD_TRAINING_HISTORY.items():
        if data['episodes'] and data['returns']:
            axes[0, 0].plot(data['episodes'], data['returns'], 
                           color=colors[method], linewidth=2, alpha=0.7, label=method)
    ...
    for method, data in METHOD_TRAINING_HISTORY.items():
        if data['losses']:
            axes[0, 1].plot(range(len(data['losses'])), data['losses'], 
                           color=colors[method], linewidth=2, alpha=0.7, label=method)
    ...
    axes[1, 0].set_title('Action Entropy Comparison')
    for method, data in METHOD_TRAINING_HISTORY.items():
        if data['entropy']:
            axes[1, 0].plot(range(len(data['entropy'])), data['entropy'], 
                           color=colors[method], linewidth=2, alpha=0.7, label=method)
    ...
    # Plot 4: Learning Efficiency (Return improvement rate)
    axes[1, 1].set_title('Learning Efficiency Comparison')
    for method, data in METHOD_TRAINING_HISTORY.items():
        if len(data['returns']) > 20:
            # Calculate moving average improvement
            window = max(10, len(data['returns'])//10)
            returns = np.array(data['returns'])
            
            # Calculate episode-to-episode improvement
            improvements = []
            for i in range(window, len(returns)):
                recent_avg = np.mean(returns[i-window:i])
                prev_avg = np.mean(returns[max(0, i-2*window):i-window])
                if prev_avg != 0:
                    improvement = (recent_avg - prev_avg) / abs(prev_avg) * 100
                    improvements.append(improvement)
            
            if improvements:
                axes[1, 1].plot(range(window, window + len(improvements)), improvements,
                               color=colors[method], linewidth=2, alpha=0.7, label=method)
    
    axes[1, 1].set_xlabel('Episode Number')
    axes[1, 1].set_ylabel('Return Improvement Rate (%)')
    axes[1, 1].legend()
    axes[1, 1].grid(True, alpha=0.3)
    axes[1, 1].axhline(y=0, color='black', linestyle='--', alpha=0.5)
    
    plt.tight_layout()
    plt.savefig('method_training_comparison.png', dpi=300, bbox_inches='tight')
    print("✅ Method training comparison plot saved: method_training_comparison.png")
    plt.show()
...
def main():
    ...
    plot_training_metrics()      # NEW: Show comprehensive training analysis
    plot_method_comparison_training()  # NEW: Compare training across methods
    ...
    import os
    folder_name = "small_instances_0.1rate"
