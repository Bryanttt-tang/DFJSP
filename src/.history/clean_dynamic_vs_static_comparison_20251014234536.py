...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 60000    # Perfect knowledge needs less training
    dynamic_timesteps = 300000   # Increased for better learning with integer timing  
    static_timesteps = 60000    # Increased for better learning
    ...
    print("\n" + "="*60)
    print("IMMEDIATE DYNAMIC RL EVALUATION ON TEST SCENARIOS")
    print("="*60)
    print("Testing trained Dynamic RL agent on all 5 test scenarios...")
    
    # Create immediate evaluation plots
    fig, axes = plt.subplots(len(test_scenarios), 1, figsize=(16, len(test_scenarios) * 3))
    if len(test_scenarios) == 1:
        axes = [axes]  # Make it a list for consistency
    
    fig.suptitle('Dynamic RL Performance on All Test Scenarios (Immediately After Training)', 
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        print(f"\nScenario {i+1}: {scenario_arrivals}")
        ...
        print(f"  Dynamic RL Makespan: {dynamic_makespan:.2f}")
        
        # Plot this scenario
        ax = axes[i]
        
        if not dynamic_schedule or all(len(ops) == 0 for ops in dynamic_schedule.values()):
            ax.text(0.5, 0.5, 'No valid schedule', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=12)
            ax.set_title(f"Scenario {i+1} - No Solution")
            continue
        
        # Calculate x-axis limit for this scenario
        max_time = 0
        if dynamic_schedule and any(len(ops) > 0 for ops in dynamic_schedule.values()):
            max_time = max([max([op[2] for op in ops]) for ops in dynamic_schedule.values() if ops])
        x_limit = max_time * 1.1 if max_time > 0 else 50
        
        # Plot operations for each machine
        for idx, machine in enumerate(MACHINE_LIST):
            machine_ops = dynamic_schedule.get(machine, [])
        ...
        for job_id, arrival_time in scenario_arrivals.items():
            if arrival_time > 0 and arrival_time < x_limit:  # Only show arrows for jobs that arrive after t=0
        ...
        ax.set_xlabel("Time" if i == len(test_scenarios)-1 else "")
        ax.set_ylabel("Machines")
        ax.set_title(f"Scenario {i+1}: Arrivals {scenario_arrivals} → Makespan: {dynamic_makespan:.2f}", 
                    fontweight='bold')
        ax.grid(True, alpha=0.3)
        ax.set_xlim(0, x_limit)
        ax.set_ylim(-0.5, len(MACHINE_LIST) + 1.0)
    ...
    # Save the immediate evaluation plot
    immediate_filename = f'dynamic_rl_immediate_evaluation_scenarios.png'
    plt.savefig(immediate_filename, dpi=300, bbox_inches='tight')
    print(f"\n✅ Saved Dynamic RL immediate evaluation: {immediate_filename}")
    plt.show()
    
    # Print summary of immediate results
    print(f"\nDynamic RL Immediate Evaluation Summary:")
    print("-" * 50)
    makespans = [r['makespan'] for r in dynamic_immediate_results if r['makespan'] != float('inf')]
    if makespans:
        avg_makespan = np.mean(makespans)
        std_makespan = np.std(makespans)
        min_makespan = np.min(makespans)
        max_makespan = np.max(makespans)
        print(f"Average Makespan: {avg_makespan:.2f} ± {std_makespan:.2f}")
        print(f"Best Performance: {min_makespan:.2f} (Scenario {[r['scenario'] for r in dynamic_immediate_results if r['makespan'] == min_makespan][0]})")
        print(f"Worst Performance: {max_makespan:.2f} (Scenario {[r['scenario'] for r in dynamic_immediate_results if r['makespan'] == max_makespan][0]})")
        
        # Show per-scenario results
        for result in dynamic_immediate_results:
            print(f"  Scenario {result['scenario']}: {result['makespan']:.2f}")
    else:
        print("❌ No valid results from Dynamic RL!")
    
    print("\nContinuing with remaining training and full comparison...")
    print("="*60)
