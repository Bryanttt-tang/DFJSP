...
def main():
    ...
    dynamic_model = train_dynamic_agent(ENHANCED_JOBS_DATA, MACHINE_LIST, 
                                       initial_jobs=[0, 1, 2], arrival_rate=arrival_rate, total_timesteps=100000)
    ...
    # Filter out jobs that arrive too late (beyond reasonable time horizon)
    filtered_arrivals = {}
    for job_id, arr_time in first_scenario_arrivals.items():
        if arr_time <= 100:  # Only include jobs that arrive within 100 time units
            filtered_arrivals[job_id] = arr_time
        else:
            print(f"  Excluding Job {job_id} (arrives at {arr_time:.2f}, too late)")
    
    print(f"Filtered arrival times for all methods: {filtered_arrivals}")
    ...
    static_makespan, static_schedule = evaluate_static_on_dynamic(
        static_model, ENHANCED_JOBS_DATA, MACHINE_LIST, filtered_arrivals)
    ...
    dynamic_makespan, dynamic_schedule = evaluate_dynamic_on_dynamic(
        dynamic_model, ENHANCED_JOBS_DATA, MACHINE_LIST, filtered_arrivals)
    
    # SPT Heuristic on dynamic scenario (first scenario only)
    print("Evaluating SPT Heuristic on first test scenario...")
    spt_makespan, spt_schedule = spt_heuristic_poisson(ENHANCED_JOBS_DATA, MACHINE_LIST, filtered_arrivals)
    ...
    colors = plt.cm.tab20.colors
    schedules_data = [
        {'schedule': dynamic_schedule, 'makespan': dynamic_makespan, 'title': 'Dynamic RL', 'arrival_times': filtered_arrivals},
        {'schedule': static_schedule, 'makespan': static_makespan, 'title': 'Static RL', 'arrival_times': filtered_arrivals},
        {'schedule': spt_schedule, 'makespan': spt_makespan, 'title': 'SPT Heuristic', 'arrival_times': filtered_arrivals}
    ]
