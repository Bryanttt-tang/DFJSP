...
class EnhancedTrainingCallback(BaseCallback):
    """Enhanced BaseCallback to track comprehensive training metrics."""
    ...
    def _on_training_start(self) -> None:
        ...
        print(f"[{self.method_name}] Training started, metrics initialized")
        
    def _on_rollout_start(self) -> None:
        """Called at the start of each rollout."""
        if self.verbose >= 1:
            print(f"[{self.method_name}] Rollout start at timestep {self.model.num_timesteps}")
    
    def _on_rollout_end(self) -> None:
        """Called at the end of each rollout - when episode statistics are available."""
        self._capture_metrics("rollout_end")
        
    def _on_step(self) -> bool:
        """Called after each step - check for new metrics."""
        # Only check every 1000 steps to avoid spam but catch metrics
        if self.model.num_timesteps % 1000 == 0 and self.model.num_timesteps > self.last_logged_timestep:
            self._capture_metrics("step")
            self.last_logged_timestep = self.model.num_timesteps
    ...
    def _capture_metrics(self, source):
        """Capture metrics from logger if available."""
        global TRAINING_METRICS
        ...
        if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
            ...
            # DEBUG: Print all available keys occasionally
            if self.verbose >= 1 and source == "rollout_end":
                available_keys = list(log_data.keys())
                print(f"[{self.method_name}] Logger keys at {source}: {available_keys}")
            
            # Episode-level metrics
            if 'rollout/ep_rew_mean' in log_data:
                ep_reward = log_data['rollout/ep_rew_mean']
                # Avoid duplicates - only add if it's a new value
                if not TRAINING_METRICS['episode_rewards'] or ep_reward != TRAINING_METRICS['episode_rewards'][-1]:
                    TRAINING_METRICS['episode_rewards'].append(float(ep_reward))
                    self.episode_count += 1
                    TRAINING_METRICS['episode_count'].append(self.episode_count)
                    if self.verbose >= 1:
                        print(f"[{self.method_name}] ✅ Episode reward captured: {ep_reward:.4f} (count: {self.episode_count}) from {source}")
            
            if 'rollout/ep_len_mean' in log_data:
                ep_len = log_data['rollout/ep_len_mean']
                if not TRAINING_METRICS['episode_lengths'] or ep_len != TRAINING_METRICS['episode_lengths'][-1]:
                    TRAINING_METRICS['episode_lengths'].append(float(ep_len))
            
            # Training metrics
            if 'train/entropy_loss' in log_data:
                entropy = log_data['train/entropy_loss']
                if not TRAINING_METRICS['action_entropy'] or entropy != TRAINING_METRICS['action_entropy'][-1]:
                    TRAINING_METRICS['action_entropy'].append(float(entropy))
            
            if 'train/policy_gradient_loss' in log_data:
                policy_loss = log_data['train/policy_gradient_loss']
                if not TRAINING_METRICS['policy_loss'] or policy_loss != TRAINING_METRICS['policy_loss'][-1]:
                    TRAINING_METRICS['policy_loss'].append(float(policy_loss))
                    if self.verbose >= 1 and source == "rollout_end":
                        print(f"[{self.method_name}] Policy loss: {policy_loss:.6f}")
                
            if 'train/value_loss' in log_data:
                value_loss = log_data['train/value_loss']
                if not TRAINING_METRICS['value_loss'] or value_loss != TRAINING_METRICS['value_loss'][-1]:
                    TRAINING_METRICS['value_loss'].append(float(value_loss))
                    if self.verbose >= 1 and source == "rollout_end":
                        print(f"[{self.method_name}] Value loss: {value_loss:.6f}")
            
            if 'train/learning_rate' in log_data:
                lr = log_data['train/learning_rate']
                if not TRAINING_METRICS['learning_rate'] or lr != TRAINING_METRICS['learning_rate'][-1]:
                    TRAINING_METRICS['learning_rate'].append(float(lr))
            
            if 'train/explained_variance' in log_data:
                ev = log_data['train/explained_variance']
                if not TRAINING_METRICS['explained_variance'] or ev != TRAINING_METRICS['explained_variance'][-1]:
                    TRAINING_METRICS['explained_variance'].append(float(ev))
            
            # Always record timesteps for this capture
            current_timesteps = self.model.num_timesteps
            if not TRAINING_METRICS['timesteps'] or current_timesteps != TRAINING_METRICS['timesteps'][-1]:
                TRAINING_METRICS['timesteps'].append(current_timesteps)
        else:
            if self.verbose >= 1:
                print(f"[{self.method_name}] ❌ No logger or name_to_value available at {source}")
...
def plot_training_metrics():
    ...
    print(f"Method: {TRAINING_METRICS.get('method_name', 'Unknown')}")
    ...
    if not TRAINING_METRICS['timesteps']:
        ...
        print("This suggests the callback is not working properly.")
        return
    ...
    if TRAINING_METRICS['episode_rewards']:
        ...
        print(f"✅ Episode rewards captured successfully!")
        print(f"  Count: {len(rewards)}")
        print(f"  Range: {min(rewards):.4f} to {max(rewards):.4f}")
        print(f"  Last 5 values: {rewards[-5:]}")
    else:
        print("❌ NO EPISODE REWARDS RECORDED!")
        return
    
    # Don't try to plot if we don't have enough data
    if len(TRAINING_METRICS['episode_rewards']) < 2:
        print("❌ Not enough data points to plot (need at least 2)")
        return
    
    print("✅ Sufficient data available, creating plots...")
    
    # Create comprehensive figure with subplots
    import matplotlib.pyplot as plt
    fig, axes = plt.subplots(3, 2, figsize=(20, 15))
    fig.suptitle(f'Training Analysis - {TRAINING_METRICS.get("method_name", "RL Agent")}', 
    ...
    ep_timesteps = timesteps[:len(ep_rewards)]  # Match lengths
    
    print(f"Plotting {len(ep_rewards)} episode rewards against {len(ep_timesteps)} timesteps")
    
    axes[0, 0].plot(ep_timesteps, ep_rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward')
    
    # Add moving average for trend if enough data
    if len(ep_rewards) > 10:
        window_size = min(10, len(ep_rewards) // 3)
        moving_avg = []
        for i in range(len(ep_rewards)):
            start_idx = max(0, i - window_size // 2)
            end_idx = min(len(ep_rewards), i + window_size // 2 + 1)
            moving_avg.append(sum(ep_rewards[start_idx:end_idx]) / (end_idx - start_idx))
        
        axes[0, 0].plot(ep_timesteps, moving_avg, 'r--', linewidth=2, alpha=0.9, label=f'Moving Avg (window={window_size})')
    ...
    # Plot 2: Policy and Value Loss (Top Right)
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['value_loss']:
        loss_timesteps = timesteps[:len(TRAINING_METRICS['policy_loss'])]
        axes[0, 1].plot(loss_timesteps, TRAINING_METRICS['policy_loss'], 'b-', linewidth=2, label='Policy Loss', alpha=0.8)
        
        # Value loss on secondary y-axis
        ax2 = axes[0, 1].twinx()
        value_loss_timesteps = timesteps[:len(TRAINING_METRICS['value_loss'])]
        ax2.plot(value_loss_timesteps, TRAINING_METRICS['value_loss'], 'r-', linewidth=2, label='Value Loss', alpha=0.8)
        
        axes[0, 1].set_xlabel('Timesteps')
        axes[0, 1].set_ylabel('Policy Loss', color='b')
    ...
    # Plot 3: Episode Length (Middle Left)
    if TRAINING_METRICS['episode_lengths']:
        len_timesteps = timesteps[:len(TRAINING_METRICS['episode_lengths'])]
        axes[1, 0].plot(len_timesteps, TRAINING_METRICS['episode_lengths'], 'purple', linewidth=2, alpha=0.8)
    ...
    # Plot 4: Learning Rate (Middle Right)
    if TRAINING_METRICS['learning_rate']:
        lr_timesteps = timesteps[:len(TRAINING_METRICS['learning_rate'])]
        axes[1, 1].plot(lr_timesteps, TRAINING_METRICS['learning_rate'], 'orange', linewidth=2, alpha=0.8)
        axes[1, 1].set_xlabel('Timesteps')
        axes[1, 1].set_ylabel('Learning Rate')
        axes[1, 1].set_title('Learning Rate Schedule')
        axes[1, 1].grid(True, alpha=0.3)
    else:
        axes[1, 1].text(0.5, 0.5, 'No Learning Rate Data', ha='center', va='center', transform=axes[1, 1].transAxes)
        axes[1, 1].set_title('Learning Rate (No Data)')
    
    # Plot 5: Action Entropy (Bottom Left)
    if TRAINING_METRICS['action_entropy']:
        entropy_timesteps = timesteps[:len(TRAINING_METRICS['action_entropy'])]
        axes[2, 0].plot(entropy_timesteps, TRAINING_METRICS['action_entropy'], 'brown', linewidth=2, alpha=0.8)
        axes[2, 0].set_xlabel('Timesteps')
        axes[2, 0].set_ylabel('Action Entropy')
        axes[2, 0].set_title('Action Entropy (Exploration)')
        axes[2, 0].grid(True, alpha=0.3)
    else:
        axes[2, 0].text(0.5, 0.5, 'No Entropy Data', ha='center', va='center', transform=axes[2, 0].transAxes)
        axes[2, 0].set_title('Action Entropy (No Data)')
    
    # Plot 6: Explained Variance (Bottom Right)  
    if TRAINING_METRICS['explained_variance']:
        ev_timesteps = timesteps[:len(TRAINING_METRICS['explained_variance'])]
        axes[2, 1].plot(ev_timesteps, TRAINING_METRICS['explained_variance'], 'cyan', linewidth=2, alpha=0.8)
        axes[2, 1].set_xlabel('Timesteps')
        axes[2, 1].set_ylabel('Explained Variance')
        axes[2, 1].set_title('Value Function Explained Variance')
        axes[2, 1].grid(True, alpha=0.3)
    else:
        axes[2, 1].text(0.5, 0.5, 'No Explained Variance Data', ha='center', va='center', transform=axes[2, 1].transAxes)
        axes[2, 1].set_title('Explained Variance (No Data)')
    
    plt.tight_layout()
    
    # Save the plot
    plot_filename = f"enhanced_ppo_training_metrics_{TRAINING_METRICS.get('method_name', 'rl_agent').lower().replace(' ', '_')}.png"
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
    print(f"✅ Training metrics plot saved as: {plot_filename}")
...
def main():
    ...
    # Store dynamic RL metrics
    dynamic_rl_metrics = TRAINING_METRICS.copy()
    ...
    # Store static RL metrics
    static_rl_metrics = TRAINING_METRICS.copy()
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        # Store dynamic RL metrics before training perfect knowledge agents
        if i == 0:
            dynamic_rl_metrics = TRAINING_METRICS.copy()
            print("\n--- Dynamic RL Training Metrics (Main Training) ---")
            plot_training_metrics()
        
        # Reset metrics only for perfect RL training
        reset_training_metrics()
        perfect_model = train_perfect_knowledge_agent(
            ENHANCED_JOBS_DATA, MACHINE_LIST,
            arrival_times=scenario_arrivals,
