...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=300000, reward_mode="makespan_increment"):
    """
    Train a dynamic RL agent on Poisson job arrivals - simplified based on successful static/perfect knowledge patterns.
    """
    print(f"\n--- Training Dynamic RL Agent on {len(jobs_data)} jobs ---")
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    
    def make_dynamic_env():
        # Simplified approach - no complex randomization during training
    ...
    # Simplified hyperparameters matching successful static RL approach
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=1e-4,            # Match static RL
        n_steps=4096,                  # Match static RL
        batch_size=512,                # Match static RL
        n_epochs=10,                   # Match static RL
        gamma=1,                    # Match static RL
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,
        vf_coef=0.5,
        max_grad_norm=0.5,
        policy_kwargs=dict(
            net_arch=[256, 128, 64],  # Simpler network like static RL
            activation_fn=torch.nn.ReLU
        )
    )
    ...
    # Simplified training approach matching static RL
    start_time = time.time()
