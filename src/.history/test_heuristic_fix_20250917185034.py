...
def test_heuristics():
    ...
    try:
        ...
        # Test optimized SPT
        print("\n3. Testing Optimized SPT Scheduler:")
        opt_spt_makespan, opt_spt_schedule = optimized_spt_scheduler(ENHANCED_JOBS_DATA, MACHINE_LIST, DETERMINISTIC_ARRIVAL_TIMES)
        print(f"   Optimized SPT makespan: {opt_spt_makespan:.2f}")
        
        # Test earliest completion
        print("\n4. Testing Earliest Completion Scheduler:")
        ec_makespan, ec_schedule = earliest_completion_scheduler(ENHANCED_JOBS_DATA, MACHINE_LIST, DETERMINISTIC_ARRIVAL_TIMES)
        print(f"   Earliest Completion makespan: {ec_makespan:.2f}")
        
        # Find the best heuristic
        results = [
            (spt_makespan, spt_schedule, "Simple SPT"),
        ...
        best_makespan, best_schedule, best_name = min(results, key=lambda x: x[0])
        
        print(f"\nResults Summary:")
        print(f"Simple SPT: {spt_makespan:.2f}")
        print(f"Basic Greedy: {greedy_makespan:.2f}")
        print(f"Optimized SPT: {opt_spt_makespan:.2f}")
        print(f"Earliest Completion: {ec_makespan:.2f}")
