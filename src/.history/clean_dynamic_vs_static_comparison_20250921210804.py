...
class StaticFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        Generate observation for event-based scheduling system.
        
        Key insight: In event-based systems, relative timing is always zero since 
        decisions are only made when machines are available. Focus on:
        1. Normalized machine free times (absolute, not relative)
        2. Processing times of next available operations
        3. Job progress and availability status
        4. Current system time (normalized)
        ...
        for machine in self.machines:
            ...
            normalized_free_time = min(1.0, machine_free_time / time_scale)
            obs.append(normalized_free_time)
        
        # 2. Next operation progress for each job (what operation they're on)
        for job_id in self.job_ids:
            next_op = self.next_operation[job_id]
            total_ops = len(self.jobs[job_id])
            if total_ops > 0:
                progress = float(next_op) / float(total_ops)  # 0 = not started, 1 = completed
            else:
                progress = 1.0  # Job has no operations
            obs.append(min(1.0, progress))
        
        # 3. Processing times of next available operations (key for scheduling decisions)
        for job_id in self.job_ids:
            next_op_idx = self.next_operation[job_id]
            
            if next_op_idx < len(self.jobs[job_id]) and job_id in self.arrived_jobs:
                # Job has next operation available - get min processing time across machines
                operation = self.jobs[job_id][next_op_idx]
                available_proc_times = [pt for machine, pt in operation['proc_times'].items() 
                                      if machine in self.machines]
                if available_proc_times:
                    min_proc_time = min(available_proc_times)
                    normalized_proc_time = min(1.0, min_proc_time / max(all_proc_times))
                    obs.append(normalized_proc_time)
                else:
                    obs.append(0.0)  # No compatible machines
            else:
                obs.append(0.0)  # Job completed or not arrived
        
        # 4. Current system time (normalized) - important for arrival timing
        normalized_time = min(1.0, current_time / time_scale)
        obs.append(normalized_time)
        
        # 5. Job arrival status (binary: arrived=1, not_arrived=0)
        for job_id in self.job_ids:
            arrival_status = 1.0 if job_id in self.arrived_jobs else 0.0
            obs.append(arrival_status)
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        3. Job progress and availability status
        4. Current system time (normalized)
        5. Job arrival status (dynamic: binary arrived/not-arrived)
        """
        obs = []
        ...
        # It should only know which jobs have already arrived
        for job_id in self.job_ids:
            arrival_status = 1.0 if job_id in self.arrived_jobs else 0.0
            obs.append(arrival_status)
...
def evaluate_perfect_knowledge_on_scenario(model, jobs_data, machine_list, arrival_times):
    """Evaluate Perfect Knowledge RL on a specific scenario."""
    env = PerfectKnowledgeFJSPEnv(jobs_data, machine_list, arrival_times, reward_mode="makespan_increment")
    ...
    while not done and steps < max_steps:
        ...
        for job_id in jobs_data.keys():
            if env.env.next_operation[job_id] > 0:
                scheduled_jobs.add(job_id)
        
        steps += 1
    
    makespan = env.env.current_makespan
    schedule = env.env.schedule
...
def evaluate_dynamic_on_dynamic(model, jobs_data, machine_list, arrival_times):
    """Evaluate Dynamic RL on dynamic scenario with specific arrival times."""
    print(f"  Dynamic RL using arrival times: {arrival_times}")
    
    # Create dynamic environment with these specific arrival times
    env = PoissonDynamicFJSPEnv(jobs_data, machine_list, 
                               initial_jobs=[0, 1, 2], 
                               arrival_rate=0.2,  # Rate doesn't matter since we override arrivals
                               reward_mode="makespan_increment")
    
    # Override the arrival times with our test scenario
    env.job_arrival_times = arrival_times.copy()
    env.arrived_jobs = {job_id for job_id, arr_time in arrival_times.items() if arr_time <= 0}
    
    print(f"  Dynamic RL environment arrival times: {env.job_arrival_times}")
    print(f"  Dynamic RL initial jobs: {env.arrived_jobs}")
    
    env = ActionMasker(env, mask_fn)
    
    obs, _ = env.reset()
    # Re-override after reset (in case reset regenerates arrivals)
    env.env.job_arrival_times = arrival_times.copy()
    env.env.arrived_jobs = {job_id for job_id, arr_time in arrival_times.items() if arr_time <= 0}
    ...
    print(f"  Dynamic RL scheduled jobs: {sorted(scheduled_jobs)} ({env.env.operations_scheduled}/{total_ops} ops)")
    
    return makespan, schedule


def evaluate_static_on_dynamic(model, jobs_data, machine_list, arrival_times):
    """Evaluate Static RL on dynamic scenario (jobs arrive over time but agent doesn't know)."""
    print(f"  Static RL evaluation on dynamic scenario (arrival times: {arrival_times})...")
    
    # Use PoissonDynamicFJSPEnv but with static RL agent
    env = PoissonDynamicFJSPEnv(jobs_data, machine_list, 
                               initial_jobs=[0, 1, 2], 
                               arrival_rate=0.2,
                               reward_mode="makespan_increment")
    
    # Override arrival times
    env.job_arrival_times = arrival_times.copy()
    env.arrived_jobs = {job_id for job_id, arr_time in arrival_times.items() if arr_time <= 0}
...
def evaluate_static_on_static(model, jobs_data, machine_list):
    """Evaluate Static RL on static scenario (all jobs at t=0)."""
    ...
    while not done and steps < max_steps:
        ...
        # Track scheduled jobs
        for job_id in jobs_data.keys():
            if env.next_operation[job_id] > 0:
                scheduled_jobs.add(job_id)
...
def verify_schedule_correctness(schedule, jobs_data, arrival_times, method_name):
    """Verify that a schedule is valid and calculate true makespan."""
    try:
        if not schedule or all(len(ops) == 0 for ops in schedule.values()):
            return False, float('inf')
        
        # Extract all operations from schedule
        all_operations = []
        ...
        for machine in schedule.keys():
            machine_ops = schedule[machine]
            machine_ops_sorted = sorted(machine_ops, key=lambda x: x[1])  # Sort by start time
            
            for i in range(len(machine_ops_sorted) - 1):
                curr_end = machine_ops_sorted[i][2]
                next_start = machine_ops_sorted[i + 1][1]
                if curr_end > next_start:
                    return False, float('inf')
        
        # Calculate makespan
        max_end_time = max([max([op[2] for op in ops]) for ops in schedule.values() if ops])
...
def schedules_identical(schedule1, schedule2):
    ...
    try:
        ...
        # Compare number of operations per machine
        for machine in schedule1.keys():
            if machine not in schedule2:
                return False
            if len(schedule1[machine]) != len(schedule2[machine]):
                return False
        
        # Compare actual operations (allowing for small floating point differences)
        for machine in schedule1.keys():
            ops1 = sorted(schedule1[machine], key=lambda x: (x[0], x[1]))
            ops2 = sorted(schedule2[machine], key=lambda x: (x[0], x[1]))
            
            for op1, op2 in zip(ops1, ops2):
                if len(op1) < 3 or len(op2) < 3:
                    return False
                if op1[0] != op2[0]:  # Different job-operation
                    return False
                if abs(op1[1] - op2[1]) > 0.01:  # Different start time
                    return False
                if abs(op1[2] - op2[2]) > 0.01:  # Different end time
                    return False
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        Generate observation for event-based Perfect Knowledge scheduling system.
        
        KEY ADVANTAGE: Perfect Knowledge RL knows exact future arrival times,
        enabling optimal planning and scheduling decisions.
        
        Components (event-based design):
        1. Normalized machine free times (absolute, not relative)
        2. Job progress (current operation / total operations)
        3. Processing times of next available operations
        4. Current system time (normalized)
        5. Exact future arrival times (PERFECT KNOWLEDGE ADVANTAGE)
        """
        obs = []
        ...
        for job_id in self.job_ids:
            ...
            if next_op_idx < len(self.jobs[job_id]):
                # Check if job will be available (either arrived or will arrive)
                arrival_time = self.job_arrival_times.get(job_id, 0.0)
                if arrival_time <= current_time or job_id in self.arrived_jobs:
                    # Job is or will be available - get min processing time across machines
                    operation = self.jobs[job_id][next_op_idx]
                    available_proc_times = [pt for machine, pt in operation['proc_times'].items() 
                                          if machine in self.machines]
                    if available_proc_times:
                    ...
                    obs.append(0.0)  # Job not available yet
            else:
                obs.append(0.0)  # Job completed
        
        # 4. Current system time (normalized) - important for planning
        normalized_time = min(1.0, current_time / time_scale)
        obs.append(normalized_time)
        
        # 5. PERFECT KNOWLEDGE ADVANTAGE: Exact future arrival times
        # This is what makes Perfect Knowledge RL superior - it knows when jobs will arrive
        for job_id in self.job_ids:
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    """
    Shortest Processing Time (SPT) heuristic for dynamic FJSP with job arrivals.
    
    This is a greedy heuristic that always schedules the operation with the shortest
    processing time among all available operations.
    """
    # Initialize state
    ...
    # Get all jobs that will arrive (not infinity)
    arriving_jobs = {job_id for job_id, arr_time in arrival_times.items() if arr_time != float('inf')}
    
    while operations_scheduled < total_operations:
        # Find all available operations at current time
        available_operations = []
        
        for job_id in arriving_jobs:
            ...
            if job_arrival_time > current_time:
                continue
            ...
            # Add all machine options for this operation
            ...
            for machine, proc_time in operation['proc_times'].items():
                if machine in machine_list:
                    available_operations.append({
                        ...
                        'machine': machine,
                        'proc_time': proc_time,
                        'earliest_start': max(current_time, 
                                            machine_next_free.get(machine, 0.0),
                                            job_arrival_time,
                                            operation_end_times[job_id][next_op_idx - 1] if next_op_idx > 0 else 0.0)
                    })
        ...
        if not available_operations:
            ...
            # Check when machines become free
            for machine, free_time in machine_next_free.items():
                if free_time > current_time:
                    next_events.append(free_time)
            
            # Check when jobs arrive
            for job_id, arrival_time in arrival_times.items():
                if arrival_time > current_time and arrival_time != float('inf'):
                    next_events.append(arrival_time)
            
            # Check when previous operations complete
            for job_id in arriving_jobs:
                next_op_idx = next_operation[job_id]
        ...
        # SPT heuristic: choose operation with shortest processing time
        best_operation = min(available_operations, key=lambda x: x['proc_time'])
        
        # Schedule the chosen operation
        job_id = best_operation['job_id']
        op_idx = best_operation['op_idx']
        machine = best_operation['machine']
        proc_time = best_operation['proc_time']
        start_time = best_operation['earliest_start']
        end_time = start_time + proc_time
        ...
        # Update current time to the end of this operation
    ...
    # Calculate makespan
    makespan = max([max([op[2] for op in ops]) for ops in schedule.values() if ops], default=0.0)
    
    return makespan, schedule


def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=60):
    """
    MILP-based optimal scheduler for dynamic FJSP with job arrivals.
    
    Uses PuLP to formulate and solve the scheduling problem optimally.
    Returns infinity makespan if solver fails or times out.
    """
    try:
        from pulp import LpProblem, LpMinimize, LpVariable, lpSum, PULP_CBC_CMD, LpStatus
        
        # Filter out jobs that don't arrive
        arriving_jobs = {job_id: arrival_times[job_id] 
                        for job_id in jobs_data.keys() 
                        if arrival_times.get(job_id, float('inf')) != float('inf')}
        
        if not arriving_jobs:
            return float('inf'), {}
        
        # Estimate time horizon (upper bound)
        total_proc_time = sum(sum(min(op['proc_times'].values()) for op in job_ops) 
                             for job_ops in jobs_data.values())
        max_arrival = max(arrival_times.values()) if arrival_times else 0
        time_horizon = int(total_proc_time + max_arrival + 50)  # Add buffer
        
        # Create problem
        prob = LpProblem("FJSP_Optimal", LpMinimize)
        
        # Decision variables
        # x[j,o,m,t] = 1 if operation o of job j starts on machine m at time t
        x = {}
        for job_id, job_ops in jobs_data.items():
            if job_id not in arriving_jobs:
                continue
            for op_idx, operation in enumerate(job_ops):
                for machine in operation['proc_times'].keys():
                    if machine in machine_list:
                        for t in range(time_horizon):
                            x[job_id, op_idx, machine, t] = LpVariable(
                                f"x_{job_id}_{op_idx}_{machine}_{t}", cat='Binary')
        ...
        for job_id, job_ops in jobs_data.items():
            ...
            for op_idx in range(len(job_ops)):
                ...
                prob += lpSum([x[job_id, op_idx, machine, t] 
                              for machine in operation['proc_times'].keys() 
                              if machine in machine_list
        ...
        for job_id, job_ops in jobs_data.items():
            if job_id not in arriving_jobs:
                continue
            for op_idx in range(len(job_ops) - 1):
                curr_op = job_ops[op_idx]
                next_op = job_ops[op_idx + 1]
                
                # Current operation must finish before next operation starts
                for m1 in curr_op['proc_times'].keys():
                    if m1 in machine_list:
                        for t1 in range(time_horizon):
                            for m2 in next_op['proc_times'].keys():
                                if m2 in machine_list:
                                    for t2 in range(time_horizon):
                                        if t2 < t1 + curr_op['proc_times'][m1]:
                                            prob += x[job_id, op_idx, m1, t1] + x[job_id, op_idx+1, m2, t2] <= 1
                ...
                for job_id, job_ops in jobs_data.items():
                    ...
                    for op_idx, operation in enumerate(job_ops):
                        if machine in operation['proc_times']:
                            ...
                            # Add all time slots where this operation would be running
                            for start_t in range(max(0, t - proc_time + 1), t + 1):
                                if start_t < time_horizon:
                                    overlapping_ops.append(x[job_id, op_idx, machine, start_t])
                
                if overlapping_ops:
                    prob += lpSum(overlapping_ops) <= 1
        
        # 4. Job arrival constraints
        for job_id, arrival_time in arriving_jobs.items():
            job_ops = jobs_data[job_id]
            if job_ops:  # First operation cannot start before arrival
                first_op = job_ops[0]
                for machine in first_op['proc_times'].keys():
                    if machine in machine_list:
                        for t in range(int(arrival_time)):
                            if t < time_horizon:
                                prob += x[job_id, 0, machine, t] == 0
        ...
        for job_id, job_ops in jobs_data.items():
            ...
            last_operation = job_ops[last_op_idx]
            
            for machine in last_operation['proc_times'].keys():
                if machine in machine_list:
                    proc_time = last_operation['proc_times'][machine]
                    for t in range(time_horizon):
                        prob += makespan >= (t + proc_time) * x[job_id, last_op_idx, machine, t]
        ...
        if prob.status != 1:  # Not optimal
            print(f"    MILP solver status: {LpStatus[prob.status]} - returning infinite makespan")
            return float('inf'), {}
        
        # Extract solution
        schedule = {m: [] for m in machine_list}
        optimal_makespan = makespan.varValue
        ...
        for job_id, job_ops in jobs_data.items():
            ...
            for op_idx, operation in enumerate(job_ops):
                for machine in operation['proc_times'].keys():
                    if machine in machine_list:
                        for t in range(time_horizon):
                            if (job_id, op_idx, machine, t) in x and x[job_id, op_idx, machine, t].varValue == 1:
                                proc_time = operation['proc_times'][machine]
                                schedule[machine].append((f"J{job_id}-O{op_idx+1}", t, t + proc_time))
...
def calculate_regret_analysis(optimal_makespan, methods_results):
    ...
    for method, makespan in methods_results.items():
        if makespan != float('inf') and optimal_makespan != float('inf'):
            ...
            print(f"{method:25s}: +{regret:5.1f}% above optimal ({makespan:.2f} vs {optimal_makespan:.2f})")
        else:
...
def diagnose_performance_similarity(perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan):
    """Diagnose why performance might be too similar across methods."""
    print("\nPERFORMANCE SIMILARITY DIAGNOSIS:")
    print("-" * 50)
    
    # Check for identical results
    results = [perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan]
    unique_results = len(set(results))
    
    if unique_results == 1:
        print("🚨 CRITICAL: All methods produced identical results!")
        print("   Possible causes:")
        print("   - All methods finding the same optimal solution")
        print("   - Bug in evaluation functions")
        print("   - Problem is too simple/constrained")
        return
    
    if unique_results == 2:
        print("⚠️  WARNING: Only 2 unique results across 4 methods")
        print("   This suggests limited solution diversity")
    
    # Check for expected hierarchy violations
    if perfect_makespan > dynamic_makespan:
        print("🚨 VIOLATION: Perfect Knowledge worse than Dynamic RL")
        print("   Perfect Knowledge should always be best")
        
    if dynamic_makespan > static_makespan:
        print("🚨 VIOLATION: Dynamic RL worse than Static RL")
        print("   Dynamic information should provide advantage")
    
    # Check gaps
    gaps = []
    if perfect_makespan != float('inf') and dynamic_makespan != float('inf'):
        gap = abs(perfect_makespan - dynamic_makespan) / min(perfect_makespan, dynamic_makespan) * 100
        gaps.append(("Perfect vs Dynamic", gap))
    
    if dynamic_makespan != float('inf') and static_makespan != float('inf'):
        gap = abs(dynamic_makespan - static_makespan) / min(dynamic_makespan, static_makespan) * 100
        gaps.append(("Dynamic vs Static", gap))
    
    small_gaps = [name for name, gap in gaps if gap < 5.0]
    if small_gaps:
        print(f"📊 Small performance gaps (<5%): {small_gaps}")
        print("   This could indicate:")
        print("   - Problem is well-structured with clear optimal solutions")
        print("   - Methods are converging to similar strategies")
        print("   - Need more challenging test scenarios")
