...
def simple_spt_heuristic(jobs_data, machine_list, arrival_times):
    """
    CORRECTED SPT heuristic - the issue was in time advancement logic.
    Fixed to properly handle current time and job arrivals.
    """
    print("\n--- Running CORRECTED SPT Heuristic Scheduler ---")
    ...
    current_time = 0.0
    arrived_jobs = {job_id for job_id, arrival in arrival_times.items() if arrival <= current_time}
    ...
    while operations_scheduled_count < total_operations:
        ...
        # Collect all ready operations from arrived jobs
        for job_id in arrived_jobs:
            ...
            if op_idx < len(jobs_data[job_id]):
                ...
                # Only consider operations that are ready (precedence satisfied)
                if job_ready_time <= current_time:
                    for machine_name, proc_time in op_data['proc_times'].items():
                        ...
                        candidate_operations.append((
                            proc_time,           # SPT priority: shortest processing time
                            earliest_start_time, # When it can start
                            job_id, 
                            op_idx, 
                            machine_name
                        ))
        ...
        # Select the operation with the shortest processing time (SPT rule)
        selected_op = min(candidate_operations, key=lambda x: x[0])
        proc_time, earliest_start_time, job_id, op_idx, machine_name = selected_op
    ...
    makespan = max(machine_next_free.values()) if machine_next_free else 0
    print(f"CORRECTED SPT Heuristic Makespan: {makespan:.2f}")
    return makespan, schedule
