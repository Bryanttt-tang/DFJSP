...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Agent can't schedule in the past relative to event_time (can't time travel!)
        ...
        # CRITICAL FIX: Smart event_time advancement logic
        # Key insight: event_time should advance based on the machine's state BEFORE scheduling
        # 
        # Case 1: Scheduling on IDLE machine (machine_available_time <= event_time)
        #         → event_time stays same (agent can continue scheduling other idle machines)
        # 
        # Case 2: Scheduling on BUSY machine (machine_available_time > event_time)
        #         → event_time advances to when machine becomes free
        #         → This reveals any jobs that arrive before machine is ready
        #
        # This allows parallel scheduling on idle machines while properly handling
        # scheduling on machines that are busy in the future
        
        if machine_available_time > self.event_time:
            # Machine was busy in the future - advance event_time to reveal jobs
            # that arrive before this machine becomes available
            # Use the minimum of machine_available_time and next arrival
            next_arrival_time = float('inf')
            for job_id, arrival_time in self.job_arrival_times.items():
                if job_id not in self.arrived_jobs and arrival_time != float('inf'):
                    next_arrival_time = min(next_arrival_time, arrival_time)
            ...
            if new_event_time > self.event_time and new_event_time != float('inf'):
                self._update_event_time_and_arrivals(new_event_time)
        # else: machine was idle (machine_available_time <= event_time)
        #       → event_time stays the same, allowing more scheduling at current time
