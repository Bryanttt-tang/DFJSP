...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # FIXED: Use consistent seed approach for fair comparison
    # Use a deterministic but consistent seed based on scenario
    arrival_seed = GLOBAL_SEED + hash(str(sorted(arrival_times.items()))) % 1000
    print(f"    Using deterministic seed: {arrival_seed} (base: {GLOBAL_SEED})")
    ...
    print(f"    🔍 DIAGNOSTIC: Arrival times being used: {arrival_times}")
    ...
    with tqdm(total=total_timesteps, desc=f"Perfect Knowledge RL (Seed {arrival_seed})", 
    ...
    try:
        ...
        if post_training_makespan < random_makespan:
            ...
            print(f"    🔴 WARNING: Training did not improve (random: {random_makespan:.2f}, trained: {post_training_makespan:.2f})")
            print(f"    🔴 This could explain why Perfect Knowledge RL underperforms!")
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Check for newly arrived jobs (deterministic) - CONSISTENT with Dynamic RL
        newly_arrived = []
        for job_id_check, arrival_time in self.job_arrival_times.items():
            if (job_id_check not in self.arrived_jobs and 
                arrival_time <= self.current_makespan and 
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_makespan, current_makespan):
        """CONSISTENT reward calculation - IDENTICAL to Dynamic and Static RL"""
        if self.reward_mode == "makespan_increment":
            if previous_makespan is not None and current_makespan is not None:
                makespan_increment = current_makespan - previous_makespan
                reward = -makespan_increment  # Negative increment
            ...
            # Default reward function - CONSISTENT across all RL methods
            reward = 10.0 - proc_time * 0.1 - idle_time
            if done:
                reward += 100.0
            return reward
    ...
    def _get_observation(self):
        ...
        # 5. Job arrival times (perfect knowledge: exact arrival times, normalized)
        # CONSISTENT with Dynamic RL - both should have identical observation format
        for job_id in self.job_ids:
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
            normalized_arrival = min(1.0, arrival_time / max_possible_makespan)
            obs.append(normalized_arrival)
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 200000    # INCREASED: Perfect knowledge needs MORE training per scenario
    dynamic_timesteps = 100000   # Baseline training across diverse scenarios  
    static_timesteps = 100000    # Baseline training on static scenarios
    ...
    print("FIXED: All three RL methods now use identical hyperparameters for fair comparison:")  
    print("       - n_steps=2048, batch_size=256, n_epochs=10, gamma=0.99")
    print("       - net_arch=[256, 256, 128], learning_rate=1e-3")
    print("FIXED: Perfect Knowledge RL gets 200k timesteps (vs 100k) since it trains per scenario")
    print("FIXED: Consistent reward functions and observation spaces across all RL methods")
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        if perfect_makespan > dynamic_makespan:
            ...
            print(f"      This suggests training issues or inconsistent implementations!")
        
        if perfect_makespan > static_dynamic_makespan:
            print(f"  🚨 CRITICAL: Perfect Knowledge RL ({perfect_makespan:.2f}) worse than Static RL ({static_dynamic_makespan:.2f})")
            print(f"      This is a serious problem - Perfect RL should always outperform Static RL!")
