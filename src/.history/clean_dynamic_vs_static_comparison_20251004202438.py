...
class TrainingCallback:
    ...
    def __init__(self, method_name):
        ...
        # Add tracking for telescoping sum test
        self.episode_step_rewards = []  # Track individual step rewards
        self.episode_makespans = []     # Track final makespans
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        # Check if we have access to environment for manual episode tracking
        if hasattr(model, 'env') and hasattr(model.env, 'envs'):
            try:
                # Get the actual environment (unwrap from DummyVecEnv)
                env = model.env.envs[0]
                # Check if this is our wrapped environment
                if hasattr(env, 'env') and hasattr(env.env, 'current_makespan'):
                    actual_env = env.env  # Get past ActionMasker
                    if hasattr(actual_env, 'env'):
                        actual_env = actual_env.env  # Get past Monitor
                    
                    # Manual episode completion detection
                    if hasattr(actual_env, 'operations_scheduled') and hasattr(actual_env, 'total_operations'):
                        if actual_env.operations_scheduled >= actual_env.total_operations:
                            # Episode just completed!
                            final_makespan = actual_env.current_makespan
                            print(f"[MANUAL EPISODE DETECTION {self.method_name}] Episode completed!")
                            print(f"  Final makespan: {final_makespan:.4f}")
                            print(f"  Operations scheduled: {actual_env.operations_scheduled}/{actual_env.total_operations}")
                            
                            # Manual episode reward recording
                            TRAINING_METRICS['episode_rewards'].append(-final_makespan)  # Use negative makespan as episode reward
        ...
        if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
            ...
            if self.step_count % 1000 == 0:  # Print every 1000 calls to avoid spam
                ...
                if 'rollout/ep_rew_mean' in log_data:
                    ...
                    print(f"[DEBUG {self.method_name}] Manually tracked episodes: {len(TRAINING_METRICS['episode_rewards'])}")
            ...
            # Try to log episode rewards from rollout data
            if 'rollout/ep_rew_mean' in log_data:
                ep_reward = log_data['rollout/ep_rew_mean']
                TRAINING_METRICS['episode_rewards'].append(ep_reward)
                if self.episode_count == 0 or ep_reward != TRAINING_METRICS['episode_rewards'][-2]:  # Only increment if new reward
                    self.episode_count += 1
                    TRAINING_METRICS['episode_count'].append(self.episode_count)
...
# Add telescoping sum test function
def test_telescoping_sum_property(jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    """
    Test the telescoping sum property: sum of step rewards should equal -final_makespan
    for makespan_increment reward mode.
    """
    print(f"\n=== TELESCOPING SUM TEST ===")
    print(f"Testing property: Σ(step_rewards) = -final_makespan")
    ...
    # Run a single episode and collect all step rewards
    obs, _ = env.reset()
    step_rewards = []
    step_makespans = []
    step_count = 0
    ...
    print(f"Initial makespan: {env.env.current_makespan:.4f}")
    ...
    while not done and step_count < 100:  # Safety limit
        ...
        step_makespans.append(env.env.current_makespan)
        step_count += 1
        
        if done or truncated:
            break
    
    # Calculate telescoping sum
    total_step_rewards = sum(step_rewards)
    final_makespan = env.env.current_makespan
    expected_sum = -final_makespan if reward_mode == "makespan_increment" else None
    
    print(f"\n=== TELESCOPING SUM RESULTS ===")
    print(f"Total steps: {len(step_rewards)}")
    print(f"Step rewards: {[f'{r:.4f}' for r in step_rewards]}")
    print(f"Final makespan: {final_makespan:.4f}")
    print(f"Sum of step rewards: {total_step_rewards:.4f}")
    
    if reward_mode == "makespan_increment":
        print(f"Expected sum (-final_makespan): {expected_sum:.4f}")
        difference = abs(total_step_rewards - expected_sum)
        print(f"Difference: {difference:.6f}")
        
        if difference < 1e-6:
            print("✅ TELESCOPING SUM PROPERTY VERIFIED!")
        else:
            print("❌ TELESCOPING SUM PROPERTY FAILED!")
            print("This indicates an issue with the reward calculation")
            
            # Debug: Show makespan increments
            print("\nDEBUG: Makespan increments vs rewards")
            prev_makespan = 0.0
            for i, (reward, makespan) in enumerate(zip(step_rewards, step_makespans)):
                increment = makespan - prev_makespan
                expected_reward = -increment
                print(f"Step {i+1}: makespan_inc={increment:.4f}, reward={reward:.4f}, expected={expected_reward:.4f}")
                prev_makespan = makespan
    else:
        print(f"Reward mode '{reward_mode}' - telescoping sum test not applicable")
    
    return total_step_rewards, final_makespan, step_rewards
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    # Run telescoping sum test before training
    print(f"\n🧪 Running telescoping sum test before training...")
    test_arrival_times = {0: 0, 1: 0, 2: 0, 3: 5, 4: 10, 5: 15, 6: 20}  # Simple test scenario
    test_telescoping_sum_property(jobs_data, machine_list, test_arrival_times, reward_mode)
    ...
    def make_dynamic_env():
        ...
        env = ActionMasker(env, mask_fn)
        env = Monitor(env, allow_early_resets=True)  # Add allow_early_resets
        return env
    ...
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,  # Increase verbosity to see episode logging
    ...
    # Test telescoping sum property on trained agent
    print(f"\n🧪 Testing telescoping sum property on trained agent...")
    test_env = PerfectKnowledgeFJSPEnv(jobs_data, machine_list, test_arrival_times, reward_mode=reward_mode)
    ...
    total_rewards = sum(step_rewards)
    final_makespan = test_env.env.current_makespan
    print(f"🧪 Trained agent test: sum_rewards={total_rewards:.4f}, final_makespan={final_makespan:.4f}")
    print(f"🧪 Expected: {-final_makespan:.4f}, Difference: {abs(total_rewards + final_makespan):.6f}")
