...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # NOTE: No need to advance event_time here anymore!
        # The automatic advancement at the START of step() ensures we always have ready ops.
        # If this scheduling action clears all ready ops, the NEXT call to step() will
        # automatically advance event_time before asking for the next action.
        
        # Reward: negative makespan increment (deterministic based on scheduling decision)
        reward = -(self.current_makespan - previous_makespan)
...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, seq_rule, route_rule="MIN"):
    ...
    while completed_operations < total_operations:
        ...
        if not ready_operations:
            # AUTOMATIC EVENT ADVANCEMENT: Advance time to next event
            # This ensures sequential decision making with limited foresight
            next_time = float('inf')
