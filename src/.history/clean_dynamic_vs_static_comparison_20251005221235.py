...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        self.max_episode_steps = self.total_operations * 2 + 50  # Much more reasonable limit
    ...
    def step(self, action):
        """BUILDER MODE: Step function with proper event-driven WAIT semantics and improved termination."""
        self.episode_step += 1
        
        # More reasonable safety check for infinite episodes
        if self.episode_step >= self.max_episode_steps:
            # Force termination with current makespan instead of huge penalty
            return self._get_observation(), -100.0, True, False, {"error": "Max episode steps reached"}
        
        job_idx, op_idx, machine_idx = self._decode_action(action)

        # Handle WAIT action
        if job_idx is None:  # WAIT action
            # IMPROVED: Check if we really need to wait
            if self.operations_scheduled >= self.total_operations:
                return self._get_observation(), -10.0, True, False, {"action_type": "WAIT_AT_TERMINAL"}
            
            # Check if there are any valid scheduling actions available
            action_mask = self.action_masks()
            scheduling_actions_available = np.any(action_mask[:-1])  # Exclude WAIT action
            
            if scheduling_actions_available:
                # There are valid scheduling actions, but agent chose WAIT - small penalty
                wait_reward = -5.0
                info = {
                    ...
                    "action_type": "UNNECESSARY_WAIT"
                }
                return self._get_observation(), wait_reward, False, False, info
            
            # No scheduling actions available - legitimate wait
            num_new_arrivals, new_event_time = self._advance_to_next_arrival()
            
            # Check if we're stuck (no new arrivals and no future arrivals)
            if num_new_arrivals == 0 and self.operations_scheduled < self.total_operations:
                # Check if there are any jobs that will arrive in the future
                future_arrivals = [job_id for job_id, arr_time in self.job_arrival_times.items() 
                                 if job_id not in self.arrived_jobs and arr_time < float('inf')]
                
                if not future_arrivals:
                    # No more jobs will arrive and we haven't scheduled everything - terminate
                    return self._get_observation(), -50.0, True, False, {"error": "No more arrivals possible"}
            
            # WAIT reward: small penalty for waiting
            wait_reward = -2.0 if num_new_arrivals > 0 else -5.0  # Less penalty if arrivals happened
            info = {
                "makespan": self.current_makespan,
                "event_time": self.event_time,
