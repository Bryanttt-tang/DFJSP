...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # This allows the agent to learn arrival patterns across episodes
        # and make more informed wait decisions
    ...
    def _update_event_time_and_arrivals(self, new_event_time):
        ...
        for job_id, arrival_time in self.job_arrival_times.items():
            if (job_id not in self.arrived_jobs and 
                ...
                # Observe arrival for predictor (if enabled)
                if self.arrival_predictor is not None:
    ...
    def _calculate_wait_reward(self, scheduling_actions_available, current_event_time):
        ...
        The wait action is CRITICAL for reactive RL because the agent must learn:
        1. When to wait for better jobs to arrive (long-term benefit)
        2. When to schedule current jobs immediately (short-term benefit)
        
        Key Insight: The reward should balance:
        - Time cost of waiting (negative)
        - Potential benefit of waiting for better jobs (positive)
        - Machine utilization (should keep machines busy)
        
        Design Philosophy:
        Option A (SIMPLE): Just penalize waiting, let agent learn from experience
            - wait_reward = -5.0 if scheduling_actions_available else -1.0
            - Pros: Simple, agent learns patterns purely from rewards
            - Cons: Slow learning, may not discover good waiting strategies
        
        Option B (GUIDED): Use arrival prediction to guide wait decisions
            - Predict when next job arrives
            - Compare wait time vs potential benefit
            - Pros: Faster learning, more intelligent exploration
            - Cons: More complex, depends on prediction quality
        
        We'll implement OPTION B with fallback to OPTION A if predictions unavailable.
        """
        
        # --- OPTION A: SIMPLE PENALTY (Baseline) ---
        # Just penalize waiting heavily if there are valid scheduling actions
        # This encourages agent to use WAIT only when truly necessary
        base_wait_penalty = -5.0 if scheduling_actions_available else -1.0
        
        # --- OPTION B: INTELLIGENT WAIT REWARD (Advanced) ---
        # Consider: 
        # 1. Time until next arrival
        # 2. Machine utilization
        # 3. Number of jobs waiting
        ...
        # Calculate time until next machine becomes free
        next_machine_free_time = float('inf')
        for machine, free_time in self.machine_next_free.items():
            if free_time > current_event_time:
                next_machine_free_time = min(next_machine_free_time, free_time)
        
        # Number of idle machines right now
        num_idle_machines = sum(1 for m, t in self.machine_next_free.items() if t <= current_event_time)
        
        # Number of arrived jobs with ready operations
        num_ready_jobs = 0
        ...
        # Case 1: No scheduling actions available (forced wait)
        # Reward should be small penalty proportional to wait time
        if not scheduling_actions_available:
            wait_time = min(next_arrival_time, next_machine_free_time) - current_event_time
            wait_time = max(0, min(wait_time, 50))  # Cap at 50 for normalization
            
            # Small time-proportional penalty
            return -0.1 * wait_time if wait_time < 50 else -5.0
        
        # Case 2: Scheduling actions available but agent chooses to wait
        # This is where intelligence matters!
        
        # Calculate machine utilization
        machine_utilization = 1.0 - (num_idle_machines / len(self.machines))
        
        # Calculate wait time until next event
        next_event_time = min(next_arrival_time, next_machine_free_time)
        wait_time = max(0, next_event_time - current_event_time)
        
        # === OPTION B.1: Use ArrivalPredictor if available ===
        if self.arrival_predictor is not None:
            # Get prediction stats
            stats = self.arrival_predictor.get_stats()
            confidence = stats['confidence']
            
            # If we have good confidence in predictions, use them
            if confidence > 0.3:  # Threshold for using predictions
                # Predict when next job will arrive
                ...
                if unarrived_jobs:
                    ...
                    if predictions:
                        ...
                        intelligent_reward = (
                            time_penalty +
                            utilization_bonus +
                            prediction_bonus  # NEW: Prediction-based bonus
                        )
                        
                        final_reward = max(-10.0, min(-0.5, intelligent_reward))
                        return final_reward
        
        # === OPTION B.2: Heuristic-based intelligent reward (no predictor) ===
        # Heuristic: Waiting is better when:
        # - High machine utilization (machines are busy anyway)
        # - Short wait time for next arrival
        # - Few ready jobs (not much to schedule now)
        
        # Time penalty (proportional to wait time)
        time_penalty = -0.5 * min(wait_time, 20)  # Max -10
        
        # Machine utilization bonus (if machines busy, waiting is less bad)
        utilization_bonus = 3.0 * machine_utilization  # 0 to +3
        
        # Job scarcity bonus (if few ready jobs, waiting might bring better options)
        scarcity_bonus = 2.0 if num_ready_jobs <= 1 else 0.0
        
        # Proximity to next arrival (if arrival is soon, waiting is better)
        if next_arrival_time < float('inf'):
            time_until_arrival = next_arrival_time - current_event_time
            if time_until_arrival <= 5:
                proximity_bonus = 3.0 * (1 - time_until_arrival / 5.0)  # 0 to +3
            else:
                proximity_bonus = 0.0
        else:
            proximity_bonus = 0.0  # No more arrivals
        
        # Combine components
        intelligent_reward = (
            time_penalty +          # Always negative (cost of time)
            utilization_bonus +     # Positive if machines busy
            scarcity_bonus +        # Positive if few jobs to schedule
            proximity_bonus         # Positive if arrival soon
        )
        
        # === CHOOSE REWARD MODE ===
        # You can toggle between simple and intelligent reward here
        
        USE_INTELLIGENT_REWARD = True  # Set to False for simple baseline
        
        if USE_INTELLIGENT_REWARD:
            # Ensure reward is negative but not too harsh
            final_reward = max(-10.0, min(-0.5, intelligent_reward))
            return final_reward
        else:
            # Use simple penalty
    ...
    def step(self, action):
        ...
        if job_idx is None:
            ...
            # Calculate wait reward BEFORE advancing time (need current state)
