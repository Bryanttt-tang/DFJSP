...
class DynamicFJSPEnv(gym.Env):
    ...
    def reset(self, seed=None):
        # Simple reset for gym compatibility (like in dynamic-tutorial.py)
        if seed is not None:
            random.seed(seed)
            np.random.seed(seed)
    ...
    def step(self, action):
        ...
        if self.episode_step >= self.max_episode_steps:
            return self._get_observation(), -1000.0, True, {"error": "Max episode steps reached"}
        ...
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            ...
            return self._get_observation(), -10.0, False, {"error": "Invalid action, continuing"}
...
def train_agent(jobs_data, machine_list, train_arrivals, log_name, total_timesteps=150000):
    ...
    # Simplified model configuration similar to dynamic-tutorial.py
    from stable_baselines3 import PPO
...
def evaluate_agent(model, jobs_data, machine_list, eval_arrivals, scenario_name):
    ...
    print(f"Evaluation complete. Best Makespan: {best_makespan:.2f}")
