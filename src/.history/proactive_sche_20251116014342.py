...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # ACTION SPACE: Select from 7 dispatching rules
    ...
    def action_masks(self):
        """Generate action masks - all rules available if work exists, wait always available."""
        ...
        # WAIT always available unless done
    ...
    def step(self, action):
        ...
        # Handle dispatching rule selection
        ready_ops = self._get_ready_operations()
        
        if not ready_ops:
            # No ready operations - penalize
            return self._get_observation(), -10.0, False, False, {"error": "no_ready_ops"}
  ...
  # Same as reactive RL for fair comparison
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        if milp_is_valid:
            ...
            print(f"  Results: Perfect={perfect_makespan:.2f} (BENCHMARK), Dynamic={dynamic_makespan:.2f}, Static(dyn)={static_dynamic_makespan:.2f}, Heuristic={spt_makespan:.2f}, MILP=INVALID")
