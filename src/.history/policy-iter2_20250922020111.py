...
def estimate_state_space(jobs_data, machines, arrivals):
    ...
    if num_jobs == 2 and total_ops == 4:  # SIMPLE
        estimated_states = "~50-100"
    elif num_jobs == 3 and total_ops == 6:  # MEDIUM  
        estimated_states = "~320 (observed)"
    elif num_jobs == 5 and total_ops == 15:  # LARGER
        estimated_states = "~50K-200K (estimated)"
    else:
        # Rough heuristic: states grow exponentially but much slower than theoretical max
        # Based on observed ratio for MEDIUM: 320 actual vs 27 theoretical next_ops
        scaling_factor = 320 / 27  # ~12x multiplier from timing constraints
        estimated_states = f"~{int(next_ops_combinations * scaling_factor):,}"
    
    print(f"Realistic reachable states: {estimated_states}")
    print(f"Reduction factor from theory: constraints eliminate most invalid combinations")
