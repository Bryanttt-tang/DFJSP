...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def action_masks(self):
        ...
        # Wait actions: CRITICAL LOGIC CHANGE
        # Only enable wait actions if BOTH conditions hold:
        # 1. There are unarrived jobs (otherwise waiting serves no strategic purpose)
        # 2. There is schedulable work available (otherwise it's a forced wait to next arrival)
        #
        # Key insight: After all jobs arrived, wait actions become MEANINGLESS
        # - Can't wait for new arrivals (none coming)
        # - Better to schedule directly (waiting just delays makespan)
        # This prevents agent from getting stuck in wait loops after all jobs arrived
        
        has_unarrived_jobs = len(self.arrived_jobs) < len(self.job_ids)
        has_schedulable_work = np.any(mask[:self.wait_action_start])  # Any scheduling actions available
        
        # FIXED: Only enable wait if there are UNARRIVED jobs
        # Once all jobs arrived, wait serves no strategic purpose
        if has_unarrived_jobs and has_schedulable_work:
            for wait_idx in range(len(self.wait_durations)):
                action_idx = self.wait_action_start + wait_idx
                mask[action_idx] = True
