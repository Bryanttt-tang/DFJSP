...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    """
    print(f"\n--- Training Perfect Knowledge RL Agent (scenario-specific) ---")
    print(f"Training arrival times: {arrival_times}")
    print(f"Initial jobs: {[j for j, t in arrival_times.items() if t == 0]}")
    print(f"Dynamic arrivals: {[(j, t) for j, t in arrival_times.items() if t > 0]}")
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    
    # Use scenario-specific seed for deterministic but varied training
    scenario_hash = hash(str(sorted(arrival_times.items())))
    scenario_seed = GLOBAL_SEED + (abs(scenario_hash) % 1000)  # Deterministic but unique per scenario
    print(f"Scenario-specific seed: {scenario_seed} (based on arrival pattern)")
    
    # Verify this is training on the correct scenario
    print(f"Verifying: Training Perfect RL for arrivals: {sorted(arrival_times.items())}")
    
    def make_perfect_env():
        # Use PerfectKnowledgeFJSPEnv for both training and evaluation consistency
        env = PerfectKnowledgeFJSPEnv(jobs_data, machine_list, arrival_times, reward_mode=reward_mode, seed=scenario_seed)
    ...
    model = MaskablePPO(
        ...
        vf_coef=0.5,
        max_grad_norm=0.5,
        seed=scenario_seed,        # Use scenario-specific seed
        policy_kwargs=dict(
...
def evaluate_dynamic_on_dynamic(dynamic_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    """Evaluate dynamic model on dynamic scenario with FIXED arrival times."""
    print(f"  Dynamic RL evaluation with fixed arrival times: {arrival_times}")
    
    # CRITICAL FIX: Use PerfectKnowledgeFJSPEnv instead of PoissonDynamicFJSPEnv
    # This ensures the exact arrival times are used, not random Poisson generation
    test_env = PerfectKnowledgeFJSPEnv(
        jobs_data, machine_list, 
        arrival_times=arrival_times,  # Use exact test arrival times
        reward_mode=reward_mode
    )
    ...
    print(f"  Dynamic RL will see arrivals at: {[t for t in arrival_times.values() if t > 0]}")
    ...
    while step_count < 1000:
        ...
        if info.get("error", "").startswith("Invalid action"):
            invalid_actions += 1
    ...
    print(f"  Dynamic RL final makespan: {makespan:.2f} after {step_count} steps")
...
def main():
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        print(f"  Training Perfect Knowledge RL for scenario {i+1} with arrivals: {scenario_arrivals}")
        ...
        print(f"  Running MILP optimal solver for scenario {i+1}...")
        ...
        # Validate Perfect RL vs Dynamic RL relationship
        if perfect_makespan > dynamic_makespan + 0.01:  # Allow small numerical tolerance
            print(f"  ⚠️  UNEXPECTED: Perfect Knowledge RL ({perfect_makespan:.2f}) > Dynamic RL ({dynamic_makespan:.2f})")
            print(f"    Perfect RL should never be worse than Dynamic RL since it has more information!")
        ...
        # Debug: Show a unique signature of each schedule to detect if they're identical
        def schedule_signature(schedule):
            """Create a unique signature for a schedule to detect identical schedules."""
            sig_parts = []
            for machine in sorted(schedule.keys()):
                machine_ops = schedule.get(machine, [])
                if machine_ops:
                    # Sort by start time and create signature
                    sorted_ops = sorted(machine_ops, key=lambda x: x[1])
                    machine_sig = ",".join([f"{op[0]}@{op[1]:.1f}-{op[2]:.1f}" for op in sorted_ops])
                    sig_parts.append(f"{machine}:[{machine_sig}]")
            return "|".join(sig_parts)
        
        dynamic_sig = schedule_signature(dynamic_schedule)
        perfect_sig = schedule_signature(perfect_schedule)
        
        if i == 0:
            # Store first scenario signature for comparison
            first_dynamic_sig = dynamic_sig
            print(f"  Dynamic RL schedule signature (scenario 1): {dynamic_sig[:100]}..." if len(dynamic_sig) > 100 else f"  Dynamic RL schedule signature (scenario 1): {dynamic_sig}")
        else:
            # Check if Dynamic RL is producing identical schedules
            if dynamic_sig == first_dynamic_sig:
                print(f"  ⚠️  WARNING: Dynamic RL schedule for scenario {i+1} is IDENTICAL to scenario 1!")
                print(f"    This suggests Dynamic RL is not adapting to different arrival patterns.")
            else:
                print(f"  ✅ Dynamic RL schedule for scenario {i+1} is different from scenario 1")
    ...
    if milp_makespan != float('inf'):
        ...
        print(f"• Static RL (dynamic): {avg_results['Static RL (dynamic)']:.2f} ± {std_results['Static RL (dynamic)']:.2f} (avg regret: +{((avg_results['Static RL (dynamic)']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
        print(f"• Static RL (static): {avg_results['Static RL (static)']:.2f} ± {std_results['Static RL (static)']:.2f} (avg regret: +{((avg_results['Static RL (static)']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
        ...
        print(f"• Static RL (dynamic): {avg_results['Static RL (dynamic)']:.2f} ± {std_results['Static RL (dynamic)']:.2f}")
        print(f"• Static RL (static): {avg_results['Static RL (static)']:.2f} ± {std_results['Static RL (static)']:.2f}")
