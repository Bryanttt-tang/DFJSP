...
def main():
    ...
    # Get schedules from first scenario if available
    if gantt_scenarios_data:
        ...
        # Fallback empty schedules
        empty_schedule = {machine: [] for machine in MACHINE_LIST}
        perfect_schedule = dynamic_schedule = static_dynamic_schedule = static_static_schedule = spt_schedule = milp_schedule = empty_schedule
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    """
    Shortest Processing Time (SPT) heuristic for dynamic job arrivals.
    At each time step, schedule the available operation with shortest processing time.
    """
    # Initialize state
    schedule = {machine: [] for machine in machine_list}
    machine_next_free = {machine: 0.0 for machine in machine_list}
    ...
    # Create list of all operations with their earliest start times
    pending_operations = []
    ...
    while operations_scheduled < total_operations:
        ...
        for job_id in jobs_data.keys():
            ...
            if next_op_idx < len(jobs_data[job_id]):
                ...
                if job_arrival_time <= current_time:
                    ...
                    if next_op_idx == 0:
                        # First operation - ready when job arrives
                        job_ready_time = job_arrival_time
                    else:
                        # Later operation - ready when previous operation completes
        ...
        if not pending_operations:
            ...
            for job_id, arrival_time in arrival_times.items():
                if arrival_time > current_time and next_operation[job_id] == 0:
            ...
            if next_events:
                ...
                # No more events - shouldn't happen if we have operations left
                current_time += 1.0
            continue
        
        # Sort by processing time (SPT) - shortest first
        pending_operations.sort(key=lambda x: x['proc_time'])
        
        # Schedule the shortest operation
        chosen_op = pending_operations[0]
        job_id = chosen_op['job_id']
        op_idx = chosen_op['op_idx']
        machine = chosen_op['machine']
        proc_time = chosen_op['proc_time']
        
        # Calculate start and end times
        machine_available_time = machine_next_free[machine]
        job_ready_time = chosen_op['job_ready_time']
    ...
    # Calculate makespan
    makespan = max([max([op[2] for op in machine_ops]) for machine_ops in schedule.values() if machine_ops])
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times):
    ...
    try:
        ...
        # Create the problem
        prob = LpProblem("FJSP_with_Arrivals", LpMinimize)
        
        # Big M for constraints
        big_m = sum(sum(max(op['proc_times'].values()) for op in job_ops) 
                   for job_ops in jobs_data.values()) + max(arrival_times.values(), default=0)
        ...
        s = {}
        # y[j1,o1,j2,o2,m] = 1 if operation (j1,o1) precedes (j2,o2) on machine m
        y = {}
        ...
        # 1. Each operation is assigned to exactly one machine
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                op_data = jobs_data[job_id][op_idx]
                prob += lpSum([x[job_id, op_idx, machine] 
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(1, len(jobs_data[job_id])):
                ...
                prob += (s[job_id, op_idx] >= 
                        s[job_id, op_idx - 1] + 
                        lpSum([x[job_id, op_idx - 1, machine] * prev_op_data['proc_times'][machine]
                              for machine in prev_op_data['proc_times'].keys()]))
        
        # 3. Job arrival constraints
        for job_id in jobs_data.keys():
            arrival_time = arrival_times.get(job_id, 0.0)
            prob += s[job_id, 0] >= arrival_time
        
        # 4. Machine capacity constraints (disjunctive constraints)
        operations_on_machine = {}
        for machine in machine_list:
            operations_on_machine[machine] = []
            for job_id in jobs_data.keys():
                for op_idx in range(len(jobs_data[job_id])):
                    op_data = jobs_data[job_id][op_idx]
                    if machine in op_data['proc_times']:
                        operations_on_machine[machine].append((job_id, op_idx))
        
        for machine in machine_list:
            ops = operations_on_machine[machine]
            for i, (job1, op1) in enumerate(ops):
                for j, (job2, op2) in enumerate(ops):
                    if i < j:  # Avoid duplicate constraints
                        # Create binary variable for precedence
                        y[job1, op1, job2, op2, machine] = LpVariable(
                            f"y_{job1}_{op1}_{job2}_{op2}_{machine}", cat='Binary')
                        
                        proc_time1 = jobs_data[job1][op1]['proc_times'][machine]
                        proc_time2 = jobs_data[job2][op2]['proc_times'][machine]
                        
                        # If both operations are on this machine, they can't overlap
                        prob += (s[job2, op2] >= 
                                s[job1, op1] + proc_time1 * x[job1, op1, machine] - 
                                big_m * (2 - x[job1, op1, machine] - x[job2, op2, machine] + y[job1, op1, job2, op2, machine]))
                        
                        prob += (s[job1, op1] >= 
                                s[job2, op2] + proc_time2 * x[job2, op2, machine] - 
                                big_m * (3 - x[job1, op1, machine] - x[job2, op2, machine] - y[job1, op1, job2, op2, machine]))
        ...
        for job_id in jobs_data.keys():
            ...
            prob += (makespan >= 
                    s[job_id, last_op_idx] + 
                    lpSum([x[job_id, last_op_idx, machine] * last_op_data['proc_times'][machine]
                          for machine in last_op_data['proc_times'].keys()]))
        
        # Solve the problem
        prob.solve(PULP_CBC_CMD(msg=0, timeLimit=300))  # 5 minute time limit
        
        if prob.status != 1:  # Not optimal
            print(f"MILP solver status: {LpStatus[prob.status]}")
            return float('inf'), {}
        ...
        for job_id in jobs_data.keys():
            for op_idx in range(len(jobs_data[job_id])):
                ...
                for machine in op_data['proc_times'].keys():
                    if x[job_id, op_idx, machine].varValue > 0.5:  # Selected machine
        ...
        print("PuLP not available - cannot compute MILP optimal solution")
        return float('inf'), {}
    except Exception as e:
...
def calculate_regret_analysis(optimal_makespan, methods_results):
    """Calculate regret (gap from optimal) for different methods."""
    ...
    for method, makespan in methods_results.items():
        if makespan != float('inf') and optimal_makespan != float('inf'):
            ...
            print(f"{method:25s}: +{regret:5.1f}% above optimal")
        else:
            regrets[method] = float('inf')
...
def generate_test_scenarios(jobs_data, initial_jobs=[0, 1, 2, 3, 4], arrival_rate=0.08, num_scenarios=10):
    ...
    scenarios = []
    for i in range(num_scenarios):
        # Use different seeds for test scenarios (1000+i)
        test_seed = 1000 + i
        ...
        if isinstance(initial_jobs, list):
            ...
            dynamic_job_ids = [j for j in job_ids if j not in initial_jobs]
        else:
            initial_job_ids = job_ids[:initial_jobs]
            dynamic_job_ids = job_ids[initial_jobs:]
        ...
        for job_id in dynamic_job_ids:
            inter_arrival_time = np.random.exponential(1.0 / arrival_rate)
            current_time += inter_arrival_time
            arrival_times[job_id] = round(current_time)  # Integer times for simplicity
...
def diagnose_performance_similarity(perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan):
    ...
    print("\n6.5. PERFORMANCE SIMILARITY DIAGNOSIS")
    print("-" * 50)
    ...
    for i, (method1, makespan1) in enumerate(methods):
        for j, (method2, makespan2) in enumerate(methods[i+1:], i+1):
            if abs(makespan1 - makespan2) < tolerance:
                identical_groups.append((method1, method2, makespan1, makespan2))
    
    if identical_groups:
        print("⚠️  IDENTICAL RESULTS DETECTED:")
        for method1, method2, makespan1, makespan2 in identical_groups:
            print(f"   {method1} ({makespan1:.2f}) ≈ {method2} ({makespan2:.2f})")
        
        print("\nPossible causes:")
        print("1. All methods found the same optimal solution")
        print("2. Problem instance is too simple (dominates constraints)")
        print("3. Training insufficient (agents not learning)")
        print("4. Evaluation bug (using same environment/model)")
        print("5. Action masking too restrictive (forcing same decisions)")
    
    # Check performance spread
    makespans = [m[1] for m in methods if m[1] != float('inf')]
    if makespans:
        spread = max(makespans) - min(makespans)
        mean_makespan = sum(makespans) / len(makespans)
        relative_spread = (spread / mean_makespan) * 100
        
        print(f"\nPerformance spread: {spread:.2f} ({relative_spread:.1f}% of mean)")
        if relative_spread < 5:
            print("🔴 LOW DIVERSITY: Methods performing very similarly")
        elif relative_spread < 15:
            print("🟡 MODERATE DIVERSITY: Some differentiation between methods")  
        else:
            print("✅ GOOD DIVERSITY: Clear performance differences")
    ...
    for scenario_idx in range(len(test_scenarios)):
        ...
        num_methods = 5 if milp_makespan != float('inf') else 4
