...
def simple_spt_heuristic(jobs_data, machine_list, arrival_times):
    """
    Simple and efficient SPT heuristic based on test3_backup.py approach.
    At each step, schedules the operation with shortest processing time.
    """
    # Initialize state
    machine_next_free = {m: 0.0 for m in machine_list}
    operation_end_times = {job_id: [0.0] * len(jobs_data[job_id]) for job_id in jobs_data}
    ...
    while operations_scheduled_count < total_operations:
        ...
        if not any(next_operation_for_job[job_id] < len(jobs_data[job_id]) for job_id in arrived_jobs):
            ...
            upcoming_arrivals = [arr for arr in arrival_times.values() if arr > min(machine_next_free.values())]
            ...
            arrived_jobs.update({job_id for job_id, arrival in arrival_times.items() if arrival <= next_arrival_time})
            ...
            if op_idx < len(jobs_data[job_id]):
                ...
                for machine_name, proc_time in op_data['proc_times'].items():
                    ...
                    candidate_operations.append((
                        proc_time,  # SPT criterion - select shortest processing time
                        earliest_start_time,
                        job_id, 
                        op_idx, 
                        machine_name
        ...
        arrived_jobs.update({j_id for j_id, arrival in arrival_times.items() if arrival <= current_time})
