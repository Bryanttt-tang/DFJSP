...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # 4. DYNAMIC RL: NO FUTURE KNOWLEDGE - Must learn arrival patterns from experience
        # REMOVED: Direct access to future arrival times (was giving unfair advantage)
        # Dynamic RL should predict arrival patterns from training, not have perfect future knowledge
        for job_id in self.job_ids:
            if job_id not in self.arrived_jobs:
                # Job not arrived yet: NO direct future arrival information available
                # Dynamic RL must learn to predict based on patterns it has observed during training
                obs.append(0.0)  # No future arrival information available
            else:
