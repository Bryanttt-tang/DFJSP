...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # - Single WAIT action: waits to next event (like reactive environment)
        num_scheduling_actions = len(self.job_ids) * len(self.machines)
        
        # SIMPLIFIED: Just ONE wait action (to next event)
        self.WAIT_ACTION = num_scheduling_actions
    ...
    def action_masks(self):
        ...
        # SIMPLIFIED: Single wait action - only enable if there are unarrived jobs OR if scheduling actions available
        ...
        has_schedulable_work = np.any(mask[:self.WAIT_ACTION])  # Any scheduling actions available
    ...
    def step(self, action):
        """Execute action with simplified wait and consistent rewards."""
        previous_makespan = self.current_makespan
        ...
        # Handle WAIT action (simplified to single action)
        if action == self.WAIT_ACTION:
            # Check if we could have scheduled instead
            action_mask = self.action_masks()
            scheduling_actions_available = np.any(action_mask[:self.WAIT_ACTION])
            ...
            # SIMPLE FIXED PENALTY (like reactive environment)
            # Heavy penalty if we could have scheduled, light penalty if forced to wait
            reward = -5.0 if scheduling_actions_available else -1.0
