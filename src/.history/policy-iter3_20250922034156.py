...
def canonicalize_state(next_ops, machine_next_free, job_ready_time, current_time, time_unit=1, cap=None):
    """
    Convert internal state to canonical RL-compatible observation format:
    1. Ready job indicators (which jobs have ready operations now)
    2. Machine idle status (which machines are idle now) 
    3. Processing times for ready operations on each machine (normalized)
    4. Future arrival info (zeros for static/perfect knowledge compatibility)
    5. Future processing times (zeros for static/dynamic compatibility)
    
    Returns hashable tuple matching RL observation space structure.
    """
    # 1. Ready job indicators (binary: 1 if job has ready operation available now, 0 otherwise)
    ready_jobs = []
    ...
    # 2. Machine idle status (binary: 1 if idle now, 0 if busy)
    machine_idle = []
    for m_idx in range(len(machines)):
        is_idle = machine_next_free[m_idx] <= current_time
        machine_idle.append(1 if is_idle else 0)
    
    # 3. Processing times for ready operations (normalized, 0 if not ready/compatible)
    max_proc_time = 10.0  # Normalization constant matching RL environments
    proc_times = []
    for j_idx in range(len(next_ops)):
        job_id = job_ids[j_idx]
        op_idx = next_ops[j_idx]
    ...
    # 4. Future arrival info (all zeros - static/perfect knowledge compatibility)
    future_arrivals = [0.0] * len(job_ids)
    
    # 5. Future processing times (all zeros - static/dynamic compatibility) 
    future_proc_times = [0.0] * (len(job_ids) * len(machines))
    
    # Combine all components into unified observation tuple
    observation = (tuple(ready_jobs), tuple(machine_idle), tuple(proc_times), 
                  tuple(future_arrivals), tuple(future_proc_times))
    
    return observation

def pretty_state(s):
    ready_jobs, machine_idle, proc_times, future_arrivals, future_proc_times = s
