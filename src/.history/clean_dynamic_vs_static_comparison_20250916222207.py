...
class StaticFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, reward_mode="makespan_increment", seed=None):
        ...
        # StaticFJSPEnv uses smaller action space: job × machine (not job × ops × machine)
        # since at each step there's at most one ready operation per job
    ...
    def _update_valid_actions(self):
        """Update the list of valid actions based on current state (static version)."""
        self.valid_actions = []
        
        for job_id in self.job_ids:
            # Skip if job hasn't arrived yet (always available for static)
    ...
    def action_masks(self):
        ...
        # For each job that has a ready operation, mark the corresponding job*machine actions as valid
        for job_idx, job_id in enumerate(self.job_ids):
            ...
            for machine_idx, machine in enumerate(self.machines):
                if machine in operation['proc_times']:
                    # Action index: job_idx * num_machines + machine_idx
                    action = job_idx * len(self.machines) + machine_idx
                    if action < self.action_space.n:
                        mask[action] = True
        
        # If no valid actions, enable all to prevent crashes
        if not np.any(mask):
            mask.fill(True)
    ...
    def step(self, action):
        """Step function using fixed action space with masking."""
        self.episode_step += 1
        ...
        # Decode action: action = job_idx * num_machines + machine_idx
        job_idx = action // len(self.machines)
        machine_idx = action % len(self.machines)
        
        # Validate decoded action
        if job_idx >= len(self.job_ids):
            return self._get_observation(), -50.0, False, False, {"error": "Invalid job index"}
        ...
        if next_op_idx >= len(self.jobs[job_id]):
            return self._get_observation(), -50.0, False, False, {"error": "Job already complete"}
        
        operation = self.jobs[job_id][next_op_idx]
        if machine not in operation['proc_times']:
            return self._get_observation(), -50.0, False, False, {"error": "Machine cannot process operation"}
