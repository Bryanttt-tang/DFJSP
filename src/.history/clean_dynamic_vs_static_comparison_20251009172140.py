...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    """
    Evaluate static model on dynamic scenario - FIXED VERSION.
    
    Key insight: Static RL agent was trained on scenarios where ALL jobs are available at t=0.
    When evaluating on sparse/late arrivals, we need to use a DIFFERENT approach:
    1. Use an environment that ONLY shows arrived jobs to the static agent
    2. Gradually reveal jobs as they arrive (simulating dynamic arrivals)
    3. This prevents the observation space mismatch and action confusion
    """
    print(f"  Static RL evaluation on dynamic scenario (FIXED APPROACH)...")
    
    # Create a special evaluation environment that simulates how static RL should handle dynamic arrivals
    test_env = StaticRLDynamicEvaluationEnv(jobs_data, machine_list, arrival_times, reward_mode=reward_mode)
    test_env = ActionMasker(test_env, mask_fn)
    ...
    print(f"    Starting evaluation with {len([j for j, t in arrival_times.items() if t == 0])} initial jobs...")
    
    while not done and step_count < max_steps:
        action_masks = test_env.action_masks()
        if not np.any(action_masks):
            # Check if we're waiting for more jobs to arrive
            if hasattr(test_env.env, 'pending_arrivals') and test_env.env.pending_arrivals:
                print(f"    Waiting for job arrivals at step {step_count}...")
                # Advance time to next arrival
                test_env.env._advance_to_next_arrival()
                obs = test_env.env._get_observation()
                continue
            else:
                print(f"    No valid actions and no pending arrivals at step {step_count}")
                break
        ...
        # Debug: Show progress every 20 steps
        if step_count % 20 == 0:
            available_jobs = len([j for j, t in arrival_times.items() if t <= test_env.env.current_makespan])
            scheduled_ops = test_env.env.operations_scheduled
            total_ops = test_env.env.total_operations
            print(f"    Step {step_count}: {available_jobs} jobs available, {scheduled_ops}/{total_ops} ops scheduled")
...
def main():
    ...
    dynamic_timesteps = 60000   # Increased for better learning with integer timing  
