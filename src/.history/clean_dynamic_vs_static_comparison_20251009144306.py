...
def plot_training_metrics():
    ...
    ALL plots use timesteps on x-axis for alignment.
    """
    global TRAINING_METRICS
    ...
    print(f"Episode timesteps recorded: {len(TRAINING_METRICS['episode_timesteps'])}")
    
    # Use timesteps for episode rewards (aligned with other plots)
    rewards = TRAINING_METRICS['episode_rewards']
    episode_timesteps = TRAINING_METRICS['episode_timesteps'][:len(rewards)]
    ...
    print(f"Episode timesteps range: {min(episode_timesteps) if episode_timesteps else 0} to {max(episode_timesteps) if episode_timesteps else 0}")
    
    # Create comprehensive figure with 3x2 subplots
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.suptitle(f'Training Progress - {TRAINING_METRICS.get("method_name", "RL Agent")} ({len(rewards)} Episodes)', 
                 fontsize=16, fontweight='bold')
    
    # Plot 1: Episode Rewards over Timesteps (Top Left) - MAIN PLOT - NOW ALIGNED
    axes[0, 0].plot(episode_timesteps, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward')
    
    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Timesteps')  # Changed from 'Episode Number'
