...
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    proc_time_variance_range=(1, 50),  # HUGE variance (was 5-30) - breaks SPT/LPT
    seed=GLOBAL_SEED
)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        # OPTIMIZED hyperparameters for FJSP scheduling with 20 JOBS
        # Key insights:
        # 1. Larger network for complex scheduling decisions
        # 2. More training epochs per rollout for sample efficiency
        # 3. HIGHER entropy needed for 20-job exploration (not lower!)
        ...
        model = MaskablePPO(
            ...
            n_epochs=15,               # ⭐ INCREASED: More epochs for 20-job complexity
            gamma=1.0,                 # Undiscounted (makespan)
            gae_lambda=0.95,           # GAE for advantage estimation
            clip_range=0.2,            # Standard PPO clipping
            ent_coef=0.05,             # ⭐ FIXED! Higher entropy for 20-job exploration (was 0.02, too low!)
...
def main():
    ...
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 2000000    # 🔥 INCREASED! Perfect RL faces FULL 20-job problem (harder than proactive's 3-5 initial jobs)
    dynamic_timesteps = 1000000  # Increased from 300k - Reactive needs more time
    ...
    print(f"   Perfect RL: {perfect_timesteps:,} timesteps (scenario-specific) [⚠️  LONGEST - faces full 20-job problem!]")
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        perfect_model = train_perfect_knowledge_agent(
            ...
            reward_mode="makespan_increment", 
            learning_rate=learning_rate,
            num_initializations=5,  # ⭐ INCREASED: More initializations for 20-job complexity
