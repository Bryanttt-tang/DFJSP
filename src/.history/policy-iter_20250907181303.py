...
# =============================================================================
# CONFIGURATION - Test both reward modes on large dataset
# =============================================================================
TEST_INSTANCE = "large"  # 7 jobs, 3-4 ops each (25,600 states)
REWARD_MODES = ["processing_time", "makespan_increment"]

# =============================================================================
# PROBLEM INSTANCES
# =============================================================================

# Large instance from test3.py (7 jobs, all arrive at t=0)
LARGE_JOBS_DATA = collections.OrderedDict({
...
LARGE_MACHINES = ['M0', 'M1', 'M2']
LARGE_ARRIVALS = {i: 0 for i in range(7)}  # All jobs arrive at t=0

class SimplifiedFJSPEnv:
    def __init__(self, jobs_data, machine_list, reward_mode="processing_time", job_arrival_times=None):
        self.jobs = jobs_data
        self.machines = machine_list
        self.job_ids = list(self.jobs.keys())
        self.num_jobs = len(self.job_ids)
        self.total_operations = sum(len(ops) for ops in self.jobs.values())
        self.reward_mode = reward_mode  # "processing_time" or "makespan_increment"
...
def print_policy_step(iteration, V, policy):
    ...
    for state, action in sorted(policy.items()):
        if action:
            print(f"  π{state}: J{action[0]}-Op{action[1]+1} on {action[2]}")
        else:
            print(f"  π{state}: Terminal")
    print("-" * 60)

def policy_iteration(env, gamma=1, theta=1e-6, reward_mode="makespan_increment"):
    print("=== POLICY ITERATION: STEP-BY-STEP DEMONSTRATION ===\n")
    print(f"Reward mode: {reward_mode}")
    if reward_mode == "makespan_increment":
        print("Reward R(s,a) = E(t) - E(t+1) where E(t) is max completion time")
        print("This encourages actions that minimize makespan growth")
    else:
        print("Reward R(s,a) = -processing_time")
        print("This encourages selecting operations with shorter processing times")
    print()
    
    # Generate all possible states dynamically based on job structure
    # State: (J1_ops_done, J2_ops_done) where each can range from 0 to total_ops_for_that_job
    all_states = []
    job_op_counts = [len(env.jobs[job_id]) for job_id in env.job_ids]
    
    # Generate all combinations of operation completion states
    ...
    print(f"Job structure: {[(f'J{job_id}', f'{len(ops)} ops') for job_id, ops in env.jobs.items()]}")
    print(f"State space: {all_states}")
    print(f"Total states: {len(all_states)}")
    print(f"Terminal state: {terminal_state}")
    print(f"Job arrivals: {env.job_arrival_times}")
    print()
    ...
    # Initialize policy with first available action
    ...
    for s in all_states:
        ...
        temp_env.state = s
        # Reset environment timing to match state
        temp_env._sync_environment_to_state(s)
        # Use maximum arrival time to ensure all jobs are considered available
        ...
        if valid_actions:
            ...
            print(f"  π{s} initialized to: J{valid_actions[0][0][0]}-Op{valid_actions[0][0][1]+1} on {valid_actions[0][0][2]}")
    
    print_policy_step(0, V, policy)
    ...
    while True:
        ...
        print("Calculating value function V(s) = E[G_t | S_t = s, π]")
        print("For each state, computing: V(s) = R(s,π(s)) + γ * V(s')")
        print()
