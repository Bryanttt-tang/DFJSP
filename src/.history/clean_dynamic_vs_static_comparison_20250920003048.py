...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        """
        Reset the environment for a new episode - CALLED AUTOMATICALLY BY PPO.
        
        TRAINING FLOW:
        1. model.learn() starts training
        2. PPO framework calls env.reset() at start of each episode
        3. This reset() method is called
        4. _reset_state() -> _generate_poisson_arrivals() creates new random arrivals
        5. Different arrival pattern for each episode = training diversity
        """
        ...
        # Note: _reset_state() calls _generate_poisson_arrivals() which uses np.random.exponential()
        # This creates different arrival patterns each episode (which is desired for training diversity)
        self._reset_state()
...
def generate_test_scenarios(jobs_data, initial_jobs=[0, 1, 2, 3, 4], arrival_rate=0.08, num_scenarios=10):
    ...
    Uses completely different seed range from training to avoid overfitting.
    """
    print(f"Generating {num_scenarios} test scenarios from {len(jobs_data)} total jobs...")
    print(f"Using test seed range: 1000-{1000 + num_scenarios - 1} (separate from training seeds)")
    
    scenarios = []
    for i in range(num_scenarios):
        np.random.seed(1000 + i)  # Use 1000+ range to avoid overlap with training seeds (0-999)
        arrival_times = {}
        ...
        if i < 5:
            ...
            current_initial = random.sample(all_jobs, 3)  # Use 3 initial jobs like first scenarios
...
def main():
    ...
    # Create separate training scenario for perfect knowledge (different from test scenarios)
    print("Creating separate training scenario for Perfect Knowledge RL...")
    np.random.seed(500)  # Use seed 500 for training (separate from test seeds 1000+)
    training_arrival_times = {}
    
    # Initial jobs (same as test scenarios for consistency)
    for job_id in [0, 1, 2]:
        training_arrival_times[job_id] = 0.0
    
    # Generate training arrivals for remaining jobs
    remaining_jobs = [j for j in ENHANCED_JOBS_DATA.keys() if j not in [0, 1, 2]]
    current_time = 0.0
    for job_id in remaining_jobs:
        inter_arrival_time = np.random.exponential(1.0 / arrival_rate)
        current_time += inter_arrival_time
        ...
        if integer_arrival_time <= 300:
            training_arrival_times[job_id] = float(integer_arrival_time)
        else:
            training_arrival_times[job_id] = float('inf')
    
    print(f"Training scenario arrivals: {training_arrival_times}")
    print("This ensures Perfect Knowledge RL doesn't overfit to test scenarios")
    ...
    perfect_model = train_perfect_knowledge_agent(ENHANCED_JOBS_DATA, MACHINE_LIST, 
                                                 arrival_times=training_arrival_times, 
    ...
    # Step 9: Create Gantt Charts for First 3 Test Scenarios
    print(f"\n9. GANTT CHARTS FOR FIRST 3 TEST SCENARIOS")
    print("-" * 60)
    
    for scenario_idx in range(min(3, len(gantt_scenarios_data))):
        scenario_data = gantt_scenarios_data[scenario_idx]
        scenario_id = scenario_data['scenario_id']
        arrival_times = scenario_data['arrival_times']
        schedules = scenario_data['schedules']
        ...
        if schedules['MILP Optimal'][0] != float('inf'):
            fig.suptitle(f'Test Scenario {scenario_id + 1} - Scheduling Methods Comparison\n' + 
            ...
            fig.suptitle(f'Test Scenario {scenario_id + 1} - RL Methods Comparison\n' + 
                         f'Arrival Times: {arrival_times}', 
                         fontsize=14, fontweight='bold')
            methods_to_plot = [
                ('Perfect Knowledge RL', schedules['Perfect Knowledge RL']),
        ...
        # Save scenario-specific Gantt chart
        scenario_filename = f'test_scenario_{scenario_id + 1}_gantt_comparison.png'
