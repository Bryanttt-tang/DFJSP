...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        Wait Action & Reward Design (CRITICAL):
        ---------------------------------------
        REWARD: Always -makespan_increment (whether wait or schedule)
        
        This creates temporal credit assignment challenges:
        
        Example:
          Path A: Schedule long job (50) on SLOW machine immediately
            → makespan += 50 * 1.5 = 75
            → reward = -75 ❌
            
          Path B: Wait (10) + Schedule long job (50) on FAST machine  
            → Wait: makespan += 0 (if machines busy) or 10 (if idle)
            → reward_wait = 0 or -10
            → Schedule: makespan += 50 * 0.7 = 35
            → reward_schedule = -35
            → CUMULATIVE: -10 + -35 = -45 ✓ (Better than -75!)
        
        The agent learns through:
        - gamma=1.0 (no discounting) → learns cumulative rewards
        - Temporal patterns: "wait then fast schedule" = better long-term
        - Value function: Q(s, wait) includes future fast scheduling benefit
        
        Why this works:
        - PPO learns Q-values that capture multi-step returns
        - Agent discovers: immediate small penalty (wait) + later big savings (fast) > immediate big penalty (slow)
        - Makespan is monotonic → clear optimization signal
        
        Wait Actions (SIMPLIFIED):
        -------------------------
        Only 2 wait options:
        - Wait 10 time units (strategic wait for known delays)
        - Wait to next event (adaptive wait for arrivals/completions)
        
        Simpler than 6 options [1,2,3,5,10,inf] to reduce complexity.
