...
def plot_training_metrics():
    ...
    # Plot 5: Entropy Loss over Rollouts (Bottom Left)
    if TRAINING_METRICS['action_entropy']:
        entropy_loss = TRAINING_METRICS['action_entropy'][:num_rollouts]  # Ensure same length
        
        axes[2, 0].plot(rollout_numbers, entropy_loss, 'orange', linewidth=2, alpha=0.7, label='Entropy Loss')
        axes[2, 0].legend()
    ...
    # Plot 6: Episode Length Mean over Rollouts (Bottom Right)
    if TRAINING_METRICS['episode_lengths']:
        ep_lengths = TRAINING_METRICS['episode_lengths'][:num_rollouts]  # Ensure same length
        length_rollouts = rollout_numbers[:len(ep_lengths)]
        
        axes[2, 1].plot(length_rollouts, ep_lengths, 'teal', linewidth=2, alpha=0.7, label='Episode Length Mean')
        axes[2, 1].legend()
        
        axes[2, 1].set_xlabel('Rollout Number')
        axes[2, 1].set_ylabel('Episode Length Mean')
        axes[2, 1].set_title('Episode Length Mean (Steps per Episode)')
