...
print(f"Problem size: {J} jobs, {M} machines, {total_operations} total operations")
...
def add_state(s):
    if s not in state_to_id:
        ...
        if len(id_to_state) % 10000 == 0:
            ...
            print(f"  States enumerated: {len(id_to_state)}, Queue size: {len(q)}, Elapsed: {elapsed:.1f}s")
...
q = deque([init_state])
states_processed = 0
max_states = 1000000  # Safety limit
timeout_seconds = 300  # 5 minute timeout

while q and len(id_to_state) < max_states:
    # Check timeout
    if time.time() - start_time > timeout_seconds:
        print(f"\nTIMEOUT: Stopping after {timeout_seconds}s with {len(id_to_state)} states")
        break
        
    s = q.popleft()
    states_processed += 1
    
    # Print progress every 1000 processed states
    if states_processed % 1000 == 0:
        elapsed = time.time() - start_time
        print(f"  Processed: {states_processed}, Total states: {len(id_to_state)}, Queue: {len(q)}, Time: {elapsed:.1f}s")
