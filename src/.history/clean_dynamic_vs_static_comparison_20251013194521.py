...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # 5. DYNAMIC RL: Arrival pattern features
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _is_valid_action(self, job_idx, op_idx, machine_idx):
        """Perfect knowledge validation: arrival time + precedence + compatibility."""
        if not (0 <= job_idx < self.num_jobs and 0 <= machine_idx < len(self.machines)):
            return False
        ...
        if op_idx == 0:
            # First operation: check if job has arrived
            job_arrival_time = self.job_arrival_times.get(job_id, 0.0)
            if self.current_makespan < job_arrival_time:
                return False
        else:
            # Later operation: check if previous operation completed
            if not self.completed_ops[job_id][op_idx - 1]:
                return False
    ...
    def action_masks(self):
        """Perfect knowledge action masks: arrival time + precedence + compatibility."""
        mask = np.full(self.action_space.n, False, dtype=bool)
        
        if self.operations_scheduled >= self.total_operations:
            return mask
        
        # Check all jobs (perfect knowledge allows scheduling any job that has arrived)
        for job_idx, job_id in enumerate(self.job_ids):
            next_op_idx = self.next_operation[job_id]
            if next_op_idx >= len(self.jobs[job_id]):
                continue
            
            # Check if job can start its next operation (arrival time constraint)
            if next_op_idx == 0:
                # First operation: check if job has arrived
                job_arrival_time = self.job_arrival_times.get(job_id, 0.0)
                job_can_start = self.current_makespan >= job_arrival_time
            else:
                # Later operation: check if previous operation completed
                job_can_start = self.completed_ops[job_id][next_op_idx - 1]
            
            if not job_can_start:
                continue
