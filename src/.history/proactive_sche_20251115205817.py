...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def action_masks(self):
        ...
        # Wait actions: Enable if there are incomplete jobs (not all jobs completed)
        # CRITICAL FIX: Must allow waiting even when no immediate scheduling actions available
        # because jobs might be blocked on precedence constraints while machines are busy.
        # Example: J3-O2 can't run until J3-O1 completes, but all machines busy -> need to wait!
        has_incomplete_jobs = len(self.completed_jobs) < len(self.job_ids)
