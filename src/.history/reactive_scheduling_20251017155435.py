...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        n_steps=92,              # IDENTICAL across all RL methods
        batch_size=23,            # IDENTICAL across all RL methods
        n_epochs=2,               # IDENTICAL across all RL methods (no special case for late arrivals)
        gamma=1,                # IDENTICAL across all RL methods
        gae_lambda=0.99,
        clip_range=0.2,
        ent_coef=0.01,             # IDENTICAL across all RL methods
