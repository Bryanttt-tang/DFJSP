...
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    proc_time_variance_range=(3, 20),  # Wide range creates wait opportunities
    seed=GLOBAL_SEED
)
  ...
  # Same as reactive RL for fair comparison
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        if milp_makespan != float('inf') and perfect_makespan < milp_makespan - TOLERANCE:
            ...
            print(f"      1. MILP solver might have terminated early (check solver status)")
            print(f"      2. MILP time limit too strict (check if optimal was proven)")
            print(f"      3. Different constraint interpretations (check schedule details)")
            print(f"      4. Numerical precision in MILP solver")
            ...
            if perfect_schedule and milp_schedule:
                ...
                for machine, ops in sorted(perfect_schedule.items()):
                    if ops:
                        print(f"    {machine}: {len(ops)} operations, last ends at {max(op[2] for op in ops):.2f}")
            ...
            print(f"\n  💡 RECOMMENDATION: If gap < 2.0, likely due to MILP solver settings.")
            print(f"     Consider: Increase MILP time limit, check solver termination status.")
            print(f"     If gap > 2.0, investigate constraint violations in RL schedule.")
            
            if gap < 2.0:
                print(f"\n  ⚠️  Gap is small ({gap:.2f}), continuing with warning instead of halting...")
                print(f"     MILP might be sub-optimal due to time limits or numerical issues.")
            else:
                print(f"\n  HALTING EXECUTION due to large gap ({gap:.2f}) - requires investigation...")
                exit(1)
