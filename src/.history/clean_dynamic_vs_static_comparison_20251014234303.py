...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        We'll snapshot them and also compute mean reward for episodes that finished during this rollout.
        """
...
def plot_gantt_chart_dynamic_rl(schedule, arrival_times, title="Dynamic RL Gantt Chart"):
    colors = plt.cm.tab20.colors
    fig, ax = plt.subplots(figsize=(14, 4))
    MACHINE_LIST = list(schedule.keys())
    max_makespan = 0
    for machine_ops in schedule.values():
        if machine_ops:
            max_makespan = max(max_makespan, max([op[2] for op in machine_ops]))
    x_limit = max_makespan * 1.1 if max_makespan > 0 else 100
...
def main():
    ...
    # --- Show Dynamic RL Gantt charts for each test scenario ---
    print("\nDYNAMIC RL GANTT CHARTS FOR ALL TEST SCENARIOS")
    for i, scenario in enumerate(test_scenarios):
        ...
        # Use existing plotting logic for a single schedule
    plot_gantt_chart_dynamic_rl(dynamic_schedule, scenario_arrivals,
             title=f"Dynamic RL - Test Scenario {i+1} (Makespan: {dynamic_makespan:.2f})")
