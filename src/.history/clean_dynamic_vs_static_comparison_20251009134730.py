...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        TRAINING_METRICS['method_name'] = self.method_name
        if self.verbose:
            print(f"[{self.method_name}] Callback init. Will record train scalars every {self.log_every_n_steps} steps.")
    ...
    def _on_step(self) -> bool:
        ...
        # ----- 2) periodically record train/* scalars from logger.name_to_value -----
        if getattr(self.model, "num_timesteps", 0) % self.log_every_n_steps == 0:
            if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
                ...
                # always record the timestep for these training samples
