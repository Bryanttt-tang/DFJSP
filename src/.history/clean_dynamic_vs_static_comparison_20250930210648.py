...
class StaticFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        print(f"[StaticFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={machine}, arrived_jobs={sorted(self.arrived_jobs)}")

        # Use softer invalid action handling like PerfectKnowledgeFJSPEnv
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            print(f"[StaticFJSPEnv] Invalid action: job={job_id}, op={op_idx}, machine={machine}")
        ...
        print(f"[StaticFJSPEnv] Scheduled: J{job_id}-O{op_idx+1} on {machine} from {start_time} to {end_time}")
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        job_id = self.job_ids[job_idx]
        machine = self.machines[machine_idx]
        print(f"[PoissonDynamicFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={machine}, arrived_jobs={sorted(self.arrived_jobs)}")

        # Use softer invalid action handling like successful environments
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            print(f"[PoissonDynamicFJSPEnv] Invalid action: job={job_id}, op={op_idx}, machine={machine}")
            return self._get_observation(), -50.0, False, False, {"error": "Invalid action, continuing"}
        ...
        print(f"[PoissonDynamicFJSPEnv] Scheduled: J{job_id}-O{op_idx+1} on {machine} from {start_time} to {end_time}")
        if newly_arrived:
            print(f"[PoissonDynamicFJSPEnv] Newly arrived jobs: {newly_arrived}")
        return self._get_observation(), reward, terminated, False, info
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        job_id = self.job_ids[job_idx]
        machine = self.machines[machine_idx]
        print(f"[PerfectKnowledgeFJSPEnv] Step {self.episode_step}: job={job_id}, op={op_idx}, machine={machine}, arrived_jobs={sorted(self.arrived_jobs)}")

        # Use softer invalid action handling like test3_backup.py
        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            print(f"[PerfectKnowledgeFJSPEnv] Invalid action: job={job_id}, op={op_idx}, machine={machine}")
            return self._get_observation(), -50.0, False, False, {"error": "Invalid action, continuing"}
        ...
        info = {"makespan": self.current_makespan}
        print(f"[PerfectKnowledgeFJSPEnv] Scheduled: J{job_id}-O{op_idx+1} on {machine} from {start_time} to {end_time}")
        if newly_arrived:
            print(f"[PerfectKnowledgeFJSPEnv] Newly arrived jobs: {newly_arrived}")
        return self._get_observation(), reward, terminated, False, info
...
def main():
    ...
    # Skip the old static RL comparison code - focus on the 10 test scenarios
