                ...
                for idx in range(self._processed_episodes, current_len):
                    ...
                    if self.verbose >= 1 and len(TRAINING_METRICS['episode_rewards']) <= 5:
                        print(f"[{self.method_name}] Episode {len(TRAINING_METRICS['episode_rewards'])}: "
                              f"reward={episode_reward:.2f}, policy_loss={self._latest_train_metrics['policy_loss']:.4f if self._latest_train_metrics['policy_loss'] is not None else 'N/A'}")
...
def plot_training_metrics():
    ...
    axes[0, 0].plot(episode_numbers, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward')
    
    # Add moving average for trend
    if len(rewards) > 10:
        window_size = min(max(10, len(rewards) // 20), 50)  # Adaptive window size
        if window_size < len(rewards):
            moving_avg = np.convolve(rewards, np.ones(window_size)/window_size, mode='valid')
            moving_episodes = episode_numbers[window_size-1:]
            axes[0, 0].plot(moving_episodes, moving_avg, 'darkgreen', linewidth=3, 
    ...
    if TRAINING_METRICS['total_loss'] and TRAINING_METRICS['timesteps']:
        ...
        if len(total_loss) > 10:
            window_size = min(20, len(total_loss) // 4)
            if window_size > 0:
                ...
                axes[0, 1].plot(moving_timesteps, moving_avg, 'darkmagenta', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[0, 1].legend()
    ...
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['timesteps']:
        ...
        if len(policy_loss) > 10:
            ...
            if window_size > 0:
                ...
                moving_timesteps = loss_timesteps[window_size-1:]
                axes[1, 0].plot(moving_timesteps, moving_avg, 'darkred', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[1, 0].legend()
    ...
    if TRAINING_METRICS['value_loss'] and TRAINING_METRICS['timesteps']:
        ...
        if len(value_loss) > 10:
            ...
            if window_size > 0:
                ...
                moving_timesteps = loss_timesteps[window_size-1:]
                axes[1, 1].plot(moving_timesteps, moving_avg, 'darkblue', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[1, 1].legend()
    ...
    if TRAINING_METRICS['action_entropy'] and TRAINING_METRICS['timesteps']:
        ...
        if len(entropy_loss) > 10:
            window_size = min(20, len(entropy_loss) // 4)
            if window_size > 0:
                moving_avg = np.convolve(entropy_loss, np.ones(window_size)/window_size, mode='valid')
                moving_timesteps = loss_timesteps[window_size-1:]
                axes[2, 0].plot(moving_timesteps, moving_avg, 'darkorange', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[2, 0].legend()
    ...
    if TRAINING_METRICS['episode_lengths']:
        ...
        axes[2, 1].plot(length_episodes, ep_lengths, 'teal', linewidth=2, alpha=0.7, label='Episode Length')
        
        # Add moving average
        if len(ep_lengths) > 10:
            window_size = min(20, len(ep_lengths) // 4)
            if window_size > 0:
                moving_avg = np.convolve(ep_lengths, np.ones(window_size)/window_size, mode='valid')
                moving_episodes = length_episodes[window_size-1:]
                axes[2, 1].plot(moving_episodes, moving_avg, 'darkslategray', linewidth=3,
                               label=f'Moving Avg (window={window_size})')
                axes[2, 1].legend()
