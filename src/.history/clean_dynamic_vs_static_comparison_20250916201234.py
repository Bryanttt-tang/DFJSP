...
class StaticFJSPEnv(gym.Env):
    ...
    Simplified version based on the working PerfectKnowledgeFJSPEnv.
    """
    ...
    def __init__(self, jobs_data, machine_list, reward_mode="makespan_increment", seed=None):
        ...
        # Create arrival times dict with all jobs at t=0
        ...
        self.action_space = spaces.Discrete(self.num_jobs * self.max_ops_per_job * len(self.machines))
    ...
    def action_masks(self):
        ...
        for action in range(self.action_space.n):
            ...
            mask[action] = self._is_valid_action(job_idx, op_idx, machine_idx)
    ...
    def step(self, action):
        ...
        # Calculate timing (all jobs available at t=0, so no arrival time check needed)
        ...
        self.machine_workload[machine_name] += proc_time
        ...
        # Record in schedule
        self.schedule[machine_name].append((f"J{job_id}-O{op_idx+1}", start_time, end_time))
...
class PoissonDynamicFJSPEnv(gym.Env):
    """
    Simplified Dynamic FJSP Environment based on the working DynamicFJSPEnv from test3_backup.py
    but with Poisson-generated arrival times instead of fixed arrival times.
    """
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        """Initialize simplified Dynamic FJSP Environment."""
        super().__init__()
        ...
        # Action and observation spaces (simplified like test3_backup.py)
    ...
    def reset(self, seed=None, options=None):
        """Reset environment - simplified like test3_backup.py"""
        if seed is not None:
            super().reset(seed=seed, options=options)
            random.seed(seed)
            np.random.seed(seed)
    ...
    def _decode_action(self, action):
        """Decode action - simplified like test3_backup.py"""
        action = int(action) % self.action_space.n
    ...
    def _is_valid_action(self, job_idx, op_idx, machine_idx):
        """Check if action is valid - simplified like test3_backup.py"""
        if not (0 <= job_idx < self.num_jobs and 0 <= machine_idx < len(self.machines)):
            return False
    ...
    def action_masks(self):
        """Generate action masks - simplified like test3_backup.py"""
        ...
        # If no valid actions, enable all actions as fallback
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, reward_mode="makespan_increment"):
    ...
    def make_perfect_env():
        # Use StaticFJSPEnv but with deterministic arrivals - simpler and matches test3_backup.py
        # Create a simple environment that uses the exact arrival times for training
        env = SimpleDeterministicFJSPEnv(jobs_data, machine_list, arrival_times, reward_mode=reward_mode)
    ...
    model = MaskablePPO(
        ...
        n_epochs=20,               # Matches test3_backup.py
        gamma=0.99,                # Matches test3_backup.py
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,
        vf_coef=0.5,
...
class SimpleDeterministicFJSPEnv(gym.Env):
    """
    Simple deterministic FJSP environment based on test3_backup.py's DynamicFJSPEnv.
    
    This uses the exact same structure as the working environment from test3_backup.py
    but with deterministic arrival times. This should give performance close to MILP optimal.
    """
    metadata = {"render.modes": ["human"]}
    ...
    def __init__(self, jobs_data, machine_list, job_arrival_times=None, reward_mode="makespan_increment"):
        ...
        # Action space: same as test3_backup.py
        self.action_space = spaces.Discrete(
            min(self.num_jobs * self.max_ops_per_job * len(self.machines), 1000)
        )
        
        # Observation space: same as test3_backup.py
        obs_size = (
            len(self.machines) +                    # Machine availability
        ...
        for job_idx, job_id in enumerate(self.job_ids):
            if job_id in self.arrived_jobs and self.next_operation[job_id] < len(self.jobs[job_id]):
                ...
                for machine_idx, machine in enumerate(self.machines):
                    if machine in self.jobs[job_id][op_idx]['proc_times']:
                        action = job_idx * (self.max_ops_per_job * len(self.machines)) + op_idx * len(self.machines) + machine_idx
                        if action < self.action_space.n:
                            mask[action] = True
                            valid_action_count += 1
    ...
    def step(self, action):
        ...
        if self.episode_step >= self.max_episode_steps:
            return self._get_observation(), 0, True, False, {"makespan": self.current_makespan}
        
        job_idx, op_idx, machine_idx = self._decode_action(action)

        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            return self._get_observation(), -100, False, False, {"makespan": self.current_makespan, "invalid": True}
        ...
        # Calculate reward (same as test3_backup.py)
    ...
    def _calculate_reward(self, proc_time, idle_time, done):
        if self.reward_mode == "makespan_increment":
            # Focus on makespan reduction
            reward = -proc_time - idle_time * 2.0
            if done:
                reward -= self.current_makespan * 0.1
            return reward
        else:
            # Standard reward
            reward = -proc_time - idle_time
            if done:
                reward += 100.0
            return reward

    def _get_observation(self):
        # Same observation structure as test3_backup.py
        ...
        for m in self.machines:
            norm_avail = self.machine_next_free.get(m, 0.0) / norm_factor
            obs.append(max(0.0, min(1.0, norm_avail)))
        
        # Operation completion status
        for job_id in self.job_ids:
            job_ops = self.completed_ops[job_id]
            # Pad to max_ops_per_job
            for i in range(self.max_ops_per_job):
                if i < len(job_ops):
                    obs.append(1.0 if job_ops[i] else 0.0)
                else:
                    obs.append(0.0)
        ...
        makespan_norm = self.current_makespan / norm_factor
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def step(self, action):
        """Step function - same as test3_backup.py"""
        self.episode_step += 1
        
        if self.episode_step >= self.max_episode_steps:
            return self._get_observation(), -1000, True, False, {"makespan": self.current_makespan}
        
        job_idx, op_idx, machine_idx = self._decode_action(action)

        if not self._is_valid_action(job_idx, op_idx, machine_idx):
            return self._get_observation(), -100, False, False, {"makespan": self.current_makespan}
    ...
    def _calculate_reward(self, proc_time, idle_time, done):
        """Reward calculation - same as test3_backup.py"""
        if self.reward_mode == "machine_utilization":
            reward = -proc_time - idle_time * 2.0
            if done:
                reward += max(0, 200 - self.current_makespan)
        elif self.reward_mode == "makespan_increment":
            reward = self.previous_makespan-self.current_makespan - 
        else:
            reward = -proc_time - idle_time
            if done:
                reward += 100
...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    """Evaluate static model on its own StaticFJSPEnv (all jobs at t=0)."""
    print(f"  Static RL evaluation (all jobs available at t=0)...")
    
    # Create static environment for evaluation (ignores arrival_times - all jobs at t=0)
...
def main():
    ...
    dynamic_timesteps = 100000   # Dynamic RL needs more time to learn patterns
    ...
    print(f"Static RL            - Makespan: {static_makespan:.2f}")
    ...
    if milp_makespan != float('inf'):
        print(f"Actual: {milp_makespan:.2f} ≤ {perfect_makespan:.2f} ≤ {dynamic_makespan:.2f} ≤ {static_makespan:.2f}")
    else:
        print(f"Actual (no MILP): {perfect_makespan:.2f} ≤ {dynamic_makespan:.2f} ≤ {static_makespan:.2f}")
    ...
    # Step 5.9: Static RL Comparison (Dynamic vs Static scenario)
    print("\n5.9. STATIC RL COMPARISON: DYNAMIC SCENARIO vs STATIC SCENARIO")
    print("-" * 60)
    print("Comparing Static RL performance when:")
    print("1. Evaluated on dynamic scenario (some jobs arrive later)")
    print("2. Evaluated on static scenario (all jobs available at t=0)")
    
    # Evaluate Static RL on pure static scenario (all jobs at t=0)
    print("Evaluating Static RL on pure static scenario...")
    static_scenario_arrivals = {job_id: 0.0 for job_id in ENHANCED_JOBS_DATA.keys()}
    static_on_static_makespan, static_on_static_schedule = evaluate_static_on_dynamic(
        static_model, ENHANCED_JOBS_DATA, MACHINE_LIST, static_scenario_arrivals)
    
    print(f"Static RL on Dynamic scenario: {static_makespan:.2f}")
    print(f"Static RL on Static scenario:  {static_on_static_makespan:.2f}")
    
    if static_on_static_makespan > 0:
        performance_diff = static_makespan - static_on_static_makespan
        if performance_diff > 0:
            print(f"✓ Static RL performs better on static scenario by {performance_diff:.2f} time units")
        else:
            print(f"⚠️ Static RL performs worse on static scenario by {abs(performance_diff):.2f} time units")
    
    # Create Static RL comparison Gantt chart
    print("\nCreating Static RL comparison Gantt chart...")
    fig_static, axes_static = plt.subplots(2, 1, figsize=(18, 7))
    fig_static.suptitle('Static RL Performance Comparison: Dynamic Scenario vs Static Scenario\n' + 
                       f'Understanding why Static RL struggles with dynamic arrivals', 
                       fontsize=16, fontweight='bold')
    
    colors = plt.cm.tab20.colors
    
    # Plot 1: Static RL on Dynamic scenario
    ax1 = axes_static[0]
    scenario_data = [
        {'schedule': static_schedule, 'makespan': static_makespan, 'title': 'Static RL on Dynamic Scenario', 'arrival_times': first_scenario_arrivals},
        {'schedule': static_on_static_schedule, 'makespan': static_on_static_makespan, 'title': 'Static RL on Static Scenario', 'arrival_times': static_scenario_arrivals}
    ]
    
    for plot_idx, data in enumerate(scenario_data):
        ...
        if arrival_times and plot_idx == 0:  # Only for dynamic scenario
            arrow_y_position = len(MACHINE_LIST) + 0.3
            for job_id, arrival_time in arrival_times.items():
                if arrival_time > 0 and arrival_time < 200:
    ...
    plt.savefig('static_rl_dynamic_vs_static_comparison.png', dpi=300, bbox_inches='tight')
    print(f"✅ Saved Static RL comparison: static_rl_dynamic_vs_static_comparison.png")
    plt.show()
    ...
    # Determine number of plots based on MILP availability
    num_plots = 5 if milp_makespan != float('inf') else 4
    fig, axes = plt.subplots(num_plots, 1, figsize=(18, num_plots * 3.5))
    
    if milp_makespan != float('inf'):
        fig.suptitle('Scheduling Comparison: MILP Optimal vs Perfect Knowledge RL vs Dynamic RL vs Static RL vs SPT\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'MILP provides theoretical optimal benchmark for regret analysis', 
                     fontsize=16, fontweight='bold')
        schedules_data = [
        ...
        fig.suptitle('Scheduling Comparison: Perfect Knowledge RL vs Dynamic RL vs Static RL vs SPT\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'(MILP optimal solution not available)', 
                     fontsize=16, fontweight='bold')
        schedules_data = [
            {'schedule': perfect_schedule, 'makespan': perfect_makespan, 'title': 'Perfect Knowledge RL', 'arrival_times': first_scenario_arrivals},
            {'schedule': dynamic_schedule, 'makespan': dynamic_makespan, 'title': 'Dynamic RL', 'arrival_times': first_scenario_arrivals},
    ...
    if milp_makespan != float('inf'):
        print("- complete_scheduling_comparison_with_milp_optimal.png: Five-method comparison with MILP benchmark")
        ...
        print("- dynamic_vs_static_gantt_comparison-7jobs.png: Four-method comparison")
        print("- arrival_distribution_analysis.png: Training arrival time analysis")
        ...
        print(f"• Performance hierarchy: {'✅ Expected' if perfect_makespan <= dynamic_makespan <= static_makespan else '❌ Unexpected'}")
    print("=" * 80)
