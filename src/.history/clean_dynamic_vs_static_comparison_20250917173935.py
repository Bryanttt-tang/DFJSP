    ...
    while operations_scheduled < total_operations:
        ...
        for job_id in arrived_jobs:
            ...
            if next_op_idx < len(jobs_data[job_id]):
                ...
                job_ready_time = (operation_end_times[job_id][next_op_idx - 1] if next_op_idx > 0 
        ...
        if not ready_operations:
            ...
            for job_id in arrived_jobs:
                ...
                if next_op_idx > 0 and next_op_idx < len(jobs_data[job_id]):
                    ready_time = operation_end_times[job_id][next_op_idx - 1]
        ...
        if job_sequencing_rule == 'SPT':
            ...
            selected_operation = min(ready_operations, 
                                   key=lambda op: min(op[2]['proc_times'].values()))
        elif job_sequencing_rule == 'LPT':
            # Select operation with longest processing time
            selected_operation = max(ready_operations, 
                                   key=lambda op: min(op[2]['proc_times'].values()))
        elif job_sequencing_rule == 'FIFO':
            # Select operation from job that arrived first
            selected_operation = min(ready_operations, 
                                   key=lambda op: arrival_times[op[0]])
        ...
        if machine_selection_rule == 'SPT':
            ...
            # Select machine with least work remaining (lowest total workload)
            selected_machine = min(available_machines, 
                                 key=lambda m: machine_workload[m])
...
def simple_spt_heuristic(jobs_data, machine_list, arrival_times):
    """
    Ultra-simple SPT heuristic based directly on test3_backup.py logic.
            ...
            if op_idx < len(jobs_data[job_id]):
                ...
                for machine_name, proc_time in op_data['proc_times'].items():
                    ...
                    candidate_operations.append((
                        proc_time,           # Key for SPT: shortest processing time first
                        earliest_start_time,
                        job_id, 
                        op_idx, 
                        machine_name
