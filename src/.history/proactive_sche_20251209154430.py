...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    KEY DESIGN:
    - Action space: 10 dispatching rule combinations (5 sequencing × 2 routing) + 1 WAIT
    ...
    - event_time controls the "frontier" of future job visibility
    
    Action Space (11 total: 10 scheduling + 1 wait):
    ...
    - 10: WAIT     - Advance time to next event without scheduling
    
    ==================================================================================
    EVENT-TIME MECHANISM & WAIT ACTION DESIGN:
    ==================================================================================
    
    RULE-BASED RL vs PROACTIVE RL - Wait Action Philosophy:
    
    RULE-BASED RL (this environment):
    ---------------------------------
    - Each action selects the CURRENT best dispatching rule for ready operations
    - WAIT action should be DISCOURAGED (only used when no jobs available)
    - event_time controls when future jobs become visible
    - After scheduling, if no ready ops remain, event_time auto-advances to next event
    - This auto-advancement is NOT a bug - it's the event-driven simulation
    - Reward = makespan increment (not affected by whether time advanced)
    
    The WAIT action exists only to help the agent advance to the next event when
    all current jobs are done but new jobs haven't arrived yet. Ideally, the agent
    should learn to AVOID wait and let the environment auto-advance instead.
    
    PROACTIVE RL (ProactiveDynamicFJSPEnv):
    ----------------------------------------
    - Each action schedules specific (operation, machine) pairs OR waits
    - WAIT action should be ENCOURAGED when beneficial (strategic waiting)
    - event_time also controls future job visibility
    - Agent learns to trade off short-term vs long-term benefits
    - Example: Wait for fast machine instead of using slow machine now
    - Reward design must handle temporal credit assignment carefully
    
    TEMPORAL CREDIT ASSIGNMENT CHALLENGE:
    -------------------------------------
    Both environments face the challenge that waiting changes the future state
    (new arrivals may occur), making it hard to attribute reward to wait decisions.
    
    Solution approaches:
    - Rule-Based RL: Discourage wait through action masking or reward shaping
    - Proactive RL: Design wait reward to capture opportunity cost of waiting
    ...
    def step(self, action):
        ...
        # ==================================================================================
        # CRITICAL: EVENT-TIME ADVANCEMENT AFTER SCHEDULING
        # ==================================================================================
        # This is NOT a bug - it's the correct behavior for event-based simulation.
        #
        # PURPOSE: event_time controls the "frontier" of what the agent can see:
        # - Prevents scheduling operations back in the past
        # - Reveals future job arrivals only when time advances
        # - Ensures sequential decision making with limited foresight
        #
        # LOGIC: After scheduling an operation, check if more ready operations exist.
        # If no ready ops AND jobs remain incomplete, advance event_time to next event.
        #
        # NOTE: This advancement changes the environment state (new arrivals may occur).
        # The reward is based on makespan increment, NOT on whether time advanced.
        # The temporal credit assignment challenge is handled by the wait action design:
        # - Rule-Based RL: Wait should be DISCOURAGED (each action selects best rule)
        # - Proactive RL: Wait should be ENCOURAGED when beneficial (long-term planning)
        # ==================================================================================
        
        # Always check for ready operations after scheduling
...
def train_rule_based_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    model = MaskablePPO(
        ...
        n_steps=1024,
        batch_size=256,            # Increased from 512
        n_epochs=5,
        gamma=1.0,
        gae_lambda=0.99,
        clip_range=0.2,
    ...
    print(f"⚠️  IMPORTANT: High entropy (0.05) to explore all {11} rule combinations")
...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, seq_rule, route_rule="MIN"):
    ...
    CRITICAL: Heuristic operates with LIMITED KNOWLEDGE (same as RL)
    ...
    =====================================================
    SEQUENTIAL DECISION MAKING (same as Rule-Based RL):
    - sim_time controls the frontier of when the agent sees future job arrivals
    - After each scheduling action, sim_time is updated if no ready ops remain
    - This ensures the agent cannot "see into the future" or schedule back in the past
    - Automatic time advancement happens when all arrived jobs are done but new jobs haven't arrived
    
    NOTE: This is NOT a bug - it mirrors the Rule-Based RL environment's event_time mechanism.
    The environment is event-based, so sim_time/event_time controls:
    1. When future jobs are revealed to the agent
    2. That the agent cannot schedule operations back in the past
    3. Automatic advancement to next event when no operations are available
    
    SEQUENTIAL DECISION FLOW:
    1. Check for new arrivals at current sim_time
    2. Identify ready operations (arrived + precedence satisfied)
    3. If no ready ops, advance sim_time to next event (arrival or machine free)
    4. Select best operation using sequencing rule
    5. Select best machine using routing rule
    6. Schedule the (operation, machine) pair
    7. After scheduling, check if more ready ops exist
    8. If no ready ops AND jobs remain, advance sim_time to next event
    ...
    while completed_operations < total_operations:
        ...
        # ==================================================================================
        # CRITICAL: EVENT-TIME ADVANCEMENT (mirrors Rule-Based RL's event_time mechanism)
        # ==================================================================================
        # This is NOT a bug - it's the correct behavior for sequential decision making.
        # 
        # PURPOSE: sim_time controls the "frontier" of what the agent can see:
        # - Prevents scheduling operations back in the past
        # - Reveals future job arrivals only when time advances
        # - Ensures fair comparison with RL agents (no perfect foresight)
        #
        # LOGIC: After each scheduling action, check if more ready operations exist.
        # If no ready ops AND jobs remain incomplete, advance sim_time to next event.
        # This is identical to Rule-Based RL's behavior (lines 1650-1658).
        # ==================================================================================
        ...
        # (same as Rule-Based RL advancing event_time when no ops ready)
...
def main():
    ...
    proactive_timesteps = 1500000  # Increased from 300k - Proactive needs even more (prediction + scheduling)
    rule_based_timesteps = 1000000  # Same as reactive - needs to explore all rule combos
    static_timesteps = 200000    # Increased for better learning
    learning_rate = 1e-3         # Optimized learning rate for PPO
