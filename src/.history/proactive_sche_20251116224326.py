...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, seq_rule, route_rule="MIN"):
    ...
    1. SEQUENCING: Which ready operation to prioritize? (Decides order)
       ...
       - EDD: Earliest due date first (score = arrival + remaining work)
    ...
    2. ROUTING: Which machine for the selected operation? (Decides assignment)
       ...
       - LEAST_LOADED: Least busy machine (score = machine_free_time + proc_time)
    
    This two-step structure allows testing all combinations:
    6 sequencing × 3 routing = 18 total strategies
    ...
    Args:
        ...
        seq_rule: Sequencing rule (FIFO, LIFO, SPT, LPT, EDD, MWKR)
        route_rule: Routing rule (MIN, MINC, LEAST_LOADED)
    ...
    while completed_operations < total_operations:
        ...
        def sequencing_score(op):
            ...
            if seq_rule == "FIFO":
                ...
                # Due date = arrival + remaining work * slack factor
                return (arr + rem * 1.5, j, oi)
            elif seq_rule == "MWKR":
                return (-rem, arr, j, oi)
            else:
        ...
        def routing_score(machine):
            ...
            elif route_rule == "LEAST_LOADED":
                # Least busy machine (earliest next free time + processing)
                return (machine_avail + proc_time, machine)
            else:
                return (proc_time, machine)  # Default to MIN
        
        best_machine = min(machine_list, key=routing_score)[1] if route_rule else \
                       min((m for m in machine_list if m in op_data['proc_times']), 
                           key=routing_score)
