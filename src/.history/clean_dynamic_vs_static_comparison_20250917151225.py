...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # Use similar action space as PerfectKnowledgeFJSPEnv for consistency
        self.action_space = spaces.Discrete(
            min(self.num_jobs * self.max_ops_per_job * len(self.machines), 1000)
        )
        
        # SIMPLIFIED observation space similar to PerfectKnowledgeFJSPEnv
        obs_size = (
            len(self.machines) +                    # Machine availability
            self.num_jobs * self.max_ops_per_job +  # Operation completion status
            self.num_jobs +                         # Job progress ratios  
            self.num_jobs +                         # Job arrival status (arrived or not)
    ...
    def step(self, action):
        """Step function - simplified based on working PerfectKnowledgeFJSPEnv."""
        self.episode_step += 1
        ...
        # Decode action using the same approach as working environments
        job_idx, op_idx, machine_idx = self._decode_action(action)
        ...
        # Calculate reward using simplified approach
        idle_time = max(0, start_time - machine_available_time)
    ...
    def _calculate_reward(self, proc_time, idle_time, done, num_new_arrivals, 
        ...
        """Simplified reward function based on working PerfectKnowledgeFJSPEnv approach."""
        
        if self.reward_mode == "makespan_increment":
            # Use the same simple approach as PerfectKnowledgeFJSPEnv
            if previous_time is not None and current_time is not None:
                makespan_increment = current_time - previous_time
                reward = -makespan_increment  # Negative increment
                
                # Small bonus for utilizing newly arrived jobs
                if num_new_arrivals > 0:
                    reward += 2.0 * num_new_arrivals  # Reduced from 5.0
    ...
    def _get_observation(self):
        """
        SIMPLIFIED observation generation based on working PerfectKnowledgeFJSPEnv approach.
        """
        norm_factor = max(self.current_time, 1.0)
        obs = []
        
        # 1. Machine availability (normalized by current time)
        for m in self.machines:
            value = float(self.machine_next_free.get(m, 0.0)) / norm_factor
            obs.append(max(0.0, min(1.0, value)))
        ...
        # 4. Job arrival status (simplified - just arrived or not)
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs:
                obs.append(1.0)  # Job has arrived
            else:
                obs.append(0.0)  # Job not yet arrived
            
        # 5. Current time (normalized)
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_time, current_time):
        """Simplified reward calculation based on working environments."""
        if self.reward_mode == "makespan_increment":
            # Simple makespan increment approach like PerfectKnowledgeFJSPEnv
            makespan_increment = current_time - previous_time
            reward = -makespan_increment  # Reward for not increasing makespan
            
            # Small bonus for task completion
            reward += 10.0
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=300000, reward_mode="makespan_increment"):
    """
    Train a dynamic RL agent using simplified approach based on successful environments.
    """
    print(f"\n--- Training Dynamic RL Agent on {len(jobs_data)} jobs ---")
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    
    def make_dynamic_env():
        # Simplified environment creation - no complex randomization
    ...
    # Use simpler hyperparameters like successful Static RL
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=0,
        learning_rate=1e-4,            # Same as Static RL
        n_steps=2048,                  # Reduced from 4096 to match successful patterns
        batch_size=256,                # Reduced from 512 for better learning
        n_epochs=10,                   # Keep the same
        gamma=1,                    # Same as working environments
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,
        vf_coef=0.5,
        max_grad_norm=0.5,
        policy_kwargs=dict(
            net_arch=[256, 128, 64],  # Same as Static RL - simpler is better
            activation_fn=torch.nn.ReLU
        )
    )
    ...
    # Simplified training loop like Static RL
    start_time = time.time()
    ...
    with tqdm(total=total_timesteps, desc="Dynamic RL", 
        ...
        # Break training into chunks for progress updates
        chunk_size = total_timesteps // 30  # 30 chunks like Static RL
        remaining_timesteps = total_timesteps
