...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # Dynamic action space: At any time, max valid actions = num_jobs * num_machines
        # (since each job can have at most one operation ready due to precedence)
    ...
    def _reset_state(self):
        ...
        # Dynamic action space: current valid actions (list of (job_id, machine_name) tuples)
        self.valid_actions = []
    ...
    def _update_valid_actions(self):
        ...
        KEY INSIGHT: Instead of complex decoding, maintain dynamic list of valid (job_id, machine_name) pairs.
        ...
        # If no valid actions, we need to advance time to next event
        if not self.valid_actions:
            self._advance_time_to_next_event()

    def _advance_time_to_next_event(self):
        """Advance time to the next event if no operations are ready"""
        future_events = []
        
        # Check for job arrivals
        for arrival_time, job_id in self.next_arrival_events:
            if arrival_time > self.current_time:
                future_events.append(arrival_time)
        
        # Check for operation completions that enable new operations
        for job_id in self.arrived_jobs:
            next_op_idx = self.next_operation[job_id]
            if next_op_idx > 0 and next_op_idx <= len(self.jobs[job_id]):
                prev_end_time = self.operation_end_times[job_id][next_op_idx - 1]
                if prev_end_time > self.current_time:
                    future_events.append(prev_end_time)
        
        if future_events:
            self.current_time = min(future_events)
            
            # Update arrived jobs for new arrivals at this time
    ...
    def step(self, action):
        ...
        if next_op_idx > 0:
            ...
            prev_op_end_time = self.arrival_times.get(job_id, 0.0)
        ...
        if terminated and self.next_arrival_events:
            ...
            if not any(self.next_operation[job_id] < len(self.jobs[job_id]) for job_id in self.arrived_jobs):
                ...
                if next_arrival_time <= self.max_time_horizon:
                    ...
                    terminated = False
                    self._update_valid_actions()  # Update actions after new arrivals
    ...
    def _calculate_reward(self, proc_time, idle_time, done, num_new_arrivals, 
        ...
        if self.reward_mode == "makespan_increment":
            ...
            """
            Combined reward mode that balances makespan minimization with machine utilization.
            ...
            makespan_increment = current_time - previous_time
            reward -= makespan_increment * 2.0  # Strong penalty for time progression
            
            # 2. Machine utilization rewards (secondary objective)
            reward -= idle_time * 3.0  # Strong penalty for idle time
            reward += 2.0  # Base reward for making progress
            
            # 3. Efficiency incentives
            reward -= proc_time * 0.1  # Small penalty for longer operations
            
            # 4. Dynamic adaptation bonuses
            if num_new_arrivals > 0:
                # Bonus for immediately utilizing newly arrived jobs
                ...
                if idle_machines > 0:
                    reward += idle_machines * 3.0
            
            # 5. Load balancing incentive
            machine_loads = [self.machine_next_free[m] for m in self.machines]
            if len(machine_loads) > 1 and max(machine_loads) > 0:
                ...
                if load_mean > 0:
                    ...
                    reward += load_balance_ratio * 2.0
            
            # 6. Future-aware scheduling
            if self.next_arrival_events:
                next_arrival_time = self.next_arrival_events[0][0]
                time_to_next_arrival = next_arrival_time - current_time
                ...
                if 0 < time_to_next_arrival <= 10.0:
                    available_machines = sum(1 for m in self.machines 
                                           if self.machine_next_free[m] <= current_time + 2.0)
                    reward += available_machines * 1.5
                
                # Penalty for creating long idle periods when more jobs are coming
            ...
            if self.next_operation[job_id] >= len(self.jobs[job_id]):
                ...
                if job_flow_time > 0:
                    flow_time_bonus = min(15.0 / job_flow_time, 8.0)
                    reward += flow_time_bonus
            
            # 8. Final completion rewards
            if done:
                # Base completion bonus
                reward += 150.0
                ...
                if current_time > 0:
                    makespan_bonus = min(300.0 / current_time, 80.0)
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=3, arrival_rate=0.08, total_timesteps=200000, reward_mode="combined_makespan_utilization"):
    ...
    print(f"Reward mode: {reward_mode} (combines makespan minimization + machine utilization)")
    ...
    # Estimate episodes based on typical episode length
    avg_episode_length = 50  # Typical steps per episode for this problem size
    estimated_episodes = total_timesteps // avg_episode_length
    print(f"Estimated training episodes: ~{estimated_episodes:,}")
    
    print(f"\nWhy this many episodes is sufficient for Poisson arrival learning:")
    print(f"• Each episode exposes agent to different arrival patterns")
    print(f"• With {estimated_episodes:,} episodes, agent sees diverse scenarios")
    print(f"• Poisson process has consistent statistical properties")
    print(f"• Agent learns to recognize patterns: early arrivals, late arrivals, clustered arrivals")
    print(f"• By episode ~{estimated_episodes//4:,}: basic adaptation learned")
    print(f"• By episode ~{estimated_episodes//2:,}: advanced utilization strategies developed")
    print(f"• By episode ~{3*estimated_episodes//4:,}: fine-tuning of dynamic responses")
    print(f"• Final episodes: policy refinement and edge case handling")
    ...
    # Enhanced hyperparameters for better learning with combined reward
    model = MaskablePPO(
        "MlpPolicy",
        vec_env,
        verbose=1,
        learning_rate=3e-4,        # Slightly higher for faster adaptation
        n_steps=2048,              # Good balance for experience collection
        batch_size=128,            # Smaller batch for more frequent updates
        n_epochs=10,               # Sufficient epochs for learning
    ...
    print(f"Agent has been exposed to ~{estimated_episodes:,} different Poisson arrival scenarios")
    print(f"This provides robust learning for dynamic job scheduling adaptability")
    return model
...
def main():
    ...
    print("\n0. TRAINING EPISODE ANALYSIS")
    print("-" * 50)
    dynamic_timesteps = 300000
    static_timesteps = 100000
    avg_episode_length = 50
    
    dynamic_episodes = dynamic_timesteps // avg_episode_length
    static_episodes = static_timesteps // avg_episode_length
    
    print(f"Dynamic RL Training:")
    print(f"  • Total timesteps: {dynamic_timesteps:,}")
    print(f"  • Estimated episodes: ~{dynamic_episodes:,}")
    print(f"  • Each episode = different Poisson arrival pattern")
    print(f"  • Learning phases:")
    print(f"    - Episodes 1-{dynamic_episodes//4:,}: Basic scheduling rules")
    print(f"    - Episodes {dynamic_episodes//4:,}-{dynamic_episodes//2:,}: Adaptation to arrivals")
    print(f"    - Episodes {dynamic_episodes//2:,}-{3*dynamic_episodes//4:,}: Machine utilization optimization")
    print(f"    - Episodes {3*dynamic_episodes//4:,}-{dynamic_episodes:,}: Advanced dynamic strategies")
    
    print(f"\nStatic RL Training:")
    print(f"  • Total timesteps: {static_timesteps:,}")
    print(f"  • Estimated episodes: ~{static_episodes:,}")
    print(f"  • Each episode = same job availability (all at t=0)")
    print(f"  • Learns optimal scheduling for known job set")
    
    print(f"\nWhy Dynamic RL needs 3x more training:")
    print(f"  • Must learn to handle uncertainty (vs. deterministic static)")
    print(f"  • Must develop adaptive strategies for various arrival patterns")
    print(f"  • Must balance current scheduling with future arrival preparation")
    print(f"  • More complex state space and decision making")
