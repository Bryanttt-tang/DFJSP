...
class ArrivalPredictor:
    ...
    Key Features:
    1. Cross-Episode Learning: Accumulates observations across all episodes
    2. Within-Episode Learning: Updates predictions as jobs arrive in current episode
    3. MLE for Poisson: λ̂ = 1 / mean(inter-arrival times)
    """
    ...
    def __init__(self, initial_rate_guess=0.05):
        ...
        self.observed_inter_arrivals = []  # All inter-arrival times across episodes
        self.current_episode_arrivals = []  # Arrival times in current episode
        self.estimated_rate = initial_rate_guess  # Current MLE estimate
        self.num_observations = 0
        
    def reset_episode(self):
        """Reset for new episode (but keep cross-episode learning)."""
        self.current_episode_arrivals = []
        
    def observe_arrival(self, arrival_time):
        """
        Observe a new job arrival in the current episode.
        Updates within-episode predictions immediately.
        ...
        # Calculate inter-arrival times within this episode
        if len(self.current_episode_arrivals) >= 2:
            sorted_arrivals = sorted(self.current_episode_arrivals)
            for i in range(1, len(sorted_arrivals)):
                inter_arrival = sorted_arrivals[i] - sorted_arrivals[i-1]
                if inter_arrival > 0:  # Valid inter-arrival time
                    self.observed_inter_arrivals.append(inter_arrival)
                    self.num_observations += 1
            
            # Update MLE estimate immediately
    ...
    def finalize_episode(self, all_arrival_times):
        ...
        """
        # Extract inter-arrival times from complete episode
        ...
        for i in range(1, len(arrival_list)):
            ...
            if inter_arrival > 0:
                # Only add if not already added during within-episode learning
                if inter_arrival not in self.observed_inter_arrivals[-10:]:  # Avoid recent duplicates
                    self.observed_inter_arrivals.append(inter_arrival)
                    self.num_observations += 1
        
        # Update MLE estimate with all accumulated data
        self._update_mle_estimate()
    
    def _update_mle_estimate(self):
        """Update MLE estimate of arrival rate λ."""
        if len(self.observed_inter_arrivals) > 0:
            ...
            if mean_inter_arrival > 0:
                ...
                self.estimated_rate = self.initial_rate
        else:
            self.estimated_rate = self.initial_rate
    
    def predict_next_arrivals(self, current_time, num_jobs_to_predict):
        """
        Predict arrival times of next N jobs using current MLE estimate.
        
        For Poisson process with rate λ:
        E[inter-arrival time] = 1/λ
        ...
        if self.estimated_rate <= 0:
            # No valid estimate yet, use initial guess
            mean_inter_arrival = 1.0 / self.initial_rate
        else:
            mean_inter_arrival = 1.0 / self.estimated_rate
        
        # Predict arrivals at regular intervals based on mean
        predictions = []
        for i in range(1, num_jobs_to_predict + 1):
            predicted_time = current_time + i * mean_inter_arrival
    ...
    def get_confidence(self):
        ...
        """
        # Sigmoid-like function: confidence approaches 1 as observations increase
        if self.num_observations == 0:
            return 0.0
        
        # 50% confidence at 10 observations, 90% at 50 observations
        confidence = 1.0 - np.exp(-self.num_observations / 20.0)
    ...
    def get_stats(self):
        ...
        return {
            ...
            'mean_inter_arrival': 1.0/self.estimated_rate if self.estimated_rate > 0 else float('inf')
        }
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def _update_predictions(self):
        ...
        predictions = self.arrival_predictor.predict_next_arrivals(
            self.event_time, len(unarrived_jobs)
        )
        
        # Map predictions to jobs
        for job_id, predicted_time in zip(unarrived_jobs, predictions):
            self.predicted_arrival_times[job_id] = predicted_time
    ...
    def step(self, action):
        ...
        # PROACTIVE SCHEDULING: Determine actual start time
        if job_id in self.arrived_jobs:
            ...
            actual_arrival = self.job_arrival_times[job_id]
            # Note: We're scheduling based on prediction, but execution uses actual arrival
        ...
        # REWARD SHAPING: Penalize mispredictions
        reward = -(self.current_makespan - previous_makespan)
        
        # Misprediction penalty: if scheduled proactively but job hadn't arrived yet
        if job_id not in self.arrived_jobs:
            # This was a proactive decision
            actual_arrival = self.job_arrival_times[job_id]
            if actual_arrival > start_time:
                # Prediction was too optimistic - job wasn't actually available
    ...
    def _execute_wait_action(self):
        ...
        # Find next event (job arrival or machine completion)
        next_events = []
        ...
        # Next machine completion
        for end_time in self.machine_end_times.values():
            if end_time > self.event_time:
                next_events.append(end_time)
        
        if not next_events:
            # No future events, check if done
            done = len(self.completed_jobs) == len(self.job_ids)
            return 0.0, done
        
        # Advance to next event
        next_time = min(next_events)
        self.event_time = next_time
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    print(f"  Predictor final stats: rate={stats['estimated_rate']:.4f}, "
          f"confidence={stats['confidence']:.2f}, obs={stats['num_observations']}")
...
def main():
    ...
    prediction_window = 10.0  # Can schedule jobs predicted to arrive in next 10 time units
