...
def plot_training_metrics():
    ...
    X-axis is rollout number for all metrics to align with verbose output.
    """
    global TRAINING_METRICS
    ...
    print(f"Rollout rewards recorded: {len(TRAINING_METRICS['episode_rewards'])}")
    ...
    # Create rollout numbers for x-axis
    ...
    print(f"Rollout rewards sample (first 5): {rewards[:5]}")
    print(f"Rollout rewards sample (last 5): {rewards[-5:]}")
    print(f"Rollout rewards range: {min(rewards):.4f} to {max(rewards):.4f}")
    print(f"Rollout numbers range: {min(rollout_numbers) if rollout_numbers else 'N/A'} to {max(rollout_numbers) if rollout_numbers else 'N/A'}")
    ...
    axes[0, 0].plot(rollout_numbers, rewards[:num_rollouts], 'g-', linewidth=2, alpha=0.8, label='Mean Episode Reward per Rollout')
    
    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Rollout Number')
    axes[0, 0].set_ylabel('Mean Episode Reward')
    axes[0, 0].set_title('Mean Episode Rewards per Rollout (from verbose)')
    ...
    # Plot 6: Episode Length over Rollouts (Bottom Right)
    if TRAINING_METRICS['episode_lengths']:
        ep_lengths = TRAINING_METRICS['episode_lengths']
        
        axes[2, 1].plot(rollout_numbers, ep_lengths[:num_rollouts], 'teal', linewidth=2, alpha=0.7, label='Mean Episode Length per Rollout')
        axes[2, 1].legend()
        
        axes[2, 1].set_xlabel('Rollout Number')
        axes[2, 1].set_ylabel('Mean Episode Length')
        axes[2, 1].set_title('Mean Episode Length per Rollout')
