...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=3, arrival_rate=0.08, total_timesteps=200000, reward_mode="combined_makespan_utilization"):
    ...
    # Add progress tracking callback for dynamic learning
        ...
        def _on_step(self) -> bool:
            if self.n_calls % self.check_freq == 0:
                ...
                if len(self.model.ep_info_buffer) > 0:
                    ...
                    if len(self.rewards_window) >= 5:
                        recent_trend = np.mean(self.rewards_window[-3:]) - np.mean(self.rewards_window[:3])
                        if recent_trend > 0:
                            print(f"     📈 Learning trend: IMPROVING (+{recent_trend:.2f})")
                        else:
                            print(f"     📉 Learning trend: declining ({recent_trend:.2f})")
                            
                    print(f"     ⚡ Learning focus: Poisson arrival adaptation & machine utilization")
                    
            return True
    
    callback = DynamicTrainingCallback(check_freq=5000, verbose=1)
    
    print(f"\nTraining for {total_timesteps:,} timesteps with enhanced reward shaping...")
    print("Focus areas:")
    print("  🎯 Stronger machine utilization penalties (reduce empty spaces)")
    print("  🎯 Enhanced future-aware scheduling rewards")
    print("  🎯 Dynamic adaptation bonuses for new arrivals")
    print("  🎯 Load balancing incentives")
...
def main():
    ...
    print("-" * 50)
    dynamic_timesteps = 500000  # INCREASED timesteps for better Poisson learning
    static_timesteps = 150000   # Also increased static for fair comparison
