...
# --- Enhanced Job Data for Dynamic Environment ---
ENHANCED_JOBS_DATA = collections.OrderedDict({
    ...
    5: [{'proc_times': {'M0': 6, 'M1': 9}}, {'proc_times': {'M1': 7, 'M2': 5}}, {'proc_times': {'M0': 4}}, {'proc_times': {'M2': 6}}],
    6: [{'proc_times': {'M1': 5, 'M2': 8}}, {'proc_times': {'M0': 6}}, {'proc_times': {'M1': 4, 'M2': 3}}],
    7: [{'proc_times': {'M0': 7, 'M2': 4}}, {'proc_times': {'M0': 5, 'M1': 6}}, {'proc_times': {'M1': 3}}, {'proc_times': {'M0': 2, 'M2': 5}}],
    8: [{'proc_times': {'M1': 4, 'M2': 6}}, {'proc_times': {'M0': 3}}, {'proc_times': {'M2': 5}}],
    9: [{'proc_times': {'M0': 8}}, {'proc_times': {'M1': 6, 'M2': 4}}, {'proc_times': {'M0': 3}}],
    10: [{'proc_times': {'M2': 7, 'M0': 5}}, {'proc_times': {'M1': 4}}],
    11: [{'proc_times': {'M1': 6}}, {'proc_times': {'M0': 5, 'M2': 7}}, {'proc_times': {'M1': 3}}],
    12: [{'proc_times': {'M0': 4, 'M1': 8}}, {'proc_times': {'M2': 6}}, {'proc_times': {'M0': 2}}],
    13: [{'proc_times': {'M2': 5}}, {'proc_times': {'M0': 6, 'M1': 4}}, {'proc_times': {'M2': 3}}],
    14: [{'proc_times': {'M1': 7, 'M2': 9}}, {'proc_times': {'M0': 5}}, {'proc_times': {'M1': 4}}]
})
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.1, 
        ...
        # Split jobs into initial and dynamic
    ...
    def _calculate_reward(self, proc_time, idle_time, done, num_new_arrivals, 
        ...
        if self.reward_mode == "makespan_increment":
            ...
            if done:
                reward += 50.0
                # Additional bonus for good final makespan
                if current_time > 0:
                    reward += max(0, 100.0 / current_time)
...
def train_poisson_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.1, 
                       total_timesteps=100000, reward_mode="dynamic_adaptation"):
    """Train an RL agent on the Poisson Dynamic FJSP environment."""
...
def evaluate_poisson_agent(model, jobs_data, machine_list, initial_jobs=5, 
                          arrival_rate=0.1, num_episodes=10, reward_mode="dynamic_adaptation"):
    ...
    for episode in range(num_episodes):
        ...
        print(f"Episode {episode+1}: Makespan={makespan:.2f}, "
              f"Jobs={total_jobs}, Steps={step_count}, Reward={episode_reward:.1f}")
    ...
    if makespans:
        ...
        # Return best result
        best_episode = min(results, key=lambda x: x['makespan'] if x['makespan'] > 0 else float('inf'))
...
def heuristic_spt_poisson(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.1, 
    ...
    arrival_times = {}
    for job_id in range(initial_jobs):
        arrival_times[job_id] = 0.0
    
    current_time = 0.0
    for job_id in range(initial_jobs, len(jobs_data)):
...
def compare_poisson_methods():
    ...
    print("\n4. RESULTS COMPARISON")
    print("="*50)
    print(f"RL Agent makespan:     {rl_makespan:.2f}")
    ...
    if rl_makespan != float('inf') and spt_makespan != float('inf'):
        if rl_makespan < spt_makespan:
            ...
            print(f"RL improvement: {improvement:.1f}% better than SPT")
        else:
            gap = ((rl_makespan - spt_makespan) / spt_makespan) * 100
            print(f"RL gap: {gap:.1f}% worse than SPT")
    
    # 5. Plot Gantt charts
    print("\n5. Plotting Gantt Charts...")
...
if __name__ == "__main__":
    print("Starting Poisson Dynamic FJSP Analysis...")
    results = compare_poisson_methods()
