...
class EnhancedTrainingCallback(BaseCallback):
    ...
    Robust callback that:
      ...
      - records rollout/* and train/* scalars at rollout boundaries (aligns with verbose)
      - if rollout/ep_rew_mean missing, compute it from newly finished episodes
    """
    def __init__(self, method_name, pbar=None, verbose=0):
        super().__init__(verbose)
        self.method_name = method_name
        ...
        # how many episode infos we've already processed from ep_info_buffer
        self._processed_episodes = 0
        # how many rollouts we have seen
    ...
    def _on_training_start(self) -> None:
        ...
        TRAINING_METRICS['method_name'] = self.method_name
        if self.verbose:
            print(f"[{self.method_name}] Callback initialized")

    def _on_step(self) -> bool:
        """Collect finished episodes incrementally from ep_info_buffer so per-episode arrays are up-to-date."""
        global TRAINING_METRICS
        ...
        # Try to get ep_info_buffer (OnPolicyAlgorithm maintains this)
        ep_buf = getattr(self.model, "ep_info_buffer", None)
        if ep_buf is not None:
            # ep_buf behaves like a deque; convert to list snapshot
            try:
                ep_list = list(ep_buf)
            except Exception:
                ep_list = []
            ...
            if total_eps > self._processed_episodes:
                # process newly finished episodes
                for idx in range(self._processed_episodes, total_eps):
                    info = ep_list[idx]
                    # SB3 ep_info contains at least 'r' (return) and 'l' (length) for Monitor/VecMonitor
                    r = float(info.get("r", np.nan)) if isinstance(info, dict) else np.nan
                    l = int(info.get("l", -1)) if isinstance(info, dict) else -1
                    ...
                    if self.verbose and len(TRAINING_METRICS['episode_rewards']) <= 5:
                        print(f"[{self.method_name}] ✅ Episode #{len(TRAINING_METRICS['episode_rewards'])} "
                              f"reward={r:.3f}, len={l}, t={t}")
                # advance processed pointer
                self._processed_episodes = total_eps

        return True

    def _on_rollout_end(self) -> None:
        """
        Called at the end of each rollout — this is where SB3 does updates and logs train/* scalars.
        We snapshot the logger keys and also compute/record rollout mean reward if necessary.
        """
        global TRAINING_METRICS
        ...
        TRAINING_METRICS['timesteps'].append(ts)

        # Copy logger contents safely
        log_copy = {}
        if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
            try:
        ...
        # Fetch rollout-ep scalars if SB3 logged them
        rollout_ep_rew = log_copy.get('rollout/ep_rew_mean', None)
        rollout_ep_len = log_copy.get('rollout/ep_len_mean', None)

        # If SB3 didn't log rollout ep means (no episodes finished during this rollout),
        # compute them from ep_info_buffer entries that finished since last processed pointer.
        computed_ep_rew = None
        computed_ep_len = None
        ep_buf = getattr(self.model, "ep_info_buffer", None)
        if rollout_ep_rew is None or rollout_ep_len is None:
            # gather newly finished episodes since last rollout (we already processed these in _on_step,
            # but _processed_episodes indicates how many total are currently in ep_info_buffer)
            # We cannot deduce which ones finished *during* this rollout without extra state.
            # So compute mean over episodes that were added since the last rollout. To do that we
            # need another pointer that stores the ep_info_buffer length at the previous rollout.
            if not hasattr(self, "_ep_count_at_last_rollout"):
                self._ep_count_at_last_rollout = 0
            if ep_buf is None:
                new_eps = []
            else:
                ep_list = list(ep_buf)
                total_eps = len(ep_list)
                start = self._ep_count_at_last_rollout
                new_eps = ep_list[start:total_eps] if total_eps > start else []
            # compute means if any new episodes exist
            if new_eps:
                rew_vals = [float(e.get('r', np.nan)) for e in new_eps if isinstance(e, dict)]
                len_vals = [int(e.get('l', -1)) for e in new_eps if isinstance(e, dict)]
                if len(rew_vals) > 0:
                    computed_ep_rew = float(np.nanmean(rew_vals))
                if len(len_vals) > 0:
                    computed_ep_len = float(np.mean(len_vals))
            # update pointer for next rollout
            self._ep_count_at_last_rollout = len(ep_list) if ep_buf is not None else 0

        # Choose which rollout means to record (prefer SB3's if present)
        if rollout_ep_rew is not None:
            TRAINING_METRICS['rollout_ep_rew_mean'].append(float(rollout_ep_rew))
        else:
            TRAINING_METRICS['rollout_ep_rew_mean'].append(computed_ep_rew)

        if rollout_ep_len is not None:
            TRAINING_METRICS['rollout_ep_len_mean'].append(float(rollout_ep_len))
        else:
            TRAINING_METRICS['rollout_ep_len_mean'].append(computed_ep_len)

        # Record train/* scalars (these exist after training update)
        def grab(key):
            v = log_copy.get(key, None)
            return float(v) if v is not None else None

        pol = grab('train/policy_gradient_loss') or grab('train/policy_loss')
        val = grab('train/value_loss')
        ent = grab('train/entropy_loss') or grab('train/entropy')
        tot = grab('train/loss')
        lr  = grab('train/learning_rate')
        ev  = grab('train/explained_variance')

        TRAINING_METRICS['policy_loss'].append(pol)
        TRAINING_METRICS['value_loss'].append(val)
        TRAINING_METRICS['action_entropy'].append(ent)
        TRAINING_METRICS['total_loss'].append(tot)
        TRAINING_METRICS['learning_rate'].append(lr)
        TRAINING_METRICS['explained_variance'].append(ev)

        # Optionally assign the rollout-level train metrics to episodes that finished in this rollout.
        # We'll find how many episodes finished since last rollout via _ep_count_at_last_rollout and map the
        # same scalar to each of those episodes (approximate mapping).
        if hasattr(self, "_ep_count_at_last_rollout"):
            # number of episodes that finished during this rollout (as computed above)
            num_new = 0
            if ep_buf is not None:
                total_eps = len(list(ep_buf))
                prev = getattr(self, "_ep_count_at_last_rollout_prev_map", 0)
                num_new = max(0, total_eps - prev)
                # append same rollout scalars to per-episode arrays for those new episodes
                for _ in range(num_new):
                    TRAINING_METRICS['episode_policy_loss'].append(pol)
                    TRAINING_METRICS['episode_value_loss'].append(val)
                    TRAINING_METRICS['episode_entropy'].append(ent)
                # update prev_map pointer
                self._ep_count_at_last_rollout_prev_map = total_eps
            else:
                # no ep buffer -> nothing to map
                pass

        # Small verbose print for first few rollouts
        if self.verbose and self._rollout_count <= 10:
            print(f"[{self.method_name}] Rollout #{self._rollout_count} ts={ts} "
                  f"ep_rew_mean={TRAINING_METRICS['rollout_ep_rew_mean'][-1]} "
                  f"policy_loss={pol} value_loss={val} entropy={ent}")
...
def plot_training_metrics():
    ...
    # Use episode_timesteps for x-axis to align with other subplots
    num_episodes = len(TRAINING_METRICS['episode_rewards'])
    rewards = TRAINING_METRICS['episode_rewards']
    episode_timesteps = TRAINING_METRICS['episode_timesteps'][:num_episodes] if 'episode_timesteps' in TRAINING_METRICS else list(range(num_episodes))
    ...
    print(f"Episode timesteps range: {min(episode_timesteps)} to {max(episode_timesteps)}")
    ...
    # Plot 1: Episode Rewards over Timesteps (Top Left) - MAIN PLOT
    axes[0, 0].plot(episode_timesteps, rewards, 'g-', linewidth=2, alpha=0.8, label='Episode Reward')

    axes[0, 0].legend()
    axes[0, 0].set_xlabel('Timesteps')
    axes[0, 0].set_ylabel('Episode Reward')
    axes[0, 0].set_title('Episode Rewards Over Training (Primary Metric)')
    ...
    # Plot 2: Total Loss over Timesteps (Top Right)
    if TRAINING_METRICS['total_loss'] and TRAINING_METRICS['timesteps']:
        total_loss = TRAINING_METRICS['total_loss']
        loss_timesteps = TRAINING_METRICS['timesteps'][:len(total_loss)]
        
        axes[0, 1].plot(loss_timesteps, total_loss, 'purple', linewidth=2, alpha=0.7, label='Total Loss')
        axes[0, 1].legend()
    ...
    # Plot 3: Policy Loss over Timesteps (Middle Left)
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['timesteps']:
        policy_loss = TRAINING_METRICS['policy_loss']
        loss_timesteps = TRAINING_METRICS['timesteps'][:len(policy_loss)]
        
        axes[1, 0].plot(loss_timesteps, policy_loss, 'r-', linewidth=2, alpha=0.7, label='Policy Loss')
        axes[1, 0].legend()
    ...
    # Plot 4: Value Loss over Timesteps (Middle Right)
    if TRAINING_METRICS['value_loss'] and TRAINING_METRICS['timesteps']:
        value_loss = TRAINING_METRICS['value_loss']
        loss_timesteps = TRAINING_METRICS['timesteps'][:len(value_loss)]
        
        axes[1, 1].plot(loss_timesteps, value_loss, 'b-', linewidth=2, alpha=0.7, label='Value Loss')
        axes[1, 1].legend()
    ...
    # Plot 5: Entropy Loss over Timesteps (Bottom Left)
    if TRAINING_METRICS['action_entropy'] and TRAINING_METRICS['timesteps']:
        entropy_loss = TRAINING_METRICS['action_entropy']
        loss_timesteps = TRAINING_METRICS['timesteps'][:len(entropy_loss)]
        
        axes[2, 0].plot(loss_timesteps, entropy_loss, 'orange', linewidth=2, alpha=0.7, label='Entropy Loss')
        axes[2, 0].legend()
        
        axes[2, 0].set_xlabel('Timesteps')
        axes[2, 0].set_ylabel('Entropy Loss')
        axes[2, 0].set_title('Policy Entropy Loss')
    ...
    # Plot 6: Episode Length over Timesteps (Bottom Right)
    if TRAINING_METRICS['episode_lengths']:
        ep_lengths = TRAINING_METRICS['episode_lengths']
        length_timesteps = episode_timesteps[:len(ep_lengths)]

        axes[2, 1].plot(length_timesteps, ep_lengths, 'teal', linewidth=2, alpha=0.7, label='Episode Length')
        axes[2, 1].legend()
