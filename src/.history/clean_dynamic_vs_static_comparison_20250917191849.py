...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, rule):
    """
    Simple and correct list scheduling implementation.
    """
    # Initialize
    machine_next_free = {m: 0.0 for m in machine_list}
    job_current_operation = {job_id: 0 for job_id in jobs_data.keys()}
    operation_end_times = {job_id: [0.0] * len(jobs_data[job_id]) for job_id in jobs_data.keys()}
    schedule = {m: [] for m in machine_list}
    
    # Create a list of all operations with their details
    all_operations = []
    ...
    # Sort operations based on the dispatching rule
    if rule == "FIFO":
        # Sort by arrival time, then by job ID, then by operation index
        all_operations.sort(key=lambda x: (x['arrival_time'], x['job_id'], x['op_idx']))
    elif rule == "LIFO":
        # Sort by arrival time (descending), then by job ID (descending), then by operation index
        all_operations.sort(key=lambda x: (-x['arrival_time'], -x['job_id'], x['op_idx']))
    elif rule == "SPT":
        # Sort by processing time, then by arrival time
        all_operations.sort(key=lambda x: (x['proc_time'], x['arrival_time'], x['job_id'], x['op_idx']))
    elif rule == "LPT":
        # Sort by processing time (descending), then by arrival time
        all_operations.sort(key=lambda x: (-x['proc_time'], x['arrival_time'], x['job_id'], x['op_idx']))
    elif rule == "EDD":
        # Estimate due dates and sort by them
        def estimate_due_date(job_id):
            total_work = sum(min(op['proc_times'].values()) for op in jobs_data[job_id])
            return arrival_times[job_id] + total_work * 1.5
        
        all_operations.sort(key=lambda x: (estimate_due_date(x['job_id']), x['arrival_time'], x['job_id'], x['op_idx']))
    
    # Process operations in order, but only schedule when feasible
    ...
    while scheduled_operations < total_operations:
        ...
        for op in all_operations:
            ...
            # Check if this operation can be scheduled
            if (job_current_operation[job_id] == op_idx and  # This is the next operation for the job
                op not in [x for x in all_operations if x.get('scheduled', False)]):  # Not already scheduled
        ...
        if not operation_scheduled:
            # If we couldn't schedule any operation, we're likely done or there's an issue
            break
...
def lifo_heuristic(jobs_data, machine_list, arrival_times):
    ...
    return _generic_heuristic(jobs_data, machine_list, arrival_times, "LIFO",
                             lambda op_data: -arrival_times[op_data[0]])
