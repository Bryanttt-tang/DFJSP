...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # If no ready operations, advance time until we have ready operations or episode ends
        while not ready_ops and self.operations_scheduled < self.total_operations:
            ...
            if next_event_time <= self.event_time or next_event_time == float('inf'):
                # No more events - shouldn't happen if operations incomplete
                break
            ...
            # Check again for ready operations
            ready_ops = self._get_ready_operations()
        
        # If still no ready ops after advancing, episode should end
        if not ready_ops:
            reward = -(self.current_makespan - previous_makespan)
            terminated = self.operations_scheduled >= self.total_operations
            return self._get_observation(), reward, terminated, False, {"auto_advance_no_ops": True}
        ...
        # Update event_time: advance only if no other idle machines remain
        # Check if there are other machines idle at current event_time (excluding the one we just scheduled)
        other_idle = any(self.machine_next_free[m] <= self.event_time for m in self.machines if m != best_machine)
        
        if not other_idle:
            # No other idle machines - advance to next event
