...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        # ACTION SPACE: Scheduling actions + Wait actions (SAME as Proactive RL)
        num_scheduling_actions = self.num_jobs * len(self.machines)
        
        # Define wait actions: [10, "next_event"] - SAME as Proactive RL
    ...
    def action_masks(self):
        """Generate action masks based on arrival and machine compatibility (SAME as Proactive RL)."""
        ...
        # Wait actions: Always enabled if there are unarrived jobs OR schedulable work
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        The Predictor Role:
        -------------------
        The arrival predictor guides the agent ONLY through observations (predicted arrival times).
        It does NOT modify rewards or action logic - the agent learns to use predictions implicitly.
    ...
    def _execute_wait_action(self, wait_duration):
        ...
        The arrival predictor guides behavior ONLY through observations (predicted arrival times).
        It does NOT affect rewards directly - agent learns to use predictions implicitly.
        
        Args:
            wait_duration: Time to wait (float) or float('inf') for next event
        ...
        # Update predictions (predictor learns from arrivals)
        self._update_predictions()
        ...
        # Reward: Negative makespan increment
        # NOTE: This reward structure has temporal credit assignment issues!
        # Wait → reward=0, Schedule after → reward=-X (looks bad but might be optimal)
        # Agent must learn cumulative Q-values to understand long-term benefits
        reward = -(self.current_makespan - previous_makespan)
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
                       total_timesteps=500000, reward_mode="makespan_increment", learning_rate=3e-4):
    """
    Train a REACTIVE RL agent on Poisson job arrivals (NO arrival prediction).
    
    This is the baseline method that:
    - Does NOT predict job arrivals
    - Does NOT have predicted arrival times in observations
    - Uses IDENTICAL training setup as Proactive RL for fair comparison
    
    The ONLY difference from Proactive RL is the absence of arrival predictor and 
    predicted arrival times in the observation space.
    """
    print(f"\n--- Training REACTIVE RL Agent on {len(jobs_data)} jobs ---")
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    print(f"⚠️  Agent does NOT predict arrivals (reactive baseline)")
    ...
    model = MaskablePPO(
        ...
        vec_env,
        verbose=1,
        learning_rate=linear_schedule(learning_rate),  # SAME: Decaying LR
        n_steps=2048,               # SAME: Larger rollout
        batch_size=512,             # SAME: Larger batches
        n_epochs=5,                 # SAME: More epochs
        gamma=1.0,                  # SAME: Undiscounted
        gae_lambda=0.95,            # SAME: GAE parameter
        clip_range=0.2,             # SAME: PPO clipping
        ent_coef=0.01,              # SAME: Entropy for exploration
        vf_coef=0.5,                # SAME: Value function coef
        max_grad_norm=0.5,          # SAME: Gradient clipping
        normalize_advantage=True,   # SAME: Normalize advantages
        seed=GLOBAL_SEED,
        policy_kwargs=dict(
            net_arch=[256, 256, 128],  # SAME: 3-layer network
            activation_fn=torch.nn.ReLU
        )
    )
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    # IMPROVED hyperparameters for proactive learning
    # Key changes from reactive agent:
    # 1. More epochs (5 vs 2) - proactive is more complex, needs more updates
    # 2. Higher initial entropy (0.01 vs 0.001) - needs exploration to learn wait timing
    # 3. Learning rate schedule - start high, decay for stability
    # 4. Larger batch size (512 vs 256) - more stable gradient estimates
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=1200, verbose=True):
    """
    MILP approach for optimal dynamic scheduling with perfect knowledge.
...
def main():
  ...
  # Same as reactive RL for fair comparison
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        TOLERANCE = 0.1
        if milp_makespan != float('inf') and dynamic_makespan < milp_makespan - TOLERANCE:
        ...
        if milp_makespan != float('inf') and perfect_makespan < milp_makespan - TOLERANCE:
