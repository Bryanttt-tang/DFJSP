...
def plot_three_method_comparison(schedules_data, method_arrival_times=None, save_path=None):
    """
    ...
    Args:
        ...
        method_arrival_times: Dict of method_name -> arrival_times dict
    """
    ...
    for plot_idx, method in enumerate(methods):
        ...
        # Get arrival times for this specific method
        arrival_times = method_arrival_times.get(method, {}) if method_arrival_times else {}
...
if __name__ == "__main__":
    ...
    # CRITICAL FIX: Generate arrival times once and use for all methods
    print("\n0. GENERATING CONSISTENT ARRIVAL TIMES...")
    print("-" * 50)
    np.random.seed(42)
    random.seed(42)
    
    # Generate arrival times that will be used by all methods
    consistent_arrival_times = {}
    
    # Initial jobs arrive at t=0
    for job_id in initial_jobs:
        consistent_arrival_times[job_id] = 0.0
    ...
    for job_id in remaining_jobs:
        inter_arrival_time = np.random.exponential(1.0 / arrival_rate)
        current_time += inter_arrival_time
        consistent_arrival_times[job_id] = current_time
    
    print(f"Consistent arrival times: {consistent_arrival_times}")
    ...
    print("-" * 50)
    spt_makespan, spt_schedule = heuristic_spt_poisson_fixed(
        ENHANCED_JOBS_DATA, MACHINE_LIST, consistent_arrival_times
    )
    schedules_data['spt'] = {
    ...
    print("-" * 50)
    dynamic_rl_makespan, dynamic_rl_schedule = evaluate_poisson_agent_fixed(
        dynamic_rl_model, ENHANCED_JOBS_DATA, MACHINE_LIST,
        consistent_arrival_times, debug=True
    )
    ...
    print("\n5. EVALUATING STATIC RL ON STATIC ARRIVALS...")
    print("-" * 50)
    print("Testing static RL agent on static job arrivals (all jobs at t=0)...")
    # Static RL should be evaluated on the environment it was trained on
    static_rl_makespan, static_rl_schedule = evaluate_poisson_agent(
        static_rl_model, ENHANCED_JOBS_DATA, MACHINE_LIST,
        initial_jobs=list(ENHANCED_JOBS_DATA.keys()),  # All jobs available at start
        arrival_rate=0.0,  # No Poisson arrivals
        num_episodes=10
    )
    schedules_data['static_rl'] = {
        ...
        'title': 'Static RL (All jobs at t=0)'
    }
    print(f"Static RL Makespan: {static_rl_makespan:.2f}")
    print("NOTE: Static RL evaluated on static environment (all jobs at t=0)")
    ...
    # Create static arrival times for Static RL (all jobs at t=0)
    static_arrival_times = {job_id: 0 for job_id in ENHANCED_JOBS_DATA.keys()}
    
    # Create a dictionary of arrival times for each method
    method_arrival_times = {
        'dynamic_rl': spt_arrival_times,    # Poisson arrivals
        'spt': spt_arrival_times,           # Poisson arrivals  
        'static_rl': static_arrival_times   # All jobs at t=0
    }
    
    plot_three_method_comparison(schedules_data, method_arrival_times, 
