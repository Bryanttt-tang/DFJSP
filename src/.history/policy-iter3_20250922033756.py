...
def canonicalize_state(next_ops, machine_next_free, job_ready_time, current_time, time_unit=1, cap=None):
    """
    Convert an internal absolute-time state into a canonical compact state:
      - machine_rem = max(0, mf - current_time)
      - job_rem = max(0, jr - current_time)
      - quantize by time_unit (int)
      - optionally cap times
      - shift-normalize so min remaining time is 0
    Returns hashable tuple: (tuple(next_ops), tuple(machine_can), tuple(job_can))
    """
    # compute remaining times
    machine_rem = [max(0, mf - current_time) for mf in machine_next_free]
    job_rem = [max(0, jr - current_time) for jr in job_ready_time]

    # cap if requested
    if cap is not None:
        machine_rem = [min(r, cap) for r in machine_rem]
        job_rem = [min(r, cap) for r in job_rem]

    # quantize (proc times are integers in these examples so time_unit=1 keeps exactness)
    if time_unit is not None and time_unit > 0:
        machine_q = [int(round(r / time_unit)) for r in machine_rem]
        job_q = [int(round(r / time_unit)) for r in job_rem]
    else:
        machine_q = [int(round(r)) for r in machine_rem]
        job_q = [int(round(r)) for r in job_rem]

    # shift-normalize -> subtract the minimum remaining time across both lists
    all_q = machine_q + job_q
    delta = min(all_q) if all_q else 0
