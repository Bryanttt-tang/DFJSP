...
def plot_gantt_chart_with_fixed_scale(schedule, makespan, title, fixed_scale=80):
    """
    Plot Gantt chart with FIXED scale across ALL plots for fair comparison.
    Short makespans will show shorter bars, long makespans will show longer bars.
    
    Args:
        schedule: Dictionary with machine schedules
        makespan: The makespan of this schedule
        title: Title for the plot
        fixed_scale: Fixed maximum time scale for all plots (default: 80)
    """
    if not schedule or not any(schedule.values()):
        print(f"No schedule data available for {title}")
        return
    
    plt.figure(figsize=(14, 6))
    
    # Color mapping for jobs - consistent colors across plots
    job_colors = plt.cm.Set3(np.linspace(0, 1, 10))
    
    machine_names = sorted(schedule.keys())  # Sort for consistent ordering
    y_positions = range(len(machine_names))
    
    for i, machine in enumerate(machine_names):
        if machine in schedule and schedule[machine]:
            for task_info in schedule[machine]:
                if len(task_info) >= 3:
                    task_name, start_time, end_time = task_info[:3]
                    
                    # Extract job number for consistent coloring
                    try:
                        job_num = int(task_name.split('-')[0][1:])  # Extract number from "J0", "J1", etc.
                        color = job_colors[job_num % len(job_colors)]
                    except (ValueError, IndexError):
                        color = 'lightblue'  # Default color
                    
                    # Draw the task bar
                    plt.barh(i, end_time - start_time, left=start_time, 
                            height=0.6, color=color, edgecolor='black', alpha=0.8)
                    
                    # Add task label
                    plt.text(start_time + (end_time - start_time) / 2, i, task_name,
                            ha='center', va='center', fontsize=8, fontweight='bold')
    
    # Formatting with FIXED scale
    plt.yticks(y_positions, machine_names)
    plt.xlabel('Time Units')
    plt.ylabel('Machines')
    plt.title(f'{title}\nMakespan: {makespan:.1f} (Lower is Better)')
    plt.grid(True, alpha=0.3, axis='x')
    
    # FIXED x-axis limits for consistent comparison
    plt.xlim(0, fixed_scale)
    
    # Add makespan line - this shows the actual performance
    plt.axvline(x=makespan, color='red', linestyle='--', linewidth=2, alpha=0.8, 
                label=f'Makespan: {makespan:.1f}')
    
    # Add performance quality indicators
    if makespan <= 35:
        quality = "Excellent"
        quality_color = 'green'
    elif makespan <= 40:
    ...
    plt.text(fixed_scale * 0.02, len(machine_names) - 0.3, 
             f'Quality: {quality}', fontsize=12, fontweight='bold', 
             color=quality_color, bbox=dict(boxstyle="round,pad=0.3", facecolor=quality_color, alpha=0.2))
    
    plt.legend(loc='upper right')
    plt.tight_layout()
    plt.show()
    
    print(f"📊 {title}: Makespan = {makespan:.1f} | Quality = {quality}")


# Enhanced plotting function that determines optimal fixed scale
def plot_comparison_gantt_charts(results_dict, title_prefix=""):
    """
    Plot multiple Gantt charts with consistent scale for fair comparison.
    
    Args:
        results_dict: Dictionary with keys as method names, values as (schedule, makespan) tuples
        title_prefix: Optional prefix for all titles
    """
    if not results_dict:
        print("No results to plot")
        return
    
    # Determine fixed scale based on maximum makespan + buffer
    max_makespan = max(makespan for _, makespan in results_dict.values())
    fixed_scale = max(80, int(max_makespan * 1.2))  # At least 80, or 120% of max makespan
    
    print(f"\n🎯 COMPARISON RESULTS (Fixed Scale: 0-{fixed_scale})")
    print("=" * 60)
    
    # Sort results by makespan for better comparison
    sorted_results = sorted(results_dict.items(), key=lambda x: x[1][1])
    
    for method_name, (schedule, makespan) in sorted_results:
        full_title = f"{title_prefix}{method_name}" if title_prefix else method_name
        plot_gantt_chart_with_fixed_scale(schedule, makespan, full_title, fixed_scale)
    
    # Print summary comparison
    print("\n📈 PERFORMANCE SUMMARY:")
    print("-" * 40)
    best_makespan = min(makespan for _, makespan in results_dict.values())
    
    for method_name, (_, makespan) in sorted_results:
        gap = ((makespan - best_makespan) / best_makespan) * 100 if best_makespan > 0 else 0
        if makespan == best_makespan:
            print(f"🥇 {method_name:20s}: {makespan:6.1f} (BEST)")
        else:
            print(f"   {method_name:20s}: {makespan:6.1f} (+{gap:.1f}%)")
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=500000, reward_mode="makespan_increment"):
    """
    Train a dynamic RL agent on Poisson job arrivals with ENHANCED curriculum learning.
    """
    print(f"\n--- Training Dynamic RL Agent on {len(jobs_data)} jobs ---")
    print(f"Timesteps: {total_timesteps:,} | Reward: {reward_mode}")
    print("Using curriculum learning: starting with lower arrival rates, increasing complexity")
    
    def mask_fn(env):
        return env.action_masks()
    
    def make_dynamic_env():
        # ENHANCED randomization for better generalization with curriculum learning
        available_jobs = list(jobs_data.keys())
        
        # Curriculum learning: start with simpler scenarios
        if hasattr(make_dynamic_env, 'episode_count'):
            make_dynamic_env.episode_count += 1
        else:
            make_dynamic_env.episode_count = 1
        
        # Gradually increase complexity
        episode = make_dynamic_env.episode_count
        max_episodes = total_timesteps // 25  # Approximate episodes
        
        if episode < max_episodes * 0.3:  # First 30% - simple scenarios
            current_arrival_rate = arrival_rate * 0.6  # Slower arrivals
            num_initial = 4  # More initial jobs
        elif episode < max_episodes * 0.7:  # Next 40% - medium scenarios  
            current_arrival_rate = arrival_rate * 0.8
            num_initial = 3
        else:  # Final 30% - full complexity
            current_arrival_rate = arrival_rate * random.uniform(0.9, 1.2)  # Vary ±20%
            num_initial = random.randint(2, 4)
        
        if isinstance(initial_jobs, list):
            selected_initial = random.sample(initial_jobs, min(num_initial, len(initial_jobs)))
        else:
            selected_initial = random.sample(available_jobs, min(num_initial, len(available_jobs)))
            
        env = PoissonDynamicFJSPEnv(
            jobs_data, machine_list, 
            initial_jobs=selected_initial,
