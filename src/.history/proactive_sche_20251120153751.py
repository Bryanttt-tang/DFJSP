...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=1000000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model = MaskablePPO(
        ...
        gae_lambda=0.95,            # GAE for advantage estimation
        clip_range=0.2,             # Standard PPO clipping
        ent_coef=0.03,              # Increased from 0.01 for better exploration
        vf_coef=0.5,                # Value function coefficient
...
def train_rule_based_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
    ...
    model = MaskablePPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.05,              # Increased from 0.01 - critical for rule exploration
        vf_coef=0.5,
        max_grad_norm=0.5,
        normalize_advantage=True,
        seed=GLOBAL_SEED,
    ...
    for job_id, operations in jobs_data.items():
        for op_idx, op_data in enumerate(operations):
            ...
            for machine_idx in compatible_machines:
                ...
                # Interval variable (optional, activated when machine_var == machine_idx)
                interval_var = model.NewOptionalFixedSizeIntervalVar(
                    start_var, proc_time, machine_var == machine_idx, 
                    f"interval{suffix}_m{machine_idx}"
                )
                intervals_for_task.append((machine_idx, interval_var, proc_time))
            ...
            for machine_idx, interval_var, proc_time in intervals_for_task:
                # If this machine is selected, end = start + proc_time
                model.Add(end_var == start_var + proc_time).OnlyEnforceIf(machine_var == machine_idx)
