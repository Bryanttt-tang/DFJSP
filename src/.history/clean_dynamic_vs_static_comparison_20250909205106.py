...
def train_static_agent(jobs_data, machine_list, total_timesteps=150000, reward_mode="combined_makespan_utilization"):
    ...
    # Custom progress callback using tqdm
    class TqdmCallback:
        def __init__(self, total_timesteps, desc="Training"):
            self.pbar = tqdm(total=total_timesteps, desc=desc, 
                           bar_format='{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} timesteps [{elapsed}<{remaining}]')
            self.last_timestep = 0
            
        def __call__(self, locals, globals):
            current_timestep = locals.get('self').num_timesteps
            progress = current_timestep - self.last_timestep
            if progress > 0:
    ...
    # Create progress bar
    progress_callback = TqdmCallback(total_timesteps, "Static RL")
    
    # Train with callback
    try:
        # Use a custom training loop with progress updates
        start_time = time.time()
        checkpoint_interval = total_timesteps // 20  # Update every 5%
        
        for i in range(20):
            timesteps_this_round = checkpoint_interval
            if i == 19:  # Last iteration
                timesteps_this_round = total_timesteps - (i * checkpoint_interval)
            
            model.learn(total_timesteps=timesteps_this_round)
            progress_callback.pbar.update(timesteps_this_round)
...
def train_dynamic_agent(jobs_data, machine_list, initial_jobs=3, arrival_rate=0.08, total_timesteps=200000, reward_mode="combined_makespan_utilization"):
    ...
    # Create progress bar
    progress_callback = TqdmCallback(total_timesteps, "Dynamic RL")
    
    # Train with callback
    try:
        # Use a custom training loop with progress updates
        start_time = time.time()
        checkpoint_interval = total_timesteps // 25  # Update every 4%
