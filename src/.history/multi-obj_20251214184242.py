...
from utils import (generate_fjsp_dataset, generate_simplified_fjsp_dataset, print_dataset_info,print_dataset_table, assign_due_dates)
...
NUM_INITIAL_JOBS = 5  # Jobs arriving at t=0
NUM_FUTURE_JOBS = 15   # Jobs arriving dynamically
INITIAL_JOB_IDS = list(range(NUM_INITIAL_JOBS))  # [0, 1, 2]

# CRITICAL DESIGN CHOICES TO MAKE HEURISTICS FAIL:
# ====================================================
# 1. EXTREME MACHINE HETEROGENEITY (variance=1.0):
#    - Fast machines: 0.6x speed (40% faster)
#    - Slow machines: 1.5x speed (50% slower)
#    - Gap: 2.5x difference!
#    → MINC heuristic fails: earliest completion ≠ best long-term choice
#
# 2. HUGE PROCESSING TIME VARIANCE (1-100):
#    - Short ops: ~10 units (wait not worth it)
#    - Long ops: ~80 units (wait 20 units saves 40+ units!)
#    → Simple rules (SPT, LPT) fail: timing context matters
#
# 3. HIGH MACHINE COMPATIBILITY (most ops work on most machines):
#    - More routing choices = more ways for greedy heuristics to fail
#    → Strategic waiting becomes critical
#
# This creates COMPLEX TRADE-OFFS that require lookahead:
# - "Schedule now on slow machine or wait for fast machine?"
# - "Fast machine free soon, but next job might need it more!"
# - "Current op is short (don't wait), but next op is long (should save fast machine)!"

# Generate dataset WITHOUT due dates (due dates will be calculated dynamically each episode)
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    machine_speed_variance=1,  # EXTREME heterogeneity (was 0.8) - breaks MINC
    proc_time_variance_range=(1, 20),  # HUGE variance (was 5-30) - breaks SPT/LPT
    due_date_tightness=1.5,  # This parameter is now stored for later use
    seed=GLOBAL_SEED
)

# IMPORTANT: Store due_date_tightness for dynamic calculation during training/evaluation
DUE_DATE_TIGHTNESS = 1.5  # Controls: due_date = arrival_time + tightness × total_min_proc_time
...
class ArrivalPredictor:
    ...
    def get_state(self):
        """
        Get predictor state for saving with model weights.
        
        This captures the learned knowledge (global statistics) that should
        be preserved between training and evaluation.
        
        Returns:
            dict: Complete predictor state
        """
        return {
            'global_n': self.global_n,
            'global_sum': self.global_sum,
            'global_estimated_rate': self.global_estimated_rate,
    ...
    def set_state(self, state):
        """
        Restore predictor state from saved checkpoint.
        
        This restores the learned knowledge (global statistics) when loading
        a trained model for evaluation.
        
        Args:
            state: Dict containing predictor state (from get_state())
        """
        self.global_n = state.get('global_n', 0)
        self.global_sum = state.get('global_sum', 0.0)
        self.global_estimated_rate = state.get('global_estimated_rate', self.initial_rate)
        self.initial_rate = state.get('initial_rate', self.initial_rate)
        self.mode = state.get('mode', 'mle')
        self.prior_shape = state.get('prior_shape', 2.0)
        self.prior_rate = state.get('prior_rate', self.prior_shape / self.initial_rate)
        
        # Update current estimates based on loaded global statistics
        self._update_mle_estimate()
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
                 ...
                 tardiness_weight=0.0, due_date_tightness=1.5):
        super().__init__()
        ...
        # Handle jobs_data format: can be simple list or dict with 'operations' and 'due_date'
        self.jobs = collections.OrderedDict()
        self.job_due_dates = {}  # Will be calculated dynamically in each episode
        for job_id, job_info in jobs_data.items():
            if isinstance(job_info, dict) and 'operations' in job_info:
                # New format with due dates (but we'll recalculate them dynamically)
                self.jobs[job_id] = job_info['operations']
            else:
                # Legacy format: job_info is directly a list of operations
                self.jobs[job_id] = job_info
        
        self.due_date_tightness = due_date_tightness  # Store for dynamic calculation
        ...
        self.tardiness_weight = tardiness_weight  # Weight for tardiness in multi-objective reward
        ...
        # UPDATED: Added tardiness-related features
        if self.cheat==False:
            obs_size = (
                ...
                self.num_jobs +                         # Due dates (normalized)
                self.num_jobs +                         # Slack time (due_date - current_time - remaining_proc_time)
                3                                       # Arrival progress, makespan progress, current_time
            )
        else:
            obs_size = (
                self.num_jobs +                         # Ready job indicators
    ...
    def _reset_state(self):
        ...
        # Tardiness tracking
        self.job_completion_times = {job_id: 0.0 for job_id in self.job_ids}
        self.total_tardiness = 0.0
        self.num_tardy_jobs = 0
    ...
    def _generate_poisson_arrivals(self):
        ...
        CRITICAL: Also calculate due dates based on these arrival times.
        Formula: due_date = arrival_time + due_date_tightness × total_min_processing_time
        """
        # Initialize arrival times for initial jobs
        ...
        # DYNAMIC DUE DATE CALCULATION: Calculate due dates based on actual arrival times
        # This ensures due dates are consistent with the Poisson arrival realization in this episode
        for job_id in self.job_ids:
            # Calculate minimum total processing time (optimistic estimate)
            total_min_proc_time = sum(
                min(op['proc_times'].values()) for op in self.jobs[job_id]
            )
            
            # Calculate due date: arrival_time + tightness × min_processing_time
            arrival_time = self.job_arrival_times.get(job_id, 0.0)
            self.job_due_dates[job_id] = arrival_time + self.due_date_tightness * total_min_proc_time
    ...
    def step(self, action):
        ...
        # Track job completion and tardiness
        if self.next_operation[job_id] >= len(self.jobs[job_id]):
            ...
            if tardiness > 0:
                self.num_tardy_jobs += 1

        # Multi-objective reward: makespan + estimated weighted tardiness
        makespan_increment = self.current_makespan - previous_makespan
        
        # Calculate ESTIMATED WEIGHTED TARDINESS for denser feedback
        tardiness_penalty = 0.0
        
        if self.tardiness_weight > 0:
            # Check all in-progress and completed jobs for potential/actual tardiness
            for j_id in self.job_ids:
                if j_id not in self.arrived_jobs:
                    continue  # Can't estimate for jobs that haven't arrived
                
                if self.job_due_dates.get(j_id, float('inf')) != float('inf'):
                    due_date = self.job_due_dates[j_id]
                    
                    if self.next_operation[j_id] >= len(self.jobs[j_id]):
                        # Job completed - use ACTUAL tardiness
                        completion_time = self.job_completion_times.get(j_id, 0.0)
                        tardiness = max(0.0, completion_time - due_date)
                        tardiness_penalty += tardiness
                    else:
                        # Job in progress - use ESTIMATED tardiness
                        # Estimate remaining time: sum of minimum processing times for remaining ops
                        remaining_time = 0.0
                        for op_idx in range(self.next_operation[j_id], len(self.jobs[j_id])):
                            operation = self.jobs[j_id][op_idx]
                            min_proc = min(operation['proc_times'].values())
                            remaining_time += min_proc
                        
                        # Estimated completion = current job end time + remaining work
                        current_job_time = max(self.operation_end_times[j_id]) if self.operation_end_times[j_id] else 0.0
                        estimated_completion = current_job_time + remaining_time
                        
                        # Estimated tardiness (weighted less than actual to avoid over-penalization)
                        estimated_tardiness = max(0.0, estimated_completion - due_date)
                        tardiness_penalty += 0.5 * estimated_tardiness
        
        # Weighted sum reward
        reward = -(makespan_increment + self.tardiness_weight * tardiness_penalty)
        ...
        info = {
            ...
            "num_tardy_jobs": self.num_tardy_jobs
        }
    ...
    def _get_observation(self):
        ...
        if not self.cheat:    
            ...
            # 5.4. Current time (normalized)
            normalized_current_time = min(1.0, self.event_time / self.max_time_horizon)
            obs.append(normalized_current_time)
            
            # 6. Due date features (for tardiness minimization)
            # 6.1. Due dates (normalized)
            for job_id in self.job_ids:
                if job_id in self.arrived_jobs:
                    due_date = self.job_due_dates.get(job_id, self.max_time_horizon)
                    normalized_due_date = min(1.0, due_date / self.max_time_horizon)
                    obs.append(normalized_due_date)
                else:
                    # Job not arrived: use 1.0 (no information leakage)
                    obs.append(1.0)
            
            # 6.2. Slack time (due_date - current_time - remaining_proc_time)
            for job_id in self.job_ids:
                if job_id in self.arrived_jobs and self.next_operation[job_id] < len(self.jobs[job_id]):
                    # Calculate remaining processing time (minimum across all machines)
                    remaining_proc_time = 0.0
                    for op_idx in range(self.next_operation[job_id], len(self.jobs[job_id])):
                        operation = self.jobs[job_id][op_idx]
                        min_time = min(operation['proc_times'].values())
                        remaining_proc_time += min_time
                    
                    due_date = self.job_due_dates.get(job_id, self.max_time_horizon)
                    slack = due_date - self.event_time - remaining_proc_time
                    # Normalize slack: positive slack -> 0.5 to 1.0, negative slack -> 0.0 to 0.5
                    if slack >= 0:
                        normalized_slack = 0.5 + min(0.5, slack / (self.max_time_horizon * 0.5))
                    else:
                        normalized_slack = max(0.0, 0.5 + slack / (self.max_time_horizon * 0.5))
                    obs.append(normalized_slack)
                elif self.next_operation[job_id] >= len(self.jobs[job_id]):
                    # Job completed: slack is 1.0 (good)
                    obs.append(1.0)
                else:
                    # Job not arrived: slack unknown, use 0.5 (neutral)
                    obs.append(0.5)
...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    KEY DESIGN:
    - Action space: 12 dispatching rule combinations (6 sequencing × 2 routing)
    ...
    - NO WAIT ACTION: Environment automatically advances to next event when stuck
    
    Action Space (12 combinations - NO WAIT):
    ...
    - 10: EDD+MIN  - Earliest due date, fastest machine
    - 11: EDD+MINC - Earliest due date, earliest completion
    
    AUTOMATIC EVENT ADVANCEMENT:
    - If no ready operations exist, step() automatically advances event_time
    - Continues advancing until ready operations appear OR episode terminates
    - This prevents agent from getting stuck without needing a WAIT action
    - Reduces action space complexity (11→10 actions)
    - Agent focuses on WHICH dispatching rule to use, not WHEN to wait
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # ACTION SPACE: 12 dispatching rule combinations (NO WAIT ACTION)
        # Actions 0-11: Rule combinations (6 sequencing × 2 routing)
        # If no ready ops, environment automatically advances to next event
    ...
    def _generate_poisson_arrivals(self):
        """
        Generate arrival times and dynamically calculate due dates.
        Formula: due_date = arrival_time + due_date_tightness × total_min_processing_time
        """
        for job_id in self.initial_job_ids:
            self.job_arrival_times[job_id] = 0.0
    ...
    def _apply_rule_combination(self, action_idx, ready_ops):
        ...
        seq_rules = ["FIFO", "FIFO", "LIFO", "LIFO", "SPT", "SPT", "LPT", "LPT", "MWKR", "MWKR", "EDD", "EDD"]
        route_rules = ["MIN", "MINC", "MIN", "MINC", "MIN", "MINC", "MIN", "MINC", "MIN", "MINC", "MIN", "MINC"]
        ...
        def sequencing_score(op):
            if seq_rule == "FIFO":
                ...
                # Get due date from job_due_dates
                job_id = op['job_id']
                due_date = self.job_due_dates.get(job_id, float('inf'))
                return (due_date, op['arrival_time'], job_id)
            else:
    ...
    def _get_next_event_time(self):
        ...
        next_event_time = min(next_arrival_time, next_machine_completion)
        return next_event_time  # Return float('inf') if no events, don't return self.event_time!
    ...
    def action_masks(self):
        """Generate action masks - enable all 10 rule combinations while episode is not complete.
        
        CRITICAL: Always enable all actions if episode is not complete!
        Even if no ready ops exist at current event_time, step() will automatically
        advance event_time until ready ops appear. Returning all-False mask would cause
        evaluation loops to break prematurely.
        """
        mask = np.zeros(self.action_space.n, dtype=bool)
        
        if self.operations_scheduled >= self.total_operations:
            # Episode complete - no actions available
            return mask
        
        # Enable all 10 rule combinations as long as episode is not complete
        # step() will handle automatic event advancement if no ready ops exist
        mask[:] = True
    ...
    def step(self, action):
        ...
        NO WAIT ACTION: If no ready operations exist, environment automatically advances
        event_time to next event (arrival or machine completion) until operations are ready.
        This prevents the agent from getting stuck without requiring a WAIT action.
        ...
        # AUTOMATIC EVENT ADVANCEMENT: If no ready operations, advance to next event
        # This happens BEFORE the agent selects a rule, so agent always sees valid actions
        ready_ops = self._get_ready_operations()
        
        # Keep advancing event_time until we have ready operations OR episode ends
        while not ready_ops and self.operations_scheduled < self.total_operations:
            next_event_time = self._get_next_event_time()
            
            # Only terminate if truly no more events (float('inf'))
            # If next_event_time > self.event_time, we can advance
            if next_event_time == float('inf'):
                # No more events but jobs incomplete - this is a real error
                print(f"  ERROR: No more events but {self.total_operations - self.operations_scheduled} operations remain")
                print(f"  Arrived jobs: {sorted(self.arrived_jobs)}")
                print(f"  Total jobs: {len(self.job_ids)}")
                reward = -1000.0
                return self._get_observation(), reward, True, False, {"error": "stuck_no_events"}
            
            if next_event_time <= self.event_time:
                # Event time not advancing - stuck in loop
                print(f"  ERROR: Event time not advancing: {self.event_time} -> {next_event_time}")
                reward = -1000.0
                return self._get_observation(), reward, True, False, {"error": "stuck_time_loop"}
        ...
        # Reward: negative makespan increment + estimated weighted tardiness
        makespan_increment = self.current_makespan - previous_makespan
        
        # Calculate ESTIMATED WEIGHTED TARDINESS (same as ProactiveDynamicFJSPEnv)
        tardiness_penalty = 0.0
        
        if hasattr(self, 'tardiness_weight') and self.tardiness_weight > 0:
            # Check all in-progress and completed jobs for potential/actual tardiness
            for j_id in self.job_ids:
                if j_id not in self.arrived_jobs:
                    continue  # Can't estimate for jobs that haven't arrived
                
                if self.job_due_dates.get(j_id) is not None:
                    due_date = self.job_due_dates[j_id]
                    
                    if self.next_operation[j_id] >= len(self.jobs[j_id]):
                        # Job completed - use ACTUAL tardiness
                        completion_time = self.operation_end_times[j_id][-1]
                        tardiness = max(0.0, completion_time - due_date)
                        tardiness_penalty += tardiness
                    else:
                        # Job in progress - use ESTIMATED tardiness
                        ...
                        current_job_time = max(self.operation_end_times[j_id]) if any(self.operation_end_times[j_id]) else 0.0
                        estimated_completion = current_job_time + remaining_time
                        
                        # Estimated tardiness (weighted less than actual)
                        estimated_tardiness = max(0.0, estimated_completion - due_date)
                        tardiness_penalty += 0.5 * estimated_tardiness
    ...
    def _get_observation(self):
        ...
        # Convert to numpy array and handle any numerical issues
        obs_array = np.array(obs, dtype=np.float32)
        obs_array = np.nan_to_num(obs_array, nan=0.0, posinf=1.0, neginf=0.0)
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
                 ...
                 tardiness_weight=2.0, due_date_tightness=1.5):
        """
        ...
        # Extract job operations (due dates will be calculated dynamically)
        self.job_operations = {}
        self.job_due_dates = {}  # Will be calculated dynamically in each episode
        for job_id, job_info in jobs_data.items():
            if isinstance(job_info, dict) and 'operations' in job_info:
                self.job_operations[job_id] = job_info['operations']
            else:
                self.job_operations[job_id] = job_info
        
        # Use job_operations instead of jobs_data for processing
        self.jobs = self.job_operations
        ...
        # ENHANCED observation space (includes prediction information and tardiness features)
        obs_size = (
            ...
            len(self.job_ids) +                      # NEW: Job due dates (normalized)
            len(self.job_ids) +                      # NEW: Job slack time (due - current - remaining work)
            3                                        # Arrival progress + Makespan progress + Current time
        )
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(obs_size,), dtype=np.float32
        )
        ...
        self.max_proc_time = self._calculate_max_processing_time()
        # IMPROVED: Calculate realistic time normalizer based on problem structure
        # Instead of arbitrary 1000, use sum of all processing times (upper bound on makespan)
        self.time_normalizer = self._calculate_time_normalizer()
    ...
    def _calculate_time_normalizer(self):
        """Calculate realistic time normalizer based on total work content.
        
        Uses: sum of minimum processing times across all operations + arrival window.
        This gives a realistic upper bound on schedule length (much better than arbitrary 1000).
        """
        total_min_proc_time = 0.0
        for job_ops in self.jobs.values():
            for operation in job_ops:
                # Use minimum processing time (best case for each operation)
                min_proc = min(operation['proc_times'].values())
                total_min_proc_time += min_proc
        
        # Add expected arrival window (for dynamic jobs)
        # Expected last arrival time = num_dynamic_jobs / arrival_rate
        num_dynamic_jobs = len(self.dynamic_job_ids)
        expected_arrival_window = num_dynamic_jobs / self.arrival_rate if self.arrival_rate > 0 else 0
        
        # Time normalizer = total work + arrival window
        # This represents realistic upper bound on makespan
        normalizer = total_min_proc_time + expected_arrival_window
        
        # Safety: ensure at least 100 (avoid division by very small numbers)
        return max(normalizer, 100.0)
    ...
    def step(self, action):
        ...
        # REWARD: Makespan increment + estimated weighted tardiness
        makespan_increment = self.current_makespan - previous_makespan
        
        # Calculate ESTIMATED WEIGHTED TARDINESS for denser feedback
        # This provides immediate penalty based on predicted tardiness, not just when job completes
        tardiness_penalty = 0.0
        ...
        if self.tardiness_weight > 0:
            ...
            for j_id in self.job_ids:
                ...
                if self.job_due_dates.get(j_id) is not None:
                    ...
                    if self.job_progress[j_id] >= len(self.jobs[j_id]):
                        # Job completed - use ACTUAL tardiness
                        completion_time = self.job_end_times[j_id]
                        tardiness = max(0.0, completion_time - due_date)
                        tardiness_penalty += tardiness
                    else:
                        # Job in progress - use ESTIMATED tardiness
                        # Estimate remaining time: sum of minimum processing times for remaining ops
                        remaining_time = 0.0
                        for op_idx in range(self.job_progress[j_id], len(self.jobs[j_id])):
                            operation = self.jobs[j_id][op_idx]
                            min_proc = min(operation['proc_times'].values())
                            remaining_time += min_proc
                        
                        # Estimated completion = current job end time + remaining work
                        current_job_time = self.job_end_times[j_id]
                        estimated_completion = current_job_time + remaining_time
                        
                        # Estimated tardiness (only if we're already past due date)
                        estimated_tardiness = max(0.0, estimated_completion - due_date)
                        
                        # Weight the estimate less than actual (e.g., 0.5x) to avoid over-penalization
                        tardiness_penalty += 0.5 * estimated_tardiness
    ...
    def _execute_wait_action(self, wait_duration):
        ...
        # CRITICAL FIX: Add strong penalty for waiting when ALL jobs have arrived
        # Once all jobs arrived, waiting is ALWAYS wasteful (only delays completion)
        if len(self.arrived_jobs) == len(self.job_ids):
            # All jobs arrived - waiting now is purely wasteful!
            # Add extra penalty proportional to wait duration
            wait_penalty = -(self.current_makespan - previous_makespan) * 2.0
            reward = min(reward, wait_penalty)  # Use larger penalty
    ...
    def _get_observation(self):
        ...
        for job_id in self.job_ids:
            if job_id in self.completed_jobs:
                ...
                if op_idx < len(self.jobs[job_id]):
                    ...
                    # FIXED: Normalize against realistic time_normalizer (not arbitrary max_time_horizon)
                    normalized_ready_time = min(1.0, job_ready_time / self.time_normalizer)
        ...
        for machine in self.machines:
            ...
            # FIXED: Use realistic time_normalizer instead of arbitrary max_time_horizon
            normalized_free_time = min(1.0, machine_free_time / self.time_normalizer)
        ...
        for job_id in self.job_ids:
            if job_id in self.completed_jobs:
                ...
                if op_idx < len(self.jobs[job_id]):
                    ...
                    for machine in self.machines:
                        if machine in operation['proc_times']:
                            # CRITICAL FIX: Normalize by max_proc_time, NOT max_time_horizon!
                            # Processing times are ~10-50, not 1000, so they were all appearing as ~0.0
                            normalized = min(1.0, operation['proc_times'][machine] / self.max_proc_time)
        ...
        for job_id in self.job_ids:
            if job_id in self.arrived_jobs or job_id in self.completed_jobs:
                ...
                pred_time = self.predicted_arrival_times[job_id]
                # FIXED: Normalize using realistic time_normalizer
                time_until = max(0, pred_time - self.event_time)
                normalized = min(1.0, time_until / self.time_normalizer)
                pred_arrivals.append(normalized)
            else:
                pred_arrivals.append(1.0)  # Far future
        obs_parts.extend(pred_arrivals)
        
        # 6. NEW: Job due dates (normalized)
        due_date_features = []
        for job_id in self.job_ids:
            if job_id in self.completed_jobs:
                due_date_features.append(0.0)
            elif self.job_due_dates.get(job_id) is not None:
                due_date = self.job_due_dates[job_id]
                normalized_due = min(1.0, due_date / self.time_normalizer)
                due_date_features.append(normalized_due)
            else:
                due_date_features.append(1.0)  # No due date = far future
        obs_parts.extend(due_date_features)
        
        # 7. NEW: Job slack time (due_date - current_time - remaining_work)
        slack_features = []
        for job_id in self.job_ids:
            if job_id in self.completed_jobs:
                slack_features.append(1.0)  # Completed = max slack
            elif job_id not in self.arrived_jobs:
                slack_features.append(0.0)  # Not arrived = unknown slack
            elif self.job_due_dates.get(job_id) is not None:
                due_date = self.job_due_dates[job_id]
                ...
                for op_idx in range(self.job_progress[job_id], len(self.jobs[job_id])):
                    operation = self.jobs[job_id][op_idx]
                    min_proc = min(operation['proc_times'].values())
                    remaining_work += min_proc
                
                # Slack = due_date - current_time - remaining_work
                slack = due_date - self.event_time - remaining_work
                # Normalize slack (can be negative for tardy jobs)
                normalized_slack = max(0.0, min(1.0, (slack + self.time_normalizer/2) / self.time_normalizer))
                slack_features.append(normalized_slack)
            else:
                slack_features.append(1.0)  # No due date = max slack
        obs_parts.extend(slack_features)
        
        # 8. Arrival progress (how many jobs have arrived so far)
        arrival_progress = len(self.arrived_jobs) / len(self.job_ids)
        obs_parts.append(arrival_progress)

        # 9. Makespan progress (current schedule length)
        # FIXED: Use realistic time_normalizer for better gradient signal
        makespan_progress = min(1.0, self.current_makespan / self.time_normalizer)
        obs_parts.append(makespan_progress)
        
        # 10. Current time (normalized)
        current_time_normalized = min(1.0, self.event_time / self.time_normalizer)
        obs_parts.append(current_time_normalized)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    Args:
        ...
        scenario_idx: Index of test scenario (for saving model)
    ...
    for init_idx in range(num_initializations):
        ...
        # IDENTICAL hyperparameters to Static RL
        # Both solve deterministic perfect knowledge problems
        # Should have similar training stability
        model = MaskablePPO(
            "MlpPolicy",
            vec_env,
            verbose=0,
    ...
    # Save the best trained model with scenario index
    model_path = f"perfect_knowledge_rl_model_scenario_{scenario_idx}.zip"
    best_model.save(model_path)
    print(f"    💾 Perfect Knowledge RL model saved to: {model_path}")
...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    model_path = "static_rl_model.zip"
    model.save(model_path)
    print(f"💾 Static RL model saved to: {model_path}")
    
    return model


def train_dynamic_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, total_timesteps=1000000, reward_mode="makespan_increment", learning_rate=3e-4, tardiness_weight=0.0):
    """
    ...
    # Save model
    model_path = "reactive_rl_model.zip"
    model.save(model_path)
...
def train_rule_based_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
                           total_timesteps=1000000, reward_mode="makespan_increment", 
                           learning_rate=3e-4,tardiness_weight=2.0):
    """
    ...
    model = MaskablePPO(
        ...
        vec_env,
        verbose=1,
        learning_rate=linear_schedule(learning_rate),  # Lower for stability
        n_steps=4096,              # ⭐ DOUBLED: Average out Poisson variance
        ...
        seed=GLOBAL_SEED,
        policy_kwargs=dict(
            net_arch=[256, 256, 128],  # Moderate network size
            activation_fn=torch.nn.ReLU
        )
    )
    ...
    # Save model
    model_path = "rule_based_rl_model.zip"
    model.save(model_path)
    print(f"💾 Model saved to: {model_path}")
...
def train_proactive_agent(jobs_data, machine_list, initial_jobs=5, arrival_rate=0.08, 
                          ...
                          reward_mode="makespan_increment", learning_rate=3e-4,
                          predictor_mode='mle', prior_shape=2.0, prior_rate=None,tardiness_weight=2.0):
    """
    ...
    # Save model
    model_path = "proactive_rl_model.zip"
    model.save(model_path)
    print(f"💾 Model saved to: {model_path}")
    
    # CRITICAL: Save predictor state separately
    # The predictor's learned statistics (global_n, global_sum) are NOT part of the neural network
    # and must be saved separately for evaluation
    import pickle
    predictor_state = vec_env.envs[0].env.arrival_predictor.get_state()
    predictor_path = "proactive_rl_predictor_state.pkl"
    with open(predictor_path, 'wb') as f:
        pickle.dump(predictor_state, f)
    print(f"💾 Predictor state saved to: {predictor_path}")
    print(f"   Global observations: {predictor_state['global_n']}")
    print(f"   Learned arrival rate: {predictor_state['global_estimated_rate']:.4f}")
...
def generate_test_scenarios(jobs_data, initial_jobs=[0, 1, 2, 3, 4], arrival_rate=0.08, num_scenarios=10):
    ...
    for i in range(num_scenarios):
        ...
        # Mean inter-arrival time = 1/λ = 1/0.08 = 12.5 time units
        # Starting from t=0 (after initial jobs), generate arrival times
        dynamic_job_ids = [j for j in jobs_data.keys() if j not in initial_jobs]
        current_time = 0.0  # Start Poisson process from t=0
        ...
        for job_id in dynamic_job_ids:
            ...
            current_time += inter_arrival  # Cumulative arrival time
        ...
        # Show first few dynamic arrival times to verify Poisson process
        dynamic_arrivals = {k: v for k, v in sorted(arrival_times.items()) if k not in initial_jobs}
        if dynamic_arrivals:
            first_few = dict(list(dynamic_arrivals.items())[:3])
            print(f"    First dynamic arrivals: {first_few}")
            print(f"    (Mean inter-arrival = 1/λ = 1/{arrival_rate:.3f} = {1/arrival_rate:.2f} time units)")
        print()
...
def evaluate_dynamic_on_dynamic(dynamic_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment", tardiness_weight=0.0):
    ...
    # CRITICAL: Recalculate due dates based on forced arrival times
    for job_id in test_env.env.job_ids:
        total_min_proc_time = sum(
            min(op['proc_times'].values()) for op in test_env.env.jobs[job_id]
        )
        arrival_time = arrival_times.get(job_id, 0.0)
        test_env.env.job_due_dates[job_id] = arrival_time + DUE_DATE_TIGHTNESS * total_min_proc_time
...
def evaluate_rule_based_on_dynamic(rule_model, jobs_data, machine_list, arrival_times, 
                                   reward_mode="makespan_increment", tardiness_weight=0.0):
    """
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
                                  reward_mode="makespan_increment", predictor_mode='map', tardiness_weight=0.0):
    """
    ...
    # CRITICAL FIX: Load predictor state from training
    # The arrival rate used for environment initialization doesn't matter since we'll:
    # 1. Override arrival times with test scenario
    # 2. Restore predictor's learned statistics from training
    import pickle
    import os
    ...
    if os.path.exists(predictor_path):
        with open(predictor_path, 'rb') as f:
            predictor_state = pickle.load(f)
        print(f"  ✅ Loaded predictor state from: {predictor_path}")
        print(f"     Global observations: {predictor_state['global_n']}")
        print(f"     Learned arrival rate: {predictor_state['global_estimated_rate']:.4f}")
        estimated_rate = predictor_state['global_estimated_rate']  # Use learned rate
    else:
        print(f"  ⚠️  WARNING: No saved predictor state found at {predictor_path}")
        print(f"     Predictor will start with 0 global observations (poor predictions!)")
        # Fallback: estimate from test scenario
        dynamic_arrivals = [t for t in arrival_times.values() if t > 1e-6 and t < float('inf')]
        if len(dynamic_arrivals) > 1:
            estimated_rate = (len(dynamic_arrivals) - 1) / max(dynamic_arrivals)
        else:
            estimated_rate = 0.05
    
    print(f"  📊 Using arrival rate for predictor: {estimated_rate:.4f}")
    
    test_env = ProactiveDynamicFJSPEnv(
        jobs_data, machine_list,
        initial_jobs=initial_job_ids,
        arrival_rate=estimated_rate,  # Use learned rate (will be overridden by predictor state)
        reward_mode=reward_mode,
        predictor_mode=predictor_mode,
        seed=GLOBAL_SEED,
        max_time_horizon=1000,  # Extended horizon to allow for WAIT actions
    ...
    # CRITICAL: Restore predictor state BEFORE evaluation
    if predictor_state is not None:
        test_env.env.arrival_predictor.set_state(predictor_state)
        print(f"  ✅ Predictor state restored successfully")
    ...
    # Debug: Verify arrival times and predictor state
    predictor_stats = test_env.env.arrival_predictor.get_stats()
    print(f"  Initially arrived jobs: {sorted(test_env.env.arrived_jobs)}")
    print(f"  Total jobs: {len(test_env.env.job_ids)}")
    print(f"  Total operations expected: {sum(len(ops) for ops in jobs_data.values())}")
    print(f"  📊 Predictor state after loading: rate={predictor_stats['estimated_rate']:.4f}, "
          f"mode={predictor_mode}, global_obs={predictor_stats['num_global_observations']}")
    
    # Verify predictor has learned knowledge
    if predictor_stats['num_global_observations'] == 0:
        print(f"  ⚠️  WARNING: Predictor has NO global observations (fresh start)!")
        print(f"      This will lead to poor predictions and suboptimal wait decisions.")
        print(f"      SOLUTION: Ensure predictor state is saved during training.")
    else:
        print(f"  ✅ Predictor has {predictor_stats['num_global_observations']} global observations (good!)")
    ...
    while step_count < max_steps:
        ...
        if action >= test_env.env.wait_action_start:
            ...
            # Debug logging for wait actions
            if wait_count <= 10:  # Log first 10 waits
                print(f"    🕐 WAIT action #{wait_count} at step {step_count}: duration={wait_duration}, "
                      f"event_time={test_env.env.event_time:.1f}, "
                      f"arrived={len(test_env.env.arrived_jobs)}/{len(test_env.env.job_ids)}, "
                      f"completed={len(test_env.env.completed_jobs)}/{len(test_env.env.job_ids)}")
...
def remaining_work_estimate(jobs_data, job_id, op_idx):
    ...
    total = 0.0
    # Handle both dict format {'operations': [...], 'due_date': ...} and list format
    ...
    for oi in range(op_idx, len(job_operations)):
        total += min(job_operations[oi]['proc_times'].values())
    return total
...
def simple_list_scheduling(jobs_data, machine_list, arrival_times, seq_rule, route_rule="MIN"):
    ...
    # Heuristic starts knowing ONLY about jobs that have arrived by current sim_time
    # This ensures NO CHEATING - same as RL agents (event-driven discovery)
    arrived_jobs = {job_id for job_id, arr_time in arrival_times.items() if arr_time <= sim_time}
    ...
    while completed_operations < total_operations:
        ...
        if not ready_operations:
            # AUTOMATIC EVENT ADVANCEMENT: Advance sim_time to next event
            # When no ready operations exist, we need to wait for:
            # 1. New job arrivals (jobs not yet discovered)
            # 2. Machine completions (which may unblock precedence-constrained ops)
            next_time = float('inf')
        ...
        def sequencing_score(op):
            ...
            if seq_rule == "FIFO":
                ...
                # Earliest Due Date: prioritize jobs with earlier due dates
        ...
        # CRITICAL: Update sim_time after scheduling (same logic as Rule-Based RL)
        # Check if there are more ready operations after this scheduling action
        ...
        # If no ready operations after scheduling AND jobs remain, advance to next event
        if not ready_operations_after and completed_operations < total_operations:
            ...
            # Check for next new job arrival
            for job_id_check, arr_time in arrival_times.items():
                if job_id_check not in arrived_jobs and arr_time > sim_time:
                    next_time = min(next_time, arr_time)
            
            # Check for next machine completion
            for machine, free_time in machine_next_free.items():
                if free_time > sim_time:
                    next_time = min(next_time, free_time)
...
def run_heuristic_comparison(jobs_data, machine_list, arrival_times, verbose=False):
    ...
    Tests:
    - Sequencing rules: FIFO, LIFO, SPT, LPT, MWKR, EDD (6 rules)
    - Routing rules: MIN, MINC (2 rules)
    - Total combinations: 6 × 2 = 12 strategies
    
    WHY MINC OFTEN DOMINATES (and how we break it):
    ------------------------------------------------
    MINC (Minimum Completion) = Schedule on machine that finishes earliest
    
    MINC works well when:
    - Low machine heterogeneity (machines similar speed)
    - Low processing time variance (operations similar duration)
    - High current machine availability
    
    → In these cases, "earliest completion" ≈ "optimal choice"
    
    MINC FAILS when (our new dataset):
    - EXTREME machine heterogeneity (2.5x speed difference!)
    - HUGE processing time variance (1-100 range!)
    - Strategic waiting opportunities (fast machine worth waiting for)
    
    → Greedy "earliest completion" != globally optimal
    → Need lookahead: "Save fast machine for long operations coming soon"
    
    This creates strategic depth for RL agents to exploit!
    """
...
# =============================================================================
# OPTIMAL SCHEDULERS: MILP (PuLP) and CP-SAT (OR-Tools)
# =============================================================================
# Two approaches for computing optimal schedules:
# 
# 1. MILP (Mixed Integer Linear Programming) via PuLP
#    - Supports multiple solvers: Gurobi (default), CBC, CPLEX
#    - Gurobi: Commercial, very fast, requires license
#    - CBC: Open-source, slower but free
#    - CPLEX: Commercial, IBM solver
#
# 2. CP-SAT (Constraint Programming) via OR-Tools
#    - Google's open-source constraint solver
#    - Often faster than MILP for scheduling problems
#    - Recommended as first choice
#
# Installation:
#   pip install pulp              # For MILP solvers
#   pip install ortools           # For CP-SAT
#   pip install gurobipy          # For Gurobi (requires license)
# =============================================================================

def milp_optimal_scheduler(jobs_data, machine_list, arrival_times, time_limit=1200, verbose=True, solver='gurobi', 
                          tardiness_weight=0.0, job_due_dates=None):
    """
    MILP approach for optimal dynamic scheduling using Gurobi's native Python API.
    Supports multi-objective optimization: minimize (makespan + tardiness_weight * total_tardiness).
    Uses caching to avoid re-solving identical scenarios.

    Args:
        jobs_data: dict mapping job_id -> list of operations or dict with 'operations' and 'due_date'
                   e.g. jobs_data[j] = [ {'proc_times': {'M1': 3, 'M2':5}}, ... ]
                   or jobs_data[j] = {'operations': [...], 'due_date': 100.0}
        ...
        solver: Not used anymore (kept for backward compatibility) - always uses Gurobi
        tardiness_weight: Weight for tardiness in objective (0.0 = makespan only, 1.0 = tardiness only)
        job_due_dates: Optional dict mapping job_id -> due_date (overrides due_dates in jobs_data)

    Returns:
        (optimal_objective, schedule) or (float('inf'), {}) if failed.
        schedule is dict: machine -> list of (op_name, start, end)
    """
    # DEBUG: Print arrival times and objective settings
    print(f"\n  🔍 MILP OPTIMAL using arrival times: {arrival_times}")
    print(f"  Using Gurobi native Python API")
    print(f"  Objective: makespan + {tardiness_weight} × total_tardiness")
    
    # Extract job operations and due dates
    job_operations = {}
    due_dates = {}
    for job_id, job_info in jobs_data.items():
        if isinstance(job_info, dict) and 'operations' in job_info:
            job_operations[job_id] = job_info['operations']
            due_dates[job_id] = job_info.get('due_date', float('inf'))
        else:
            job_operations[job_id] = job_info
            due_dates[job_id] = float('inf')
    
    # Override with explicit due_dates if provided
    if job_due_dates is not None:
        due_dates.update(job_due_dates)
    
    # Use job_operations instead of jobs_data for processing
    jobs_data_ops = job_operations
    ...
    try:
        ...
        print("❌ Gurobi not installed. Install with: pip install gurobipy")
        print("   Note: Requires valid Gurobi license (free academic license available)")
        return float('inf'), {}
    ...
    ops = [(j, oi) for j in jobs_data_ops for oi in range(len(jobs_data_ops[j]))]

    # Compute safe BIG_M: total processing time + max arrival
    all_proc_times = []
    for j, oi in ops:
        all_proc_times.extend(jobs_data_ops[j][oi]['proc_times'].values())
    total_proc = sum(all_proc_times) if all_proc_times else 0.0
    max_arrival = max(arrival_times.values()) if arrival_times else 0.0
    max_due_date = max(due_dates.values()) if due_dates and any(d != float('inf') for d in due_dates.values()) else max_arrival + total_proc
    BIG_M = max(total_proc + max_arrival, max_due_date) + 1.0  # safe upper bound

    # Create Gurobi model
    try:
        model = gp.Model("PerfectKnowledge_FJSP_Optimal")
        model.Params.TimeLimit = time_limit
        model.Params.OutputFlag = 1 if verbose else 0
    except Exception as e:
        print(f"❌ Failed to create Gurobi model: {e}")
        print("   Check your Gurobi license with: grbgetkey YOUR-LICENSE-KEY")
        return float('inf'), {}
    ...
    for op in ops:
        ...
        for m in machine_list:
            if m in jobs_data_ops[j][oi]['proc_times']:
                x[j, oi, m] = model.addVar(vtype=GRB.BINARY, name=f"x_{j}_{oi}_{m}")
    ...
    for op in ops:
        j, oi = op
        s[j, oi] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"s_{j}_{oi}")
        c[j, oi] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"c_{j}_{oi}")

    # Job completion times (completion of last operation)
    job_completion = {}
    for j in jobs_data_ops.keys():
        job_completion[j] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"job_comp_{j}")
    
    # Tardiness variables (job_completion - due_date, or 0 if early)
    tardiness = {}
    for j in jobs_data_ops.keys():
        if due_dates[j] != float('inf'):
            tardiness[j] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"tardiness_{j}")
    ...
    # Objective: minimize weighted sum of makespan and total tardiness
    if tardiness_weight > 0 and any(d != float('inf') for d in due_dates.values()):
        total_tardiness = gp.quicksum(tardiness[j] for j in tardiness.keys())
        model.setObjective(Cmax + tardiness_weight * total_tardiness, GRB.MINIMIZE)
        print(f"  Multi-objective: Cmax + {tardiness_weight} × ΣTardiness")
    else:
        model.setObjective(Cmax, GRB.MINIMIZE)
        print(f"  Single-objective: Cmax only")

    # Constraints
    for op in ops:
        j, oi = op
        # 1. assignment to exactly one compatible machine
        compatible_machines = [m for m in machine_list if m in jobs_data_ops[j][oi]['proc_times']]
        if not compatible_machines:
        ...
        model.addConstr(
            gp.quicksum(x[j, oi, m] for m in compatible_machines) == 1,
            name=f"assign_{j}_{oi}"
        )

        # 2. completion time = start + processing time (depends on chosen machine)
        model.addConstr(
            c[j, oi] == s[j, oi] + gp.quicksum(
                x[j, oi, m] * jobs_data_ops[j][oi]['proc_times'][m] 
                for m in compatible_machines
            ),
            name=f"completion_{j}_{oi}"
        )
        ...
        if oi > 0:
            ...
            model.addConstr(
                s[j, oi] >= arr,
                name=f"arrival_{j}_{oi}"
            )

        # 4. makespan constraint
        model.addConstr(Cmax >= c[j, oi], name=f"makespan_{j}_{oi}")
    
    # 5. Job completion time (completion of last operation)
    for j in jobs_data_ops.keys():
        last_op_idx = len(jobs_data_ops[j]) - 1
        model.addConstr(
            job_completion[j] == c[j, last_op_idx],
            name=f"job_completion_{j}"
        )
    
    # 6. Tardiness constraints (tardiness = max(0, completion - due_date))
    for j in jobs_data_ops.keys():
        if due_dates[j] != float('inf'):
            model.addConstr(
                tardiness[j] >= job_completion[j] - due_dates[j],
                name=f"tardiness_def_{j}"
            )
            model.addConstr(
                tardiness[j] >= 0,
                name=f"tardiness_nonneg_{j}"
            )

    # 7. disjunctive sequencing constraints for operations on same machine
    # y[op1, op2, m] = 1 if op1 before op2 on machine m
    y = {}
    for m in machine_list:
        # operations compatible with m
        ops_on_m = [op for op in ops if m in jobs_data_ops[op[0]][op[1]]['proc_times']]
        # pairwise sequencing
        for op1_idx in range(len(ops_on_m)):
            for op2_idx in range(op1_idx + 1, len(ops_on_m)):
                ...
                # create sequencing binary var
                y[op1, op2, m] = model.addVar(vtype=GRB.BINARY, name=f"y_{j1}_{oi1}_{j2}_{oi2}_{m}")
                
                # create both_on_m auxiliary variable (linearization of AND)
                both_on_m = model.addVar(vtype=GRB.BINARY, name=f"both_{j1}_{oi1}_{j2}_{oi2}_{m}")
                model.addConstr(both_on_m <= x[j1, oi1, m])
                model.addConstr(both_on_m <= x[j2, oi2, m])
                model.addConstr(both_on_m >= x[j1, oi1, m] + x[j2, oi2, m] - 1)

                # sequencing constraints (big-M formulation)
                # If both operations on same machine, one must come before the other
                # op1 comes after op2 OR y=1 OR both_on_m=0
                model.addConstr(
                    s[j1, oi1] >= c[j2, oi2] - BIG_M * (1 - y[op1, op2, m]) - BIG_M * (1 - both_on_m),
                    name=f"seq1_{j1}_{oi1}_{j2}_{oi2}_{m}"
                )
                # op2 comes after op1 OR y=0 OR both_on_m=0
                model.addConstr(
                    s[j2, oi2] >= c[j1, oi1] - BIG_M * y[op1, op2, m] - BIG_M * (1 - both_on_m),
                    name=f"seq2_{j1}_{oi1}_{j2}_{oi2}_{m}"
                )

    # Update model before optimization
    model.update()
    
    # Print total number of decision variables
    num_vars = model.NumVars
    num_binary = model.NumBinVars
    num_continuous = num_vars - num_binary
    print(f"\n📊 MILP Model Statistics:")
    print(f"   Total decision variables: {num_vars}")
    print(f"   - Binary variables: {num_binary}")
    print(f"   - Continuous variables: {num_continuous}")
    ...
    try:
        ...
        error_msg = str(e)
        if "size-limited license" in error_msg.lower() or "model too large" in error_msg.lower():
            print(f"⚠️  Gurobi license limitation: {e}")
            print(f"    Falling back to OR-Tools CP-SAT solver...")
            try:
                return ortools_cp_sat_scheduler(jobs_data, machine_list, arrival_times, time_limit, verbose)
            except Exception as ortools_error:
                print(f"❌ OR-Tools also failed: {ortools_error}")
                return float('inf'), {}
        else:
    ...
    # Check status
    status = model.Status
    solver_proven_optimal = (status == GRB.OPTIMAL)
    
    if status == GRB.OPTIMAL:
        print("✅ True optimal solution found (proven by solver).")
    elif status == GRB.TIME_LIMIT:
        print(f"⚠️  Time limit reached. Solution may be suboptimal.")
    ...
    if status == GRB.OPTIMAL or status == GRB.TIME_LIMIT:
        try:
            optimal_makespan = Cmax.X
        except Exception:
            optimal_makespan = float('inf')

        # Build schedule from variable values
        for op in ops:
            j, oi = op
            for m in machine_list:
                if (j, oi, m) in x and x[j, oi, m].X > 0.5:
                    st = s[j, oi].X
                    en = c[j, oi].X
        ...
        for m in schedule:
            schedule[m].sort(key=lambda tup: tup[1])
        ...
        if math.isfinite(actual_makespan) and abs(actual_makespan - optimal_makespan) <= 1e-6:
            ...
            try:
                cached_result = {
                    ...
                    'arrival_times': arrival_times,
                    'solver_proven_optimal': solver_proven_optimal
                }
...
def ortools_cp_sat_scheduler(jobs_data, machine_list, arrival_times, time_limit=300, verbose=True):
    """
    OR-Tools CP-SAT solver for optimal dynamic scheduling.
    Often faster than MILP for job shop scheduling problems.
    
    Args:
        jobs_data: dict mapping job_id -> list of operations
        machine_list: list of machine names
        arrival_times: dict mapping job_id -> arrival_time
        time_limit: Solver time limit in seconds
        verbose: if True prints solver progress
    ...
    """
    print(f"\n  🔍 OR-Tools CP-SAT using arrival times: {arrival_times}")
    ...
    for job_id, operations in jobs_data.items():
        for op in operations:
            all_proc_times.append(max(op['proc_times'].values()))
    horizon = int(sum(all_proc_times) + max(arrival_times.values()) + 100)
    
    # Decision variables
    # task_vars[(job_id, op_idx)] = (start_var, end_var, interval_var, machine_var)
    task_vars = {}
    machine_to_intervals = {m: [] for m in machine_list}
    
    for job_id, operations in jobs_data.items():
        for op_idx, op_data in enumerate(operations):
            suffix = f"_j{job_id}_o{op_idx}"
            
            # Start time variable
            start_var = model.NewIntVar(0, horizon, f"start{suffix}")
            
            # Machine assignment variable (use machine index)
            compatible_machines = [i for i, m in enumerate(machine_list) if m in op_data['proc_times']]
            if not compatible_machines:
                print(f"❌ No compatible machines for Job {job_id} Op {op_idx}")
                return float('inf'), {}
            
            machine_var = model.NewIntVarFromDomain(
                cp_model.Domain.FromValues(compatible_machines), 
                f"machine{suffix}"
            )
            
            # For each possible machine, create interval and enforce with indicator
            intervals_for_task = []
            machine_booleans = []  # Boolean variables for machine selection
            
            for machine_idx in compatible_machines:
                machine_name = machine_list[machine_idx]
                proc_time = int(op_data['proc_times'][machine_name])
                
                # Create boolean variable for "this machine is selected"
                is_selected = model.NewBoolVar(f"select{suffix}_m{machine_idx}")
                machine_booleans.append((machine_idx, is_selected))
                
                # Interval variable (optional, activated when this machine is selected)
                interval_var = model.NewOptionalFixedSizeIntervalVar(
                    start_var, proc_time, is_selected, 
                    f"interval{suffix}_m{machine_idx}"
                )
                intervals_for_task.append((machine_idx, interval_var, proc_time, is_selected))
                machine_to_intervals[machine_name].append(interval_var)
            
            # Link boolean variables to machine_var
            # Exactly one machine must be selected
            model.AddExactlyOne([is_selected for _, is_selected in machine_booleans])
            
            # Link machine_var to boolean variables
            for machine_idx, is_selected in machine_booleans:
                model.Add(machine_var == machine_idx).OnlyEnforceIf(is_selected)
            
            # End time = start + processing time (depends on selected machine)
            end_var = model.NewIntVar(0, horizon, f"end{suffix}")
            
            # Enforce end = start + proc_time for selected machine
            for machine_idx, interval_var, proc_time, is_selected in intervals_for_task:
                # If this machine is selected, end = start + proc_time
                model.Add(end_var == start_var + proc_time).OnlyEnforceIf(is_selected)
            
            task_vars[(job_id, op_idx)] = (start_var, end_var, intervals_for_task, machine_var)
    ...
    for job_id, operations in jobs_data.items():
        for op_idx in range(len(operations) - 1):
            curr_end = task_vars[(job_id, op_idx)][1]
            next_start = task_vars[(job_id, op_idx + 1)][0]
            model.Add(next_start >= curr_end)
    
    # 2. Arrival time constraints
    for job_id, arr_time in arrival_times.items():
        if job_id in jobs_data and len(jobs_data[job_id]) > 0:
            first_op_start = task_vars[(job_id, 0)][0]
            # Use ceiling to ensure we don't violate arrival constraint due to floating-point precision
            model.Add(first_op_start >= math.ceil(arr_time))
    ...
    for (job_id, op_idx), (start_var, end_var, _, _) in task_vars.items():
        if op_idx == len(jobs_data[job_id]) - 1:  # Last operation of job
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        if status == cp_model.OPTIMAL:
            print(f"✅ OR-Tools found OPTIMAL solution: {optimal_makespan:.2f}")
        else:
            print(f"⚠️  OR-Tools found FEASIBLE solution: {optimal_makespan:.2f} (not proven optimal)")
        
        # Build schedule
        schedule = {m: [] for m in machine_list}
        
        for (job_id, op_idx), (start_var, end_var, intervals_for_task, machine_var) in task_vars.items():
            start_time = solver.Value(start_var)
            end_time = solver.Value(end_var)
            selected_machine_idx = solver.Value(machine_var)
            machine_name = machine_list[selected_machine_idx]
            
            schedule[machine_name].append((f"J{job_id}-O{op_idx+1}", float(start_time), float(end_time)))
        ...
        # Validate
        actual_makespan = validate_schedule_makespan(schedule, jobs_data, arrival_times)
        if math.isfinite(actual_makespan):
            if verbose:
                print(f"✅ OR-Tools solution validated. Makespan = {actual_makespan:.2f}")
            return actual_makespan, schedule
        else:
            print("❌ OR-Tools solution validation failed")
            return float('inf'), {}
    else:
        print(f"❌ OR-Tools did not find solution. Status: {solver.StatusName(status)}")
        return float('inf'), {}
...
def main():
    ...
    tardiness_weight = 0.0  # Weight for tardiness in objective (0.0 = makespan only, >0 = multi-objective)
    
    arrival_rate = 0.2 # LOWER arrival rate to create more dynamic scenarios
    # With λ=0.5, expected inter-arrival = 2 time units (faster than most job operations)
    
    # Step 1: Training Setup
    print("\n1. TRAINING SETUP")
    print("-" * 50)
    perfect_timesteps = 500000    # Perfect knowledge with MULTIPLE INITIALIZATIONS
    ...
    rule_based_timesteps = 500000  # Same as reactive - needs to explore all rule combos
    ...
    print(f"   Perfect RL: {perfect_timesteps:,} timesteps (scenario-specific)")
    print(f"   Reactive RL: {dynamic_timesteps:,} timesteps (+700k, batch=1024, ent=0.03)")
    print(f"   Proactive RL: {proactive_timesteps:,} timesteps (+1.2M, batch=1024, ent=0.05)")
    print(f"   Rule-Based RL: {rule_based_timesteps:,} timesteps (+700k, batch=1024, ent=0.05)")
    print(f"   Static RL: {static_timesteps:,} timesteps")
    print(f"")
    print(f"🔧 SOLVER IMPROVEMENTS:")
    print(f"   - Gurobi with automatic OR-Tools fallback if license limited")
    print(f"   - OR-Tools CP-SAT often faster for scheduling problems")
    print(f"")
    print(f"💡 KEY IMPROVEMENTS:")
    print(f"   - Increased timesteps: RL agents need more training to beat heuristics")
    print(f"   - Higher entropy (0.03-0.05): Better exploration of wait/rule actions")
    print(f"   - Larger batches (1024): More stable gradient updates")
    print(f"   - Rule-Based RL should now match/exceed best heuristic (flexible action space)")
    print(f"")
    ...
    # Save Proactive RL training metrics to JSON file
    import json
    proactive_metrics = {
        'mean_episode_reward': TRAINING_METRICS['rollout_ep_rew_mean'],
        ...
        'action_entropy': TRAINING_METRICS['action_entropy'],
        'kl_divergence': TRAINING_METRICS['kl_divergence'],
        'explained_variance': TRAINING_METRICS['explained_variance'],
        'timesteps': proactive_timesteps,
        'arrival_rate': arrival_rate
    }
    
    metrics_filename = f'proactive_rl_training_metrics_rate_{arrival_rate}.json'
    with open(metrics_filename, 'w') as f:
        json.dump(proactive_metrics, f, indent=2)
    print(f"💾 Proactive RL training metrics saved to: {metrics_filename}")
    
    # Plot the training metrics
    plot_training_metrics()
    
    # Create a dedicated plot for mean episode reward with enhanced visualization
    if TRAINING_METRICS['rollout_ep_rew_mean']:
        import numpy as np
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
        fig.suptitle(f'Proactive RL Training Analysis (λ={arrival_rate}, {proactive_timesteps:,} timesteps)', 
                     fontsize=18, fontweight='bold')
        
        rollout_numbers = list(range(1, len(TRAINING_METRICS['rollout_ep_rew_mean']) + 1))
        mean_rewards = TRAINING_METRICS['rollout_ep_rew_mean']
        
        # Plot 1: Mean Episode Reward with smoothing
        ax1.plot(rollout_numbers, mean_rewards, 'b-', linewidth=1.5, alpha=0.4, label='Raw Mean Reward')
        
        # Add smoothed curve (simple moving average)
        if len(mean_rewards) > 20:
            window_size = min(20, len(mean_rewards) // 5)
            smoothed = np.convolve(mean_rewards, np.ones(window_size)/window_size, mode='same')
            ax1.plot(rollout_numbers, smoothed, 'r-', linewidth=2.5, label=f'Smoothed (window={window_size})')
        
        ax1.set_xlabel('Rollout Number', fontsize=14)
        ax1.set_ylabel('Mean Episode Reward', fontsize=14)
        ax1.set_title('Mean Episode Reward Over Training', fontsize=15, fontweight='bold')
        ax1.grid(True, alpha=0.3)
        ax1.legend(fontsize=12)
        
        # Add statistics text box
        final_mean = np.mean(mean_rewards[-10:]) if len(mean_rewards) >= 10 else np.mean(mean_rewards)
        initial_mean = np.mean(mean_rewards[:10]) if len(mean_rewards) >= 10 else mean_rewards[0]
        improvement = ((final_mean - initial_mean) / abs(initial_mean)) * 100 if initial_mean != 0 else 0
        
        stats_text = f"Initial (avg first 10): {initial_mean:.2f}\n" \
                    f"Final (avg last 10): {final_mean:.2f}\n" \
                    f"Improvement: {improvement:+.1f}%"
        ax1.text(0.02, 0.98, stats_text, transform=ax1.transAxes, 
                fontsize=11, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
        
        # Plot 2: Learning Progress (cumulative improvement)
        if len(mean_rewards) > 1:
            cumulative_best = [max(mean_rewards[:i+1]) for i in range(len(mean_rewards))]
            ax2.plot(rollout_numbers, cumulative_best, 'g-', linewidth=2, label='Best Reward So Far')
            ax2.fill_between(rollout_numbers, cumulative_best, alpha=0.3, color='green')
            
            ax2.set_xlabel('Rollout Number', fontsize=14)
            ax2.set_ylabel('Best Mean Reward Achieved', fontsize=14)
            ax2.set_title('Learning Progress (Best Performance)', fontsize=15, fontweight='bold')
            ax2.grid(True, alpha=0.3)
            ax2.legend(fontsize=12)
        
        plt.tight_layout()
        
        reward_plot_filename = f'proactive_rl_mean_ep_reward_rate_{arrival_rate}.png'
        plt.savefig(reward_plot_filename, dpi=300, bbox_inches='tight')
        print(f"📊 Proactive RL mean episode reward plot saved to: {reward_plot_filename}")
        plt.close()
    ...
    rule_based_model = train_rule_based_agent(
        ...
        initial_jobs=INITIAL_JOB_IDS,
        arrival_rate=arrival_rate,
        total_timesteps=rule_based_timesteps,  # Use dedicated rule_based_timesteps
        reward_mode="makespan_increment",
        learning_rate=learning_rate
    )
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        # STEP 1: Compute Optimal Solution FIRST (to use as target for Perfect RL)
        # Try OR-Tools CP-SAT first (usually faster for scheduling), fallback to MILP if unavailable
        print(f"  Computing Optimal Solution for scenario {i+1}...")
        
        # Try OR-Tools CP-SAT first (if available)
        ortools_available = True
        try:
            print(f"    Attempting OR-Tools CP-SAT solver...")
            ortools_start_time = time.time()
            optimal_makespan, optimal_schedule = ortools_cp_sat_scheduler(ENHANCED_JOBS_DATA, MACHINE_LIST, scenario_arrivals, time_limit=300)
            ...
            if optimal_makespan != float('inf'):
                ...
                milp_solve_time = ortools_solve_time
            else:
        ...
        if not ortools_available:
            # Fallback to MILP (will try Gurobi, then fallback to CBC internally)
            print(f"    OR-Tools not available or failed, using MILP solver...")
            
            # Calculate due dates based on arrival times and due_date_tightness
            milp_due_dates = {}
            for job_id in ENHANCED_JOBS_DATA.keys():
                total_min_proc_time = sum(
                    min(op['proc_times'].values()) for op in ENHANCED_JOBS_DATA[job_id]
                )
                arrival_time = scenario_arrivals.get(job_id, 0.0)
                milp_due_dates[job_id] = arrival_time + DUE_DATE_TIGHTNESS * total_min_proc_time
        ...
        perfect_model = train_perfect_knowledge_agent(
            ...
            reward_mode="makespan_increment", 
            learning_rate=learning_rate,
            num_initializations=1,  # ⭐ Try 3 random initializations
            milp_optimal=milp_makespan if milp_is_valid else None,  # ⭐ Pass MILP for early stopping
            scenario_idx=i  # ⭐ Pass scenario index for model saving
        )
        if i == 0:
        ...
        proactive_makespan, proactive_schedule = evaluate_proactive_on_dynamic(
            proactive_model, ENHANCED_JOBS_DATA, MACHINE_LIST, scenario_arrivals, predictor_mode='map')
    ...
    # Helper function to calculate total tardiness from schedule
    def calculate_total_tardiness(schedule, jobs_data, arrival_times):
        """Calculate total tardiness given a schedule and job due dates."""
        if not schedule:
            return 0.0
        
        # Extract job completion times from schedule
        job_completion_times = {}
            ...
            for op_data in ops:
                if len(op_data) >= 3:
                    ...
                    if 'J' in job_op:
                        try:
                            job_id = int(job_op.split('J')[1].split('-')[0])
                            # Track latest completion time for each job
        ...
        for job_id, completion_time in job_completion_times.items():
            # Get due date from jobs_data
            job_info = jobs_data.get(job_id)
            if job_info:
                if isinstance(job_info, dict) and 'due_date' in job_info:
    ...
    if milp_makespan != float('inf'):
        ...
        schedules_data = [
            {'schedule': milp_schedule, 'makespan': milp_makespan, 'title': 'MILP Optimal (Benchmark)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(milp_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': perfect_schedule, 'makespan': perfect_makespan, 'title': 'Perfect Knowledge RL', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(perfect_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': proactive_schedule, 'makespan': all_results['Proactive RL'][0], 'title': 'Proactive RL (Learned Predictions)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(proactive_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': dynamic_schedule, 'makespan': dynamic_makespan, 'title': 'Reactive RL (Reactive)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(dynamic_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': rule_based_schedule, 'makespan': all_results['Rule-Based RL'][0], 'title': 'Rule-Based RL (Learned Rule Selection)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(rule_based_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': static_dynamic_schedule, 'makespan': static_dynamic_makespan, 'title': 'Static RL (on dynamic scenario)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(static_dynamic_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': spt_schedule, 'makespan': spt_makespan, 'title': 'Best Heuristic', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(spt_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)}
        ]
    else:
    ...
    for plot_idx, data in enumerate(schedules_data):
        ...
        tardiness = data.get('tardiness', 0.0)
        ...
        ax.set_title(f"{title} (Makespan: {makespan:.2f}, Total Tardiness: {tardiness:.2f})", fontweight='bold')
    ...
    # Create a focused 5-method Gantt chart comparison (MILP, Perfect RL, Proactive RL, Rule-Based RL, Best Heuristic)
    print(f"\n7b. FOCUSED 5-METHOD GANTT CHART COMPARISON")
    print("-" * 60)
    
    if milp_makespan != float('inf'):
        fig_5method, axes_5method = plt.subplots(5, 1, figsize=(18, 17.5))
        fig_5method.suptitle('Focused Scheduling Comparison: 5 Key Methods\n' + 
                             f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                             f'Comparing: MILP Optimal, Perfect RL, Proactive RL, Rule-Based RL, Best Heuristic', 
                             fontsize=16, fontweight='bold')
        
        schedules_data_5method = [
            {'schedule': milp_schedule, 'makespan': milp_makespan, 'title': 'MILP Optimal (Benchmark)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(milp_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': perfect_schedule, 'makespan': perfect_makespan, 'title': 'Perfect Knowledge RL', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(perfect_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': proactive_schedule, 'makespan': all_results['Proactive RL'][0], 'title': 'Proactive RL (Learned Predictions)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(proactive_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
            {'schedule': rule_based_schedule, 'makespan': all_results['Rule-Based RL'][0], 'title': 'Rule-Based RL (Learned Rule Selection)', 'arrival_times': first_scenario_arrivals, 'tardiness': calculate_total_tardiness(rule_based_schedule, ENHANCED_JOBS_DATA, first_scenario_arrivals)},
        ...
        max_makespan_5method = 0
        for data in schedules_data_5method:
            schedule = data['schedule']
            if schedule and any(len(ops) > 0 for ops in schedule.values()):
                schedule_max_time = max([max([op[2] for op in ops]) for ops in schedule.values() if ops])
                max_makespan_5method = max(max_makespan_5method, schedule_max_time)
        
        consistent_x_limit_5method = max_makespan_5method * 1.1 if max_makespan_5method > 0 else 100
        
        for plot_idx, data in enumerate(schedules_data_5method):
            schedule = data['schedule']
            makespan = data['makespan']
            title = data['title']
            arrival_times = data['arrival_times']
            ...
            ax = axes_5method[plot_idx]
            
            if not schedule or all(len(ops) == 0 for ops in schedule.values()):
                ax.text(0.5, 0.5, 'No valid schedule', ha='center', va='center', 
                       transform=ax.transAxes, fontsize=14)
                ax.set_title(f"{title} - No Solution")
                ax.set_xlim(0, consistent_x_limit_5method)
                ax.set_ylim(-0.5, len(MACHINE_LIST) + 2.0)
                continue
            ...
            if arrival_times:
                ...
                for job_id, arrival_time in arrival_times.items():
                    if arrival_time > 0 and arrival_time < consistent_x_limit_5method:
                        ax.axvline(x=arrival_time, color='red', linestyle='--', alpha=0.7, linewidth=2)
                        ax.annotate(f'Job {job_id} arrives', 
                                   xy=(arrival_time, arrow_y_position), 
                                   xytext=(arrival_time, arrow_y_position + 0.5),
            ...
            ax.set_yticks(range(len(MACHINE_LIST)))
            ax.set_yticklabels(MACHINE_LIST)
            ax.set_xlabel("Time" if plot_idx == len(schedules_data_5method)-1 else "")
            ax.set_ylabel("Machines")
            ax.set_title(f"{title} (Makespan: {makespan:.2f}, Total Tardiness: {tardiness:.2f})", fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, consistent_x_limit_5method)
        ...
        # Add legend
        legend_elements_5method = []
        for i in range(len(ENHANCED_JOBS_DATA)):
            color = colors[i % len(colors)]
            initial_or_poisson = ' (Initial)' if i < 3 else ' (Poisson)'
            legend_elements_5method.append(plt.Rectangle((0, 0), 1, 1, facecolor=color, 
                                              alpha=0.8, label=f'Job {i}{initial_or_poisson}'))
        
        fig_5method.legend(handles=legend_elements_5method, loc='center', bbox_to_anchor=(0.5, 0.02), 
                  ncol=len(ENHANCED_JOBS_DATA), fontsize=10)
        
        plt.tight_layout(rect=[0, 0.06, 1, 0.96])
        
        filename_5method = 'focused_5method_gantt_comparison.png'
        plt.savefig(filename_5method, dpi=300, bbox_inches='tight')
        print(f"✅ Saved focused 5-method Gantt chart comparison: {filename_5method}")
        plt.close(fig_5method)
    else:
        print("⚠️  Skipping 5-method chart (MILP invalid)")
    ...
    for scenario_idx in range(len(gantt_scenarios_data)):
        ...
        for plot_idx, (method_name, (makespan, schedule)) in enumerate(methods_to_plot):
            ...
            # Calculate tardiness for this schedule
            tardiness = calculate_total_tardiness(schedule, ENHANCED_JOBS_DATA, arrival_times)
            ...
            ax.set_ylabel("Machines")
            ax.set_title(f"{method_name} (Makespan: {makespan:.2f}, Total Tardiness: {tardiness:.2f})", fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, x_limit_scenario)
            ax.set_ylim(-0.5, len(MACHINE_LIST) + 1.5)
