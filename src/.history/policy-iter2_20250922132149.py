...
# Analyze the state space breakdown
print("=== STATE SPACE ANALYSIS ===")
next_ops_counts = {}
for sid in range(num_states):
    state = id_to_state[sid]
    next_ops = state[0]
    if next_ops not in next_ops_counts:
        next_ops_counts[next_ops] = 0
    next_ops_counts[next_ops] += 1

print("States by job progress (next_ops):")
for next_ops, count in sorted(next_ops_counts.items()):
    finished_ops = sum(next_ops)
    print(f"  {next_ops}: {count} states (total ops completed: {finished_ops})")
    
print(f"\nTheoretical next_ops combinations: {len(next_ops_counts)}")
print(f"Actual states: {num_states}")
print(f"Average states per next_ops combination: {num_states / len(next_ops_counts):.1f}")
print("The explosion comes from timing constraints creating many distinct machine/job ready time combinations!")
