...
while q and len(id_to_state) < MAX_STATES:
    ...
    for action in legal_actions:
        ...
        # reward: negative makespan increment (use -0.01 small penalty per step to avoid positive cycles)
        new_mk = compute_makespan_internal(new_machine_next_free, new_job_ready)
        reward = -(new_mk - cur_makespan) - 0.01
...
if incomplete_mdp:
    ...
    while True:
        ...
        print(f"  Step {step}: state {cur_sid}, next_ops={cur_next_ops}, finished_ops={finished_ops}/{total_operations}, action={a}")
        if a is None:
            print(f"  No action available (terminal state)")
            break
        if finished_ops >= total_operations:
        ...
        # reward: compute old makespan before update, new makespan after update
        old_mk = compute_makespan_internal([cur_mnext[i] if i != m_idx else cur_time for i in range(len(cur_mnext))], 
                                         [cur_jready[i] if i != j_idx else cur_time for i in range(len(cur_jready))])
        new_mk = compute_makespan_internal(cur_mnext, cur_jready)
        reward = -(new_mk - old_mk)
        total_reward += reward
