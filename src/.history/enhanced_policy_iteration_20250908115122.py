...
class EnhancedFJSPEnv:
            ...
            if completed_ops[job_idx] < len(self.jobs[job_id]):
                ...
                for machine, proc_time in operation['proc_times'].items():
                    ...
                    if verbose:
                        ...
                        # Calculate when this operation would start/end
                        job_ready_time = self.job_arrival_times[job_id]
                        if completed_ops[job_idx] > 0:
                            # For subsequent operations, job is ready when current makespan allows
    ...
    def transition(self, state, action, verbose=False):
        ...
        # Job ready time: arrival time for first operation
        job_ready_time = self.job_arrival_times[job_id]
        if completed_ops[job_idx] > 0:
            # For subsequent operations, job is ready when current makespan allows
            # (This is a simplification - in reality we'd track exact job completion times)
            job_ready_time = current_makespan
