...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_next_event_time(self):
        ...
        CRITICAL: Only consider events that are STRICTLY IN THE FUTURE (> event_time).
        This prevents getting stuck when machines are idle at the current event_time.
        """
        # Find next arrival time (must be in the future)
        next_arrival_time = float('inf')
        for job_id, arrival_time in self.job_arrival_times.items():
            if (job_id not in self.arrived_jobs and 
                arrival_time != float('inf') and 
                arrival_time > self.event_time):  # CRITICAL: Must be in the future
                next_arrival_time = min(next_arrival_time, arrival_time)
        
        # Find earliest machine completion time (must be in the future)
        ...
        return next_event_time if next_event_time != float('inf') else float('inf')
    ...
    def step(self, action):
        ...
        # CRITICAL FIX: Check if there are no valid actions available
