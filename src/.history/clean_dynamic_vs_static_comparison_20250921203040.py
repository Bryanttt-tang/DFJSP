...
class StaticFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        Generate compact, informative observation for fully observed MDP.
        
        IMPROVED DESIGN - More compact and decision-focused:
        1. Machine availability (relative time until free)
        2. Schedulable operations with their processing times
        3. Job progress and dependencies
        4. Resource utilization information
        5. Arrival status (for static: all available)
        """
        obs = []
        
        # Calculate time scale for normalization (more robust)
        all_proc_times = []
        ...
        if all_proc_times:
            min_proc_time = min(all_proc_times)
            max_proc_time = max(all_proc_times)
            avg_proc_time = np.mean(all_proc_times)
            time_scale = max(avg_proc_time * 15, max_proc_time * 3, 10.0)  # More intelligent scaling
        else:
            time_scale = 50.0
        
        current_time = self.current_makespan
        
        # 1. MACHINE STATE (relative timing) - 3 features per machine
        for machine in self.machines:
            # Time until machine becomes free (key for scheduling decisions)
            time_until_free = max(0.0, self.machine_next_free[machine] - current_time)
            normalized_time_free = min(1.0, time_until_free / time_scale)
            obs.append(normalized_time_free)
            
            # Machine utilization (how busy it's been)
            total_scheduled_time = sum(op[2] - op[1] for op in self.schedule[machine])
            utilization = min(1.0, total_scheduled_time / max(current_time, 1.0))
            obs.append(utilization)
            
            # Number of operations already scheduled on this machine (workload indicator)
            num_ops_scheduled = len(self.schedule[machine])
            normalized_workload = min(1.0, num_ops_scheduled / max(self.total_operations / len(self.machines), 1))
            obs.append(normalized_workload)
        
        # 2. SCHEDULABLE OPERATIONS (operation-centric view) - More decision-relevant
        schedulable_ops = []
        for job_id in self.job_ids:
            ...
            next_op_idx = self.next_operation[job_id]
            if next_op_idx < len(self.jobs[job_id]):
                op_data = self.jobs[job_id][next_op_idx]
                ...
                # Add schedulable operation info
                schedulable_ops.append({
                    'job_id': job_id,
                    'op_idx': next_op_idx,
                    'ready_time': job_ready_time,
                    'proc_times': op_data['proc_times'],
                    'job_progress': next_op_idx / len(self.jobs[job_id])
                })
        
        # Pad/truncate to fixed number of schedulable operations for consistent obs size
        max_schedulable_ops = self.num_jobs  # At most one op per job can be schedulable
        
        for i in range(max_schedulable_ops):
            if i < len(schedulable_ops):
                op = schedulable_ops[i]
                
                # Time until operation can start (relative timing)
                time_until_ready = max(0.0, op['ready_time'] - current_time)
                obs.append(min(1.0, time_until_ready / time_scale))
                
                # Job progress (how much of job is completed)
                obs.append(op['job_progress'])
                
                # Processing times on each machine (or 0 if not compatible)
                for machine in self.machines:
                    if machine in op['proc_times']:
                        proc_time = op['proc_times'][machine]
                        normalized_proc_time = min(1.0, proc_time / time_scale)
                        obs.append(normalized_proc_time)
                    else:
                        obs.append(0.0)  # Not compatible with this machine
                
                # Remaining operations in job (dependency information)
                remaining_ops = len(self.jobs[op['job_id']]) - op['op_idx'] - 1
                normalized_remaining = remaining_ops / max(self.max_ops_per_job - 1, 1)
                obs.append(min(1.0, normalized_remaining))
                
            else:
                # No operation available - pad with zeros
                features_per_op = 2 + len(self.machines) + 1  # ready_time + progress + proc_times + remaining
                obs.extend([0.0] * features_per_op)
        
        # 3. GLOBAL STATE (compact)
        # Current time (normalized)
        obs.append(min(1.0, current_time / time_scale))
        
        # Overall progress (operations completed / total operations)
        progress = self.operations_scheduled / max(self.total_operations, 1)
        obs.append(progress)
        
        # 4. ARRIVAL INFORMATION (environment-specific)
        # For static environment: simple binary indicators
        for job_id in self.job_ids:
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        Generate compact, informative observation for dynamic MDP.
        
        IMPROVED DESIGN - More compact and decision-focused:
        1. Machine availability (relative time until free + utilization + workload)
        2. Schedulable operations with processing times and dependencies
        3. Job progress and remaining work
        4. Global scheduling progress
        5. Arrival status (arrived/not-arrived only, NO future knowledge)
        """
        obs = []
        ...
        # 4. ARRIVAL INFORMATION (NO FUTURE KNOWLEDGE)
        # CRITICAL: Dynamic RL should NOT know future arrival times for generalizability!
        # Only knows which jobs have already arrived
        for job_id in self.job_ids:
            obs.append(1.0 if job_id in self.arrived_jobs else 0.0)
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _get_observation(self):
        """
        Generate compact, informative observation for perfect knowledge MDP.
        ...
        3. Job progress and remaining work
        4. Global scheduling progress
        5. PERFECT ARRIVAL KNOWLEDGE (knows exact future arrival times)
        """
        obs = []
        ...
        # 4. PERFECT ARRIVAL KNOWLEDGE (knows exact future arrival times - this is the advantage)
        for job_id in self.job_ids:
...
def plot_training_metrics():
    ...
    if TRAINING_METRICS['value_loss']:
        axes[1, 0].plot(timesteps[:len(TRAINING_METRICS['value_loss'])], TRAINING METRICS['value_loss'], 'g-', linewidth=2)
...
def spt_heuristic_poisson(jobs_data, machine_list, arrival_times):
    """
    Shortest Processing Time (SPT) heuristic for dynamic FJSP with Poisson arrivals.
    
    Algorithm:
    1. At each decision point, select the operation with shortest processing time
    2. Among machines that can process it, choose the one available earliest
    3. Handle job arrivals dynamically
    
    Returns:
        makespan: Total completion time
        schedule: Dictionary mapping machines to list of (job_op, start_time, end_time)
    """
    # Initialize state
    machine_next_free = {m: 0.0 for m in machine_list}
    schedule = {m: [] for m in machine_list}
    completed_ops = {job_id: [False] * len(jobs_data[job_id]) for job_id in jobs_data.keys()}
    operation_end_times = {job_id: [0.0] * len(jobs_data[job_id]) for job_id in jobs_data.keys()}
    next_operation = {job_id: 0 for job_id in jobs_data.keys()}
    ...
    while operations_scheduled < total_operations:
        ...
        for job_id, arr_time in arrival_times.items():
            if job_id not in arrived_jobs and arr_time <= current_time and arr_time != float('inf'):
                arrived_jobs.add(job_id)
                newly_arrived.add(job_id)
        
        # Collect all schedulable operations
        schedulable_ops = []
        ...
        for job_id in arrived_jobs:
            ...
            if next_op_idx < len(jobs_data[job_id]):
                ...
                # Calculate when this operation can start
                if next_op_idx == 0:
                    job_ready_time = arrival_times.get(job_id, 0.0)
                else:
                    job_ready_time = operation_end_times[job_id][next_op_idx - 1]
                
                operation_ready_time = max(job_ready_time, current_time)
                
                # Find minimum processing time across all compatible machines
                ...
                for machine in machine_list:
                    if machine in op_data['proc_times']:
                        ...
                        start_time = max(machine_available_time, operation_ready_time)
                        proc_time = op_data['proc_times'][machine]
                        
                        # SPT: Choose operation and machine combination with shortest processing time
                        # Break ties by earliest start time
                        if proc_time < min_proc_time or (proc_time == min_proc_time and start_time < best_start_time):
                ...
                if best_machine is not None:
                    schedulable_ops.append({
                        ...
                        'end_time': best_start_time + min_proc_time
                    })
        
        if not schedulable_ops:
            # No operations available, advance time to next job arrival or machine availability
            next_event_time = float('inf')
            
            # Check for future job arrivals
            for job_id, arr_time in arrival_times.items():
                if job_id not in arrived_jobs and arr_time > current_time and arr_time != float('inf'):
                    next_event_time = min(next_event_time, arr_time)
            
            # Check for machine availability
            for machine_time in machine_next_free.values():
                if machine_time > current_time:
                    next_event_time = min(next_event_time, machine_time)
        ...
        # SPT: Choose operation with minimum processing time
        best_op = min(schedulable_ops, key=lambda op: (op['proc_time'], op['start_time'], op['job_id'], op['op_idx']))
        
        # Schedule the selected operation
        job_id = best_op['job_id']
        op_idx = best_op['op_idx']
        machine = best_op['machine']
        start_time = best_op['start_time']
        end_time = best_op['end_time']

        # Update state
        machine_next_free[machine] = end_time
        operation_end_times[job_id][op_idx] = end_time
        completed_ops[job_id][op_idx] = True
    ...
    # Calculate makespan
    makespan = max(machine_next_free.values()) if machine_next_free else 0.0
...
def milp_optimal_scheduler(jobs_data, machine_list, arrival_times):
    """
    MILP optimal scheduler for FJSP with job arrivals.
    
    Uses PuLP to solve the scheduling problem optimally.
    
    Returns:
        makespan: Optimal makespan (or inf if failed)
        schedule: Optimal schedule (or empty dict if failed)
    """
    try:
        # Create the problem
        prob = LpProblem("FJSP_Scheduling", LpMinimize)
        
        # Calculate a reasonable upper bound for makespan
        total_proc_time = 0
        for job_data in jobs_data.values():
            for op in job_data:
                total_proc_time += min(op['proc_times'].values())
        
        max_arrival = max(arrival_times.values()) if arrival_times else 0
        if max_arrival == float('inf'):
            max_arrival = 100  # Fallback for jobs that never arrive
        
        time_horizon = max_arrival + total_proc_time + 50  # Conservative upper bound
        ...
        # Create variables for each feasible assignment
            ...
            for op_idx, op_data in enumerate(jobs_data[job_id]):
                ...
                for machine in machine_list:
                    if machine in op_data['proc_times']:
                        ...
                        arrival_time = arrival_times.get(job_id, 0)
                        if arrival_time == float('inf'):
                            continue  # Skip jobs that never arrive
                        
                        # Only consider reasonable start times
                        earliest_start = int(max(0, arrival_time))
                        latest_start = int(time_horizon - op_data['proc_times'][machine])
                        
                        for t in range(earliest_start, min(latest_start + 1, int(time_horizon))):
                            var_name = f"x_{job_id}_{op_idx}_{machine}_{t}"
                            x[job_id][op_idx][machine][t] = LpVariable(var_name, cat='Binary')
        ...
        for job_id in jobs_data.keys():
            if arrival_times.get(job_id, 0) == float('inf'):
                continue
            for op_idx in range(len(jobs_data[job_id])):
                op_data = jobs_data[job_id][op_idx]
                constraint = lpSum([
                    x[job_id][op_idx][machine][t]
                    for machine in machine_list
                    if machine in op_data['proc_times'] and machine in x[job_id][op_idx]
                    for t in x[job_id][op_idx][machine].keys()
                ]) == 1
                prob += constraint
        
        # Constraint 2: Precedence constraints within jobs
        for job_id in jobs_data.keys():
            if arrival_times.get(job_id, 0) == float('inf'):
                continue
            for op_idx in range(1, len(jobs_data[job_id])):
                prev_op_data = jobs_data[job_id][op_idx - 1]
                curr_op_data = jobs_data[job_id][op_idx]
                
                # Previous operation end time
                prev_end = lpSum([
                    (t + prev_op_data['proc_times'][machine]) * x[job_id][op_idx-1][machine][t]
                    for machine in machine_list
                    if machine in prev_op_data['proc_times'] and machine in x[job_id][op_idx-1]
                    for t in x[job_id][op_idx-1][machine].keys()
                ])
                
                # Current operation start time
                curr_start = lpSum([
                    t * x[job_id][op_idx][machine][t]
                    for machine in machine_list
                    if machine in curr_op_data['proc_times'] and machine in x[job_id][op_idx]
                    for t in x[job_id][op_idx][machine].keys()
                ])
                
                prob += curr_start >= prev_end
        
        # Constraint 3: Machine capacity (no overlapping operations)
        for machine in machine_list:
            for t in range(int(time_horizon)):
                ...
                for job_id in jobs_data.keys():
                    ...
                    for op_idx, op_data in enumerate(jobs_data[job_id]):
                        if machine in op_data['proc_times'] and machine in x[job_id][op_idx]:
                            proc_time = op_data['proc_times'][machine]
                            for start_t in x[job_id][op_idx][machine].keys():
                                if start_t <= t < start_t + proc_time:
                                    overlapping_ops.append(x[job_id][op_idx][machine][start_t])
                
                if overlapping_ops:
                    prob += lpSum(overlapping_ops) <= 1
        
        # Constraint 4: Job arrival times
        for job_id in jobs_data.keys():
            arrival_time = arrival_times.get(job_id, 0)
            if arrival_time == float('inf'):
                continue
            if len(jobs_data[job_id]) > 0:
                first_op_data = jobs_data[job_id][0]
                first_start = lpSum([
                    t * x[job_id][0][machine][t]
                    for machine in machine_list
                    if machine in first_op_data['proc_times'] and machine in x[job_id][0]
                    for t in x[job_id][0][machine].keys()
                ])
                prob += first_start >= arrival_time
        ...
        for job_id in jobs_data.keys():
            ...
            if len(jobs_data[job_id]) > 0:
                ...
                last_op_data = jobs_data[job_id][last_op_idx]
                job_completion = lpSum([
                    (t + last_op_data['proc_times'][machine]) * x[job_id][last_op_idx][machine][t]
                    for machine in machine_list
                    if machine in last_op_data['proc_times'] and machine in x[job_id][last_op_idx]
                    for t in x[job_id][last_op_idx][machine].keys()
                ])
                prob += C_max >= job_completion
        ...
        for job_id in jobs_data.keys():
            ...
            for op_idx, op_data in enumerate(jobs_data[job_id]):
                for machine in machine_list:
                    if machine in op_data['proc_times'] and machine in x[job_id][op_idx]:
                        for t in x[job_id][op_idx][machine].keys():
                            if x[job_id][op_idx][machine][t].varValue == 1:
                                start_time = t
                                proc_time = op_data['proc_times'][machine]
                                end_time = start_time + proc_time
                                schedule[machine].append((f"J{job_id}-O{op_idx+1}", start_time, end_time))
        
        # Sort operations in schedule by start time for each machine
        for machine in machine_list:
...
def calculate_regret_analysis(optimal_makespan, method_results):
    """Calculate regret (gap from optimal) for each method."""
    regret_results = {}
    
    print(f"Regret Analysis (gap from optimal makespan {optimal_makespan:.2f}):")
    print("-" * 50)
    
    for method, makespan in method_results.items():
        if makespan != float('inf') and optimal_makespan != float('inf'):
            ...
            print(f"{method:25s}: +{regret:6.1f}% ({makespan:.2f})")
        else:
...
def diagnose_performance_similarity(perfect_makespan, dynamic_makespan, static_makespan, heuristic_makespan):
    """Diagnose why different methods might be giving similar performance."""
    print(f"\n🔍 PERFORMANCE SIMILARITY DIAGNOSIS")
    print("-" * 50)
    
    # Check if makespans are suspiciously similar
    ...
    if max_makespan - min_makespan < 0.1:
        print("🚨 SUSPICIOUS: All methods giving nearly identical results!")
        print("   Possible causes:")
        print("   1. Problem is too constrained (only one feasible solution)")
        print("   2. All methods converging to same local optimum")
        print("   3. Bug in evaluation - methods using same environment state")
        print("   4. Test scenario too simple")
    elif max_makespan - min_makespan < 2.0:
        print("⚠️  CAUTION: Methods giving very similar results")
        print("   This might indicate:")
        print("   1. Problem characteristics favor similar strategies")
        print("   2. Arrival patterns don't create significant scheduling challenges")
        print("   3. Methods are all reasonably good at this problem size")
    else:
        print("✅ GOOD: Methods showing meaningful performance differences")
    
    # Analyze method-specific issues
    if abs(perfect_makespan - dynamic_makespan) < 0.1:
        print("🚨 Perfect Knowledge RL ≈ Dynamic RL: Perfect knowledge not being utilized!")
    
    if abs(dynamic_makespan - static_makespan) < 0.5:
        print("🚨 Dynamic RL ≈ Static RL: Dynamic arrivals not providing advantage!")
        print("   Possible reasons:")
        print("   - Dynamic RL not learning arrival patterns effectively")
        print("   - Static RL evaluation environment incorrectly using arrival information")
        print("   - Test scenarios don't benefit much from dynamic information")
    
    if min(perfect_makespan, dynamic_makespan, static_makespan) >= heuristic_makespan:
        print("🚨 All RL methods ≥ Heuristic: RL not learning effectively!")
        print("   RL training may need:")
        print("   - More timesteps")
        print("   - Better reward function")
    ...
    methods = [
        ...
        ("Heuristic", heuristic_makespan)
    ]
    methods.sort(key=lambda x: x[1])
    
    print(f"\nActual ranking (best to worst):")
    for i, (method, makespan) in enumerate(methods, 1):
        if makespan != float('inf'):
            print(f"{i}. {method}: {makespan:.2f}")
        else:
            print(f"{i}. {method}: Failed")
    
    expected_order = ["Perfect Knowledge RL", "Dynamic RL", "Static RL", "Heuristic"]
    actual_order = [method for method, _ in methods if _ != float('inf')]
    
    if actual_order == expected_order:
        print("✅ Performance follows expected hierarchy")
    else:
        print(f"❌ Performance hierarchy unexpected!")
...
def main():
    ...
    print(f"⚠️  IMPORTANT: Test scenarios use seeds 1000-1009, training used seed {GLOBAL_SEED}")
    ...
    print("CRITICAL: Dynamic RL observation ONLY includes arrived/not-arrived status")
    print("         It does NOT know specific future arrival times for generalizability")
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        # Debug: Check for impossible results
        if milp_makespan != float('inf') and dynamic_makespan < milp_makespan:
            print(f"  🚨 ERROR: Dynamic RL ({dynamic_makespan:.2f}) outperformed MILP Optimal ({milp_makespan:.2f})!")
            print(f"      This is theoretically impossible - there's a bug in evaluation or MILP")
        
        if perfect_makespan > dynamic_makespan:
            print(f"  🚨 WARNING: Perfect Knowledge RL ({perfect_makespan:.2f}) worse than Dynamic RL ({dynamic_makespan:.2f})")
            print(f"      Perfect RL should always be better since it knows exact arrival times")
    ...
    print(f"Perfect Knowledge RL      - Avg Makespan: {avg_results['Perfect Knowledge RL']:.2f} ± {std_results['Perfect Knowledge RL']:.2f} (avg regret: +{((avg_results['Perfect Knowledge RL']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
    print(f"Dynamic RL (Poisson)      - Avg Makespan: {avg_results['Dynamic RL']:.2f} ± {std_results['Dynamic RL']:.2f} (avg regret: +{((avg_results['Dynamic RL']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
    print(f"Static RL (on dynamic)    - Avg Makespan: {avg_results['Static RL (dynamic)']:.2f} ± {std_results['Static RL (dynamic)']:.2f} (avg regret: +{((avg_results['Static RL (dynamic)']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
    print(f"Static RL (on static)     - Avg Makespan: {avg_results['Static RL (static)']:.2f} ± {std_results['Static RL (static)']:.2f} (avg regret: +{((avg_results['Static RL (static)']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
    print(f"Best Heuristic            - Avg Makespan: {avg_results['Best Heuristic']:.2f} ± {std_results['Best Heuristic']:.2f} (avg regret: +{((avg_results['Best Heuristic']-avg_results['MILP Optimal'])/avg_results['MILP Optimal']*100):.1f}%)")
    ...
    import os
    folder_name = "small_instances"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
        print(f"Created folder: {folder_name}")
    
    # Generate Gantt charts for all 10 scenarios
    for scenario_idx in range(len(test_scenarios)):
        ...
        if scenario_idx < len(gantt_scenarios_data):
            schedules = gantt_scenarios_data[scenario_idx]['schedules']
        else:
            # For scenarios beyond the first 3, we need to run evaluations
            print(f"Evaluating scenario {scenario_idx + 1} for Gantt chart generation...")
            
            # Train Perfect Knowledge RL specifically for this scenario
            print(f"  Training Perfect Knowledge RL for Gantt scenario {scenario_idx + 1}...")
            perfect_model_gantt = train_perfect_knowledge_agent(ENHANCED_JOBS_DATA, MACHINE_LIST, 
                                                               arrival_times=arrival_times, 
                                                               total_timesteps=perfect_timesteps,
                                                               reward_mode="makespan_increment", learning_rate=learning_rate)
            
            # Evaluate all methods for this scenario
            perfect_makespan, perfect_schedule = evaluate_perfect_knowledge_on_scenario(
                perfect_model_gantt, ENHANCED_JOBS_DATA, MACHINE_LIST, arrival_times)
            ...
            spt_makespan, spt_schedule = spt_heuristic_poisson(ENHANCED_JOBS_DATA, MACHINE_LIST, arrival_times)
            
            milp_makespan, milp_schedule = milp_optimal_scheduler(ENHANCED_JOBS_DATA, MACHINE_LIST, arrival_times)
        ...
        # Create figure for this scenario - 5 methods only (no static RL comparison)
