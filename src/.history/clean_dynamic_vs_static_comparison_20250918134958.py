...
# Global tracking for detailed episode monitoring
DETAILED_EPISODES = []  # Store first 10 episodes with full details
TRAINING_REWARDS = []   # Track rewards over time
TRAINING_RETURNS = []   # Track episode returns
...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Store detailed information for first 10 episodes
        if TRAINING_EPISODE_COUNT <= 10:
            episode_detail = {
                'episode': TRAINING_EPISODE_COUNT,
                'arrival_times': self.job_arrival_times.copy(),
                'initial_jobs': list(self.initial_job_ids),
                'dynamic_jobs': list(self.dynamic_job_ids),
                'arrived_jobs_at_start': list(self.arrived_jobs),
                'total_operations': self.total_operations,
                'step_rewards': [],  # Will be filled during episode
                'step_actions': [],  # Will be filled during episode
                'episode_return': 0.0,  # Will be calculated at end
                'final_makespan': 0.0   # Will be filled at end
            }
    ...
    def step(self, action):
        ...
        global TRAINING_EPISODE_COUNT, DETAILED_EPISODES, TRAINING_REWARDS, TRAINING_RETURNS, TRAINING_MAKESPANS
        if TRAINING_EPISODE_COUNT <= 10 and len(DETAILED_EPISODES) >= TRAINING_EPISODE_COUNT:
            current_episode = DETAILED_EPISODES[TRAINING_EPISODE_COUNT - 1]
            current_episode['step_rewards'].append(reward)
            current_episode['step_actions'].append(action)
            current_episode['episode_return'] += reward
            current_episode['final_makespan'] = self.current_makespan
            
            if terminated:
                # Episode finished, store final statistics
                TRAINING_RETURNS.append(current_episode['episode_return'])
                TRAINING_MAKESPANS.append(current_episode['final_makespan'])
        
        # Always track rewards for plotting
...
def visualize_first_10_episodes():
    """
    Visualize the first 10 training episodes showing job arrivals, rewards, and actions.
    """
    global DETAILED_EPISODES
    
    if not DETAILED_EPISODES:
        print("No detailed episode data recorded!")
        return
    
    print(f"\n=== FIRST 10 EPISODES ANALYSIS ===")
    print(f"Recorded {len(DETAILED_EPISODES)} detailed episodes")
    
    # Create comprehensive visualization
    fig = plt.figure(figsize=(20, 15))
    
    # Plot 1: Job Arrivals Timeline for each episode
    ax1 = plt.subplot(3, 2, 1)
    colors = plt.cm.Set3.colors
    
    for i, episode in enumerate(DETAILED_EPISODES):
        episode_num = episode['episode']
        arrival_times = episode['arrival_times']
        
        # Plot arrival times for this episode
        for job_id, arrival_time in arrival_times.items():
            if arrival_time != float('inf') and arrival_time > 0:
                ax1.scatter(arrival_time, episode_num, 
                           color=colors[job_id % len(colors)], 
                           s=100, alpha=0.7, label=f'Job {job_id}' if i == 0 else "")
                ax1.text(arrival_time + 0.5, episode_num, f'J{job_id}', 
                        fontsize=8, va='center')
    
    ax1.set_xlabel('Arrival Time')
    ax1.set_ylabel('Episode')
    ax1.set_title('Job Arrival Times Across First 10 Episodes')
    ax1.grid(True, alpha=0.3)
    ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    
    # Plot 2: Episode Returns
    ax2 = plt.subplot(3, 2, 2)
    episode_nums = [ep['episode'] for ep in DETAILED_EPISODES if 'episode_return' in ep]
    returns = [ep['episode_return'] for ep in DETAILED_EPISODES if 'episode_return' in ep]
    
    if returns:
        ax2.bar(episode_nums, returns, color='skyblue', alpha=0.7)
        ax2.set_xlabel('Episode')
        ax2.set_ylabel('Total Return')
        ax2.set_title('Episode Returns (First 10 Episodes)')
        ax2.grid(True, alpha=0.3)
        
        # Add value labels on bars
        for i, (ep, ret) in enumerate(zip(episode_nums, returns)):
            ax2.text(ep, ret + max(returns)*0.01, f'{ret:.1f}', 
                    ha='center', va='bottom', fontsize=9)
    
    # Plot 3: Final Makespans
    ax3 = plt.subplot(3, 2, 3)
    makespans = [ep['final_makespan'] for ep in DETAILED_EPISODES if 'final_makespan' in ep]
    
    if makespans:
        ax3.bar(episode_nums, makespans, color='lightcoral', alpha=0.7)
        ax3.set_xlabel('Episode')
        ax3.set_ylabel('Final Makespan')
        ax3.set_title('Final Makespans (First 10 Episodes)')
        ax3.grid(True, alpha=0.3)
        
        # Add value labels on bars
        for i, (ep, ms) in enumerate(zip(episode_nums, makespans)):
            ax3.text(ep, ms + max(makespans)*0.01, f'{ms:.1f}', 
                    ha='center', va='bottom', fontsize=9)
    
    # Plot 4: Reward Distribution per Episode
    ax4 = plt.subplot(3, 2, 4)
    episode_reward_data = []
    episode_labels = []
    
    for episode in DETAILED_EPISODES:
        if 'step_rewards' in episode and episode['step_rewards']:
            episode_reward_data.append(episode['step_rewards'])
            episode_labels.append(f"Ep {episode['episode']}")
    
    if episode_reward_data:
        bp = ax4.boxplot(episode_reward_data, labels=episode_labels, patch_artist=True)
        for patch in bp['boxes']:
            patch.set_facecolor('lightgreen')
            patch.set_alpha(0.7)
        
        ax4.set_xlabel('Episode')
        ax4.set_ylabel('Step Rewards')
        ax4.set_title('Reward Distribution per Episode')
        ax4.grid(True, alpha=0.3)
        plt.setp(ax4.get_xticklabels(), rotation=45)
    
    # Plot 5: Number of Jobs Arrived per Episode
    ax5 = plt.subplot(3, 2, 5)
    jobs_arrived = []
    for episode in DETAILED_EPISODES:
        arrived_count = sum(1 for t in episode['arrival_times'].values() if t != float('inf'))
        jobs_arrived.append(arrived_count)
    
    ax5.bar(episode_nums, jobs_arrived, color='gold', alpha=0.7)
    ax5.set_xlabel('Episode')
    ax5.set_ylabel('Number of Jobs')
    ax5.set_title('Total Jobs per Episode (Including Arrivals)')
    ax5.grid(True, alpha=0.3)
    
    # Add value labels
    for i, (ep, count) in enumerate(zip(episode_nums, jobs_arrived)):
        ax5.text(ep, count + 0.1, str(count), ha='center', va='bottom', fontsize=9)
    
    # Plot 6: Episode Length (Number of Steps)
    ax6 = plt.subplot(3, 2, 6)
    episode_lengths = [len(ep['step_rewards']) for ep in DETAILED_EPISODES if 'step_rewards' in ep]
    
    if episode_lengths:
        ax6.bar(episode_nums, episode_lengths, color='mediumpurple', alpha=0.7)
        ax6.set_xlabel('Episode')
        ax6.set_ylabel('Number of Steps')
        ax6.set_title('Episode Length (Steps to Completion)')
        ax6.grid(True, alpha=0.3)
        
        # Add value labels
        for i, (ep, length) in enumerate(zip(episode_nums, episode_lengths)):
            ax6.text(ep, length + max(episode_lengths)*0.01, str(length), 
                    ha='center', va='bottom', fontsize=9)
    
    plt.tight_layout()
    plt.savefig('first_10_episodes_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    for episode in DETAILED_EPISODES:
        ...
        arrivals = {k: v for k, v in episode['arrival_times'].items() if v != float('inf')}
        dynamic_arrivals = {k: v for k, v in arrivals.items() if v > 0}
        
        print(f"\nEpisode {ep_num}:")
        print(f"  Initial jobs: {episode['initial_jobs']}")
        print(f"  Dynamic arrivals: {dynamic_arrivals}")
        print(f"  Total jobs: {len(arrivals)}")
        print(f"  Steps: {len(episode.get('step_rewards', []))}")
        print(f"  Return: {episode.get('episode_return', 0):.2f}")
        print(f"  Final makespan: {episode.get('final_makespan', 0):.2f}")
