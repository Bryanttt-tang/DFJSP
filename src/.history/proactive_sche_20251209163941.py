...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def action_masks(self):
        """Generate action masks - enable all 10 rule combinations when there are ready ops.
        
        NOTE: If no ready ops exist, step() will automatically advance event_time before
        asking agent for action, so action_masks() will always see at least one ready op.
        """
        ...
        if ready_ops:
            # Enable all 10 rule combinations
            mask[:] = True
        # If no ready ops, step() will handle automatic advancement before getting action
