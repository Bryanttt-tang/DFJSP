...
class TrainingCallback:
    ...
    def __init__(self, method_name):
        ...
        self.episode_buffer = []  # Buffer to collect episode data
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        # Track episode-level metrics from environment
        if hasattr(model, 'env') and hasattr(model.env, 'get_attr'):
            try:
                # Get episode statistics from vectorized environment
                episode_rewards = model.env.get_attr('episode_rewards')[0] if hasattr(model.env.envs[0], 'episode_rewards') else []
                episode_lengths = model.env.get_attr('episode_lengths')[0] if hasattr(model.env.envs[0], 'episode_lengths') else []
                ...
                if current_episode_count > self.last_episode_count:
                    ...
                    for i in range(new_episodes):
                        ep_idx = self.last_episode_count + i
                        if ep_idx < len(episode_rewards) and ep_idx < len(episode_lengths):
                            TRAINING_METRICS['episode_rewards'].append(episode_rewards[ep_idx])
                            TRAINING_METRICS['episode_lengths'].append(episode_lengths[ep_idx])
                            TRAINING_METRICS['episode_timesteps'].append(model.num_timesteps)
                    
                    self.last_episode_count = current_episode_count
            except:
                pass  # Silently handle any attribute errors
        
        # Periodic progress update (every 50 calls to reduce output noise)
        self.step_count += 1
        if self.step_count % 50 == 0:
            recent_episodes = TRAINING_METRICS['episode_rewards'][-10:] if TRAINING_METRICS['episode_rewards'] else []
            if recent_episodes:
                avg_recent_reward = sum(recent_episodes) / len(recent_episodes)
                print(f"    {self.method_name} - Step {model.num_timesteps}: Recent 10 episodes avg reward = {avg_recent_reward:.2f}")
...
def plot_comprehensive_training_metrics():
    """
    Create comprehensive training visualization showing episode-level progress.
    This helps verify that training is working correctly with decreasing loss and improving rewards.
    """
    global TRAINING_METRICS
    ...
    print(f"Total episodes recorded: {len(TRAINING_METRICS['episode_rewards'])}")
    
    # Create comprehensive figure with multiple subplots
    fig = plt.figure(figsize=(20, 16))
    gs = fig.add_gridspec(4, 3, hspace=0.3, wspace=0.3)
    
    fig.suptitle('Comprehensive PPO Training Analysis - Episode & Step Level Metrics', 
    ...
    if TRAINING_METRICS['episode_rewards'] and TRAINING_METRICS['episode_timesteps']:
        ax1.plot(TRAINING_METRICS['episode_timesteps'], TRAINING_METRICS['episode_rewards'], 
                color=colors['reward'], alpha=0.6, linewidth=1, label='Episode Reward')
        
        # Add moving average for trend
        if len(TRAINING_METRICS['episode_rewards']) > 10:
            window_size = min(50, len(TRAINING_METRICS['episode_rewards']) // 4)
            moving_avg = []
            timesteps_avg = []
            for i in range(window_size-1, len(TRAINING_METRICS['episode_rewards'])):
                avg_reward = sum(TRAINING_METRICS['episode_rewards'][i-window_size+1:i+1]) / window_size
                moving_avg.append(avg_reward)
                timesteps_avg.append(TRAINING_METRICS['episode_timesteps'][i])
            
            ax1.plot(timesteps_avg, moving_avg, color=colors['reward'], 
                    linewidth=3, label=f'{window_size}-Episode Moving Average')
        
        ax1.set_xlabel('Training Steps')
        ax1.set_ylabel('Episode Reward')
        ax1.set_title('Episode Rewards (Should Increase Over Time)')
        ax1.legend()
        ...
        if len(TRAINING_METRICS['episode_rewards']) > 20:
            ...
            if improvement > 0:
                ax1.text(0.02, 0.98, f'✅ Improving (+{improvement:.1f})', 
                        transform=ax1.transAxes, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            else:
                ax1.text(0.02, 0.98, f'🔴 Declining ({improvement:.1f})', 
                        transform=ax1.transAxes, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        ax1.text(0.5, 0.5, 'No episode reward data', ha='center', va='center', 
                transform=ax1.transAxes)
        ax1.set_title('Episode Rewards (No Data)')
    
    # 2. Episode Lengths Over Time
    ax2 = fig.add_subplot(gs[0, 1])
    if TRAINING_METRICS['episode_lengths'] and TRAINING_METRICS['episode_timesteps']:
        ax2.plot(TRAINING_METRICS['episode_timesteps'], TRAINING_METRICS['episode_lengths'], 
                color=colors['length'], alpha=0.6, linewidth=1, label='Episode Length')
        
        # Add moving average
        if len(TRAINING_METRICS['episode_lengths']) > 10:
            window_size = min(50, len(TRAINING_METRICS['episode_lengths']) // 4)
            moving_avg = []
            timesteps_avg = []
            for i in range(window_size-1, len(TRAINING_METRICS['episode_lengths'])):
                avg_length = sum(TRAINING_METRICS['episode_lengths'][i-window_size+1:i+1]) / window_size
                moving_avg.append(avg_length)
                timesteps_avg.append(TRAINING_METRICS['episode_timesteps'][i])
            
            ax2.plot(timesteps_avg, moving_avg, color=colors['length'], 
                    linewidth=3, label=f'{window_size}-Episode Moving Average')
        
        ax2.set_xlabel('Training Steps')
        ax2.set_ylabel('Episode Length (Steps)')
        ax2.set_title('Episode Lengths (Efficiency Indicator)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
    else:
        ax2.text(0.5, 0.5, 'No episode length data', ha='center', va='center', 
                transform=ax2.transAxes)
        ax2.set_title('Episode Lengths (No Data)')
    
    # 3. Policy Loss Over Time
    ax3 = fig.add_subplot(gs[0, 2])
    if TRAINING_METRICS['policy_loss'] and TRAINING_METRICS['timesteps']:
        steps_policy = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['policy_loss'])]
        ax3.plot(steps_policy, TRAINING_METRICS['policy_loss'], 
                color=colors['loss'], linewidth=2, label='Policy Loss')
        ax3.set_xlabel('Training Steps')
        ax3.set_ylabel('Policy Loss')
        ax3.set_title('Policy Loss (Should Stabilize)')
        ax3.legend()
        ax3.grid(True, alpha=0.3)
    else:
        ax3.text(0.5, 0.5, 'No policy loss data', ha='center', va='center', 
                transform=ax3.transAxes)
        ax3.set_title('Policy Loss (No Data)')
    
    # 4. Value Loss Over Time
    ax4 = fig.add_subplot(gs[1, 0])
    if TRAINING_METRICS['value_loss'] and TRAINING_METRICS['timesteps']:
        steps_value = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['value_loss'])]
        ax4.plot(steps_value, TRAINING_METRICS['value_loss'], 
                color=colors['loss'], linewidth=2, label='Value Loss', alpha=0.8)
        ax4.set_xlabel('Training Steps')
        ax4.set_ylabel('Value Loss')
        ax4.set_title('Value Loss (Should Decrease Initially)')
        ax4.legend()
        ax4.grid(True, alpha=0.3)
        
        # Add trend analysis
        if len(TRAINING_METRICS['value_loss']) > 10:
            early_loss = np.mean(TRAINING_METRICS['value_loss'][:len(TRAINING_METRICS['value_loss'])//4])
            late_loss = np.mean(TRAINING_METRICS['value_loss'][-len(TRAINING_METRICS['value_loss'])//4:])
            loss_change = ((late_loss - early_loss) / early_loss) * 100
            
            if loss_change < -10:
                ax4.text(0.02, 0.98, f'✅ Decreasing ({loss_change:.1f}%)', 
                        transform=ax4.transAxes, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
            elif abs(loss_change) < 10:
                ax4.text(0.02, 0.98, f'🟡 Stable ({loss_change:+.1f}%)', 
                        transform=ax4.transAxes, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
            else:
                ax4.text(0.02, 0.98, f'🔴 Increasing ({loss_change:+.1f}%)', 
                        transform=ax4.transAxes, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        ax4.text(0.5, 0.5, 'No value loss data', ha='center', va='center', 
                transform=ax4.transAxes)
        ax4.set_title('Value Loss (No Data)')
    
    # 5. Action Entropy Over Time
    ax5 = fig.add_subplot(gs[1, 1])
    if TRAINING_METRICS['action_entropy'] and TRAINING_METRICS['timesteps']:
        steps_entropy = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['action_entropy'])]
        ax5.plot(steps_entropy, TRAINING_METRICS['action_entropy'], 
                color=colors['entropy'], linewidth=2, label='Action Entropy')
        ax5.set_xlabel('Training Steps')
        ax5.set_ylabel('Action Entropy')
        ax5.set_title('Action Entropy (Exploration Level)')
        ax5.legend()
        ...
        if final_entropy > 0.5:
            ax5.text(0.02, 0.98, '✅ High exploration', transform=ax5.transAxes, 
                    verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
        elif final_entropy > 0.1:
            ax5.text(0.02, 0.98, '🟡 Moderate exploration', transform=ax5.transAxes, 
                    verticalalignment='top', bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
        else:
            ax5.text(0.02, 0.98, '🔴 Low exploration', transform=ax5.transAxes, 
                    verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))
    else:
        ax5.text(0.5, 0.5, 'No entropy data', ha='center', va='center', 
                transform=ax5.transAxes)
        ax5.set_title('Action Entropy (No Data)')
    
    # 6. Learning Rate Over Time
    ax6 = fig.add_subplot(gs[1, 2])
    if TRAINING_METRICS['learning_rate'] and TRAINING_METRICS['timesteps']:
        steps_lr = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['learning_rate'])]
        ax6.plot(steps_lr, TRAINING_METRICS['learning_rate'], 
                color=colors['lr'], linewidth=2, label='Learning Rate')
        ...
        ax6.text(0.5, 0.5, 'No learning rate data', ha='center', va='center', 
                transform=ax6.transAxes)
        ax6.set_title('Learning Rate (No Data)')
    
    # 7. Explained Variance
    ax7 = fig.add_subplot(gs[2, 0])
    if TRAINING_METRICS['explained_variance'] and TRAINING_METRICS['timesteps']:
        steps_ev = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['explained_variance'])]
        ax7.plot(steps_ev, TRAINING_METRICS['explained_variance'], 
                color='green', linewidth=2, label='Explained Variance')
        ax7.set_xlabel('Training Steps')
        ax7.set_ylabel('Explained Variance')
        ax7.set_title('Explained Variance (Value Function Quality)')
        ax7.legend()
        ax7.grid(True, alpha=0.3)
        ax7.axhline(y=0, color='red', linestyle='--', alpha=0.5, label='Zero Line')
    else:
        ax7.text(0.5, 0.5, 'No explained variance data', ha='center', va='center', 
                transform=ax7.transAxes)
        ax7.set_title('Explained Variance (No Data)')
    
    # 8. Clip Fraction
    ax8 = fig.add_subplot(gs[2, 1])
    if TRAINING_METRICS['clip_fraction'] and TRAINING_METRICS['timesteps']:
        steps_clip = TRAINING_METRICS['timesteps'][:len(TRAINING_METRICS['clip_fraction'])]
        ax8.plot(steps_clip, TRAINING_METRICS['clip_fraction'], 
                color='orange', linewidth=2, label='Clip Fraction')
        ax8.set_xlabel('Training Steps')
        ax8.set_ylabel('Clip Fraction')
        ax8.set_title('Clip Fraction (Policy Change Magnitude)')
        ax8.legend()
        ax8.grid(True, alpha=0.3)
    else:
        ax8.text(0.5, 0.5, 'No clip fraction data', ha='center', va='center', 
                transform=ax8.transAxes)
        ax8.set_title('Clip Fraction (No Data)')
    ...
    if TRAINING_METRICS['episode_rewards']:
        ...
        summary_text += f"Episodes Completed: {len(rewards)}\n"
        summary_text += f"Reward Stats:\n"
        summary_text += f"  • Final: {rewards[-1]:.2f}\n"
        summary_text += f"  • Mean: {np.mean(rewards):.2f}\n"
        summary_text += f"  • Std: {np.std(rewards):.2f}\n"
        summary_text += f"  • Best: {max(rewards):.2f}\n"
        summary_text += f"  • Worst: {min(rewards):.2f}\n\n"
        
        # Trend analysis
        if len(rewards) > 20:
            early = np.mean(rewards[:len(rewards)//4])
            late = np.mean(rewards[-len(rewards)//4:])
            improvement = late - early
            summary_text += f"Reward Trend:\n"
            summary_text += f"  • Early avg: {early:.2f}\n"
            summary_text += f"  • Late avg: {late:.2f}\n"
            summary_text += f"  • Improvement: {improvement:+.2f}\n"
            summary_text += f"  • Status: {'✅ Learning' if improvement > 0 else '❌ Declining'}\n\n"
    
    if TRAINING_METRICS['episode_lengths']:
        lengths = TRAINING_METRICS['episode_lengths']
        summary_text += f"Episode Length Stats:\n"
        summary_text += f"  • Mean: {np.mean(lengths):.1f}\n"
        summary_text += f"  • Range: {min(lengths)}-{max(lengths)}\n\n"
    
    if TRAINING_METRICS['action_entropy']:
        entropy = TRAINING_METRICS['action_entropy']
        summary_text += f"Exploration Stats:\n"
        summary_text += f"  • Current entropy: {entropy[-1]:.3f}\n"
        summary_text += f"  • Exploration level: "
        if entropy[-1] > 0.5:
            summary_text += "High ✅\n"
        elif entropy[-1] > 0.1:
    ...
    # Check 1: Reward improvement
    if len(TRAINING_METRICS['episode_rewards']) > 20:
        early_r = np.mean(TRAINING_METRICS['episode_rewards'][:len(TRAINING_METRICS['episode_rewards'])//4])
        late_r = np.mean(TRAINING_METRICS['episode_rewards'][-len(TRAINING_METRICS['episode_rewards'])//4:])
        if late_r > early_r:
            health_score += 1
            summary_text += "• Reward improving ✅\n"
        else:
            summary_text += "• Reward declining ❌\n"
        total_checks += 1
    
    # Check 2: Entropy level
    if TRAINING_METRICS['action_entropy']:
        if TRAINING_METRICS['action_entropy'][-1] > 0.1:
            health_score += 1
            summary_text += "• Exploration adequate ✅\n"
        else:
            summary_text += "• Low exploration ❌\n"
        total_checks += 1
    
    # Check 3: Loss stability
    if len(TRAINING_METRICS['value_loss']) > 10:
        recent_loss_std = np.std(TRAINING_METRICS['value_loss'][-20:])
        if recent_loss_std < 1.0:  # Reasonable threshold
            health_score += 1
            summary_text += "• Loss stable ✅\n"
        else:
            summary_text += "• Loss unstable ❌\n"
        total_checks += 1
    
    if total_checks > 0:
        health_percentage = (health_score / total_checks) * 100
        summary_text += f"\nOverall Health: {health_percentage:.0f}%"
    ...
    print("✅ Comprehensive training analysis saved: comprehensive_training_analysis.png")
    plt.show()

def plot_training_metrics():
    """
    Enhanced version that calls the comprehensive training analysis.
    """
...
def main():
    ...
    plot_training_metrics()      # Show comprehensive PPO exploration and learning metrics
