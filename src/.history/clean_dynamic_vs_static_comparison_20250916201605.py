...
def main():
    ...
    # Determine number of plots based on MILP availability (now 6 plots with both static scenarios)
    num_plots = 6 if milp_makespan != float('inf') else 5
    fig, axes = plt.subplots(num_plots, 1, figsize=(18, num_plots * 3.5))
    
    if milp_makespan != float('inf'):
        fig.suptitle('Comprehensive Scheduling Comparison: MILP vs Perfect Knowledge vs Dynamic vs Static RL (2 scenarios) vs SPT\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'Static RL tested on both dynamic arrivals and static (all t=0) scenarios', 
        ...
        schedules_data = [
            ...
            {'schedule': static_static_schedule, 'makespan': static_static_makespan, 'title': 'Static RL (on static scenario)', 'arrival_times': static_arrivals},
            {'schedule': spt_schedule, 'makespan': spt_makespan, 'title': 'SPT Heuristic', 'arrival_times': first_scenario_arrivals}
        ]
    else:
        fig.suptitle('Scheduling Comparison: Perfect Knowledge vs Dynamic vs Static RL (2 scenarios) vs SPT\n' + 
                     f'Test Scenario: Jobs 0-2 at t=0, Jobs 3-6 via Poisson arrivals\n' +
                     f'Static RL tested on both dynamic arrivals and static (all t=0) scenarios', 
                     fontsize=16, fontweight='bold')
        schedules_data = [
