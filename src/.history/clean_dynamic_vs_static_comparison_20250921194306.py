...
def main():
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        # CRITICAL VALIDATION: Check for impossible results
        if milp_makespan != float('inf'):
            violations = []
            if perfect_makespan < milp_makespan - 0.01:  # Allow small numerical tolerance
                violations.append(f"Perfect RL ({perfect_makespan:.2f}) < MILP ({milp_makespan:.2f})")
            if dynamic_makespan < milp_makespan - 0.01:
                violations.append(f"Dynamic RL ({dynamic_makespan:.2f}) < MILP ({milp_makespan:.2f})")
            if static_dynamic_makespan < milp_makespan - 0.01:
                violations.append(f"Static RL ({static_dynamic_makespan:.2f}) < MILP ({milp_makespan:.2f})")
            if spt_makespan < milp_makespan - 0.01:
                violations.append(f"Heuristic ({spt_makespan:.2f}) < MILP ({milp_makespan:.2f})")
            
            if violations:
                print(f"  ⚠️  IMPOSSIBLE RESULTS DETECTED:")
                for violation in violations:
                    print(f"    - {violation}")
                print(f"  This indicates bugs in evaluation or MILP solver!")
