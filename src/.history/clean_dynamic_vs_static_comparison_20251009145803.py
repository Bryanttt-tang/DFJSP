...
class EnhancedTrainingCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if rollout_ep_rew is not None:
            TRAINING_METRICS['episode_rewards'].append(float(rollout_ep_rew))
        else:
            TRAINING_METRICS['episode_rewards'].append(computed_ep_rew)

        if rollout_ep_len is not None:
            TRAINING_METRICS['episode_lengths'].append(float(rollout_ep_len))
        else:
            TRAINING_METRICS['episode_lengths'].append(computed_ep_len)
