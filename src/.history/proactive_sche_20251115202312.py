...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def _execute_wait_action_with_predictor_guidance(self, wait_duration):
        ...
        # CRITICAL: Update makespan correctly
        # Makespan = max(machine_end_times) if any machine is working
        # Makespan = event_time if ALL machines idle (time passes with no work)
        max_machine_time = max(self.machine_end_times.values())
        
        if max_machine_time <= self.event_time:
            # ALL machines are idle at current event_time - time is wasted!
            # This is BAD: waited while machines idle
            self.current_makespan = self.event_time
        else:
            # Some machines are still working - makespan = latest machine end time
            # This is OK: machines working during wait
            self.current_makespan = max_machine_time
