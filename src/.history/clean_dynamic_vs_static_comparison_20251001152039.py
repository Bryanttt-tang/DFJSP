...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    pr    print(f"Training Static RL for {total_timesteps:,} timesteps with seed {GLOBAL_SEED}...")ing completed in {training_time:.1f}s!")
...
def evaluate_static_on_dynamic(static_model, jobs_data, machine_list, arrival_times, reward_mode="makespan_increment"):
    ...
    # Use PerfectKnowledgeFJSPEnv to properly handle arrival times for static agent
    # The static agent was trained assuming all jobs at t=0, but we evaluate with actual arrival times
    test_env = PerfectKnowledgeFJSPEnv(jobs_data, machine_list, arrival_times, reward_mode=reward_mode)
    test_env = ActionMasker(test_env, mask_fn)
    ...
    # Handle late arrivals by allowing time advancement (same as Perfect Knowledge RL)
    latest_arrival = max([t for t in arrival_times.values() if t != float('inf')] + [0])
...
def evaluate_static_on_static(static_model, jobs_data, machine_list, reward_mode="makespan_increment"):
    ...
    # Create static arrival times (all jobs at t=0) for evaluation
    static_arrival_times = {job_id: 0.0 for job_id in jobs_data.keys()}
    
    # Use PerfectKnowledgeFJSPEnv with all arrival times = 0 for static evaluation
