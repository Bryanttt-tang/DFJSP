...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05, 
        ...
        self.wait_penalty_per_time = 0.01  # Penalty per time unit for waiting
    ...
    def _update_arrived_jobs(self):
        """Update arrived_jobs based on current event_time."""
        newly_arrived = set()
    ...
    def _advance_to_next_event(self):
        """Advance event_time to next event (job arrival or machine completion)."""
        ...
        # Find next machine completion time
        next_machine_completion = float('inf')
        for machine_free_time in self.machine_next_free.values():
            if machine_free_time > self.event_time:
                next_machine_completion = min(next_machine_completion, machine_free_time)
        ...
        if next_event_time != float('inf'):
            ...
            # No future events, advance by a small step
            self.event_time += 1.0
        
        # Update arrived jobs based on new event time
        num_new_arrivals = self._update_arrived_jobs()
        
        return num_new_arrivals, self.event_time

    def _advance_to_next_arrival(self):
        """WAIT ACTION: Advance event_time to next job arrival or machine completion."""
        num_new_arrivals, new_time = self._advance_to_next_event()
        return num_new_arrivals > 0
    ...
    def step(self, action):
        ...
        if job_idx is None:
            ...
            old_event_time = self.event_time
            num_new_arrivals, new_time = self._advance_to_next_event()
            delta_t = new_time - old_event_time
            wait_penalty = -self.wait_penalty_per_time * delta_t
            
            # Additional penalty for waiting when other actions were available
            ...
            if num_new_arrivals == 0 and not future_arrivals_possible and not np.any(self.action_masks()[:-1]):
                 return self._get_observation(), -100, True, False, {"error": "Stuck with no future arrivals"}

            info = {"action_type": "WAIT", "event_time": self.event_time, "delta_t": delta_t}
            return self._get_observation(), wait_penalty, False, False, info
        ...
        # Update event time to the end of this operation
        old_event_time = self.event_time
        self.event_time = max(self.event_time, end_time)
        
        # Check for new arrivals that might have occurred during this operation
        num_new_arrivals = self._update_arrived_jobs()
