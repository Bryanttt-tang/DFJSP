...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_makespan, current_makespan):
        ...
        # Fallback for other modes if needed
            ...
            reward = - proc_time * 1 - 2 * idle_time
            # if done:
            #     reward += 100.0
            return reward
...
class PerfectKnowledgeFJSPEnv(gym.Env):
    ...
    def _calculate_reward(self, proc_time, idle_time, done, previous_makespan, current_makespan):
        ...
        if self.reward_mode == "makespan_increment":
            ...
            # Default reward function
            reward = 10.0 - proc_time * 1 - 2 * idle_time
            # if done:
            #     reward += 100.0
            return reward
...
def train_static_agent(jobs_data, machine_list, total_timesteps=300000, reward_mode="makespan_increment", learning_rate=3e-4):
    ...
    def make_static_env():
        env = PerfectKnowledgeFJSPEnv(jobs_data, machine_list, static_arrival_times, reward_mode='else')
        env = ActionMasker(env, mask_fn)
        # env = Monitor(env)
        return env
...
def plot_training_metrics():
    ...
    fig.suptitle(f'Training Progress - {TRAINING_METRICS.get("method_name", "RL Agent")}', fontsize=16, fontweight='bold')
...
def main():
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        print(f"    Dynamic RL training time: {dynamic_end_time - dynamic_start_time:.2f} seconds")
    ...
    print(f"    Perfect RL training time: {perfect_end_time - perfect_start_time:.2f} seconds")
    print(f"    Dynamic RL training time: {dynamic_end_time - dynamic_start_time:.2f} seconds")
    print(f"    Best Heuristic training time: {spt_end_time - spt_start_time:.2f} seconds")
    print(f"    MILP Optimal training time: {milp_end_time - milp_start_time:.2f} seconds")
