...
ENHANCED_JOBS_DATA, MACHINE_LIST, MACHINE_METADATA = generate_simplified_fjsp_dataset(
    ...
    proc_time_variance_range=(5, 20),  # HUGE variance (was 5-30) - breaks SPT/LPT
    seed=GLOBAL_SEED
)
...
class ProactiveDynamicFJSPEnv(gym.Env):
    ...
    def __init__(self, jobs_data, machine_list, initial_jobs=5, arrival_rate=0.05,
        ...
        # - Wait actions: Multiple durations for flexibility
        num_scheduling_actions = len(self.job_ids) * len(self.machines)
        
        # CRITICAL FIX: Add multiple wait durations as originally intended
        # Agent learns when to: (1) wait for machine, (2) wait for arrival
        # Options: 5, 10, 20 time units, or "next_event" (float('inf'))
        self.wait_durations = [5.0, 10.0, 20.0, float('inf')]
        num_wait_actions = len(self.wait_durations)
...
def train_perfect_knowledge_agent(jobs_data, machine_list, arrival_times, total_timesteps=100000, 
    ...
    for init_idx in range(num_initializations):
        ...
        model = MaskablePPO(
            ...
            batch_size=1024,            # Good batch size for stability
...
def main():
    ...
    static_timesteps = 30000    # Increased for better learning
