...
def plot_gantt_chart(schedule_log, machines, makespan, title="Gantt Chart"):
    """Plot Gantt chart for the schedule"""
    if not any(schedule_log.values()):
        print("No operations to plot in Gantt chart")
        return
    ...
    for idx, machine in enumerate(machines):
        machine_ops = schedule_log.get(machine, [])
        ...
        for op_data in machine_ops:
            ...
            # Use different colors for different jobs
...
def print_schedule_details(schedule_log, machines, makespan):
    """Print detailed schedule information"""
    print(f"\n📊 DETAILED SCHEDULE (Makespan: {makespan}):")
    print("="*50)
    
    for machine in machines:
        ops = schedule_log.get(machine, [])
        if ops:
            print(f"{machine}:")
            for op in sorted(ops, key=lambda x: x['start']):
                print(f"  J{op['job_id']}-O{op['op_idx']+1}: {op['start']:.1f} → {op['end']:.1f} (duration: {op['end']-op['start']:.1f})")
        else:
...
def enhanced_policy_iteration(env, gamma=0.9, theta=1e-6, max_iterations=100):
    ...
    while iteration < max_iterations:
        ...
        print("Goal: Find value of each state under current policy")
        print()
        ...
        while True:
            ...
            for state_idx, state in enumerate(all_states):
                ...
                print(f"  🔸 State {state_idx + 1}: {state}")
                print(f"      Jobs completed: {state_info['operations_completed']}")
                print(f"      Machine free times: {state_info['machine_free_times']}")
                print(f"      Job ready times: {state_info['job_ready_times']}")
                print(f"      Current makespan: {state_info['current_makespan']}")
                print(f"      Current policy action: J{action[0]}-O{action[1]+1} on {action[2]}")
                print(f"      → Next state: Jobs completed: {next_state_info['operations_completed']}, Makespan: {next_state_info['current_makespan']}")
                print(f"      💰 Reward calculation: R(s,a) = {state_info['current_makespan']:.1f} - {next_state_info['current_makespan']:.1f} = {reward:.3f}")
                print(f"      📈 Value update: V(s) = {reward:.3f} + {gamma} × {V[next_state]:.3f} = {V[state]:.3f}")
                print(f"      📊 Change: {v_old:.3f} → {V[state]:.3f} (Δ = {abs(v_old - V[state]):.6f})")
                print()
            
            print(f"    ✅ Max delta = {delta:.6f} (convergence threshold: {theta})")
            if delta < theta:
                print(f"    🎉 Policy evaluation CONVERGED in {eval_iter} iterations!")
                break
            else:
                print(f"    ⏳ Continuing evaluation (delta > threshold)...")
            print()
        ...
        print("Goal: Improve policy by selecting best action for each state")
        print()
        ...
        for state_idx, state in enumerate(all_states):
            ...
            print(f"    Job ready times: {state_info['job_ready_times']}")
            print(f"    Current makespan: {state_info['current_makespan']}")
            print(f"    🔍 Evaluating all possible actions:")
            
            for action_idx, (action, _) in enumerate(valid_actions):
                next_state, reward = env.transition(state, action)
                q_value = reward + gamma * V[next_state]
                
                next_state_info = env.get_state_info(next_state)
                is_best = "⭐" if q_value > best_q else "  "
                
                print(f"      {is_best} Action {action_idx + 1}: J{action[0]}-O{action[1]+1} on {action[2]}")
                print(f"          → Next state: Jobs={next_state_info['operations_completed']}, Makespan={next_state_info['current_makespan']}")
                print(f"          💰 Q-value = R + γ*V(s') = {reward:.3f} + {gamma}*{V[next_state]:.3f} = {q_value:.3f}")
            ...
            if old_action != best_action:
                ...
                print(f"        Old: J{old_action[0]}-O{old_action[1]+1} on {old_action[2]}")
                print(f"        New: J{best_action[0]}-O{best_action[1]+1} on {best_action[2]} (Q = {best_q:.3f})")
            else:
                print(f"    ✅ Policy UNCHANGED: J{best_action[0]}-O{best_action[1]+1} on {best_action[2]} (Q = {best_q:.3f})")
            print()
        
        if policy_stable:
            print(f"🎉 POLICY CONVERGED in {iteration} iterations!")
            print("All states have optimal actions - algorithm complete!")
            break
        else:
            print(f"📈 Policy improved, continuing to iteration {iteration + 1}...")
        print()
...
def execute_optimal_policy(env, policy):
    ...
    step = 0
    total_reward = 0
    schedule_log = collections.defaultdict(list)
    ...
    while not env.is_done(env.state):
        ...
        print(f"   Job ready times: {current_info['job_ready_times']}")
        ...
        # Log operation for Gantt chart
        job_id, op_idx, machine = action
        proc_time = env.jobs[job_id][op_idx]['proc_times'][machine]
        
        # Calculate actual timing for Gantt chart
        job_idx = env.job_ids.index(job_id)
        machine_idx = env.machines.index(machine)
        
        current_ops = current_state[:env.num_jobs]
        machine_free_times = current_state[env.num_jobs:env.num_jobs+env.num_machines]
        job_ready_times = current_state[env.num_jobs+env.num_machines:env.num_jobs+env.num_machines+env.num_jobs]
        
        start_time = max(machine_free_times[machine_idx], job_ready_times[job_idx])
        end_time = start_time + proc_time
        
        schedule_log[machine].append({
            'job_id': job_id,
            'op_idx': op_idx,
            'start': start_time,
            'end': end_time
        ...
        print(f"   Job ready times: {next_info['job_ready_times']}")
    ...
    final_makespan = final_info['current_makespan']
    ...
    print_schedule_details(schedule_log, env.machines, final_makespan)
    
    # Show Gantt chart
    print("\n📈 Generating Gantt Chart...")
    plot_gantt_chart(schedule_log, env.machines, final_makespan, "Optimal Schedule from Policy Iteration")
    
    return final_makespan, total_reward
...
def main():
    ...
    print("🎯 Expected optimal makespan: 4")
    print()
    ...
    print(f"\n🎯 SUMMARY:")
    print(f"="*50)
    print(f"✅ Optimal makespan achieved: {makespan}")
    print(f"💰 Total reward: {total_reward:.3f}")
    print(f"🎉 Expected makespan (4): {'✅ CORRECT!' if makespan == 4 else '❌ INCORRECT - Please debug'}")
    
    # Verify the solution
    if makespan == 4:
        print(f"\n🎊 SUCCESS! The policy iteration found the optimal solution!")
        print(f"   The algorithm correctly identified the minimum makespan of 4.")
    else:
        print(f"\n⚠️  WARNING! Expected makespan was 4, but got {makespan}")
        print(f"   This indicates a potential issue in the implementation.")
