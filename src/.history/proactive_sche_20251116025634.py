...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    Episode Structure:
    - Each step schedules exactly ONE operation (same as Reactive RL)
    - Episode length = total_operations (same as Reactive RL)
    - Agent learns WHICH rule to apply at each decision point
    ...
    def step(self, action):
        """
        EVENT-BASED SCHEDULING: Schedule ONE operation per step using selected rule.
        
        Similar to Reactive RL, each step schedules exactly one operation.
        The difference is that the action selects a RULE (FIFO, SPT, etc.) which then
        chooses which specific operation to schedule, rather than directly selecting
        a (job, machine) pair.
        """
        self.episode_step += 1
        previous_makespan = self.current_makespan
        ...
        if not ready_ops:
            ...
            # After time advancement, check for ready operations again
            ready_ops = self._get_ready_operations()
            
            # If still no ready operations, return with small penalty
            if not ready_ops:
                reward = -(self.current_makespan - previous_makespan)
                terminated = self.operations_scheduled >= self.total_operations
                return self._get_observation(), reward, terminated, False, {"info": "time_advanced"}
...
def evaluate_rule_based_on_dynamic(rule_model, jobs_data, machine_list, arrival_times, 
    ...
    while step_count < max_steps:
        ...
        if not any(action_masks):
            print(f"    ❌ No valid actions at step {step_count}")
            print(f"       Ops scheduled: {test_env.env.operations_scheduled}/{test_env.env.total_operations}")
            print(f"       Arrived jobs: {sorted(test_env.env.arrived_jobs)}/{len(test_env.env.job_ids)}")
    ...
    # Check if hit max steps without completion
    if step_count >= max_steps:
        print(f"    ⚠️  Hit max_steps={max_steps} limit!")
        print(f"       Ops scheduled: {test_env.env.operations_scheduled}/{test_env.env.total_operations}")
...
def evaluate_proactive_on_dynamic(proactive_model, jobs_data, machine_list, arrival_times, 
    ...
    test_env = ProactiveDynamicFJSPEnv(
        jobs_data, machine_list,
        initial_jobs=[k for k, v in arrival_times.items() if v == 0.0],
    ...
    while step_count < max_steps:
        ...
        if done or truncated:
            print(f"    Episode terminated at step {step_count} (scheduled={schedule_count}, waited={wait_count})")
            print(f"       Completed jobs: {sorted(test_env.env.completed_jobs)}/{len(test_env.env.job_ids)}")
            print(f"       Done={done}, Truncated={truncated}")
            break
