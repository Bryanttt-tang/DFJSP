...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # DON'T advance event time during scheduling - event time represents the current frontier
        # Event time should only advance through WAIT actions or when we need to reveal arrivals
        # However, we should check if any jobs arrive between current event_time and start_time
        if start_time > self.event_time:
            # Check for arrivals between current event_time and start_time, but don't advance event_time
            for job_id_check, arrival_time in self.job_arrival_times.items():
                if (job_id_check not in self.arrived_jobs and 
                    arrival_time != float('inf') and 
                    self.event_time < arrival_time <= start_time):
                    # Job arrives during this scheduling window, reveal it
