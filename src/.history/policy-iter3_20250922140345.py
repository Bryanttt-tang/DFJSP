...
def canonicalize_state(next_ops, machine_next_free, job_ready_time, current_time, time_unit=1, cap=None):
    ...
    # (d) Current makespan (global timing context)
    current_makespan = compute_makespan_internal(machine_next_free, job_ready_time)
    
    # Compact 4-tuple observation including global makespan
    observation = (tuple(ready_jobs), tuple(machine_idle), tuple(proc_times), current_makespan)
    
    return observation

def pretty_state(s):
    ready_jobs, machine_idle, proc_times, makespan = s
    return f"ready_jobs={ready_jobs}, machine_idle={machine_idle}, proc_times_sample={proc_times[:6]}..., makespan={makespan}"
...
with open(csv_filename, 'w', newline='') as csvfile:
    ...
    writer.writerow(['state_id', 'ready_jobs', 'machine_idle', 'proc_times_sample', 'makespan', 'num_actions', 'actions'])
    for sid, canon in enumerate(id_to_state):
        ready_jobs, machine_idle, proc_times, makespan = canon
        ...
        writer.writerow([sid, list(ready_jobs), list(machine_idle), list(proc_times[:6]), makespan, len(acts), actions_str])
print("CSV saved.")
...
if incomplete_mdp:
    ...
    while True:
        ...
        if cur_canon not in state_to_id:
            ...
            print(f"  Current state: next_ops={cur_next_ops}, mnext={cur_mnext}, jready={cur_jready}, time={cur_time}")
            break
        cur_sid = state_to_id[cur_canon]
        a = policy[cur_sid]
        finished_ops = sum(cur_next_ops)
        print(f"Step {step+1}: current state sid={cur_sid}, finished_ops={finished_ops}/{total_operations}, action={a}")
        if a is None:
            print("  No action available - terminating")
            break
        if finished_ops >= total_operations:
            print("  All operations completed - terminating")
            break
        step += 1
        if isinstance(a, tuple) and len(a) == 1 and a[0] == 'WAIT':
