...
class TrainingCallback:
    ...
    def __call__(self, locals_dict, globals_dict):
        ...
        if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
            ...
            # FIXED: Better episode reward and length logging
            # Try multiple possible keys for episode rewards
            episode_reward_keys = ['rollout/ep_rew_mean', 'rollout/episode_reward', 'episode_reward', 'ep_rew_mean']
            episode_length_keys = ['rollout/ep_len_mean', 'rollout/episode_length', 'episode_length', 'ep_len_mean']
            
            for key in episode_reward_keys:
                if key in log_data and log_data[key] is not None:
                    TRAINING_METRICS['episode_rewards'].append(float(log_data[key]))
                    break
            
            for key in episode_length_keys:
                if key in log_data and log_data[key] is not None:
                    TRAINING_METRICS['episode_lengths'].append(float(log_data[key]))
                    break
            
            TRAINING_METRICS['timesteps'].append(model.num_timesteps)
            
            # Debug: Print available keys occasionally to diagnose logging issues
            if len(TRAINING_METRICS['timesteps']) % 100 == 1:  # Print every 100 calls
                available_keys = list(log_data.keys())
                reward_keys = [k for k in available_keys if 'reward' in k.lower() or 'ep_rew' in k.lower()]
                if not reward_keys:
                    print(f"Debug: No episode reward keys found. Available keys: {available_keys[:10]}...")
        
        # Extract episode-level data directly from the environment if possible
        if hasattr(model, 'env') and hasattr(model.env, 'get_attr'):
            try:
                # Try to get episode statistics directly from Monitor wrapper
                episode_returns = model.env.get_attr('episode_returns')
                episode_lengths = model.env.get_attr('episode_lengths')
                
                if episode_returns and episode_returns[0]:  # Check if we have data
                    recent_returns = episode_returns[0][-10:]  # Get last 10 episodes
                    if recent_returns:
                        TRAINING_METRICS['episode_rewards'].extend(recent_returns)
                
                if episode_lengths and episode_lengths[0]:
                    recent_lengths = episode_lengths[0][-10:]
                    if recent_lengths:
                        TRAINING_METRICS['episode_lengths'].extend(recent_lengths)
