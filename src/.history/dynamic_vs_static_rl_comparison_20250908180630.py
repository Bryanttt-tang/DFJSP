...
class StaticFJSPWrapper(PoissonDynamicFJSPEnv):
    """Wrapper to make PoissonDynamicFJSPEnv behave like static FJSP."""
    
    def __init__(self, jobs_data, machines, **kwargs):
        # Force all jobs to be initial (available at t=0)
        super().__init__(
            jobs_data=jobs_data,
            machines=machines,
            initial_jobs=list(jobs_data.keys()),  # All jobs available at start
            arrival_rate=0.0,  # No dynamic arrivals
