...
class PoissonDynamicFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = self._calculate_reward(
            ...
            machine, machine_available_time, job_id
        )
    ...
    def _calculate_reward(self, proc_time, idle_time, done, num_new_arrivals, 
                         previous_time, current_time, machine=None, machine_available_time=None, job_id=None):
        ...
        elif self.reward_mode == "utilization_maximization":
            """New reward mode that maximizes machine utilization and minimizes makespan."""
            reward = 0.0
            
            # 1. Base scheduling reward
            reward += 5.0  # Basic reward for making progress
            
            # 2. Strong penalty for idle time (key improvement)
            reward -= idle_time * 5.0  # Much stronger penalty than before
            
            # 3. Encourage efficient operation selection
            reward -= proc_time * 0.2  # Small penalty for longer operations
            
            # 4. Machine utilization bonus
            current_utilization = self.machine_workload[machine] / max(current_time, 1.0)
            reward += current_utilization * 2.0  # Bonus for keeping machines busy
            
            # 5. Dynamic adaptation rewards
            if num_new_arrivals > 0:
                # Strong bonus for immediately utilizing newly arrived jobs
                reward += num_new_arrivals * 10.0
                
                # Extra bonus if we use idle machines for new arrivals
                machine_was_idle = (machine_available_time <= current_time)
                if machine_was_idle:
                    reward += 5.0  # Bonus for using idle machine
            
            # 6. Load balancing incentive
            machine_loads = [self.machine_next_free[m] for m in self.machines]
            if len(machine_loads) > 1 and max(machine_loads) > 0:
                load_variance = np.var(machine_loads)
                normalized_variance = load_variance / (max(machine_loads) ** 2)
                reward -= normalized_variance * 3.0  # Penalty for unbalanced loads
            
            # 7. Future-aware scheduling
            if self.next_arrival_events:
                next_arrival_time = self.next_arrival_events[0][0]
                time_to_next_arrival = next_arrival_time - current_time
                
                if 0 < time_to_next_arrival <= 8.0:  # Next arrival is soon
                    # Count how many machines will be available
                    available_machines = sum(1 for m in self.machines 
                                           if self.machine_next_free[m] <= next_arrival_time + 1.0)
                    
                    if available_machines >= 1:
                        reward += 2.0  # Bonus for keeping machines available for new arrivals
                    
                    # Penalty for creating long idle periods before arrivals
                    if idle_time > 3.0 and time_to_next_arrival > idle_time:
                        reward -= 3.0
            
            # 8. Early completion bonus for individual jobs
            if self.next_operation[job_id] >= len(self.jobs[job_id]):
                # Job completed - bonus based on how quickly it was done
                job_arrival_time = self.arrival_times.get(job_id, 0.0)
                job_completion_time = current_time - job_arrival_time
                if job_completion_time > 0:
                    completion_efficiency = 20.0 / job_completion_time
                    reward += min(completion_efficiency, 10.0)
            
            # 9. Final completion rewards
            if done:
                # Base completion bonus
                reward += 200.0
                
                # Strong bonus for efficient final makespan
                if current_time > 0:
                    makespan_efficiency = 400.0 / current_time
                    reward += min(makespan_efficiency, 100.0)
                
                # Machine utilization bonus at completion
                total_workload = sum(self.machine_workload.values())
                total_capacity = current_time * len(self.machines)
                if total_capacity > 0:
                    final_utilization = total_workload / total_capacity
                    reward += final_utilization * 50.0
                
                # Perfect load balancing bonus
                final_loads = [self.machine_next_free[m] for m in self.machines]
                if len(final_loads) > 1 and max(final_loads) > 0:
                    load_balance = 1.0 - (np.std(final_loads) / max(final_loads))
                    reward += load_balance * 30.0
