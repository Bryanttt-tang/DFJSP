...
class DispatchingRuleFJSPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        NO WAIT ACTIONS - if no operations ready, automatically advances time to next event.
        This matches the heuristic scheduler behavior.
        ...
        # Discover new arrivals at current event_time
        for job_id, arr_time in self.job_arrival_times.items():
            if job_id not in self.arrived_jobs and arr_time <= self.event_time:
                self.arrived_jobs.add(job_id)
        ...
        if not ready_ops:
            # No ready operations - advance time to next event (similar to heuristic)
            next_event_time = self._get_next_event_time()
            if next_event_time > self.event_time and next_event_time != float('inf'):
            ...
            reward = -(self.current_makespan - previous_makespan)
            terminated = self.operations_scheduled >= self.total_operations
            return self._get_observation(), reward, terminated, False, {"auto_advance": True}
        ...
        # Update event_time and makespan
        # If no other idle machines, advance event_time to next event
        other_idle = any(self.machine_next_free[m] <= self.event_time for m in self.machines if m != best_machine)
        if not other_idle:
        ...
        # Reward: negative makespan increment (consistent with other methods)
        reward = -(self.current_makespan - previous_makespan)
