...
def test_dynamic_actions_environment(instance_file="test_instance.txt"):
    ...
    if valid_results:
        ...
        if optimal_makespan < float('inf') and rl_makespan < float('inf'):
            ...
            print("\nPOSSIBLE REASONS FOR OPTIMALITY GAP:")
            print("1. REWARD DESIGN ISSUES:")
            print("   - Even with makespan_increment reward and γ=1, there are subtleties:")
            print("   - Expected return = Σ(-makespan_increment) ≠ -final_makespan exactly")
            print("   - Completion bonus can distort the objective")
            print("   - Early termination may cause sub-optimal action selection")
            
            print("\n2. TRAINING/EXPLORATION ISSUES:")
            print("   - PPO is a policy gradient method, not guaranteed to find global optimum")
            print("   - Action masking reduces exploration space but may miss optimal paths")
            print("   - Network capacity may be insufficient for complex instances")
            print("   - Training may not have converged to optimal policy")
            
            print("\n3. ENVIRONMENT APPROXIMATION:")
            print("   - Discrete action space vs continuous optimization problem")
            print("   - Sequential decision making vs global optimization")
            print("   - Observation space may not capture all relevant information")
            
            print("\n4. THEORETICAL LIMITATIONS:")
            print("   - FJSP is NP-hard; RL provides heuristic, not exact solution")
            print("   - Local optima in policy space")
            print("   - Value function approximation errors")
        ...
        if 'RL Dynamic Actions' in results and results['RL Dynamic Actions'] < float('inf'):
            schedule_data.append((env.schedule, "RL Dynamic Actions", results['RL Dynamic Actions']))
        else:
