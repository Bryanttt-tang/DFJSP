...
def train_agent(jobs_data, machine_list, train_arrivals, log_name, total_timesteps=50000, reward_mode="makespan_increment"):
    ...
    if total_operations <= 15:
        ...
        def make_vec_env():
            env = DynamicFJSPEnvV2(jobs_data, machine_list, train_arrivals, reward_mode=reward_mode)
            # Don't wrap with Monitor here to avoid action_masks issue
            return env
...
def evaluate_agent(model, jobs_data, machine_list, eval_arrivals, scenario_name, reward_mode="makespan_increment"):
    ...
    num_episodes = 5  # Reasonable number for evaluation
    print(f"Running {num_episodes} evaluation episodes...")
    
    episode_results = []
    
    # Check if we have a simple policy model
        ...
        while not done and step_count < max_steps:
            ...
            if is_simple_model:
                ...
                try:
                    ...
                    # Note: model.predict() works with single observations for evaluation
    ...
    if valid_episodes:
        ...
        model_type = "Heuristic Policy" if is_simple_model else "Neural Network"
