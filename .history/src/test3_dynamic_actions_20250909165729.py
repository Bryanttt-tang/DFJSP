...
def test_dynamic_actions_environment():
    ...
    # Load test instance
    try:
        spec = importlib.util.spec_from_file_location("test_instance", "test_instance.txt")
        test_instance = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(test_instance)
        
        jobs_data = test_instance.jobs_data
        machine_list = test_instance.machine_list
        job_arrival_times = test_instance.job_arrival_times
    ...
    while step_count < 20:  # Test first 20 steps
        # Get valid actions
        ...
        for info in valid_actions_info[:5]:  # Show first 5
            print(f"  {info}")
        if len(valid_actions_info) > 5:
            print(f"  ... and {len(valid_actions_info) - 5} more")
