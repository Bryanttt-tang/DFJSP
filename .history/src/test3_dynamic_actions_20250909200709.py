...
def test_dynamic_actions_environment(instance_file="test_instance.txt", skip_milp_large=False):
    ...
    try:
        ...
        step_count = 0
        total_reward = 0
        # Set max_steps proportional to problem size
        max_steps = total_operations * 5  # Much more generous limit
        
        # Initialize progress tracking
        print(f"\nStarting RL scheduling with {total_operations} operations...")
        ...
        while step_count < max_steps:
            ...
            # Progress update for large instances
            operations_done = env.operations_scheduled
            operations_total = total_operations
            progress = (operations_done / operations_total) * 100
            
            # Update progress bar
            if operations_done > 0:  # Avoid division by zero
                bar_length = 50
                filled_length = int(bar_length * operations_done // operations_total)
                bar = "█" * filled_length + "░" * (bar_length - filled_length)
                
                # Print progress bar with overwrite (carriage return)
                print(f"\rProgress: [{bar}] {progress:.1f}% ({operations_done}/{operations_total}) | Steps: {step_count} | Reward: {total_reward:.1f}", end="", flush=True)
        ...
        if step_count >= max_steps:
            ...
            print(f"  This suggests the environment may have issues with time advancement")
            results['RL Dynamic Actions'] = float('inf')
        else:
            # Successfully completed
            rl_schedule = env.schedule
